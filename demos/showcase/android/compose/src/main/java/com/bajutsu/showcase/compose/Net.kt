...
/**
 * The showcase's network calls (SPEC §6), over OkHttp so BajutsuAndroid's interceptor can observe them
 * (BE-0283 — the Android peer of BajutsuKit's `URLProtocol` on iOS). Each call mirrors its outcome to a
 * `*.status` value (`loading` → `done`/`error`) so a scenario can wait on the response before
 * asserting. Any HTTP response counts as `done`; only a transport failure is `error`.
 */
object Net {
