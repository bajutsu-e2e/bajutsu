...
| 10 | Notices（一覧） | `notices` タブ | タブ・長いリスト（スクロール） | `notice` | §5.5 |
...
**スクロール先の要素（scroll-to-element）**の対象です——シナリオはリストを `swipe`（1 回の swipe で約
100pt スクロール、§6.2）して対象行が現れるまでスクロールし、それから `tap` します。データロードを伴う Stable
カタログとは別の、素朴なリスト → 詳細フローで、ナビゲーション・スクロール・crawl のきれいな対象です。
