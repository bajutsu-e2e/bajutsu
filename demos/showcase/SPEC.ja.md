...
| `SHOWCASE_TAB` | 初期タブ：`stable`/`search`/`log`/`notices`/`profile` | `stable` |
...
| Deeplink（host） | 効果 |
|---|---|
| `…://stable` / `search` / `log` / `notices` / `profile` | そのタブを選択 |
| `…://horse/<id>` | Stable タブを選択し、`<id>` の Horse Detail を push |
| `…://notice/<id>` | Notices タブを選択し、`<id>` の Notice Detail を push |
| `…://permissions` | Profile タブを選択し、Permissions 画面（OS アラート画面）を push |

## 5. 画面別仕様

**オンボーディング → ログイン**（モーダルのゲート）、その後 5 タブのメイン UI。操作可能
...
| # | 画面 | 到達方法 | 種別 | 名前空間 | 仕様 |
|---|---|---|---|---|---|
| 1 | オンボーディング | 起動（コールド） | モーダルゲート | `onboarding` | §5.0 |
| 2 | ログイン | オンボーディング後 | モーダルゲート | `auth` | §5.0 |
| 3 | Stable（カタログ一覧） | `stable` タブ | タブ・リスト | `stable` | §5.1 |
| 4 | Horse Detail | Stable 行 / `…://horse/<id>` | push | `horse`, `nav` | §5.1 |
| 5 | Search | `search` タブ | タブ・フィルタ一覧 | `search` | §5.2 |
| 6 | Log | `log` タブ | タブ・フォーム＋モーダル | `log` | §5.3 |
| 7 | — Filter シート | `log.openFilter` | sheet（detent） | `log` | §5.3 |
| 8 | — Gallery カバー | `log.openGallery` | フルスクリーンカバー | `log` | §5.3 |
| 9 | — Delete ダイアログ | `log.openDelete` | アクションシート | `log` | §5.3 |
| 10 | Notices（一覧） | `notices` タブ | タブ・リスト | `notice` | §5.5 |
| 11 | Notice Detail | Notices 行 / `…://notice/<id>` | push | `notice`, `nav` | §5.5 |
| 12 | Profile | `profile` タブ | タブ・グループドリスト | `profile` | §5.4 |
| 13 | Account | `profile.openAccount` | push | `account` | §5.4 |
| 14 | Permissions | `profile.openPermissions` / `…://permissions` | push・**OS アラート** | `perm` | §5.4 |
| 15 | About | `profile.openAbout` | push | `about` | §5.4 |

タブの並び（左から）：**Stable・Search・Log・Notices・Profile**。
...
### 5.5 タブ：Notices — `notice` 名前空間（素朴なリスト → 詳細）

`NavigationStack`（SwiftUI）/ `UINavigationController`（UIKit）が、**3 件**の静的な notice の素朴な縦
リストを持ちます。データロードを伴う Stable カタログとは別の、最小のリスト → 詳細フローで、ナビゲーション
シナリオや crawl のきれいな対象になります。3 件の notice は両アプリで同一にシードします（id は `1`/`2`/`3`）。

- `notice.title` — ナビタイトル「Notices」
- `notice.row.<id>` — notice ごとに 1 つ（`notice.row.1`/`2`/`3`）。`<id>` はデータ由来。タップで Notice Detail を push。集合アサーションは `idMatches: "notice.row.*"` + `count`。

**Notice Detail**（push。`…://notice/<id>` でも到達可能）：
- `notice.detail.title` — notice のタイトル（画面の識別要素。ナビタイトルには id を付けない）
- `notice.detail.body` — notice の本文
- `nav.back` — 戻るボタン（予約名前空間 `nav`）
...
`#if ACCESSIBLE` で囲みます。純粋に VoiceOver の意味付けのためだけの label は無条件のままでよいです。したがって `-noax` ビルドは識別子もミラー値も
