...
| 10 | Permissions | `permissions` タブ / `…://permissions` | タブ・**OS アラート** | `perm` | §5.4 |
...
### 5.4 タブ：Permissions（`perm` 名前空間、**OS アラート画面**）

`NavigationStack`（SwiftUI）/ `UINavigationController`（UIKit）。**OS レベルのアラートを意図的に上げる唯一の
画面**（§7）です。alert-guard のフローへ直接到達できるよう、トップレベルのタブに昇格しました。
