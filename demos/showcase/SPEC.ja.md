...
| `SHOWCASE_TAB` | 初期タブ：`stable`/`search`/`log`/`notices`/`permissions` | `stable` |
...
> **認証ゲートはありません**。アプリは起動直後からタブ UI に入るので、シナリオは `SHOWCASE_TAB` で
> 対象タブに着地します（`sample` フィクスチャと同じクリーン状態注入）。
...
| 2 | Horse Detail | Stable 行 / `…://horse/<id>` | push | `horse` | §5.1 |
...
| 9 | Notice Detail | Notices 行 / `…://notice/<id>` | push | `notice` | §5.5 |
...
- **戻る** — 標準のシステム戻るボタン（ナビゲーションスタックが用意します）。idb は OS 由来の id `BackButton` で引きます。アプリ定義の戻る id はありません。
...
- `log.intense` — ボタン式トグル「Intense」（素の Toggle/UISwitch は iOS 26 では idb のタップで切り替わりません）。`log.intense.value` = `on`/`off`
- `log.segment.<one|two|three>` — ボタン式のセグメントコントロール（native の `Picker(.segmented)` / `UISegmentedControl` は iOS 26 では idb のタップで切り替わりません）。選択中のボタンが `selected` トレイトを持ち、選択内容は `log.segment.value`（`one`/`two`/`three`、既定は `one`）にミラーされます
...
- `log.openDelete` → **アクションシート**（confirmationDialog / UIAlertController ではなく、素のボタンによる自前のオーバーレイ。iOS 26 では idb がアラートのアクションを駆動できないためです）：選択肢 `log.dialog.archive`、`log.dialog.delete`（破壊的）、`log.dialog.cancel`。結果は `log.dialog.value`（`none`/`archive`/`delete`）にミラー
...
- `perm.requestNotif` — ボタン → `UNUserNotificationCenter.requestAuthorization`。**SpringBoard の通知プロンプト**を上げる（プロセス外で idb には見えず、run の vision alert guard が消すか、`dismissAlerts` で「Allow」を叩く）。
...
**System** — アプリ内で完結するペーストボードの往復で、idb のアプリ内クエリでは本来観測できない状態をミラーします。
...
**スクロール先の要素（scroll-to-element）**の対象になります。シナリオはリストを `swipe`（1 回の swipe で約
100pt スクロール、§6.2）して対象行が現れるまでスクロールし、それから `tap` します。データロードを伴う Stable
...
- **戻る** — 標準のシステム戻るボタン。idb は OS 由来の id `BackButton` で引きます（§5.1 参照）。
