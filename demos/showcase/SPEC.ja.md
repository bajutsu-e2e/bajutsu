...
| 2 | Horse Detail | Stable 行 / `…://horse/<id>` | push | `horse`, `nav` | §5.1 |
...
| 9 | Notice Detail | Notices 行 / `…://notice/<id>` | push | `notice`, `nav` | §5.5 |
...
`-a11y` アプリの `targets.<name>.idNamespaces`（[DESIGN §7.3](../../DESIGN.md)）用。予約共有名前空間
`nav` は `defaults.reservedNamespaces` から来ます。
