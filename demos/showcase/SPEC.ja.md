...
> **認証ゲートはありません**。アプリは起動直後からタブ UI に入り、Stable タブに着地します
> （`SHOWCASE_TAB` で別のタブも選べます）。カタログは**固定**（馬 5 頭）で、シードを注入する
> launch-env の口はありません（BE-0079）。シナリオはアプリ自身のデータを観測するだけで、データ状態を
> 注入できません。同様に、push で開く画面へ起動時に直行する近道もありません（§4 を参照）。
