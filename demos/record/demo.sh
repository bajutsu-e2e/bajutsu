#!/usr/bin/env bash
#
# A guided lifecycle demo against the bundled `sample` app:
...
#
# Prereqs: a booted Simulator, the idb client, and the sample app built
# (`make sample-build`). See demos/record/README.md.
#
...
run_scenario() {  # run the scenario; returns bajutsu's exit code (0 pass, 1 fail)
  uv run bajutsu run "$SCENARIO" --app sample --config "$CONFIG" --backend idb --no-network
}
...
[ -d "$APP_PATH" ] \
  || { echo "Sample app not built at $APP_PATH. Build it first: make sample-build"; exit 1; }
echo "ok: idb, a booted Simulator, and the built sample app are present."
...
echo "   uv run bajutsu record $SCENARIO --app sample --config $CONFIG --backend idb \\"
echo "     --goal \"increment the counter twice and check it reads 2\")"

# --- 2) EXECUTE --------------------------------------------------------------
note "2/3  Run the generated scenario on the sample app (idb) — expect PASS"
run_scenario
...
note "Done — you generated, executed, and modified a scenario against the sample app."
