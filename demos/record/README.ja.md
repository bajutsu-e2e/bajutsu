...
1. **著作（AI）**：`bajutsu record` が本物の Tier-1 ループを実行します。**Claude** が、起動中のアプリ上で
   ...
   進行を追えます。表示されるのは、アプリのインストール／起動、🧭 目標の検討、🗺️ 事前プラン、その後は各ターン
...
4. **診断（Diagnose）**：セレクタのラベルを変える（`Log in` → `Log In`）ことで、テストの足元でセレクタが
   ...
   **診断**します（カテゴリと、捕捉した要素ツリーからの likely fix）。triage は助言にとどまり、失敗を説明
...
[`generate_from_nl.py`](generate_from_nl.py) は単独で動くので、目標を反復するのに便利です:
...
[`bajutsu/record.py`](../../bajutsu/record.py)）。差し替えているのは「頭脳」だけです。この
...
明確なエラーを送出します。これは、エージェントが存在しない要素を求めたときに `record` が示すのと同じ失敗形態です。
