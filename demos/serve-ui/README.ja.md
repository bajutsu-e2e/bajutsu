...
- **AI 駆動のフロー**：Record の Generate、Crawl の Start、Enrich の提案、Claude での Triage。
  ゲートの合否経路に LLM を置かないという設計により、どの決定的な網からも除外します。
- **過去の run を前提とするフロー**：埋め込みレポートの中身、visual の Approve、Triage
  （ルールベースでも）、History の項目、Coverage の run 合成。run 履歴はホスト側の状態なので、
  その中身のアサートはマシン依存になります。背後の operation は pytest が覆っています。
