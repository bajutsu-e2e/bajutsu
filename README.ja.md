...
> （idb / RocketSim）は実装済みだが実機未検証で、デバイス上での E2E 実行は未確認。
...
自然言語ゴール ──(record, Tier 1 / AI)──▶ シナリオ (YAML) ◀──(人手編集)
   ...
   Orchestrator  ── observe → act → verify (run, Tier 2; 決定的・AI 非依存)
        ...
        ▼
 RocketSim / idb バックエンド   ← 1 つの Driver IF に統一（テストは fake driver）
        │
        ▼
...
- **simctl コマンド層**、**idb / RocketSim 出力パーサ**、**doctor** 規約スコア
...
- idb / RocketSim バックエンドの subprocess 実行。出力パーサはテスト済みだが、外部 CLI のサーフェスと
...
```yaml
defaults:
  backend: [rocketsim, idb]   # UI 安定度順; 最初に利用可能なものが actuator
  device: "iPhone 15"
  locale: en_US
...
├── drivers/idb.py         # idb バックエンド（ヘッドレス・座標 tap）
├── drivers/rocketsim.py   # RocketSim バックエンド（semantic tap）
...
- **M1 — 完了（実機検証待ち）。** 決定的ランナー: env (simctl) + ドライバ + シナリオ +
  アサーション + 軽量証跡 + manifest + アプリ別 config + `run` / `doctor`。完了条件: 同一シナリオが
  RocketSim / idb 両方で通り、config だけで対象アプリを切り替えられること。*（ロジックは実装・
  テスト済み。「両バックエンドで同一シナリオが通る」は実機での確認が必要。）*
