...
bajutsu record --app <name> --goal "..." [--out file]     # AI 探索 + 記録（Tier 1・要 API キー / ログイン）
bajutsu crawl  --app <name> [--max-screens N]             # AI 幅優先クロール → スクリーンマップ（Tier 1）
bajutsu doctor --app <name>                               # 環境チェック + 現在画面の規約スコア
...
ものが同梱されています（例: [`demos/features/demo.config.yaml`](demos/features/demo.config.yaml)、
...
apps:
  sample:                   # iOS アプリ — idb 経由で Simulator を操作
...
- **[features](demos/features/README.md)**（`make -C demos features`）。シナリオ著作の機能（タグ、
  パラメータ化した共有ステップ、シークレット）を実機 Simulator で示します。
- **[webui](demos/features/WEBUI.md)**（`make -C demos webui`）。**Web UI** で Simulator を操作し、
  あらゆる証跡（スクリーンショット・動画・ログ・通信（観測 + モック）・ビジュアルリグレッション・
  システムアラート突破）をブラウザで集めます。iOS 開発者向けの目玉デモです。
- **[record](demos/record/README.md)**（`make -C demos record`）。起動中アプリに対する本物の Claude に
