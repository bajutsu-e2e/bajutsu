...
[`implement-be`](../implement-be/SKILL.md). That workflow hands these steps to a fresh Agent-tool
...
[`implement-be`](../implement-be/SKILL.md) step 12 states it when handing these steps to a
subagent. The statement changes nothing about the fix-and-reply work below; it reaches only the
[`record-issue`](../record-issue/SKILL.md) sub-step, which cannot observe a human's presence itself
...
[`record-issue`](../record-issue/SKILL.md) as a sub-step for each such finding, handing it the
