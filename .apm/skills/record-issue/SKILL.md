...
[`pr-followup`](../pr-followup/SKILL.md) is the caller wired today. A calling skill may not skip any
...
[`ideation`](../ideation/SKILL.md) or [`propose-and-build`](../propose-and-build/SKILL.md) instead,
which step 1 decides. An issue already filed is shipped by
[`fix-issue`](../fix-issue/SKILL.md), typically after
[`task-select`](../task-select/SKILL.md) surfaces the issue as a candidate — so what this skill
...
the answer itself. Only the loop layer knows: [`implement-be`](../implement-be/SKILL.md) step 12
...
- [`ideation`](../ideation/SKILL.md) and [`propose-and-build`](../propose-and-build/SKILL.md) —
  step 1's two escalation targets, for a finding substantial enough to need a Detailed design.
- [`roadmap-filter`](../roadmap-filter/SKILL.md) — the read-only survey step 2 runs against the
  roadmap.
- [`pr-followup`](../pr-followup/SKILL.md) — the calling skill wired today, and the source of the
  unattended turn's pending-draft path.
- [`task-select`](../task-select/SKILL.md) and [`fix-issue`](../fix-issue/SKILL.md) — the consumers
