...
This skill is the plain-issue sibling of [`implement-be`](../implement-be/SKILL.md), which ships a
...
2. Point at the skill that fits instead —
   [`ideation`](../ideation/SKILL.md) when the design still needs shaping, or
   [`propose-and-build`](../propose-and-build/SKILL.md) when the design is settled enough to
...
Run [`implement-be`](../implement-be/SKILL.md) steps 3–5 with the issue body in place of a BE
...
Run [`implement-be`](../implement-be/SKILL.md) steps 6 and 7: build to the plan matching the
...
[`references/self-review.md`](../implement-be/references/self-review.md) defines, and don't open the
...
yourself**, exactly as
[`implement-be`](../implement-be/SKILL.md) step 10 does and for the same reason: this skill's
...
Run [`implement-be`](../implement-be/SKILL.md) steps 11 and 12 against the new PR, unmodified —
including its [`references/pr-followup-loop.md`](../implement-be/references/pr-followup-loop.md),
...
[`pr-followup`](../pr-followup/SKILL.md) work runs in a fresh Agent-tool subagent so the
...
- [`implement-be`](../implement-be/SKILL.md) — the BE-item counterpart this skill mirrors, and
  the source of steps 4–8's procedure.
- [`ideation`](../ideation/SKILL.md) and
  [`propose-and-build`](../propose-and-build/SKILL.md) — step 3's two escalation targets.
- [`pr-followup`](../pr-followup/SKILL.md) — the skill step 8's loop runs each iteration.
- [`task-select`](../task-select/SKILL.md) — the read-only survey that ranks plain issues as
