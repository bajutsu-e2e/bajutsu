...
[`ideation`](../ideation/SKILL.md)'s job, so hand off rather than drafting it yourself.
...
2. **Start a fresh Agent-tool subagent** and give it
   [`pr-followup`](../pr-followup/SKILL.md)'s steps directly, **stating that no human is in the
   turn** — the loop runs unattended, and only this layer knows that, so
   [`record-issue`](../record-issue/SKILL.md) would otherwise wait on an approval nobody is there to
   give. Its task for
