...
def test_run_command_builder() -> None:
    ...
    erased = srv.run_command("s.yaml", "demo", erase=True, system_alert_handling=True)
    assert "--erase" in erased
    assert "--no-erase" not in erased
    assert "--system-alert-handling" in erased
    assert "--no-erase" in srv.run_command("s.yaml", "demo", erase=False)  # explicit override
    # system_alert_handling defaults to None: no flag, so each scenario's systemAlertHandling (on)
    # decides.
    assert "--system-alert-handling" not in cmd and "--no-system-alert-handling" not in cmd
    # False forces the guard off for the run (mirrors --no-erase).
    assert "--no-system-alert-handling" in srv.run_command(
        "s.yaml", "demo", system_alert_handling=False
    )
...
def test_record_command_builder() -> None:
    ...
    assert (
        "--erase" not in cmd and "--no-erase" not in cmd and "--no-system-alert-handling" not in cmd
    )
    # Explicit overrides mirror run_command.
    assert "--no-erase" in srv.record_command("o.yaml", "demo", "g", erase=False)
    assert "--no-system-alert-handling" in srv.record_command(
        "o.yaml", "demo", "g", system_alert_handling=False
    )
...
def test_crawl_command_builder() -> None:
    ...
    assert "--no-system-alert-handling" in srv.crawl_command(
        "demo", out="o", system_alert_handling=False
    )
