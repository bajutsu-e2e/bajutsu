...
def test_http_run_boots_pool_and_passes_workers(tmp_path: Path) -> None:
    ...
    try:
        body = json.dumps(
            {"scenario": "smoke.yaml", "target": "demo", "udid": "A,B", "workers": 2}
        ).encode()
        req = urllib.request.Request(
            f"http://127.0.0.1:{port}/api/run",
            data=body,
...
def test_http_run_rejects_scenario_outside_dir(tmp_path: Path) -> None:
    ...
    try:
        status, resp = _post(port, "/api/run", {"scenario": str(secret), "target": "demo"})
        assert status == 400 and "inside the target's scenarios dir" in resp["error"]
    finally:
        server.shutdown()
        server.server_close()
...
def test_http_run_rejects_path_traversal(tmp_path: Path) -> None:
    server, port = _run_server(tmp_path)
    try:
        status, resp = _post(port, "/api/run", {"scenario": "../../etc/passwd", "target": "demo"})
...
def test_http_run_rejects_nonexistent_scenario(tmp_path: Path) -> None:
    server, port = _run_server(tmp_path)
    try:
        status, resp = _post(port, "/api/run", {"scenario": "ghost.yaml", "target": "demo"})
...
def test_http_run_rejects_unknown_backend(tmp_path: Path) -> None:
    ...
    try:
        status, resp = _post(
            port, "/api/run", {"scenario": "smoke.yaml", "target": "demo", "backend": "rm -rf /"}
        )
...
def test_http_run_rejects_invalid_udid(tmp_path: Path) -> None:
    ...
    try:
        status, resp = _post(
            port, "/api/run", {"scenario": "smoke.yaml", "target": "demo", "udid": "A;rm -rf /"}
        )
