...
scenarios dir, it returns the generated native test source (XCUITest or Playwright) plus a
filename. It is a structural mapping — deterministic, no device, no AI, no verdict."""
...
def _project(tmp_path: Path) -> tuple[Path, Path, Path]:
    """A scenarios dir shared by an iOS target (`demo`) and a web target (`site`).

    `site` carries a `baseUrl`, so it is a web target and Playwright codegen applies; `demo`
    is iOS and XCUITest applies. Both point at the same scenarios dir so one `smoke.yaml`
    exercises either emit path."""
