...
def test_capabilities_track_the_role_gate_on_activating_a_project(
    tmp_path: Path, serve_engine: Callable[..., Engine]
) -> None:
    # Activating rebinds the config the whole deployment reads, so the route is admin-only. The
    # comparison view's per-row Activate button reads this flag to disable itself with a reason,
    # rather than letting a viewer press and confirm only to meet a 403 (#1720). Read through the
    # same `forbidden_for_role` gate the transport applies to that very route.
    from bajutsu.serve.operations.config import ACTIVATE_ADMIN_ONLY_REASON, serve_capabilities
    from bajutsu.serve.server.db import SqlRepository
    from bajutsu.serve.server.models import Base
    ...
    repository.upsert_user("root", org_id="acme", github_login="root", email="root@x", role="admin")
    repository.upsert_user(
        "edit", org_id="acme", github_login="edit", email="edit@x", role="editor"
    )
    state = srv.ServeState(runs_dir=tmp_path / "runs", cwd=tmp_path, repository=repository)

    assert serve_capabilities(state, "root")["activate"] == {"available": True, "reason": None}
    # An editor may trigger a run but not repoint the binding, so the button is theirs to see and
    # not to press.
    editor = serve_capabilities(state, "edit")["activate"]
    assert editor == {"available": False, "reason": ACTIVATE_ADMIN_ONLY_REASON}
    # No identity at all (a local serve, a shared-token session) leaves no role to fail.
    assert serve_capabilities(state, None)["activate"]["available"] is True


def test_capabilities_offer_activation_on_a_database_less_serve(tmp_path: Path) -> None:
    # Without a database the transport's role gate short-circuits to full access, so reporting the
    # button as unavailable would grey out a control that in fact works. The flag reports, it never
    # gates.
    from bajutsu.serve.operations.config import serve_capabilities

    state = srv.ServeState(runs_dir=tmp_path / "runs", cwd=tmp_path)
    assert serve_capabilities(state, "anyone")["activate"] == {"available": True, "reason": None}
