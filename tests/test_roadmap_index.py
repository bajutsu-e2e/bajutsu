...
def test_status_bucket_maps_each_status() -> None:
    assert bri.STATUS_BUCKET["Implemented"] == "Implemented"
    assert bri.STATUS_BUCKET["In progress"] == "In progress"
    assert bri.STATUS_BUCKET["Proposal"] == "Proposals"
    assert bri.STATUS_BUCKET["Proposal (deferred)"] == "Deferred"
