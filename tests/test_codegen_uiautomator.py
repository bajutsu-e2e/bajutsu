...
def test_tap_waits_for_the_target_before_clicking() -> None:
    ...
    # observed as a flake on the very first tap right after launch under CI load. The message
    # names the selector so a timeout is legible straight from the CI console.
    code = _gen("- name: x\n  steps:\n    - tap: { id: search }\n")
    lines = [line.strip() for line in code.splitlines()]
    wait_idx = lines.index(
        'assertTrue("target not found within 10000ms: byId(\\"search\\")", '
        'device.wait(Until.hasObject(byId("search")), 10000L))'
    )
...
def test_delete_and_select_also_wait_before_focusing() -> None:
    ...
    assert code.count('device.wait(Until.hasObject(byId("form.note")), 10000L)') == 2
