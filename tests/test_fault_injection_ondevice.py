...
def test_a_hang_past_the_retry_budget_is_ridden_out_by_crash_recovery(
    ...
    base.resolve_unique(elements, _MARKER)
    # A read that recovers by re-issuing can come back on a freshly re-spawned runner. Discard the
    # module lease once this contract is proven so the next case never inherits a half-turned-over
    # resident (old port / dead process) and fails before it injects its own fault.
    lease.invalidate()
