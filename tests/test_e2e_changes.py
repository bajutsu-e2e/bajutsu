...
def test_real_ios_workflow_declares_the_scenario_keyed_jobs() -> None:
    # Pin the map against the real workflow: the four scenario-keyed jobs the narrowing acts on after
    # the gating jobs folded into one `run` (the consolidation), and smoke.yaml declared by both `run`
    # and `bundled-runner`. A job rename or a scenario move surfaces here rather than silently
    # mis-narrowing the lane. `run` declares its six scenarios as a folded block scalar, so this also
    # pins the block-scalar path of `job_scenario_map` against the real workflow.
    text = lane_workflow_text("ios")
    assert text is not None
    ...
    assert job_map["run"] == {
        ...
        "demos/showcase/scenarios/search.yaml",
        "demos/showcase/scenarios/notices.yaml",
        "demos/showcase/scenarios/permission.yaml",
        "demos/showcase/scenarios/permission_system_alert.yaml",
    }
    assert job_map["actuation"] == {
        "demos/showcase/scenarios/navigation.yaml",
        "demos/showcase/scenarios/device.yaml",
        "demos/showcase/scenarios/push.yaml",
...
def test_main_narrows_a_scenario_only_ios_change_to_the_affected_jobs(
    ...
    # (which declares it after the consolidation); main() emits shared=false and the narrowed affected
    # set. The job-to-scenario map is read from the real workflow (via lane_workflow_text), while the
    # changed file lives in the temp repo.
    _init_repo(tmp_path, monkeypatch)
    _commit(tmp_path, "README.md", "seed")
    _git(tmp_path, "branch", "pr")
    main_tip = _commit(tmp_path, "bajutsu/runner/pipeline.py", "unrelated on main")
