...
the batch/CI periphery (`analytics`, `cloud`, `github`), and the evidence/report pair (`evidence`,
`report`). Still to move here: the rest of the deterministic pipeline (`drivers`, `orchestrator`,
`runner`, `config`, `platform_lifecycle`) — so membership here says nothing about whether a
