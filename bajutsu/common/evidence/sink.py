...
class RunArtifactWriter:
    ...
    def write_screen_map(self, name: str, screen_map: dict[str, Any]) -> Path:
        ...
        so the full key/known-value pass still runs over the map's strings. Only its *keys* are
        exempt: those are the crawl's own control schema (`kind`, `label`, `key`, `path`,
        `fingerprint`), which `screenmap_from_dict` reads back on `--continue`, so a target naming an
        app body field after one of them would break the resume rather than protect anything
        (BE-0331).
        """
        return self._write_json(
            name,
            self._redactor.redact_screen_map(screen_map),
            scrub_text=True,
            scrub_keys=False,
        )
    ...
    def _write_json(
        self, name: str, data: Any, *, scrub_text: bool = False, scrub_keys: bool = True
    ) -> Path:
        ...
        # resume path and the web UI's live poll both read back (BE-0331). `scrub_keys=False` drops
        # the key half for a document whose names are Bajutsu's schema, not the app's.
        if scrub_text:
            data = self._redactor.redact_structure(data, keys=scrub_keys)
