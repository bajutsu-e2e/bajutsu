...
def _composed_cache_hit(state: ServeState, org: str, shas: dict[str, str]) -> bool:
    ...
    Restoring carries no `scenarios` display name, so its composition id is the unsalted form
...
def _legs_cached(state: ServeState, org: str, shas: dict[str, str]) -> bool:
    ...
    display name is part of the composition's identity — which the restore path cannot reconstruct,
    since a stored record carries no such name. `_compose_and_bind` can still rebuild the tree from
    these legs with no object store (`_fetch_artifact` resolves this same cache), so holding them all
    is the second way a no-store deployment has something to restore from.
    """
