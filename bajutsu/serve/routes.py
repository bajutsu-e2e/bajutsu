...
class RequestCtx(Protocol):
    ...
    def session_id(self) -> str | None:
        """This request's session id, or None without a session cookie.

        The two endpoints that read or write the session's own org selection need the id itself, not
        just who the caller is: one reads the GitHub facts the session recorded, the other writes the
        selection back onto that row (session-scoped org selection).
        """
        ...

    def caller(self) -> Caller | None:
        """Who this request is — login, acting org, and role — or None without a session.

        The whole caller rather than the login alone, so an operation scopes by the org this
        session selected instead of re-deriving it from the user row (session-scoped org selection).
        """
        ...
...
ROUTES: tuple[Route, ...] = (
    ...
    Route(
        "GET",
        "/api/scenarios",
        lambda state, ctx: ops.list_scenarios(state, ctx.query("target"), actor=ctx.caller()),
    ),
    Route(
        "GET",
        "/api/targets",
        lambda state, ctx: ops.list_targets_payload(state, actor=ctx.caller()),
    ),
    ...
    Route(
        "GET",
        "/api/config",
        lambda state, ctx: ops.config_info(state, actor=ctx.caller(), session_id=ctx.session_id()),
    ),
    ...
    Route("GET", "/api/apikey", lambda state, ctx: ops.api_key_info(state, ctx.caller())),
    Route(
        "GET",
        "/api/claudecodetoken",
        lambda state, ctx: ops.claude_code_token_info(state, ctx.caller()),
    ),
    Route(
        "GET", "/api/gitcredential", lambda state, ctx: ops.git_credential_info(state, ctx.caller())
    ),
    ...
    Route("GET", "/api/secrets", lambda state, ctx: ops.scenario_secrets_info(state, ctx.caller())),
    Route("GET", "/api/provider", lambda state, ctx: ops.provider_info(state, ctx.caller())),
    ...
    Route(
        ...
        lambda state, ctx: ops.runs_payload(
            state, actor=ctx.caller(), scenario=ctx.query("scenario")
        ),
    ),
    Route(
        "GET", "/api/projects", lambda state, ctx: ops.list_projects_view(state, actor=ctx.caller())
    ),
    Route(
        "GET",
        "/api/projects/{name}/runs",
        lambda state, ctx: ops.project_runs(state, ctx.path_param("name"), actor=ctx.caller()),
    ),
    ...
    Route("GET", "/api/orgs", lambda state, ctx: ops.list_orgs_view(state, actor=ctx.caller())),
    # Which org this session acts as (session-scoped org selection). Any signed-in identity may
    # switch among the orgs it already qualifies for, so this one carries no role gate.
    Route(
        "POST",
        "/api/session/org",
        lambda state, ctx: ops.switch_org(
            state, ctx.body(), session_id=ctx.session_id(), caller=ctx.caller()
        ),
    ),
    Route(
        "GET",
        "/api/metrics/projects",
        lambda state, ctx: ops.project_metrics_view(state, actor=ctx.caller()),
    ),
    Route(
        "GET",
        "/api/crawl/runs",
        lambda state, ctx: ops.crawl_runs_payload(state, actor=ctx.caller()),
    ),
    ...
    Route(
        "GET",
        "/api/runs/trash",
        lambda state, ctx: ops.trashed_runs_payload(state, actor=ctx.caller()),
    ),
    Route(
        "GET",
        "/api/artifacts/exists",
        lambda state, ctx: ops.artifact_exists(
            state, ctx.query("kind"), ctx.query("sha256"), actor=ctx.caller()
        ),
    ),
    Route(
        "GET",
        "/api/compose/current",
        lambda state, ctx: ops.compose_current(state, actor=ctx.caller()),
    ),
    Route(
        "GET",
        "/api/scenario",
    ...
    Route(
        "GET",
        "/stats",
        lambda state, ctx: ops.stats_html(state, actor=ctx.caller()),
        content_type=_HTML,
    ),
    Route(
        "GET",
        "/flakiness",
        lambda state, ctx: ops.flakiness_html(state, actor=ctx.caller()),
        content_type=_HTML,
    ),
    Route(
        "GET",
        "/usage",
        lambda state, ctx: ops.usage_html(state, actor=ctx.caller()),
        content_type=_HTML,
    ),
    ...
    Route(
        ...
        lambda state, ctx: (
            ops.bind_git_config(state, str(ctx.body().get("git") or ""), actor=ctx.caller())
            if "git" in ctx.body()
    ...
    Route(
        ...
        lambda state, ctx: ops.set_api_key(
            state, str(ctx.body().get("value", "") or ""), ctx.caller()
        ),
    ),
    Route(
        "POST",
    ...
    Route(
        "POST",
        "/api/provider",
        lambda state, ctx: ops.set_provider(state, ctx.body(), ctx.caller()),
    ),
    ...
    Route(
        "POST",
        "/api/secrets",
        lambda state, ctx: ops.set_scenario_secret(state, ctx.body(), ctx.caller()),
    ),
    Route(
        "POST", "/api/theme", lambda state, ctx: ops.upload_theme(state, ctx.body(), ctx.caller())
    ),
    Route(
        "POST",
        "/api/compose",
        lambda state, ctx: ops.bind_composition(state, ctx.body(), actor=ctx.caller()),
    ),
    Route("POST", "/api/ant/login", lambda state, ctx: ops.ant_login(state), local_only=True),
    Route(
        "POST", "/api/run", lambda state, ctx: ops.start_run(state, ctx.body(), actor=ctx.caller())
    ),
    Route(
        "POST",
        "/api/run-set",
        lambda state, ctx: ops.start_run_set(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/projects",
        lambda state, ctx: ops.register_project(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/projects/{name}/run",
        lambda state, ctx: ops.run_project(
            state, ctx.path_param("name"), ctx.body(), actor=ctx.caller()
        ),
    ),
    Route(
        "POST",
        "/api/projects/{name}/activate",
        lambda state, ctx: ops.activate_project(state, ctx.path_param("name"), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/orgs",
        lambda state, ctx: ops.create_org(state, ctx.body(), actor=ctx.caller()),
    ),
    ...
    Route(
        ...
        lambda state, ctx: ops.update_org_membership(
            state, ctx.path_param("slug"), ctx.body(), actor=ctx.caller()
        ),
    ),
    Route(
        "POST",
        "/api/record",
        lambda state, ctx: ops.start_record(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/crawl",
        lambda state, ctx: ops.start_crawl(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/triage",
        lambda state, ctx: ops.start_triage(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/scenario",
        lambda state, ctx: ops.save_scenario(state, ctx.body(), actor=ctx.caller()),
    ),
    ...
    Route(
        "POST",
        "/api/audit",
        lambda state, ctx: ops.audit_scenario(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/codegen",
        lambda state, ctx: ops.generate_codegen(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/approve",
        lambda state, ctx: ops.approve_baseline(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/scenario/resolve",
        lambda state, ctx: ops.resolve_scenario_pick(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/enrich",
        lambda state, ctx: ops.start_enrich(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/doctor",
        lambda state, ctx: ops.doctor_check(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/coverage",
        lambda state, ctx: ops.coverage_view(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/capture/start",
        lambda state, ctx: ops.start_capture(state, ctx.body(), actor=ctx.caller()),
        local_only=True,
    ),
    Route(
        "POST",
        "/api/capture/mark",
        lambda state, ctx: ops.mark_capture(state, ctx.body(), actor=ctx.caller()),
        local_only=True,
    ),
    Route(
        "POST",
        "/api/capture/finish",
        lambda state, ctx: ops.finish_capture(state, ctx.body(), actor=ctx.caller()),
        local_only=True,
    ),
    ...
    Route(
        "POST",
        "/api/capture/resolve",
        lambda state, ctx: ops.resolve_capture_pick(state, ctx.body(), actor=ctx.caller()),
        local_only=True,
    ),
    Route(
        "POST",
        "/api/capture/close",
        lambda state, ctx: ops.close_capture(state, ctx.body(), actor=ctx.caller()),
        local_only=True,
    ),
    Route(
        "POST",
    ...
    Route(
        ...
        lambda state, ctx: ops.bulk_delete_runs(state, ctx.body(), actor=ctx.caller()),
    ),
    Route(
        "POST",
        "/api/runs/{run_id}/restore",
        lambda state, ctx: ops.restore_run(state, ctx.path_param("run_id"), actor=ctx.caller()),
    ),
    # --- DELETE ---
    Route(
        "DELETE",
    ...
    Route(
        ...
        lambda state, ctx: ops.deregister_project(
            state, ctx.path_param("name"), actor=ctx.caller()
        ),
    ),
    Route(
        "DELETE",
        "/api/orgs/{slug}",
        lambda state, ctx: ops.delete_org(state, ctx.path_param("slug"), actor=ctx.caller()),
    ),
)
