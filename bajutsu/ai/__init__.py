...
investigation paths (`record`, `triage`, `--system-alert-handling`, `crawl`, MCP enrich) talk to a model
...
point is that model-free `credential_gap` lookup — `run --system-alert-handling`'s alert guard (itself a
