...
def credential_gap() -> str | None:
    """What the SDK AI path is missing to authenticate with the configured provider, or ``None`` when it can.

    The Anthropic provider needs ``ANTHROPIC_API_KEY``; Bedrock authenticates with the
