...
never a re-resolved predicate that could match a different element. The runner content-addresses that
handle from the element's identity (identifier / label / traits), so a re-snapshot of an unchanged
screen re-issues the *identical* handle; a handle goes stale only when the element leaves the screen
or changes identity (BE-XXXX). A `stale` reply is still treated as a trigger to re-query rather than
an immediate failure (BE-0289): the actuation is re-issued only while the same selector still resolves
