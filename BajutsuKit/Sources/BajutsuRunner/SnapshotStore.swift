...
///
/// A handle is derived from the element's identity (identifier, label, traits),
/// not from a per-snapshot generation counter, so an element that is still on
/// screen keeps the same handle across snapshots. A redundant `/elements` read —
/// one the Python driver's transport-retry seam re-sends — therefore re-issues
/// the identical handle rather than stranding it, which is the failure class
/// this addresses (BE-0312). A handle goes stale only when the element it names
/// leaves the screen or changes identity; a handle string that was never issued
/// is not-found. `value` and `frame` are excluded from the derivation on purpose:
/// a screen settling after a transition re-snapshots repeatedly, and a `frame`
/// that shifts or a `value` that ticks (a loading label, a countdown, a spinner)
/// between two interleaved reads would give one unchanged element two handles and
/// reopen the race — and neither buys actuation precision, since the runner acts
/// through the resolved `backingElement`. This is the same identity
/// `PositionPath.attributesMatch` uses (BE-0287). The `h-` prefix is kept as an
/// opaque-handle namespace; only what follows it changes from a generation-index
/// to an identity encoding.
final class SnapshotStore {
    ...
    func refreshSnapshot(elements: [ElementSnapshot]) -> [(handle: String, snapshot: ElementSnapshot)] {
        lock.withLock {
            ...
            // Two identity-identical elements in one snapshot (say, two buttons both
