...
[「推敲後の textlint 検証（必須）」](#推敲後の-textlint-検証必須)に置く（実行環境は
[`tech-writing`](../tech-writing/SKILL.md) スキルと共有している）。

## 文体（敬体・常体）

文書の種別で文体を選ぶ。このリポジトリのドキュメント（`docs/ja/`）とロードマップ項目（`*-ja.md`）は、
敬体（ですます調）で書く。常体（だ・である調）は使わない。書籍・記事の原稿は、その媒体の慣例に従う
（多くは常体）。どちらの場合も、一つの文書のなかで敬体と常体を混在させない。敬体にするのは文末の述語だけで、
...
textlint による機械的な検証は、英語・日本語の両方の文章に共通の実行環境として
[`tech-writing`](../tech-writing/SKILL.md#mandatory-textlint-verification-after-drafting) スキルに
一本化されている。実行コマンド、`--fix` の扱い、ルールの変え方はすべてそちらに書いてある。

指摘がすべて消えるまで推敲と再実行を繰り返す約束、textlint はあくまで機械的な下限であって
これを通しただけで上の規範を満たしたことにはならないという注意点は、日本語の文章にもそのまま
適用される。設定ファイル [`../tech-writing/textlint/.textlintrc.json`](../tech-writing/textlint/.textlintrc.json)
は `textlint-rule-preset-ja-technical-writing`（日本語の技術文書向けの定番プリセット）を含む、
このスキルの規範向けの日本語ルールを有効にしている。プリセットの指摘が本スキルの規範や、
このリポジトリで意図的に許している表記（全角・日本語文字、ダッシュの扱いなど）と衝突するときは、
同じファイルで調整する。
