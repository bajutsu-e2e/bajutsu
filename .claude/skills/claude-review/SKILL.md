...
This packages the same judge-only review/plan pass [`ideation`](../../../.apm/skills/ideation/SKILL.md) step 5
...
example `pr-followup` runs it on `opus`). Give that subagent **only** the contract text and the
...
- **Working diff, no live PR:** return the classified findings as structured text to whoever
  ...
  anywhere.
- **Live PR:** post inline comments with `gh api` directly — an interactive session has no
  `mcp__github_inline_comment` tool, which exists only inside the claude-code-action environment the
  CI workflow runs in. Post every new fix instruction, escalation, and question as an inline PR
