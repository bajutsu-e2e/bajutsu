...
- **A leak in the backend-agnostic `Driver` abstraction** — the deterministic core taking on
  knowledge of a specific backend (idb / Playwright / Android). This is the seam prime directive 3
  protects; a leak here makes every future backend harder to add.
