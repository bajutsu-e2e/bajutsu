...
to tap, expressed as fractions of the image width and height in [0,1] (left edge x=0, \
right edge x=1, top y=0, bottom y=1).
...
LOCATOR_TOOL: list[dict[str, Any]] = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "x": {"type": "number", "description": "button center x as a fraction [0,1]"},
                "y": {"type": "number", "description": "button center y as a fraction [0,1]"},
...
def _decision_of(message: Any) -> AlertDecision:
...
class ClaudeAlertLocator:
    ...
    def locate(self, screenshot_png: bytes, instruction: str | None) -> AlertDecision:
        ...
        text = "Clear the blocking system prompt if one is present."
        if instruction:
