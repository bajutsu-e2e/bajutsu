...
Since BE-0159 every item lives directly under ``roadmaps/`` (the per-``Status`` folders are retired),
so the "which directories are items" walk is a single flat scan — :func:`iter_item_dirs` — shared
here too, replacing the per-script four-folder ``CATEGORIES`` loops the flat layout collapsed.
...
def iter_item_dirs(roadmap: Path) -> Iterator[Path]:
    ...
    flat level — so this is one scan. Yields both numbered and placeholder directories (callers narrow
    with :func:`numbered_match` / :func:`is_placeholder_dir`); non-item entries (the index pages,
    stray files) are skipped. Empty when ``roadmap`` is not a directory, so callers need no separate
    existence guard.
    """
