...
- **Cross-link resolution.** Every markdown link to an item's file must resolve to a file that
  ...
  top-level ``README*`` / ``CLAUDE.md`` (BE-0096). Since BE-0159 every item lives at a permanent
  flat ``roadmaps/BE-NNNN-<slug>/`` path, so promotion no longer moves a file and cannot rot a link;
  this still catches the plain mistakes a fixed layout doesn't prevent — a typo'd slug, a link to a
...
# Markdown outside roadmaps/ that links *into* it (BE-0096): every page under docs/, plus the
# repo-root README/CLAUDE files. Checked so a typo'd or stale item link there is caught too.
...
def _item_dirs(roadmap: Path) -> dict[str, Path]:
    """Map each item's ``BE-NNNN-slug`` directory name to its absolute path."""
