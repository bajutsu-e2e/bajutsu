...
def _parse_english(item_dir: Path) -> tuple[str, str, str | None]:
    ...
    Title and intro default to ``''`` when absent; Status is ``None`` if the file is unreadable or
    carries no ``Status`` field — those items are skipped, since there is nothing to track against.
    """
