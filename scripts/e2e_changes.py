...
def affected_jobs(changed_scenarios: Iterable[str], job_map: dict[str, set[str]]) -> set[str]:
    ...
    A scenario reused across jobs (``smoke.yaml``, declared by both ``smoke`` and ``bundled-runner``)
...
def _affected_or_fallback(changed: list[str], lane: str) -> list[str] | None:
    ...
    declared: set[str] = set().union(*job_map.values()) if job_map else set()
    if any(scenario not in declared for scenario in changed_scenarios):
        return None
