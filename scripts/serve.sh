...
# time (e.g. `no available actuator among ['idb']`, or `ModuleNotFoundError: playwright`).
...
# reinstalled.
#
# When a local `--config <file>` is passed, provision that config's actual backends — idb, web
# (Playwright), or both — so a web-target UI doesn't get an idb-only sync that prunes Playwright
# (and vice versa). Otherwise (no config, or a `github:`/`https:` spec install.sh can't resolve to
# a local file) fall back to the idb backend: serve's historical iOS-first default.
#
