...
セキュリティ上の問題は**非公開で**報告してください。公開の Issue や Pull Request は
作成せず、修正が用意できるまで詳細を公開しないようにお願いします。
...
報告は数日以内の受領連絡を目標とし（ベストエフォート）、修正の進捗を随時お知らせします。
...
- **API キー / シークレット。** `ANTHROPIC_API_KEY` が必要なのは AI パス（`record`、
  `run --dismiss-alerts`）だけです。API キーはコミットしたり共有したりせず、`.env`
  （gitignore 済み）に保管してください。決定的な `run`／CI ゲートにシークレットは不要です。
- **取得したエビデンス。** `runs/` 配下の実行成果物（スクリーンショット、page source、
  ログ）には、テスト対象アプリの機密情報が含まれることがあります。共有・Pull Request への
  添付・CI へのアップロードの前に内容を確認してください。
