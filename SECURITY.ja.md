...
- **`setClipboard` はデバイス全体で共有されるクリップボードに書き込みます。** ペースト操作の
  ために使う `setClipboard` シナリオステップは、対象のクリップボードへ直接値を書き込みます。
  iOS Simulator では `simctl pbcopy`、Android では Android のクリップボードプロバイダを使います。
  このクリップボードは、同じ Simulator やデバイスにアクセスできる他のプロセスからも読み取れます。
  シークレットやワンタイムパスコードをこの経路で書き込まないようにしてください。
