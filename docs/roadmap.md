...
| 機能 | 概要 | 優先度 | 状態 | 出典 / 関連 |
|---|---|---|---|---|
| `trace` / triage コマンド | 失敗証跡を AI が読み、原因要約・修正提案を出す（人間レビュー前提）。現状 CLI 未実装の DESIGN コンセプト | P2 | 💡 | [DESIGN §3.1 / §12](../DESIGN.md)、[architecture.md](architecture.md#implementation-status) |
...
| 機能 | 概要 | 優先度 | 状態 | 出典 / 関連 |
|---|---|---|---|---|
| doctor の実行可能性ゲート | 現状はスコアのみ。env / 権限 / ツール存在などの runnability チェックを追加 | P1 | 💡 | [configuration.md](configuration.md)、`bajutsu/doctor.py` |
