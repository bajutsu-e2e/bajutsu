...
| [`ci.yml`](../.github/workflows/ci.yml) (`web-e2e` job) | Linux | push to `main`, every PR | the **web (Playwright) backend** smoke — `playwright install chromium`, then `make -C demos/web e2e` runs the `demos/web` scenarios deterministically against a browser. **No Mac / Simulator**, so it proves the core is platform-neutral on the same gate |
