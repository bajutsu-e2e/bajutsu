...
| [`web-e2e.yml`](../.github/workflows/web-e2e.yml) | Linux | manual + every PR + merge queue (required `E2E (web)` check) | the **web (Playwright) backend** lane (BE-0279), three jobs against a headless Chromium: **smoke (playwright)** runs the `demos/web` scenarios (`make -C demos/web e2e`); **dogfood (serve UI)** drives Bajutsu's own serve SPA (BE-0058); **conformance (playwright)** runs the BE-0114 driver contract against the real browser. **No Mac / Simulator**, so it proves the core is platform-neutral. All three are deterministic and host-independent, so they are path-gated by a `changes` detector (`scripts/e2e_changes.py`, `E2E_LANE=web`) and feed a single always-reporting `E2E (web)` job — the required check; a PR that can't affect the web path skips the browser jobs and the gate passes |
...
- **Element-tree golden** — the iOS/Android `golden` jobs. Deterministic, but the tree is read through
  an upstream dependency (`idb_companion`, the on-device server) whose drift could redden it
  independently of any Bajutsu change. Excluded from the aggregators' `needs:`.
