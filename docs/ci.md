...
`make check` fails on the pull request that splits the next one. Two more guards close the rest of the
class. The shared core sweeps the two per-backend directories (`bajutsu/drivers/` and
...
or a deletion fails the gate instead of leaving a lane that stops running with no signal at all.
