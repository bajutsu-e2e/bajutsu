...
| [`android-e2e.yml`](../.github/workflows/android-e2e.yml) | Linux | manual + push to `main` / PRs touching the adb backend, the Android showcase, the shared scenarios, or deps | the **Android (adb) backend** smoke (BE-0208) — boots an x86_64 API 34 AVD under KVM (`reactivecircus/android-emulator-runner`), builds the Compose showcase APK, and runs the core id/tap/type/value scenarios through `--backend android` (`make -C demos/showcase/android e2e`). **No Mac / Simulator** — the third backend's Linux twin of the idb and web e2e lanes. Path-gated like `web-e2e.yml`, not a required check. The AVD is x86_64 (not the local validation's arm64) so KVM accelerates it on the x86_64 runner |
