...
hook step-for-step; every check except `actionlint`, `gitleaks`, and `apm` (three tools CI
installs separately, each step skipping with a notice when its tool is absent) runs identically on a
fresh clone via `uv` alone, which is what makes "green locally" predict "green in CI".
