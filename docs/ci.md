...
| [`ios-e2e.yml`](../.github/workflows/ios-e2e.yml) | macOS | manual + push to `main` / PRs touching the idb backend, the SwiftUI showcase, the shared scenarios, or deps | the **iOS visual-regression (VRT) lane** — the iOS twin of `android-e2e.yml`'s `e2e-visual`. Boots a Simulator, builds the SwiftUI showcase, and pixel-compares the Stable catalog against the committed `baselines_ios/` baseline over idb (`make -C demos/showcase e2e-visual`), masking the status bar + the "Liquid Glass" tab bar. Path-gated, **not a required check**: a pixel baseline is host-specific (the Simulator renderer varies by Xcode / device / OS), so a drift surfaces on this lane rather than blocking PRs, and the captured screenshot is uploaded (`ios-e2e-visual-run`) to re-record the baseline from. Deterministic — no LLM (the committed element-tree golden still runs on the weekly idb-monitor). |
