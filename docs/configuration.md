...
The AI paths — `record`, `triage --ai`, and the `--system-alert-handling` guard — reach the model through
...
- **Fail closed.** `record`, `triage --ai`, and an explicitly-requested `--system-alert-handling` exit with
