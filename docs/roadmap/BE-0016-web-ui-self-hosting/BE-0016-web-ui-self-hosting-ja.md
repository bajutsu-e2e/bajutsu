...
- **段階 A — 今日すぐ使える。** 既存の stdlib `bajutsu serve`（`bajutsu/serve.py`、すでに
  [BE-0011](../BE-0011-local-web-ui-serve/BE-0011-local-web-ui-serve-ja.md) として実装済み）に、
  運用設定（LaunchAgent・自動ログイン・Tailscale）を加えるだけで*今日*動くもの。新規コードは
  ほぼ不要です。
- **段階 B — 将来。**
  [BE-0015](../BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md) の将来の
  マルチテナント版を、各マネージドサービスをセルフホスト OSS（オープンソースソフトウェア）に
  置き換えて完全自前化したもの。BE-0015 で提案された（未実装の）コントロールプレーンに依存します。

項目全体としては提案ですが、その中で段階 A は今日すぐ使えるベースラインです。

## 動機

テスト基盤をマネージドクラウドに置けない・置きたくないチームがあります（コスト・データの所在・
ポリシーなどの理由）。そうしたチームは自前のハードウェアで Web UI を動かしたいと考えます。bajutsu の
...
[BE-0011](../BE-0011-local-web-ui-serve/BE-0011-local-web-ui-serve-ja.md) として実装済みです。この
段階では、**ほぼコード変更なし**でチームから安全に到達できるようにします —— 既存の `bajutsu serve` で
**今日すぐ動かせます**。
...
シナリオパス**を実行します（`bajutsu/serve.py` の `run_command`/`do_POST`）。
...
[BE-0015](../BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md) のアーキテクチャを
...
`app`（コントロールプレーン —— **未実装**、
[BE-0015](../BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md) 参照）・`postgres`・
...
[BE-0015](../BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md) のコントロールプレーンが
実在するようになってから移行します。

## 検討した代替案

- **`LaunchAgent` ではなく `LaunchDaemon`** —— 却下。daemon には GUI / Aqua セッションがなく、Simulator は
  それなしには動きません。ユーザ単位の `LaunchAgent`（＋自動ログイン＋caffeinate）は好みの問題ではなく必須です。
- **公開インターネットへの `0.0.0.0` バインド** —— 却下。`serve.py` は未認証でクライアント指定のシナリオパスを
  実行するため、公開バインドは未認証・任意パスの攻撃面になります。プライベートな tailnet（Tailscale）を使い、
  社内ホスト名が必要な場合のみ Caddy + Basic 認証を前段に置きます。
- **スキーマ別 / DB 別テナント vs 共有スキーマ + `org_id` + RLS** —— 自前運用では却下。テナント別の
  スキーマ/DB はマイグレーションと接続管理のオーバーヘッドを増やします。共有スキーマ + `org_id` + Postgres RLS の
  方が運用負荷をはるかに抑えつつ隔離を実現できます。
- **純 FIFO スケジューリング vs 重み付き公平スケジューリング** —— 却下。純 FIFO では 1 テナントが希少な Mac
  プールを独占できてしまいます。テナント別キューとクォータを尊重するラウンドロビンのディスパッチャにより、
  競合下でもプールを公平に保てます。

## 参考

`bajutsu/serve.py`、[cli.md](../../ja/cli.md#serve)、[ci.md](../../ja/ci.md)、
