...
**インライン限定の再利用（YAML アンカー／ファイルへのテンプレートエンジン適用）。** 不採用。アンカーは 1 ファイル内のテキストは複製できても、ファイルをまたげず、名前付き引数もきれいには扱えません。汎用テンプレートエンジンはシナリオファイルに任意のロジックを招き入れます。明示的な `params` 契約を持つ名前付きコンポーネントのほうが範囲が狭く、検証され（未知・不足の param は失敗する）、入力を自己文書化します。
