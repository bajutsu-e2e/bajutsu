...
1. **It needs no macOS and no device emulator.** A `BrowserContext` is the whole "device", so the
   backend runs on Linux and fits *inside* the current `make check` / CI gate ([ci](../../ci.md))
...
emulator**, so it fits the *existing* Linux `make check` / CI gate ([ci](../../ci.md)) from day one.
...
[drivers.md](../../drivers.md), [ci.md](../../ci.md), [concepts.md](../../concepts.md),
