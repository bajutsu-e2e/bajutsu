...
なしで Linux 上で動き、既存の `make check` / CI ゲートに収まり、能力モデルの大きい端を行使するからで ——
コアがプラットフォーム中立であることの最低コストの証明です。
...
1. **macOS もデバイスエミュレータも不要。** `BrowserContext` がそのまま「デバイス」なので、バックエンドは
   Linux 上で動き、動いた日から現行の `make check` / CI ゲート（[ci](../../ja/ci.md)）の *内側*に収まります。
...
項目にします。摩擦最小・到達範囲最大・コアが本当にプラットフォーム中立であることの最低コストの証明です。
...
idb と Android は小さい端（座標 actuation・モックネットワーク）、Playwright は大きい端（意味的・ネイティブ
...
*既存の* Linux `make check` / CI ゲート（[ci](../../ja/ci.md)）に動いた日から収まります。ネイティブ network +
video + 意味的操作が `capabilities()` の **大きい端**を行使します。摩擦最小・到達範囲最大・コアがプラットフォーム
中立であることの最低コストの証明です。Android（[BE-0007](../BE-0007-android-backend/BE-0007-android-backend-ja.md)）は
...
[drivers.md](../../ja/drivers.md)、[ci.md](../../ja/ci.md)、[concepts.md](../../ja/concepts.md)、
