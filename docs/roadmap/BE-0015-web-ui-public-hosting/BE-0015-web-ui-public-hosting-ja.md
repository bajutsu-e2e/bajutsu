...
ローカルの Web UI（`bajutsu serve`、[bajutsu/serve/](../../../bajutsu/serve/)）は Tier 1 の
オーサリング向け便利機能です。`127.0.0.1` にバインドし、認証はなく、同一ホスト上で `bajutsu run`
を subprocess 起動するだけです。つまり単一マシン・単一ユーザのツールであり、公開ホスティングは
ここにある3つの制約を取り除くためのものです。

- **共有できない。** シナリオのオーサリングも失敗の調査も共同作業ですが、run ツリー・ライブログ・
  レポートはすべて1台のノート PC に留まります。同僚のレポートを開く、進行中のオーサリングを見る、
  結果をレビューする、いずれもファイルを手でコピーしないとできません。
- **オーサリングする全員に設定済みの Mac が要る。** run は iOS Simulator を駆動し、Simulator は
  macOS 上にしか存在しません。そのため今は、設定済みの macOS マシンを持つ人しかオーサリングも run
  もできません。共有サービスなら、ブラウザさえあればマネージドな Mac プールに対して誰でも実行
  できます。
- **UI がホストに縛られている。** リクエストごとに配信マシン上で subprocess を起動するため
  （[jobs.py](../../../bajutsu/serve/jobs.py) の `run_job`）、1ホストを超えてスケールできず、ある
  ユーザの run を別のユーザから隔離することもできません。

公開ホスティングは、このノート PC ごとの便利機能を共有インフラへ変えます。ログイン済みユーザが
プロジェクトとシナリオを選び、マネージドな Mac プールで実行し、ライブログを見て、できたレポートを
URL で共有します。これはまさに*オーサリング体験*のトピックです。机ごとに macOS を要する前提を
取り除き、run とレポートを一級の共有可能な成果物にします。
...
その際、prime directives（[CLAUDE.md](../../../CLAUDE.md)）を侵してはいけません。`run` は完全に
決定論的なままで、合否判定の経路には大規模言語モデル（LLM: Large Language Model）を一切入れません。
決定論的ゲートは使い捨ての継続的インテグレーション（CI: Continuous Integration）ランナー上に置いた
ままにします。ホステッドの Mac プールが担うのは対話的オーサリングだけで、回帰テストの物量は担いません。
