...
| `--ios-tipkit-handling / --no-ios-tipkit-handling` | scenario › config › off | override every scenario's `iosTipKitHandling` — dismiss a blocking Apple **TipKit** tip, the framework-owned popover no author-written selector can name. iOS only (inert on other backends) and **off** unless asked for, since a tip is sometimes the assertion's own subject; omit and it resolves each scenario's value, then the target's `iosTipKitHandling` config, then off. Needs no credential ([scenarios](scenarios.md)) |
