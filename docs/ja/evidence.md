...
>
> **Android では、ターゲット側でも `targets.<name>` に `nativeZ: true` を書いて要求する必要があります。** 値を得るために、resident server は画面の読み取りごとに全ノードを走査します。走査にかかる時間は 20 ミリ秒から 100 ミリ秒で、アプリがビューを1つも opt-in していなくても変わりません（[BE-0407](../../roadmaps/BE-0407-step-latency-driver-internal-tuning/BE-0407-step-latency-driver-internal-tuning-ja.md) の作業単位 18）。`BajutsuZOrder.report(view)` を呼んでいても `nativeZ: true` を書かなければ、値は `null` になります。報告されない他の場合と同じ、正直な欠落です。iOS に対応するフラグはありません。レスポンダはアプリが BajutsuKit をリンクしているときだけ応答するので、opt-in していないターゲットが問い合わせの費用を払うことはないからです。
...
> 残ります（DESIGN §10）。どのステップでも、スクリーンショットは動作の**前後の両方**で撮りますが、
> ツリーを取得するのは動作のあとだけです（BE-0407）。動作の前に撮る `before.png` は、そのステップが
> 動作の対象とした画面を示し、動作のあとに撮る `after.png` と、そのとき取得する `elements` は、その動作が
> 残した画面を示します。前のステップの `after.png` から何も操作していなければ、`before.png` は新たに
> 撮り直さず、その画素をそのまま複製できます。割り込みの回復ステップ、`handleSystemAlert` ステップ、
> `interrupts` を宣言したシナリオのどのステップでも、複製せず常に新たに撮影します。`systemAlertHandling`
> のもとで動くどのステップでも同様です。非同期の割り込み画面がステップの間に現れていないという前提が、
> これらの場合には成り立たないためです。`systemAlertHandling` は既定でオンです。既定の設定のままの
> `run` はどのステップでも新たに撮影し、複製が効くのはシナリオかターゲット設定でガードを切った場合だけ
> です。スクリーンショットはどちらも
> `capture` の指定によらないため、
> `capture` を絞っても、ステップから 2 枚のスクリーンショットが失われることはありません。`elements.json`
> のファイル名は 1 つなので、動作後に取得したツリーがそのステップの記録として残ります。これは `after.png`
> が写す画面であり、ビューアが要素の枠を描く基準とする画面でもあります。
...
- **確認済みの開始時刻が、レポートのステップ/通信ログのタイムスタンプを、単に録画を要求した瞬間ではなく動画の実際の開始に合わせて補正します。** 本番の呼び出し箇所は`start_video`（iOS）と`start_screenrecord`（Android）に`confirm_started=True`を渡します。両者は録画の起動のあと、実在の信号を待ちます。iOS が待つのは、`simctl io recordVideo`が最初のフレームを処理した時点で自身の標準エラー出力へ書く`Recording started`の行です。Android はデバイス側のプロセスが現れることを確認します（より弱い保証です。プロセスが存在しても、そのエンコーダーがすでにフレームを出力しているとは限りませんが、それでも実在の信号であり、当て推量よりは早い時点を示します）。確認できた`time.monotonic()`の値は`Interval.true_start`に記録されます。`intervals.adopt`は、事前録画されたインターバルを移すときも`true_start`をそのまま引き継ぐため、`adopt`が動く前に確認済みだったAndroidの値が失われることはありません。web アクチュエータはポーリングせず、録画対象のページが生成された直後に`true_start`を記録します。`record_video_dir`はコンテキスト配下のページに対する録画を有効にするだけで、動画そのものはページが生成されるまで存在しないため、記録するタイミングは`new_context()`ではなく`new_page()`の直後です。待ちが一度も確認できなかった場合は`true_start`が`None`のままとなり、基準時刻は`scenario_start`に戻ります。当て推量の数値が入ることはありません。

  出力ファイルではなくレコーダー自身の通知を待つことは、iOS では動作の要になります。`simctl io recordVideo`は録画中に mp4 を書き進めません。ファイルは録画のあいだ 0 バイトのままで、確定処理のときに一括で書き出されます。以前の確認処理はそのファイルの成長をポーリングしていたため、成功しようがなく、シナリオごとに上限をまるごと消費していました。緑の run でも同じです。しかも iOS のレーンは、上限を超過しても正常な経路では損をしないという判断から、その値を 20 秒へ引き上げていました。`simctl`自身の`--help`が標準エラー出力の行を待つべき信号として案内しており、その行はおよそ 0.15 秒で届きます。

  この待ちをどれだけ続けられるかは、いまも調整できる値です。`simctl`と`adb`の起動のばらつきは、開発機よりも負荷の高い継続的インテグレーション（CI）のマシンで目に見えて大きくなり、待ちが諦めると、そのシナリオは補正を丸ごと失うからです。`BAJUTSU_VIDEO_START_TIMEOUT`（秒）が、コンパイル時のデフォルト値である5秒を置き換えます。すでに同じ仕組みを持つ3つの`BAJUTSU_XCUITEST_*`のタイムアウトと並ぶ扱いです。現在この値を上書きしているレーンはありません。
- **確定した録画そのものが原点を示し、その測定値が上の開始確認より優先されます。** `true_start`はいずれも**代理の信号**です。レコーダー自身の開始の通知、現れたデバイス側のプロセス、生成済みのブラウザページのいずれも、録画が最初のフレームを収めた瞬間からそれぞれ固有の距離だけ離れて届きます。レポートのシークは、その距離のぶんだけずれます。録画そのものが、この問いに答えられます。確定したクリップは自分の長さを申告し、`Interval.stop()`は録画が終わった瞬間を知っています。したがって、引き算がそのまま原点になります（`measured_start = ended_at - duration`）。長さは外部ツールを使わずコンテナから読みます。[`evidence/media.py`](../../bajutsu/common/evidence/media.py)が、`simctl`と`screenrecord`が書くムービーヘッダと、Playwright のレコーダーが書く Matroska のセグメントを読みます。
  ...
  この2つをまとめて抑えるのが`Interval.spawned_at`です。確認を必要としない唯一の時刻だからです。録画が最初のフレームを収める瞬間は、その起動時刻と`_ORIGIN_STARTUP_CEILING`との間にあります。この上限は、レコーダーが立ち上がるまでにかかりうる時間についての主張であり、レーンがどれだけ待つつもりでいるかとは別のものです。`BAJUTSU_VIDEO_START_TIMEOUT`を使わないのはそのためです。両者が同じ値だったころ、iOS のレーンが設定していた20秒がこの範囲を静かに広げ、どの録画も取り得ない原点まで受け入れていました。範囲の外にある原点は、2つの入力のどちらかがこの録画を表していないことを示すので、読み取り側が捨て、その録画は`true_start`による基準時刻を保ちます。
...
- **あるジェスチャの跡は、次のジェスチャが始まるまで残ります。** タイマーで消える仕組みは
  ...
  有効にした run のスクリーンショットは無効の run と異なるものになります。アプリ内の制御チャネルを
  運べる場合（後述）、`visual` アサーションはマーカーを隠した状態で自分の撮影を得ます。この場合に
  かぎって、フラグと画素の比較は同じ run に同居できます。運べない場合は、画素を比較する run では
  フラグを無効のままにしてください。
  Android にはこのチャネルがありません。シナリオを走らせるレーンは端末全体で OS の
  `show_touches` と `pointer_location` を有効化し、要素ツリーを比較するレーンと画素を比較する
  レーンにかぎって、その対を手動で無効に戻しています（`demos/showcase/android/Makefile`）。
...
ほかの種別はアクセシビリティツリー、通信の記録、クリップボードのいずれかを読みます。これらのレーンの
アプリは、いずれも今日時点では `-DBAJUTSU_ENABLE_CONTROL_CHANNEL` を付けてビルドしていません。そのため、
どのレーンであっても `visual` シナリオを加えると、ビルドにこの設定が入るまで失敗します（後述）。

マーカー自体は判定の経路の外にあります。ところが、マーカーを隠すチャネルは判定の経路の内側にあります。
チャネルが起動している run で、アプリが完了報告を返さないコマンドがあると、そのシナリオは失敗します。撮影が
済んだあとにマーカーを戻す 2 つ目のコマンドについても同じです。どちらの向きでも完了報告を待つのは、
意図した設計です。実行ループが、確認できていない状態を確認済みとして報告してはならないからです。
したがって `--touch-markers` が判定に影響しないのは、チャネルが起動していないあいだにかぎります。

判定でスクリーンショットを比較するシナリオは、その 1 回の撮影のあいだだけマーカーを隠します。
`visual` アサーションが読むのは、まさにマーカーが描き込まれた画像だからです。実行ループは動作中の
アプリにマーカーを下ろすよう求め、アプリからの完了報告を待ちます。その後でシャッターを切り、
2 つ目のコマンドでマーカーを戻します。マーカーはジェスチャに追従して動き、決まった領域に収まらないので、
マスクでは救えません。

この要求はアプリ内制御チャネル
（[BE-0365](../../roadmaps/BE-0365-in-app-control-channel/BE-0365-in-app-control-channel-ja.md)）を通ります。
チャネルの実体は、アプリがすでに報告を送っている collector 上のコマンドキューです。アプリは自分では
ソケットを開かず、collector の run ごとのトークンがコマンドも守ります。固定の時間だけ待つのではなく
アプリの完了報告を待つことが、この撮影を確実なものにしています。

チャネルを運べるのは `xcuitest` actuator だけで、しかもネットワーク収集が有効なときに限ります。
BajutsuKit のポーリングが動くのは、実際に動作する Simulator プロセスの内側だけだからです。
`fake` では collector は立ちますが、それをポーリングするものが何もありません。`playwright` はドライバ経由で
ネットワークを観測するので、この種の collector 自体がありません。`adb` は iOS と同じホスト側の
受け口へ報告するため、collector 自体は立ちます。ただし、端末側にはコマンドへ答えるポーリングが
ありません。チャネルを `xcuitest` に限っているのは、collector の形ではなく actuator の判定です。
`xcuitest` actuator とネットワーク収集の両方がそろったときにかぎって、`--touch-markers` は
判定がスクリーンショットを読むシナリオのチャネルを起動します。それ以外では、スクリーンショットを
比較するシナリオもこのチャネルが登場する前と同じ挙動のままで、マーカーを描きません。その分だけ、
調査者はそのシナリオのタッチの証跡を失います。このチャネルが取り除こうとしているのは、まさにその
トレードオフであり、運べる場面では取り除き、運べない場面ではこれまでどおり払うことになります。

`--backend` に候補を複数渡した run では、actuator の条件を二重に満たす必要があります。collector を
前もって立てるのは run 全体の actuator ですが、actuator の解決自体はシナリオごとに走るからです
（BE-0240）。`--backend web,ios` の run は collector を 1 つも立てません。この run のなかで
`xcuitest` へ切り替わったシナリオには、コマンドを運ぶ先がありません。そのため、そのシナリオも
マーカーを描かないままになります。

マーカーを描かないという扱いは、その扱いを受けたシナリオの内側にとどまります。あるシナリオが
チャネルを起動し、次のシナリオはマーカーなしのまま、という組み合わせも起こります。run は
シナリオごとに、そのシナリオ自身の起動環境でアプリを起動し直します。マーカーなしのシナリオは、
マーカーの環境変数が渡されなかったプロセスで走ります。同じ run の残りのシナリオは、変わらずマーカーを描きます。

チャネルが動く場合、アプリ側では二重に閉じられており、そのうちの 1 つはビルド設定です。`BajutsuKit` は
`-DBAJUTSU_ENABLE_CONTROL_CHANNEL` を渡してビルドしないかぎりチャネルをコンパイル対象から外します。
コンパイルされていても、起動環境に `BAJUTSU_CONTROL_CHANNEL=1` がなければチャネルは動きません。この値は
`--touch-markers` が与えます。ビルド設定なしで作られたアプリは何も応答せず、そのシナリオは
マーカーの写った画像を比較する代わりに、2 つの関門を挙げて失敗します。

run ループが読むのは起動環境の 2 つの値だけで、`--touch-markers` が組にした値なのか、シナリオが
自分で置いた値なのかを見分けられません。ここで言う起動環境は、ターゲット自身の `launchEnv` と
シナリオ自身の `launchEnv` を、起動処理と同じ順序で重ね合わせた値です。ターゲットがどちらかの値を
全シナリオに固定している場合も、実際に起動したアプリが受け取る値のとおりに判定します。この起動環境に
`BAJUTSU_CONTROL_CHANNEL=1` がすでに置かれているシナリオが、スクリーンショットを読む判定を持たない
こともあります。その組み合わせでは、`--touch-markers` はマーカーの値を与えません。マーカーの値を足すと
2 つの値がそろってしまい、ベースライン画像のディレクトリがある run で `expect` を走らせるシナリオ
なら、求めていないチャネルに入って失敗してしまうからです。そのためマーカーを描かないという扱いに
なり、run はその旨を標準エラー出力に書きます。
...
テスト対象アプリ自体がシナリオの途中で異常終了すると、失敗したステップのメッセージがその事象を名指し、
シナリオの証跡ディレクトリに `app-crash/` サブディレクトリが作られます。中身は、プラットフォーム自身が
そのクラッシュに対して書いたレポートです。これがないと、失敗は「存在するはずのセレクタが見つからない」という
`ElementNotFound` として読めてしまいます。調べる人は、`id` の改名と画面の未ロードという2つの誤った説明を
潰してからでないと、アプリ自体を疑うところまで辿り着けません。

これは診断であって判定ではありません。シナリオが失敗するのはステップが失敗したからであり、他のあらゆる
終端ステップ失敗とまったく同じ扱いです。ここにあるどの処理も合否を決めません（プライムディレクティブ1）。
クラッシュの分類は**ステップループの内側**で完結するので、シナリオはそのまま最後まで進みます。失敗した
ステップのスクリーンショットも、実行中の動画記録も、`after: on: error` のルールも、いつもどおり残ります。
バックエンドクラッシュの再試行ループ（前掲の `crash-diagnostics/`、BE-0421）が動くこともありません。あちらが
名指すのはテスト基盤側の障害であり、こちらが名指すのはテスト対象アプリ自体の不具合だからです。
...
この検査は**事後的**です。ドライバに問い合わせるのは、ステップ自身のアクション、待機、アサーションが
すでに失敗した後だけであり、定期的なポーリングはしません。ポーリングすると、構造上めったに起きない失敗を
捕まえるために、緑の run も含めた全ステップに往復が1回増えます。事後的な形でも、そのステップがすでに
失敗している以上、事象が起きたまさにそのステップで捕捉できます。

この形には2つの死角があり、どちらも意図して受け入れています。塞ぐにはシナリオ終端での無条件の問い合わせが
必要で、それはこの設計が避けている「緑の run すべてに乗るコスト」そのものだからです。

- シナリオの**最後**のステップが引き起こしたクラッシュのうち、そのステップ自身の操作が成功として報告される
  もの（アプリが死ぬ直前にランナーがタップを届けた場合など）は、問い合わせの対象になりません。`expect` も
  失敗以外の `after` ルールも宣言していないシナリオは、**緑のまま**になります。
- 同じ形でシナリオレベルの `expect` がある場合は、**赤になりますが分類されません**。`expect` はステップの
  結果ではなくアサーション結果を生むので、この検査に到達しません。読む人にはクラッシュではなく
  アサーションの不一致として見えます。

このシグナルを動かすために書くシナリオは、クラッシュのきっかけとなるステップの後にもう1ステップ置きます。
そうすれば、きっかけのステップではなく後続のステップが失敗し、問い合わせの対象になります。成功時または常時
実行される `after` ルールでも足ります。その後処理ステップ自身の結果が検査に到達するので、クラッシュを
見逃さず、1ステップ遅れて捕捉できます。

crawl も同じ証跡を `crashes/crash-NNN/app-crash/` に、そのクラッシュの再現シナリオと並べて記録します。
crawl 側の*検知*は従来どおりアクセシビリティツリーのヒューリスティックのままです。画面を覆うシステム
アラートで誤検知しうるので、取得のほうだけをドライバによる積極的な確認で条件付けています。確認できない
検知では、アラートの可能性があるものに時間制限付きの探索を払うことはせず、証跡なしでクラッシュだけを
記録します。Android のトムストーン層は `run` 専用です。必要な `adb root` が `adbd` を再起動するため、
walk 全体で1つの環境を持ち続ける crawl のレーンでは復旧できないからです。

書き出しはすべて、前掲のバックエンド側クラッシュ証跡と同じくマスキングを通る text 経路を使います。
クラッシュレポートは、クラッシュしたアプリが秘密情報を書き出しうるテキストだからです。
