...
> 撮り直さず、その画素をそのまま複製します。ただし、割り込みの回復ステップ、`handleSystemAlert`
> ステップ、`interrupts` を宣言したシナリオのどのステップ、そして `systemAlertHandling`（既定でオン）
> のもとで動くどのステップでも、この複製は行わず常に新たに撮影します。非同期の割り込み画面がステップの
> 間に現れていないという前提が、これらの場合には成り立たないためです。スクリーンショットはどちらも
> `capture` の指定によらないため、
