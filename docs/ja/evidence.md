...
>
> この 3 つはいずれも、各ステップがすでに記録している 2 枚のスクリーンショットに**上乗せ**して証跡を
> 指示するものです。したがって、どれかで `screenshot.after` を挙げても結果は変わらず、挙げなくても
> 失うものはありません。`before.png` と `after.png` は、3 つが何を要求したかによらず取得します
> （後述）。
...
> どちらも `capture` の指定によらないため、`capture` を絞ってもステップから 2 枚のスクリーンショットが
> 失われることはありません。`elements.json` のファイル名は 1 つなので、動作後の取得が動作前のツリーを
> 置き換えます。実行のあとに残るツリーは、動作が生んだ画面、すなわち `after.png` が写す画面と、
> ビューアが要素の枠を描く基準とする画面を表します。
