...
この書き出しがベストエフォートになるのは、証跡を失う場面だけです。タイムアウト時点の要素ツリーの読み取りに失敗した場合、または書き出し自体に失敗した場合は、診断を破棄し、その旨を `warning` で開示します。本来のタイムアウトをクラッシュで覆い隠すことはありません。どちらの場合も、run の失敗はそのタイムアウトのままです。
