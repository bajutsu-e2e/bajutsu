...
> 残ります（DESIGN §10）。スクリーンショットは、どのステップでも動作の**前後の両方**で取得します。
> 動作の前に取る `before.png` と `elements.json` は、そのステップが動作の対象とした画面を示し、
> 動作のあとに取る `after.png` は、その動作が残した画面を示します。どちらも `capture` の指定には
> よらないため、`capture` を絞ってもステップから 2 枚のスクリーンショットが失われることはありません。
