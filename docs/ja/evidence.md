...
ステップ前のベースライン取得と、ステップ後の取得の2回）。Sink は、1回の呼び出しでそのステップが
発生させるすべての種類を見られるとは限りません。複数回の呼び出しは互いに独立していません。
`FileSink` は各種類を固定名（`elements.json`、`before.png` / `after.png`）で書き出すため、後の
呼び出しが同じ種類を指定すると、先の呼び出しの内容を上書きします。
