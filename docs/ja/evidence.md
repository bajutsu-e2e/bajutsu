...
ステップ前のベースライン取得と、ステップ後の取得の2回）。そのため Sink は、1回の呼び出しでその
ステップが要求する `kind` のすべてを見られると前提してはいけません。複数回の呼び出しは互いに
独立してもいません。`FileSink` は `kind` ごとに固定名（`elements.json`、`before.png` / `after.png`）
...
この書き出しは、1点だけベストエフォートです。タイムアウト時点の要素ツリーの読み取り、または書き出し自体が失敗した場合、診断は落とされ、クラッシュで本来のタイムアウトを覆い隠す代わりに `warning` で開示します。どちらの場合も、run の失敗はそのタイムアウトのままです。
