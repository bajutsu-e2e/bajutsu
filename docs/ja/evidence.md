...
`capture` は同じ `step_id` に対して、1ステップの中で複数回呼び出されることがあります（たとえば、
ステップ前のベースライン取得と、ステップ後の取得）。そのため Sink は、1回の呼び出しでその
ステップが要求する `kind` のすべてを見られると前提してはいけません。また、複数回の呼び出しは互いに
独立していません。`FileSink` は `kind` ごとに固定名（`elements.json`、`before.png` / `after.png`）
で書き出すため、後の呼び出しが先の呼び出しと同じ `kind` を指定すると、先の内容を上書きします。
...
ベストエフォートなのは証跡だけで、判定は失いません。タイムアウト時点の要素ツリーの読み取りに失敗した場合に失うのは `elements` の項目だけで、`readiness` と `trace`、`provenance` を残したままファイルを書き出します。ファイル全体を失うのは、書き出し自体に失敗した場合です。どちらの損失も `warning` で開示し、本来のタイムアウトをクラッシュで置き換えることはありません。run の失敗は、いずれの場合もそのタイムアウトのままです。
