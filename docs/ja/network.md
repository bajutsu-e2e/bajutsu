...
`bodyMatches` が照合するのは**報告されたボディ**です。アプリ側の SDK が報告するのは各ボディの先頭
64 KB までに限られます。この機能が対象とする表明にはそれで足りますし、上限を設けない場合は試験対象
アプリが全ペイロードの複製を保持することになり、実測でシナリオあたり数ギガバイトに達しました。切り
詰めたボディには元の大きさを併記する（`requestBodyBytes` / `responseBodyBytes`）ので、報告が完全で
ないのに完全に見えることはありません。照合はこの範囲の中身に対して書いてください。
