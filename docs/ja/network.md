...
の URL はそのまま解決します）。同じポート番号を**両側**でバインドできなければならないため、コレクタは
ポートを OS に選ばせず、小さな予約帯から取ります。OS が選ぶポートは、ホストのエフェメラルポート範囲
（ephemeral range）に属します。エミュレータが自身のソケットへ割り当てる範囲も同じなので、ゲストが
すでに握っているポートに時折ぶつかり、橋渡しのバインドが失敗します。コレクタ、トークン検査、
アサーションパイプラインは同一です。
