...
| `--runs-dir` | `runs` | run ツリーを書き出すディレクトリ。作業ディレクトリと出力先を分けられる。`serve` はアップロードされたバンドルを展開先で走らせつつ、その run を `serve` のストアに残すためにこれを使う（[BE-0073](../../roadmaps/implemented/BE-0073-serve-zip-bundle-upload/BE-0073-serve-zip-bundle-upload-ja.md)） |
...
- **`--html <path>`** を渡すと、同じ数値を **自己完結した HTML レポート**にも書き出します（CSS は埋め込み、JavaScript も外部アセットも無し。ディスクから直接開けます）。次元ごとにカバレッジバーを描き、gap と off-namespace の一覧を目立たせます。エンドポイントと観測 id のセクションは `--runs` を渡したときだけ描画します。テキスト（または `--json`）の出力は変わらず、書き出し先は標準エラー出力で知らせます。
...
アプリを探索して画面マップをリアルタイムに描き（`python -m bajutsu crawl ...`）、**Upload** はブラウザから
持ち込んだ config・シナリオ・ビルド済みアプリバイナリの `.zip` バンドルを実行します（展開したツリーに対する
`python -m bajutsu run ...`。[BE-0073](../../roadmaps/implemented/BE-0073-serve-zip-bundle-upload/BE-0073-serve-zip-bundle-upload-ja.md)）。リクエストごとに CLI を
...
- **Upload** タブは、ホストのファイルシステムに触れないブラウザ利用者が、ホスト型の `serve` に**自分のスイートを
  持ち込む**ためのものです（[BE-0073](../../roadmaps/implemented/BE-0073-serve-zip-bundle-upload/BE-0073-serve-zip-bundle-upload-ja.md)）。
  レイアウトが動くローカル checkout そのものの `.zip`、つまり `bajutsu.config.yaml`・その `scenarios` ツリー・
  config の `appPath` が指すビルド済みアプリバイナリ（`.app` ディレクトリ・zip 化した `.app`・`.ipa`）を投下し、
  バンドルが見せるターゲットとシナリオを選んで **Run** を押します。`serve` はそれを封じ込めたサンドボックス（`runs/` の
  兄弟で、`--root` ではない）へ展開し、`bajutsu run` をその展開先から走らせ（config の相対パスがバンドルを基準に
  解決される）、`--runs-dir` を `serve` のストアへ向けます。そして**展開先は実行後に削除します**。アップロードされた
  ものは残りません。アップロードのファイル名と zip の **sha256** は run の `manifest.json`（`provenance`）に記録するので、
  「この run は何を実行したか」を後から辿れます。バンドルは**秘匿情報を運びません**。`${secrets.*}` は通常の run と
  同じく `serve` ホストの環境から解決します（[BE-0032](../../roadmaps/implemented/BE-0032-secret-variables/BE-0032-secret-variables-ja.md)）。
  展開は zip-slip（絶対パス・`..`・symlink エントリは拒否）と zip-bomb（エントリ数・展開後の総サイズ・エントリごとの
  圧縮率の上限）に対して堅牢で、エンドポイントは他のリクエストと同じトークン認証の裏にあります。任意のバイナリを
  アップロードして実行することは、認証済みの単一 Mac の `serve` でのみ公開されます。
