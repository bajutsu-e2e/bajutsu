...
行はつねに画面の一番上に固定され、各コマンドへの応答は下のスクロール可能な領域に積み重なります。入力行の
すぐ下の行には常にバナーが表示され、現在のモードと、`Tab` でモードを切り替えられることを示します。この表を
確認しなくても、キー割り当てが画面上で常に見える状態になります。

| キー | 入力行での動作 | スクロール領域での動作 |
|---|---|---|
| `Tab` | スクロール領域に切り替えます | 入力行に戻ります |
| `↑` / `↓` | 直前や直後に入力したコマンドをヒストリから呼び出します | 出力を 1 行ずつ上下にスクロールします |
| `k` / `j` | 動作しません | 出力を 1 行ずつ上下にスクロールします（vim 風、`↑`/`↓` と同じ動作） |
