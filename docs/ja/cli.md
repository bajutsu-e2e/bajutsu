...
代替する手段です。この領域はどのモードでも描画され続けるため、スクロール領域を見ているあいだや、下端まで
`Tab` で戻らずに次のコマンドを入力しているあいだに、コマンドの応答が画面の外に届くことがあります。そうなった
瞬間にバナーへ `[new output ↓]` が付き（そのときのモードの文言はそのまま）、表示が下端に戻ると消えます。
