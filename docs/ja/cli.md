...
| `--alert-poll-interval` | 未設定 | ネイティブのシステムアラート presence 照会の間隔（秒）。シナリオ自身の `pollInterval` の下位、ターゲットの上位に位置します。ガードのもう 1 つのキーである `rules` にフラグはありません。1 エントリはプロンプトと選択の組であり、フラグの値 1 つでは読みやすく運べないため、宣言はシナリオファイルとターゲット config に置いたままにしています（[BE-0406](../../roadmaps/BE-0406-system-alert-declared-prompts/BE-0406-system-alert-declared-prompts-ja.md)） |
...
| `--browser` | アプリの `browser`（既定 chromium） | web backend: この run で使う Playwright のレンダリングエンジン。`chromium` / `firefox` / `webkit` から選びます。省略時はターゲットの `browser` config に従います（[configuration](configuration.md)、[BE-0076](../../roadmaps/BE-0076-web-cross-browser-engines/BE-0076-web-cross-browser-engines-ja.md)） |
| `--browsers` | "" | web backend: **クロスブラウザマトリクス**を実行します。エンジンのカンマ区切りリスト（例 `chromium,firefox,webkit`）を指定すると、各シナリオがエンジンごとに1回ずつ走り、全エンジンが合格して初めて run 全体が緑になります（[configuration](configuration.md#クロスブラウザマトリクス--browsersbe-0076)）。エンジンを1つだけ指定するのは `--browser` と同じです |
...
- **履歴は OS バージョンごとに分かれます**（[BE-0358](../../roadmaps/BE-0358-device-os-as-a-first-class-fact/BE-0358-device-os-as-a-first-class-fact-ja.md)）。グループのキーには、その run が動いたデバイスの OS バージョンも入ります。値は、各 `manifest.json` がシナリオごとに記録している `device_runtime` のラベルを解析したものです。1 つのシナリオを iOS 18.6 と iOS 26.5 で走らせれば、履歴は 2 つに分かれます。それぞれが自分の証跡だけで分類されるので、バージョンが違うから判定が違う場合に報告されるのは、フレーキネスではなく、その OS 上で再現する差異です。同じバージョンの 2 通りの綴り（`iOS 18.6` と `iOS 18.6.1`）は同じ OS として扱うため、パッチリリースをまたいで履歴が割れることはありません。ラベルのない run や解釈できない run（web の run と、ラベルを読む前に記録された run）は、どのバージョンの履歴とも合流せず、`unknown OS` としてまとまります。出力は、その履歴が何件あるかを述べます。
...
- 参照している id とは、シナリオが指定する `id` / `idMatches` のすべてです。ステップ、入れ子の制御フロー、`within` スコープ、アサーションを横断して集めます。run が後から束縛するプレースホルダは含みません。components（`params.*`）とデータ行（`row.*`）はマップを組み立てる前に展開されており、展開後に残る `${vars.*}` や `${secrets.*}` のトークンはどの要素も指していないので、マップはトークンの文字列を id として数えず対象から外します。
...
**テスト影響分析**です。ソースの変更から、その変更がどのシナリオステップに影響しうるかを報告します（AI 非依存。[BE-0321](../../roadmaps/BE-0321-test-impact-analysis/BE-0321-test-impact-analysis-ja.md)）。`coverage` の逆にあたります。`coverage` がスイートを前向きに辿って行使するアプリ面を見るのに対し、`impact` は同じ静的解析を反転し、各[安定した id](glossary.md#シナリオのオーサリング)、画面、アサート対象のエンドポイントから、それを参照する `(scenario, step)` の組への対応表を作ります。そのうえで変更を `git` の diff として読み、開発者が見直すべきステップを報告します。開発者は push 前に危険なステップだけを把握でき、CI パイプラインは素早いフィードバックのためにそれらを先に流せます。

```bash
bajutsu impact --target <name> [--config ...] [--range HEAD~1..HEAD] [--diff <path>|-] [--repo <dir>] [--json]
```

- **変更は `git` の diff です。** `--range` は diff を取るリビジョン範囲を指定します（既定の `HEAD` は作業ツリーを `HEAD` と比較します）。`--repo` は対象のリポジトリを選びます（既定は現在のディレクトリ）。あるいは `--diff <path>` はファイルから、`--diff -` は標準入力から unified diff を読みます。すでに diff を持っている CI パイプラインが渡す形です。作業ツリーモード（`..` を含まない範囲）では、追跡されていない新規ファイルも取り込みます。参照する id を求めて読み込み、バイナリなら帰属不能として印を付けるので、新しい画面が黙って見落とされません。
- **一致は単純な文字列検索です。** スイートがすでに宣言している[安定した id](glossary.md#シナリオのオーサリング)（および画面名やエンドポイントのパス）を対象に、その id のリテラルが diff の追加行または削除行に現れれば、その id は*触れられた*とみなします。言語ごとの構文解析を必要とせず、アプリ固有の知識も埋め込まないので、Swift でも Kotlin でも TypeScript でも同じように働きます。
- 影響を受ける `(scenario, step)` の組を、それを引き当てた触れられたリテラルとともに（*どの*ステップかだけでなく*なぜ*かがわかるように）、テキスト、または CI が消費する `--json` で**報告します**。
- **健全性には限界があり、その限界を隠さず示します。** 走査が保証できない変更は*帰属不能*です。参照されるリテラルを一つも編集しない変更（ハンドラのロジックや共有ヘルパーなど）と、文字列一致では中身を見られない変更（バイナリファイル）です。変更がそうした部分を含むとき、レポートは**不完全**と印を付け、帰属不能なファイルを列挙します。CI で絞り込むなら、このシグナルを受けて**フルスイートへ退避**しなければなりません（`--json` の `complete` が `false` になります）。過剰選択が安全な方向です。短い id や一般的な id は影響集合を広げうるものの、影響を受けるステップが黙って飛ばされることはありません。
- **CI での安全な使い方。** 影響集合は*仕組み*であり、方針はチームが選びます。安全な既定は加算的です。スイート全体は流したまま、影響を受けるステップを先に並べる、あるいは人にフラグとして示します。マージ前の run を影響集合まで絞り込むのは opt-in で、健全なのは2つの安全策があるときに限ります。上記の**保守的な退避**と、**フルスイートの安全 cadence**（決定的なスイート全体を、main ブランチへのマージ時、夜間、あるいはリリース時に依然として流すこと）です。これにより、走査が取りこぼしたステップも出荷前に捕まえられます。いずれの場合も合否の判定は決定的なランナーに残ります。
- **助言的かつ read-only** です。シナリオを実行も編集もせず、**CI ゲートにもなりません**。**影響集合が何であっても終了 0**（config / scenarios ディレクトリが無い、シナリオが読めない、または `git` / diff の読み取りに失敗したときだけ終了 2）です。どの段階でもモデルは使いません。
...
- シナリオの系列は BE-0049 の `(scenarioHash, name)` という同一性に、その run が動いたデバイスの OS バージョンを加えて束ねます（[BE-0358](../../roadmaps/BE-0358-device-os-as-a-first-class-fact/BE-0358-device-os-as-a-first-class-fact-ja.md)）。1 つの OS 上で指紋が一定のまま判定が反転すれば、真の flaky です。シナリオを編集するか、同じシナリオを別の OS バージョンで走らせれば、新しい系列が始まります。各系列は自分の OS を明示し、所要時間もその系列の中で平均します。2 つの OS バージョンの計測値が、どちらも実際には走っていない 1 つの平均に混ざることはありません。`provenance.scenarioHash` を持たない run は run レベルの傾向には数えますが、シナリオの系列には加われません（スキップとして報告します）。
...
完了した run を1つの可搬な `.zip` にまとめます。`report.html` に加えて `manifest.json`、`junit.xml`、`ctrf.json`（[reporting](reporting.md#ctrfjson)）、実行した `scenario.yaml`、**すべての**証跡（スクリーンショット、動画、`network.json` …）を含みます（[BE-0060](../../roadmaps/BE-0060-run-report-zip-export/BE-0060-run-report-zip-export-ja.md)）。`runs/<id>/` のツリー全体を単一の `<id>/` フォルダ直下に収めるので、`report.html` の**相対**リンクがオフラインで解決します。ダブルクリックで開け、サーバは要りません。
...
完了した run の `report.html` を**保存済みデータ**から、**現行の**テンプレで再描画します（`junit.xml` と `ctrf.json`〈[reporting](reporting.md#ctrfjson)〉も再出力）。デバイスも LLM も使わず、再実行もしません（[BE-0068](../../roadmaps/BE-0068-regenerable-reports/BE-0068-regenerable-reports-ja.md)）。テンプレ改善やレンダリングのバグ修正が、再実行せずに過去の run にも届きます。合否は保存済みモデルから読むだけで、再計算しません。
...
| `--system-alert-handling` | on | オーサリング中のプロンプトを片付ける（要 API キー） |
| `--max-steps` | `30` | オーサリングのターン数（＝トークン消費の上限）に上限を設けます（[BE-0194](../../roadmaps/BE-0194-record-turn-payload-diet/BE-0194-record-turn-payload-diet-ja.md)） |
...
語彙に届くようになりました。加えて、`tap` だけはセレクタを一切経由しない生の座標も受け付けます。
...
`#<digits>` で終わる id や label、あるいは `@` や `label:` で始まる id や label には、この方法では届きません。
実際の値は `tree` や `find` で確かめてください。これは既知の制約です。
...
行はつねに画面の一番上に固定され、各コマンドへの応答は下のスクロール可能な領域に積み重なります。
...
は、1 行ずつ読むプレーンなシェルにフォールバックします。上のコマンドはどちらの形でも同じように動作し、`exit`
/ `quit` はどちらの形でもシェルを抜けます。Ctrl-D はプレーンなフォールバックでのみ同じ働きをします。curses
...
- オーサリング（Record と Crawl）の **AI プロバイダ**は **Settings → AI プロバイダ** の一箇所で選びます。
  ...
  **Anthropic CLI**（`ant`。Pro / Max / Console のシートに対するブラウザ経由の OAuth（SSO）サインイン。
  BE-0163。専用の `/api/ant/login` ルートを持ちます）、**Claude Code CLI**（`claude-code`。Claude Code
  の Pro / Max / Console シートに課金します。BE-0176。ヘッダレスなホストで対話的な `claude setup-token`
  が実行できない場合に備え、長期間有効な OAuth トークンを write-once でパネルに保持します。
...
- **ホスティング向けフラグ（応用）。** `--emit-launchagent` は `serve` を単一 Mac 上でトークン認証付きの LaunchAgent として動かす launchd plist を出力します。`--backend server`（と `--asgi`）はホスティング用の FastAPI コントロールプレーンに切り替えます。`--upload-exec`（または環境変数 `BAJUTSU_UPLOAD_EXEC`）は、アップロードされたバンドルの `launchServer` コマンドを実行してよいかを決めます（`deny` / `reuse` / `sandbox`、既定は `sandbox`）。`--allow-remote-build`（または環境変数 `BAJUTSU_ALLOW_REMOTE_BUILD`）は、UI で bind した **Git** ソースの config の `build` コマンドをホスト上で実行することを許可します（既定はオフ）。いずれも [self-hosting](self-hosting.md) で扱います。
...
## 実行時の診断（`BAJUTSU_LOG_LEVEL`）

`BAJUTSU_LOG_LEVEL` は、実行が自身についてどれだけ語るかを決めます。既定は `WARNING` で、この
つまみが無かった頃と同じ出力です。診断を有効にするのは常に意図的な操作になります。

```bash
BAJUTSU_LOG_LEVEL=DEBUG uv run bajutsu run --scenario …
```

`DEBUG` が要るのは、ステップが失敗したのにアサーションの文言では理由が分からないときです。つまり
タイミングの失敗で、問うべきなのはアサーションが何と言ったかではなく、アクチュエータがどのツリー
から解決したかです。adb バックエンドでは、各読み取りとそのデバイスイベントマーク、読み取り遅延の
バリアがいつ張られ何によって閉じたか、ツリーがどう落ち着いたか、各セレクタが解決したフレーム、
そしてジェスチャがデバイスへ渡ったか座標注入へ縮退したかを記録します。これらを並べて読むと、
タップが実際にどこへ落ちたのか、その下の画面が追いついていたのかが分かります。

このうち 2 つは debug ではなく警告です。どちらも、ドライバが保証できない画面に対してアクチュエータ
がこれから解決しようとしていることを意味するからです。読み取り遅延のバリアが予算を使い切った場合と、
