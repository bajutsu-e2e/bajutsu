...
- 無効化された操作要素（`notEnabled`）はタップせず、画面ごとに `blocked` として報告します。遷移を洗い出すため、
  ...
  **`--guide ai`** はパイプラインで動きます——まず画面を決定的に検査し、その操作を Claude に渡して**組合せ**を
  ...
  `--guide ai` は**アクセシビリティツリーで指定できないタブバー**（`tab` トレイトを持たない画像だけの独自バーなど）
  にも対応します——ツリーにタブが現れないときは、システムアラートガードと同じ要領で画像認識（vision）から位置を割り出し、
  各タブを座標でタップします（タブを優先して切り替えてから深掘りする方針は同じ）。
