...
1つのファイルの**中だけ**で使い回すステップ列に、専用のファイルは要りません。シナリオファイルの先頭の `components:` に宣言し、下のシナリオから名前で呼び出せます（[BE-0422](../../roadmaps/BE-0422-inline-scenario-components/BE-0422-inline-scenario-components.md)）。コンポーネントは、それを使うシナリオの隣に並びます。1つのファイルしか使わない名前で、スイートのディレクトリ一覧が増えることもありません。
