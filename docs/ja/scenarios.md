...
```yaml
- setPickerValue: { sel: { id: form.school }, value: "大学" }   # ホイールを「大学」の行へ動かす
```
...
複数コンポーネントのピッカー（年のホイールと月のホイールが並ぶもの）は、コンポーネントごとに独立した `pickerWheel` 要素として現れます。`sel` は常にそのうちの 1 つを指します。区別には、どのセレクタも持っている `within` / `traits` / `index` を使い、コンポーネントごとに 1 ステップずつ書きます。
