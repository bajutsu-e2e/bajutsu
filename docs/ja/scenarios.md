...
（[`app/sample/scenarios/smoke.yaml`](../../app/sample/scenarios/smoke.yaml) 実物）
...
（[`app/sample/scenarios/permission.yaml`](../../app/sample/scenarios/permission.yaml) 実物）
...
（[`app/sample/scenarios/gestures.yaml`](../../app/sample/scenarios/gestures.yaml) 実物）
...
> （[`app/sample/scenarios/network_mock.yaml`](../../app/sample/scenarios/network_mock.yaml) 実物）
...
モックは `BAJUTSU_MOCKS` env で BajutsuKit に渡される（`dump_mocks`, `scenario.py:638`）。形式的な形は
...
`expand_components`（`scenario.py:474`）が run の前にコンポーネントの置換済みステップへ置き換える
...
`--exclude` が `--tag` に優先する（`select_scenarios`, `scenario.py:560`）。両フラグはカンマ区切り可。
...
シークレットの環境変数名を config で宣言する（`secrets: [API_TOKEN, ...]`）。宣言した各名 `X` は
環境から解決され、**アクション時**に実行ステップへ `${secrets.X}` として置換される。シナリオファイルは
**トークン**を保ち値は持たず、リテラル値は証跡で **自動マスク**される —— だからシークレットは安全に
コミット・レビューできる。`${params.*}` / `${row.*}`（ロード時展開）と異なり、この名前空間は
run ループが解決する。
...
> `vars.*`（UI 値を実行時に取り込む）は **未実装** —— `${...}` プリミティブは対応できるが、run ループは
> `vars.*` を束縛しない。
