...
`scroll` は画面外の要素を表示領域に入れます。1 ステップ分スクロールしてツリーを再クエリし、`to` が解決してそのフレームの**中心**が画面内に入った時点で止まります。中心は続く `tap` が狙う点なので、ビューポートより高い要素でも、中心が入りさえすれば成功します。`maxScrolls` 分（既定 15）を使い切ったとき、またはスクロールしても対象領域が変化せず、再読み取りでも変化がないとき、決定論的に失敗します。領域が末尾に達して対象がそこにないということなので、`to` の打ち間違いは上限まで回さずに早く失敗します。再読み取りが効くのは Android です。Android ではジェスチャがリストを動かした後にアクセシビリティツリーが公開されるため、1 回だけ読み取ったツリーはスクロール前の画面を表し、末尾に達した状態と同じに見えます。確認にかかる時間は、Android で `scroll` が失敗するときの最後のステップで数秒です。読み取りが遅れない web と iOS では、時間がかかりません。`within` はジェスチャと末尾判定を 1 つのスクロール可能なコンテナに限定します。省略すると画面全体をスクロールします。
