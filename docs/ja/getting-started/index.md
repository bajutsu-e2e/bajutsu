...
`run` / `doctor` / `record` / `crawl` / `repl` / `codegen` / `trace` / `triage` / `approve` / `serve` / `mcp` / `worker` / `lint` / `schema` / `audit` / `coverage` / `export` / `flakiness` / `impact` / `report` / `stats` のコマンドが表示されるはずです
