...
> `demos/features/app/` の小さな自己完結 SwiftUI アプリ。Bajutsu の **全プリミティブ**（全ステップ種別・全
...
> オンボーディング → ログインの認証フロー越しに網羅するテスト用フィクスチャ。
>
> 詳細は [`demos/features/app/README.md`](../../demos/features/app/README.md)。ここでは Bajutsu 機能との対応を要約する。
...
- シナリオ例は [`demos/features/app/scenarios/`](../../demos/features/app/scenarios)。
...
`bajutsusample://async` / `bajutsusample://system`（いずれもログイン済みで開く）。`network` タブだけは
deeplink を持たない（`SAMPLE_TAB=network` で開く）。
...
| Gestures (`SAMPLE_TAB=gestures`) | `gest.doubletap`（+ `gest.doubletap.value`） / `gest.pinch`（+ `.value`） / `gest.rotate`（+ `.value`） — double-tap は idb で駆動可。pinch / rotate は真のマルチタッチが要るため実機経路は生成 XCUITest |
...
| Network (`SAMPLE_TAB=network`) | `net.fetch` / `net.get-query` / `net.post`（redaction 用に秘匿ヘッダ + body を載せる） / `net.delete` ・ `net.status`（value） ・ `net.captured.*`（method / status / duration / url を BajutsuKit から読み戻し） — BajutsuKit + `BAJUTSU_COLLECTOR` が必要 |
...
`make -C demos/features ui-test` は **codegen 経由**の経路を回す: シナリオから XCUITest を生成し、xcodebuild で実行する
