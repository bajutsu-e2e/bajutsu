...
| **中** | `sonnet` | 中 | ロードマップの発想と起草（`ideation`）、技術文書執筆と翻訳のレビュー（`english-tech-writing`・`japanese-tech-writing`）、PR レビュー |
...
- [`tech-writing`](../../.claude/skills/tech-writing/SKILL.md)：`sonnet`（中）
- [`english-tech-writing`](../../.claude/skills/english-tech-writing/SKILL.md)：`sonnet`（中）
...
矛盾するためです。指摘は再実行に分散させず、一度のパスですべて挙げることを目指します。
...
- **[`tech-writing`](../../.claude/skills/tech-writing/) スキルに従う。** ここのすべての
  ...
  空ける、主語と述語を近づける、能動態を選ぶ、冗語を削る）を定めます。書いたあとではなく、書く前・
  推敲する前に呼び出します。このスキルは、英語と日本語それぞれの言語レイヤーの上位に立つ傘です。
  英語の散文には [`english-tech-writing`](../../.claude/skills/english-tech-writing/) を併せて適用します
  （シリアルコンマ、*that* / *which*、ダッシュ、数の表記といった英語固有の作法）。日本語の散文には
  下記の [`japanese-tech-writing`](../../.claude/skills/japanese-tech-writing/) を適用します。以下の
