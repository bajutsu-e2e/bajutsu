...
実行結果を開かないと意味の通じない、単語 1 語だけの名前（`docs`、`build`、`deploy`）は残しません。模範例は `e2e.yml` と `swift.yml` です（BE-0122）。
...
そして `main` のブランチ保護ルールセットは、このうちいくつかを文字列そのままで固定しています。`check`（`ci.yml`）、`E2E`（`e2e.yml`）、`require two approvals for BE proposals`（`roadmap-proposal-approvals.yml`）の 3 つです。
