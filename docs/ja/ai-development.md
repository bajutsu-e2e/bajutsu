...
`## 詳細設計` / `## 検討した代替案` / `## 参考`。埋められる範囲だけ記入し、不明は `TBD` とします。**Author は
GitHub のアカウント名で明記**します —— `* Author: [@handle](https://github.com/handle)`。最初にその項目を
...
`roadmaps/implemented/` へ（同じ ID・slug のまま）**移動**し、インデックスも再生成します（ローカルで行うなら
...
ことはありません。マイルストーン M1–M4 は `BE-0001`–`BE-0004`（可決・実装済み）です。
