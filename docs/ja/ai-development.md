...
> 始めてください。** 最初の提案と最初の実装を手を動かしながら辿る walkthrough です。このページは、その規則
...
| `conformance` | 実バックエンド上のドライバ契約（BE-0114） | ✓ | *欠落* | ✓ |
| `codegen` / `gestures` | ネイティブテスト出力 / マルチタッチ（idb は不可） | ✓ | — | — |
| `fallback` | resident と `uiautomator dump` の読み取り経路が一致するか（BE-0245） | — | ✓（ステップ） | — |

この構成を健全に保つ規則が 2 つあります。**必須レーンは 1 つだけです。** 必須ステータスチェックはルールセットが固定するジョブの `name:`（前述）であり、E2E レーンでそこに含まれるのは iOS の `E2E` 集約ジョブだけです。したがって、それがゲートする iOS のジョブは `needs:` で配線し、Android と Web は `paths:` フィルタだけでゲートされる非必須レーンです。**ホスト依存や upstream に脆いチェックは必須ゲートから外します。** `visual` はレンダラによってベースラインが変わるピクセル比較であり、element ツリーの `golden` は制御外の upstream `idb_companion` に対して走ります。どちらも PR ごとにシグナルとして走らせますが、`E2E` ゲートの `needs:` からは除外するので、ドリフトはマージを止めずに表れます。表の空欄（Android に実機 conformance がないこと）は、その項目自体で埋めるべき実在のカバレッジの穴であり、列を落とす理由ではありません。
...
アイデアを出荷可能なコードにする作業は、3 つのスキルが作る三角形を通ります。起草するか、出荷するか、
その両方かです。
...
ルール、BE ID のライフサイクルです。おかげで、汎用のレビュアーには拾えないものを拾えます。
...
移ります）。ディレクトリは移動しません（BE-0159）。同じ `roadmaps/BE-NNNN-<slug>/` のパスがその項目を生涯
...
- **[`tech-writing`](../../.claude/skills/tech-writing/) スキルに従う。** ここのすべての
  ドキュメントとすべての BE ロードマップ項目に対する、両言語の正式な散文規範です。言語に依存しない
  執筆技法（上から下へ推敲する、主眼を冒頭で述べる、各文の文末を最も重要な要素のために空ける、主語と
  述語を近づける、能動態を選ぶ、冗語を削る）と、英語固有の作法（シリアルコンマ、*that* / *which*、
  ダッシュ、数の表記）を定めます。書いたあとではなく、書く前・推敲する前に呼び出します。以下のルールは、
  この節とそのスキルが共有する具体的な期待です。日本語の散文では、`tech-writing` は下記の
  [`japanese-tech-writing`](../../.claude/skills/japanese-tech-writing/) スキルの上位に立ちます。
...
- **日本語の文章は `japanese-tech-writing` スキルに従う。** 日本語版を新規に書くときも、英語版を
  ...
  この規範に沿った自然な日本語にします。これは [`tech-writing`](../../.claude/skills/tech-writing/)
  アンブレラの下位に位置します（上記）。日本語の散文では両方を適用します。
