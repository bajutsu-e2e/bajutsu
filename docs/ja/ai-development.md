...
エージェントでも同じ）**です。レビュアー（自動レビュアーの Claude Code。後述します。あるいは人間）が
...
すべての pull request は **Claude Code** が自動でレビューします。
...
- **オンデマンド。** 自動レビューとは別に、PR に `@claude review` と書く（あるいはレビュースレッドに返信する）
  ことで、新しいレビューや特定コメントへの追随を依頼できます。
