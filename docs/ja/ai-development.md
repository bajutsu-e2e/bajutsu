...
手で採番しても競合は残ります。同時に進む 2 つのブランチが同じ次の ID を選ぶことがあり、互いのまだ merge
していない番号は見えないからです。これを自分で見張る必要はありません。roadmap を更新する PR が merge されると、
**`roadmap-id-repair`** ワークフローが、roadmap を更新している open な PR それぞれで採番をやり直します。その PR が
新しく導入した項目（slug がまだ `main` に無い項目）の番号を merge 済みの項目がすでに持っていれば、次の空き番号へ
割り当て直し、ディレクトリを移動してファイルや相互参照、PR タイトルを書き換え、修正をブランチへ push します
（ローカルで同じ処理を行うなら `make roadmap-id-repair`）。優先されるのは `main` で、merge 済みの項目が番号を保ち、
まだ open な側が動きます。BE ID が変わるのはこの場合だけで、しかもそのブランチ自身が新しく導入した、まだ merge
されていない項目に限られます。古い `main` から引き継いだだけの項目（slug がすでに `main` にある項目）は rebase で
解消するもので、採番し直しません。`BE-XXXX` プレースホルダで書き起こせば、この競合は最初から避けられます。
`roadmap-id` ワークフローが PR の時点で ID を割り当て、`main` と他の open な PR にすでにある番号をすべて避ける
からです。
