...
- **オンデマンド。** オープンと再オープンのレビューより後は、すべて依頼して走らせます。メンテナまたは
  ...
  無視されても PR には何も残りません。コメント起因の実行はデフォルトブランチに対して走るため、PR の
  ブランチで絞り込んでも見つかりません。依頼した側は、コメントを投稿する前に時刻を控え、
  `gh run list --workflow "Claude review" --created ">＜控えた時刻＞"` のように作成時刻で絞り込んで、
  実行が始まったかを確かめてください。無反応を「指摘なしのレビュー」と読み違えないためです。
