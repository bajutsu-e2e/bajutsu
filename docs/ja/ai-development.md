...
失敗します。下限を `Makefile` の pytest 行の `--cov-fail-under` フラグではなく coverage.py 自身の
設定に置いているのは、下限を読む側すべてが1つの宣言的な情報源を共有するためです。シェルのレシピ行
から拾った値では、後の編集で誰にも気付かれずに下限が変わってしまいます。
