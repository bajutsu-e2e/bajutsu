...
`roadmaps/<implemented|in-progress|proposals|deferred>/BE-NNNN-<slug>/` ディレクトリに、英語版 `BE-NNNN-<slug>.md`
と日本語版 `BE-NNNN-<slug>-ja.md`（ID と slug は同一）を入れます。**BE** は *Bajutsu Evolution* の略で、`NNNN` は
**ゼロ詰め 4 桁で単調増加する** ID です。各項目は `状態` が示すフォルダに置きます（BE-0078）。`roadmaps/implemented/`
（`実装済み`）、`roadmaps/in-progress/`（`実装中`）、`roadmaps/proposals/`（`提案`）、`roadmaps/deferred/`（`提案（保留）`）です。

ロードマップ項目を追加するとき:

1. **次の ID を採番する** = 既存の最大 `BE-NNNN` + 1（すべてのフォルダを数えます）。現在の最大は次で確認します。
   ```bash
...
2. **項目ディレクトリと両言語のファイルを作成する**（新規項目はまず提案なので `roadmaps/proposals/` の下に）。
   ...
   再生成してください。項目の `状態`（バケット）+ `Topic` が並ぶセクションを決めるので、既存トピックの項目なら表の手編集は
...
フィールドが唯一の真実の源です。項目のフォルダと、index のどのバケットに並ぶかの両方を決めます（BE-0078）。

| 状態 | フォルダ / index バケット |
|---|---|
| `実装済み` | `roadmaps/implemented/`。出荷済み |
| `実装中` | `roadmaps/in-progress/`。可決され、PR が進行中 |
| `提案` | `roadmaps/proposals/`。検討中 |
| `提案（保留）` | `roadmaps/deferred/`。見送り |

項目が進んだら**状態を更新**してインデックスを再生成します（行は自動で正しいバケットへ移ります）。状態が変わったら
`状態` を設定し、ディレクトリをその状態に対応するフォルダへ移します。**`roadmap-promote`** ワークフローが PR 上で
移動と再生成を行います（ローカルで行うなら `make roadmap-promote`）。`状態` が項目の置き場所（どのサブディレクトリか）
を決める唯一の基準であり、両者が食い違うと `make test` が失敗します。そのため、項目が誤ったフォルダに置かれたまま
merge されることはありません。マイルストーン M1–M4 は `BE-0001`–`BE-0004`（可決され、実装済み）です。
