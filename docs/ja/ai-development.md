...
**項目に着手する前に、そのトラッキング Issue を確認してください**（`label:roadmap-tracking BE-NNNN
in:title` で検索します）。担当者が付いていなければ、作業を拾うときに、他の GitHub Issue と同じように
**自分をアサイン**します。トラッキング Issue を手でクローズしてはいけません。同期処理が行います。
