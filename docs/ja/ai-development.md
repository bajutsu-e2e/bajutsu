...
- **省略禁止、単体で完結。**
  ...
  スキルの自己完結の規範に従います。読者がリポジトリの他のページを何も読んでいなくても、文書を最初から
  最後まで追えるようにします。略語は初出でフルスペルと頭字語を並べて展開し（role-based access
  control（RBAC）のように）、以降は頭字語だけを使います。用語は初出の箇所で定義します。この規範は
  `docs/` に限らず、ロードマップ項目を含め、用語が現れるあらゆる箇所に適用されます。この規範が扱うのは、
  文書を読み進めるのに読者が必要とする用語・手順・背景という対象領域の内容であり、次の項目が扱う横断的な
  プロセス規範とは別の観点です。
- **指示語で読者を後戻りさせない。**
