...
を与え、その App ID と秘密鍵を `ROADMAP_BOT_APP_ID` / `ROADMAP_BOT_PRIVATE_KEY` の Actions secret として
...
BE-0061 のハードニング（[BE-0061](../../roadmaps/implemented/BE-0061-be-id-allocation-hardening/BE-0061-be-id-allocation-hardening-ja.md)：原子的な
`refs/be-claims/*` の claim、`roadmap-id-repair` と `roadmap-claims-gc` のワークフロー）は多層防御として残します。
マージ時採番のもとでは大半が冗長で（`main` に触れる allocate の実行は同時に最大 1 つで、つねに最新の `main`
を読みます）、これを撤去するのは将来の任意のフォローアップです。
...
4. **秘密鍵を生成**し、App ID とともに `ROADMAP_BOT_PRIVATE_KEY` と `ROADMAP_BOT_APP_ID` の Actions secret
