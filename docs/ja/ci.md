...
| [`web-e2e.yml`](../../.github/workflows/web-e2e.yml) | Linux | 手動 + 全 PR + マージキュー（必須の `E2E (web)` チェック） | **web（Playwright）バックエンド**のレーン（BE-0279）で、ヘッドレス Chromium に対する 3 ジョブから成ります。**smoke (playwright)** は `demos/web` のシナリオを実行し（`make -C demos/web e2e`）、**dogfood (serve UI)** は Bajutsu 自身の serve SPA を駆動し（BE-0058）、**conformance (playwright)** は BE-0114 のドライバコントラクトを実ブラウザに対して実行します。**Mac / Simulator 不要**で、コアがプラットフォーム非依存であることを示します。3 つとも決定論的で実行環境に依存しないため、`changes` 検出ジョブでパスゲートし（`scripts/e2e_changes.py`、`E2E_LANE=web`）、常に結果を報告する単一の `E2E (web)` ジョブ（必須チェック）に集約します。web 経路に影響しえない PR はブラウザジョブを飛ばし、ゲートは合格します |
...
- **要素ツリーの golden**、すなわち iOS / Android の `golden` ジョブ。決定論的ですが、ツリーは上流依存
  （`idb_companion` などの実機側サーバ）を通して読み取るため、その変化が Bajutsu 側の変更と無関係に赤く
  しえます。集約ジョブの `needs:` から外します。
