...
条件に使う `shared` フラグと `affected` ジョブ配列を出力します。ゲート担当のジョブが 1 つの `run` にまとまった後
（統合）、絞り込みの対象となるシナリオキー付きジョブは `run` / `actuation` / `golden` / `bundled-runner` です。
`run` は自分のシナリオを畳み込みブロックスカラーで宣言しますが、フィルタはこれも同じように読み取ります。それ以外の
場合はすべて、従来どおりレーン全体を発火させます。共有コードの変更（任意のシナリオに影響しうるドライバ / ランナー /
アプリ / ワークフローのコード）、シナリオを宣言しない次元ジョブ（`codegen` / `conformance` / `visual`）、そして
ジョブがシナリオでキーづけられていないレーン（Android と web）です。この判断はレーン全体の側に倒すので、変更が
壊しえたジョブを取りこぼすことはありません。そして `git` の差分とワークフロー自身の宣言だけを読むので、経路に LLM は
入らず、実行の合否判定にも一切影響しません。
