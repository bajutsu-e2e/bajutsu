...
| [`ios-e2e.yml`](../../.github/workflows/ios-e2e.yml) | macOS | 手動 + idb バックエンド、SwiftUI showcase、共有シナリオ、依存を触る `main` への push / PR | **iOS の視覚回帰（VRT）レーン**で、`android-e2e.yml` の `e2e-visual` に対応する iOS 版です。Simulator を起動し、SwiftUI showcase をビルドして、Stable カタログをコミット済みの `baselines_ios/` ベースラインと idb 上でピクセル比較します（`make -C demos/showcase e2e-visual`）。ステータスバーと「Liquid Glass」タブバーはマスクします。パスゲートで**必須チェックにはしません**。ピクセルのベースラインはホスト依存（Simulator のレンダラが Xcode / デバイス / OS で変わります）なので、ドリフトは PR をブロックせずこのレーンに現れ、採取したスクリーンショットを `ios-e2e-visual-run` としてアップロードし再採取に使います。決定的で LLM は使いません（コミット済みの要素ツリー golden は引き続き週次の idb-monitor で走ります） |
