...
### Android E2E の失敗が残す診断情報（BE-0367）

[`android-e2e.yml`](../../.github/workflows/android-e2e.yml) のジョブは、どのシナリオのアサーションでも
説明できない理由で落ちることがあります。adb バックエンドが要素ツリーを読む常駐 UI Automator サーバの
無応答、エミュレータ自身の描画の固まり、`timeout-minutes` の使い切りといった理由です。以前のアーティファクトに残るのは、シナリオかジョブが終わらなかったという事実だけでした。
原因の証拠は何もありません。いまは3層の収集がその原因を記録します。それぞれが他の2層には
見えないものを見ます。`bajutsu` 自身が知るのは、停止が「いつ」始まったかです。デバイス側の掃き出しは、その瞬間の
エミュレータの状態を読みます。ホストのサンプラは、1点ではなくジョブ全体を見る唯一の層です。成果物は
すべて `runs/diagnostics/` の下に置かれます。この場所は各ジョブの既存の `Upload run artifacts`
ステップがすでに拾っているので、新しくアップロードの配線を増やしていません。ここで集めたものは合否に
一切届きません。どの層も観測者であり、合否はこれまでと同じ手順で決まります。

**第1層：`bajutsu` の内側にある停止時プローブ**。 発火点は2つだけで、どちらも意図的に絞ってあります。
1つめは常駐チャネルの階層読み取りのフォールバックで、ドライバがチャネルを諦めてリース残りを
`uiautomator dump` のサブプロセスに切り替える地点です。読み取りチャネルが一時的に不安定なのではなく、
失われたことを意味する地点にあたります。2つめは、デバイス側の `screenrecord` が動いているだけでなく
実際にバイトを吐いているかどうかを見る新しい確認で、描画が固まった状態と、そもそも録画が始まって
いない状態とを切り分けます。捕捉先は環境変数 `BAJUTSU_STALL_DIAGNOSTICS` が指すディレクトリです。
これが設定されているとき、どちらの瞬間も `dumpsys SurfaceFlinger --latency`、`logcat` の末尾、ホストの
`ps` と `top` のスナップショットを取ります。置き場所は `<ディレクトリ>/stall-NN-<発火点>/` です。各プローブは
それぞれ短いサブプロセスのタイムアウトを持ちます。失敗は例外にせず成果物のなかに書き残し、1回の実行で
取るキャプチャは最大3件です。したがって、クラッシュを繰り返す実行がディスクと実時間を使い果たすことはありません。
環境変数を設定しなければ、これらの経路はすべて何もしません。ローカルの実行がこのフックのために
払うコストはゼロです。Android レーンでは `runs/diagnostics/stalls` を指しています。アクチュエーションの
経路は意図的に除いてあります。常駐チャネルのエラーのうち2つは、まったく健全な実行でも発火するからです。
アクチュエーション用のエンドポイントを持たない古いサーバと、二重に操作しないようドライバが「届いた」と
扱う応答の喪失の2つです。ここでキャプチャすると、本当の停止が使うべき1回の実行あたりの上限を先に
使い切ってしまいます。この環境変数の名前にバックエンドの接頭辞はありません。操作者が1つ設定すれば、
ジョブがどのバックエンドを駆動していてもフックが有効になります。

**第2層：エミュレータステップの内側から掃き出すエミュレータの状態**。 Android レーンでデバイスに触る
ステップは、すべて `reactivecircus/android-emulator-runner` ステップの内側で走ります。このステップは
Android Virtual Device（AVD）を起動し、`script:` を実行し、終わりに AVD を落とします。
したがって、その後ろに置いた通常のステップは、デバイスが1台も繋がっていない状態で走り、何も
集められません。ステップレベルの `if: failure()` による制御を足しても同じです。デバイス側の掃き出しは
[`scripts/collect_android_diagnostics.sh`](../../scripts/collect_android_diagnostics.sh) という
スクリプトに置き、各ジョブ自身の `script:` の末尾から呼びます。実行コマンド自身の終了コードで
段を切り替える形は、このレーンの `poll_cpuinfo` ポーラが失敗する実行を生き延びるためにすでに使って
いる形と同じです。段は2つあります。常時の段はどの実行でも払える軽さです。全バッファの `logcat -d`
ダンプに加えて `dumpsys meminfo`、`getprop`、`adb devices -l` の環境スナップショットを取ります。
全バッファのダンプが読むのは、シナリオごとの `deviceLog` インターバル証拠と違って、デバイス自身が
保持するリングバッファです。そのため、どのシナリオの取得も始まる前に落ちたジョブについても示せるものが
残ります。失敗時のみの段は重く、必要な実行のためにとってあります。1つは Android 自身の総合収集
ツールである `adb bugreport` です。もう1つは root 権限での取得で、`/data/tombstones` の
ネイティブクラッシュレポートと、`/data/anr/` のトレースを引き上げます。`/data/anr/` のトレースは、
Application Not Responding（ANR）の記録です。この掃き出しを呼ぶのは、エミュレータを起動する
6ジョブ（`smoke`、`golden`、`network`、`conformance`、`fault-injection`、`visual`）です。
`uiautomator (codegen)` は対象外です。Gradle の `connectedAndroidTest` を直接駆動し、この収集が
相乗りできるものを `runs/` の下に何も書かないからです。

**第3層：ジョブ全体にわたるホストのテレメトリ**。 停止がホストの飢餓と相関しているかどうかは、
停止した時点のホストの負荷が記録されていて初めて言えます。composite action
[`.github/actions/collect-android-diagnostics`](../../.github/actions/collect-android-diagnostics/action.yml)
は各ジョブのエミュレータステップを外側から挟みます。前に置く `start` フェーズが、バックグラウンドの
サンプラを起動します。サンプラは `top` と `free` の出力を約20秒ごとに
`runs/diagnostics/host-telemetry.log` へ追記します。後ろに置く `collect` フェーズが、そのサンプラを
止めます。読むのは Linux ランナーだけなので、
この層はステップとして成立します。そして外側から挟むからこそ、エミュレータステップごと落ちたジョブも
カバーできます。このサンプラはレーンの既存の `poll_cpuinfo` 入力を置き換えず、並存します。両者は
別の側から別のものを測っているからです。`poll_cpuinfo` は adb 越しにゲスト側の見え方を読み、この
サンプラはホスト自身の負荷を読みます。この違いは、`poll_cpuinfo` がオプトインであるのに対してこちらが
常時オンである理由でもあります。ホスト側で20秒ごとに取るサンプルは adb のトラフィックを一切
発生させず、頻度は100分の1です。`poll_cpuinfo` をデフォルトで無効にしている観測者効果を持ちません。
サンプラは
合否の経路の外にいる観測者であり、その間隔は待機ではなくサンプリングの周期なので、実行ループにおける
固定待機の禁止には触れません。
...
- **`bajutsu` の内側。** ストールがいつ起きたかを知っているのは実行中のプロセスだけだからです。
  ...
  あります。1回あたりの実時間の予算と、トリガーごとに2回という回数です。クラッシュを繰り返すジョブが
  証跡収集に `timeout-minutes` を使い果たすことはなく、劣化したデバイスが復帰できるかを判断するために
  クラッシュ復帰が必要とする実時間の予算を食うこともありません。上限をトリガーごとに分けているのは、
  映像の警告がこのランナーでは緑の実行を含む全シナリオで発火するためです。上限を共有にすると、
  説明したい対象であるランナーのクラッシュが届く前に使い切られてしまいます。どちらの環境変数もこのレーンの外では
