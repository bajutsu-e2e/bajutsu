...
| [`e2e.yml`](../../.github/workflows/e2e.yml) | macOS | 手動 + アプリ/SDK/ランタイムを触る PR | 2 ジョブ: **smoke (idb)** はサンプルをビルドしてシミュレータを起動し、idb バックエンドで `smoke.yaml` を実行します（driver + simctl + idb）。**xcuitest (codegen)** はシナリオからネイティブ XCUITest を生成し（`make -C demos/features ui-test`）`xcodebuild` で実行します（テスト時に bajutsu / idb / AI は不要）。macOS の 2 ジョブは `changes` 検出ジョブでパスゲートし、常に結果を報告する単一の `E2E` ジョブを必須チェックにしています |
