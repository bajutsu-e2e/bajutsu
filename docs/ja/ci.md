...
ツリーを読む常駐 UI Automator サーバの無応答、エミュレータ自身の描画の固まり、`timeout-minutes` の
使い切りです。どの場合もプロセスは1つも死んでいないので、クラッシュレポートの走査は何も見つけません。そして、デバイスの描画サービスが固まったのか、ホストがデバイスから資源を奪ったのかを
