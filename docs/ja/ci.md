...
2. [`bajutsu-e2e`](../../.github/actions/bajutsu-e2e/action.yml) composite action で **bajutsu を実行**します。
   ...
   このアクションの run ステップは `--erase` を渡すため、各シナリオの前にシミュレータを消去します（手順 1 でインストールしたアプリも含みます）。ターゲット設定には、ビルドしたものと同じ `.app` を指す `appPath` を宣言してください。消去後に bajutsu がそれを再インストールする仕組みだからです。宣言しなければ、run はもはやシミュレータ上に存在しないバンドル ID を起動しようとします。`xcuitest.deviceType: device` のターゲットは、erase が simctl 専用の操作であるため、このアクションを利用できません。
