...
許可リストの内部にある1つの区別が、保守上の危険を抱えています。許可リストは、サブパッケージをディレクトリの
接頭辞によって一括して拾う一方、トップレベルのモジュールは名前で指定します。フィルタはその名前の末尾へ
`.py` を付けて照合するので、許可リストの項目は単一ファイルのモジュールだけに届きます。モジュールが
パッケージになった日から、新しいパッケージ配下のファイルはどれも照合されなくなります。そしてレーンの
`changes` ジョブは `relevant=false` を報告し、常に結果を報告する集約ジョブは1つのジョブも走らせずに
合格します。`bajutsu/config` と `bajutsu/platform_lifecycle` はどちらも、リファクタリングがパッケージへ
分割した時点で、名前による指定のずれを起こしました。結果として、iOS レーンがまさに動かすためにある
XCUITest のコールド起動のコードを変更しても、iOS のレーン全体は飛ばされていました。したがって、
パッケージになったモジュールは一括して拾うグループに移します。名前で指定した残りの項目は、
`tests/test_e2e_changes.py` がソースツリーと照合します。次に分割するプルリクエストでは、`make check` が
落ちます。

このクラスの残りは、さらに3つのガードで塞いでいます。1つめの対象は、バックエンドごとの2つの
ディレクトリ、すなわち `bajutsu/drivers/` と `bajutsu/platform_lifecycle/environments/` です。
共有のコアが、各レーンが名指しする leaf を除いて一括して拾います。そしてテストが、どちらの配下にも
「1つもレーンを発火させないファイル」がないことを確認します。したがって、新しく足したバックエンドの
モジュールは、見落とされるのではなく過剰に発火します。2つめのテストは、フィルタ内の素のパスをすべて
ツリーへ解決します。リネームや削除は、レーンが黙って走らなくなる代わりに、ゲートの失敗として現れます。
3つめのテストは、共有の除外リストを各レーンが名指しする leaf の和集合と突き合わせます。一方のリストにだけ
追加したドライバや environment は、全レーンを黙って過剰に発火させる代わりに、ゲートの失敗として現れます。
