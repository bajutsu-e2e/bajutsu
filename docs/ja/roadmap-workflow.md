...
プレースホルダーを使う理由は次のとおりです。ID は永続で単調増加であり、同時に多数のブランチが進行しています。
...
4. **計画し、コードを書く前に合意を得る**：ロードマップ項目を丸ごと実装するのは大きく、後戻りが難しい作業
   ...
   （と、AI が座ってよい場所とよくない場所）、テスト、両言語で動かすべきドキュメント、絶対指針との緊張、を
   計画に挙げます。
5. **実装する**：設計どおりに、コードベースの肌理に合わせて作ります。厳格な `mypy`、設定済みの `ruff`、
...
出荷の *閉じた* 部分（このコードは仕様を満たしゲートを通るか）からきれいに切り離します。議論を重ねて形になった
提案は、一行のチケットよりはるかによい仕様です。そしてその仕様に枠づけられた実装は、行き当たりばったりの変更
よりはるかにレビューしやすいものです。この循環こそが要点です。
