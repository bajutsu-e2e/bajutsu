...
> 走らせ、シナリオを読み、同梱の showcase アプリを Simulator 上で動かし、HTML レポートを開けている
...
> どのコマンドが Claude を使い、どれが使わないかは [Claude を使う機能と使わない機能](ai-boundary.md)
> にまとめてあります。
...
> `[bedrock]`）が Claude の経路のための SDK を追加します。分離の詳細は
...
公開する識別子の一覧は [showcase](showcase.md) にまとまっています。

---

## ステップ 4：ショーケースアプリをビルドする（Xcode 必要）
