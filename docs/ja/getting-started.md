...
> インストールからテストが green になるまでを、手を動かして辿るチュートリアルです。読み終える
> 頃には、Bajutsu をインストールし、ユニットテストを走らせ、シナリオを読み、同梱サンプルアプリを
> Simulator 上で動かし、HTML レポートを開けているはずです。他のページがリファレンス（各機能が何を
> するか）であるのに対し、このページはチュートリアル（この手順を順にやる）です。
...
> codegen はいずれも実装済みでユニットテスト済みです。idb バックエンド経由の実機実行は `make` ターゲット
> 経由で動きますが、まだ堅牢化の途上です。実機ステップで不審な挙動が出たら、安定しているユニットテストに
> 戻ってください。
...
`uv` は `pyproject.toml` / `uv.lock` を読んで隔離された `.venv` を構築します。プロジェクトの
コマンドは `uv run`（例 `uv run bajutsu …`）を前置してこの環境で動かします。
...
`run` / `doctor` / `record` / `codegen` / `trace` / `triage` / `serve` が見えるはずです（完全な仕様は
[cli](cli.md)）。
...
インメモリの fake driver に対して検証する 405 のテストで、**Simulator には一切触れません**。
...
ここが green なら、このマシンでエンジンは動作します。ステップ 4 以降はすべてこの上に積み上がります。
...
リスト、そして **機械チェック可能なアサーション** の `expect` ブロックを持ちます。同梱の smoke テスト
[`demos/features/app/scenarios/smoke.yaml`](../../demos/features/app/scenarios/smoke.yaml) を開いてみます。
...
- **セレクタは `accessibilityIdentifier` 優先です**（`{ id: home.title }`）。安定でローカライズ非依存です。
  曖昧なセレクタは「最初に一致した何か」をタップせず、即座に失敗します（[selectors](selectors.md)）。
- **待機は sleep ではなく条件です**（`wait: { for: …, timeout: 5 }`）。要素が現れるまで、timeout まで
  ポーリングします。

文法の全体（全ステップ種別・待機・アサーション）は [scenarios](scenarios.md)、このアプリが公開する
識別子の一覧は [sample-app](sample-app.md) を参照してください。
...
同梱されています。Simulator 向けにビルドします。

```bash
make -C demos/features sample-build         # xcodegen generate -> iOS Simulator 向けに xcodebuild
```

`demos/features/app/build/…` 下に `BajutsuSample.app` ができます（`.xcodeproj` と `build/` は gitignore 済みで、
`project.yml` が正です）。launch-env フックと識別子カタログは [sample-app](sample-app.md) を参照してください。
...
一発で通す経路は `make` ターゲットです。ビルド済みアプリを install し、smoke シナリオと `doctor` を
booted デバイスで実行します。
...
または CLI を直接叩きます（同じ手順を書き下したものです）。
...
- `--app sample` は [`bajutsu.config.yaml`](../../bajutsu.config.yaml) の `apps.sample` を選びます
  ...
  すべて config に置きます（[configuration](configuration.md)）。
- `--backend idb` で actuator を選び、`--udid booted` で現在 boot 中の Simulator を狙います。
- `--no-erase` は先頭で `simctl erase` せず、install 済みアプリをそのまま使います。
...
`run` は **全シナリオ合格で終了コード 0、いずれか失敗で 1** となり、この終了コードが CI（継続的インテグレーション）ゲート
です（[run-loop](run-loop.md)）。

> 環境の問題（booted Simulator が無い・idb 未インストール）に当たったら、まず
> `uv run bajutsu doctor --app sample` を走らせます。必要な CLI と booted デバイスの ✓/✗
...
ターミナルを離れずにテキストのタイムラインとして実行を眺めることもできます。

```bash
uv run bajutsu trace               # runs/ 下の最新実行
```

各フォーマットと `runs/` レイアウトの詳細は [reporting](reporting.md) を参照してください。

---

## 次に読むもの

これでループ一周分が揃いました。インストール → ユニットテスト → シナリオ → 実機実行 → レポートです。
同じシナリオ形式を使う入口があと 2 つあります。

- **AI でオーサリング。** Claude にゴールへ向けて探索させ、シナリオを書かせます（Tier 1）。
  ...
  ```
  Web UI（`make serve`）の **API key** ボタンでも、実行中のセッション用に同じキーを設定できます。伏字
  （redact）表示と表示切り替え（reveal）を備えますが、メモリ上に保持するだけでディスクには書き込みません。
  再起動をまたいで保持したい場合は引き続き `.env` を使ってください。オーサリングループとシステムアラート
  ガードの仕組みは [recording](recording.md) を参照してください。
- **ネイティブ XCUITest を吐く。** シナリオを Swift へ変換します（テスト時に Bajutsu ランタイム・AI は
  ...
  ```
  構造マッピングは [codegen](codegen.md) を参照してください。`make -C demos/features ui-test` で end-to-end に実行できます。

ここからは各リファレンスページが個別に深掘りしています。まず根拠を [concepts](concepts.md) で確認し、
続けて [ドキュメント索引](README.md) の推奨読書順に従うとよいでしょう。サンプル以外の自前アプリを
オンボーディングするには [configuration](configuration.md#新しいアプリのオンボーディング) を参照してください。
