...
**データベースを繋いだデプロイでは、この4つのフィールドのうち3つを読むのは一度きりです**（[BE-0375](../../roadmaps/BE-0375-serve-org-lifecycle-management/BE-0375-serve-org-lifecycle-management-ja.md)）。`serve` は各 org の `members`、`githubOrgs`、`editorTeam` を、その org を最初に見た時点でデータベースへ写します。写す契機は起動時と、新しい config を bind したときです。以後のサインインはデータベースだけを見て解決します。メンバーシップの編集は、この config ファイルではなく Orgs ページから行い、この3つのフィールドを後から書き換えても効果はありません。`serve` は、まだこの3つを宣言しているエントリの org 名を挙げて警告を記録するので、operator は編集が消えたと戸惑う代わりに、ファイルが決定権を持たなくなったことに気付けます。ここで引き続き効くのは `targets` です。したがって、そうしたデプロイでは `targets:` だけに削ぎ落としたエントリが本来の姿になります。最初の写しより前に削ぎ落としても安全なので、順序を誤る余地はありません。`targets` だけを宣言したエントリは写しの対象から外れるため、行を作ることも、その org に一度だけ許された写しを使い切ることもなく、あとから `members` を戻せばそのときに効きます。同じ名前の target を2つの org がそれぞれ要求してもよく、どちらの org もその target に対して認可されます。bind している config が1つである以上、その名前が解決する `targets:` の定義は両者で共有されます。データベースを繋いでいないデプロイは、以上のいずれとも無関係で、引き続きすべてのフィールドをこのファイルから読みます。
