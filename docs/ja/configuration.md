...
複数の org に該当する login は、サインイン時にそのうち1つに入ります（明示の `members` が優先で、次に `githubOrgs` が一致する最初の org）。そのうえで、いま使っているブラウザのセッションがどの org として振る舞うかを、ヘッダのセレクトボックスから選べます。`editorTeam` は org ごとの宣言なので、ロールも選んだ org に従います。この選択が何をキャッシュし、管理者がどう終わらせられるかは[セルフホスティング](self-hosting.md#2-github-oauth-を足す任意)を参照してください。
