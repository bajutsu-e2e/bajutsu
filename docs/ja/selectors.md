...
RocketSim / idb のどちらの出力も、ドライバが共通の `Element`（TypedDict）へ正規化する。
...
実機のバックエンドはどちらも使える semantic tap を持たないため、抽象側は **常に `query()` で
候補数を検証してから** 操作し、確定した要素の frame 中心をタップする。これで「曖昧なら失敗」の
挙動が RocketSim / idb / fake で同一になる（各ドライバの `tap` 実装は [drivers](drivers.md) 参照）。

idb では `id` は要素ツリー（`AXUniqueId`）から直接得られる。RocketSim はプロトコルが
accessibilityIdentifier を報告しないため、`identifier` は `query()` 時に **アプリ単位の idmap で
復元する**（[drivers](drivers.md#識別子の復元idmap)）。どちらの場合も解決は正規化済みの
`identifier` を見るので、同じ `id` セレクタが両バックエンドで動く。
