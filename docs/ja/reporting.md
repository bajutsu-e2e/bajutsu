...
`RunResult` 以下はすべて dataclass なので、`manifest_dict` でステップ / expect の結果がそのまま落ちます。除外されるフィールドはひとつもありません。タイムスタンプは絶対的な実時刻（エポック秒）なので、記録した run のプロセスが終了したあとも同じ意味を保ちます（[evidence](evidence.md#区間証跡video--devicelog--apptrace)参照）。
...
- `wall_offset_s`（シナリオごと）: その run が単調増加時計の値を実時刻へ変換するために足した差分です。下の通信ログを、ステップと同じ基準時刻へ揃えるのに使います。計算し直さず記録しておくので、両者が実時刻の時計の同じ1回の読み取りから導かれます。
