...
UI の各操作は CLI コマンド（`record`、`run`、`crawl`、`stats`、`lint`）に対応します。ここでの説明で
オプションの全体像が必要になったら、[CLI リファレンス](cli.md) が拠り所です。
...
必要です。`make serve` は `127.0.0.1` に bind し、ブラウザで UI を開きます。オプションの全体像
（`--port`、`--config`、`--root`、`--runs`、`--baselines`、`--host`、`--token`、`--max-concurrent-runs`、
`--evidence-store`）は [CLI リファレンス](cli.md#serve) にあります。
