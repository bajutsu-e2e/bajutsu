...
ヘッダには 8 つのトップレベルタブがあります。**Record**、**Replay**、**Crawl**、**Author**、
**Stats**、**Flaky**、**Usage**、**Coverage** です。9 つ目の **Metrics** は、
...
そのまま使います。開始と終了のトークンを `none` にしたテーマは、その遷移を即座に描画します。OS のアクセシビリティ
...
`${secrets.*}` のプレースホルダは書かれたとおりに現れ、解決されることはない（実行時にサーバの環境から
解決されます）ので、ファイル以上の開示はありません。ただし伏せ字にはしません。ローカルやアップロードした
...
config の Stats ダッシュボードが開きます。比較が入り口、プロジェクトごとの表示がドリルダウンです。ほかの
...
は、Crawl タブ自身の **History** サブタブに一覧されます。どの crawl も配信する `screenmap.json` を手が
...
2. 過去の run がないときは、**Start live session** を押すと対象を起動して現在の画面に対して要素を選べます。Edit に run は要りません。**Stop session** で保存せずに終了します。
...
**フラッキーなシナリオを診断する。** このパネルがフラッキーなシナリオを名指ししたあと、その成功 run と失敗 run を
まとめて読んで差分を説明する AI の Run 横断の*修正提案*は、CLI の機能 `bajutsu triage --flaky` です
（[CLI リファレンス](cli.md#run-横断のフラッキートリアージ--flaky)を参照）。これは設計上 CLI 専用です。この
