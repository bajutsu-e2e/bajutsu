...
エディタはトークン契約からフォームを自動生成します。色トークンごとにネイティブのカラースウォッチ、モーション
トークンごとにテキストフィールドが並びます。編集内容はライブプレビューとしてそのまま反映され、ページのリロードは
不要です。エディタからは次の操作ができます。

- **Save to Local Draft** — 編集中のテーマをブラウザのローカルストレージに保存します。ページを再読み込みしても
  残りますが、ブラウザ固有の保存であり、ほかの serve セッションとは共有されません。
- **Export** — 編集したテーマを `custom-theme.css` としてダウンロードします。`--themes` が受け付けるドロップイン
