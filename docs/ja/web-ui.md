...
config 名の隣の **View** を押すと、タブが実際に動かしている config を確認できます。ダイアログには config の
生の YAML、そのパス、そして Git リポジトリから取得したものであれば取得元（リポジトリ、指定した `ref`、
解決されたコミット SHA）が表示されます。これが効くのは Git ソースのときです。Git ソースのアクティブなパスは
content-addressed なキャッシュ上の不透明な場所（`…/gitsrc/<host>/<owner>/<repo>/<sha>/…`）になるため、
パスだけではどのコミットが bind されているかわかりません。取得元の行が、実際に bind されているコミットを
示します。YAML はそのまま表示するので、`${secrets.*}` のプレースホルダも書かれたとおりに現れます。これらは
実行時にサーバの環境から解決されるので、秘匿情報が表示されることはありません。
