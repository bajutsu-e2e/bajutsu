...
バインディングで、ヘッダには**スイッチャー**（config ビューアの隣のピッカー）と **Projects** ボタンが
加わります。どちらもプロジェクトが 1 件以上登録されるまで隠れているので、単一 config の `serve` は従来
どおりに見えます。

プロジェクトの登録は [`bajutsu project` の CLI](cli.md#project)（`project add` と `rm`）で行います。ストア
は共有なので、そこで追加したプロジェクトはスイッチャーにも現れ、その逆も成り立ちます。スイッチャーで
プロジェクトを選ぶ、あるいは Projects 一覧の行で **Run** を押すと、そのプロジェクトが **active** になります。
サーバは再起動なしでアクティブな config をそのプロジェクトのソースへ貼り替え、どのタブ（Replay、Record、
Crawl、Stats のダッシュボード）もそれに対して動きます。Projects 一覧の各行は、プロジェクト名とその config
ソース、最新の実行結果を示します。ソースがアップロードされたバンドルのプロジェクトには切り替えられません
（再展開するチェックアウトがないためです）。その config を bind するには、もう一度アップロードしてください。
切り替えは config を貼り替えるので、ホスト型のデプロイでは config を bind するのと同じく admin の操作です。
...
名前空間外の id、重複））です。ホスト環境を調べる読み取り専用・AI 不使用の診断で、run を
ゲートすることはありません。
