...
swipe / wait）し、**機械チェック可能なアサーション**で結果を検証します。backend を差し替えるだけで、同じシナリオが iOS Simulator（idb）でもブラウザ
（Playwright）でも動きます。中心にあるのは、AI を CI（継続的インテグレーション）ゲートに持ち込まないという考え方
