...
UI 操作は「**最も安定する手段から順に**」試す。下に行くほど壊れやすい。

| 順 | 選択（どの要素） | 操作（どう叩く） | 安定性 |
|---|---|---|---|
| 1 | `id` で一意解決 | ネイティブ semantic tap（RocketSim） | 最安定（レイアウト / 翻訳 / 座標すべてに非依存） |
| 2 | `id` で一意解決 | frame 中心へ座標 tap（idb 等） | 選択は id で安定、actuation のみ座標 |
| 3 | `label` / `traits` で解決 | semantic / 座標 tap | 選択がローカライズに弱い |
| 4 | `index` / 生座標 | 座標 tap | レイアウト変化で壊れる。最終手段 |

これがそのまま **backend の並び順**になる: `backend: [rocketsim, idb]` は「先頭ほど操作が安定」。
実際に操作を担う **actuator はリストで最初に利用可能なバックエンド**で、run 開始時に 1 つ確定し
run 中固定（2 ドライバが同一デバイスを操作する非決定性を避ける）。手元は RocketSim（順 1）、
ヘッドレス CI は idb（順 2）へ自動降格する。選択は常に `id` なのでシナリオは不変
