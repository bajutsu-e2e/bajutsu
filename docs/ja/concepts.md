...
4. **合否は機械チェックのみ** — 「成功した気がする」を排除。`exists`/`value`/`label`/`count`/
   `enabled`/`disabled`/`selected` の機械アサーション（[selectors](selectors.md#アサーション評価)）。
...
actuation（どう叩くか）の話ではない。実機のバックエンドはどちらも同じく frame 中心の座標 tap で
操作するので、順位で変わるのは要素の選び方だけ。下に行くほど壊れやすい。
...
> actuation は常に frame 中心への座標 tap。当初 RocketSim はネイティブ semantic tap を持つ前提で
> idb の上位に置いていたが、実機で確認した結果その前提は崩れた — `rs/1` プロトコルは
> accessibilityIdentifier を公開せず、RocketSim も座標で操作し、識別子は
> [idmap](drivers.md#識別子の復元idmap) で復元する。

これは **backend の並び順**にも表れる: `backend: [rocketsim, idb]` は手元で優先する actuator を
先頭に置く。実際に操作を担う **actuator はリストで最初に利用可能なバックエンド**で、run 開始時に
1 つ確定し run 中固定（2 ドライバが同一デバイスを操作する非決定性を避ける）。手元は RocketSim、
ヘッドレス CI は idb へ自動降格する。選択は常に `id` なのでシナリオは不変
