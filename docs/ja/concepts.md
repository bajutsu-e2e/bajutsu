...
安定度順で書き、プラットフォーム対応のレジストリを通して解決します。プラットフォームトークン（`ios` / `web` / `fake`。Android は予定）が
その actuator に展開され（`ios` → `idb`、`web` → `playwright`）、最初に利用可能なものが選ばれます。選択は常に `id` で行うため、
シナリオは変わりません（[drivers](drivers.md#バックエンド選択と-actuator)）。
...
| クリーン環境 | `runner/launch.py` `launch_driver` · `env.py` `Env.erase` |
...
| 証跡ルール | `orchestrator/loop.py` `_collect_captures` · `evidence.py` |
