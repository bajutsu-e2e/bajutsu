...
*画面*や*データ状態*へ起動時に直行する launch-env の近道は、意図的にありません（BE-0079）。アプリは常に
Stable タブで、固定のカタログを持って起動するので、シナリオは他のすべての画面を UI を操作して辿り、
データは注入せずアプリ自身のものを観測します。
