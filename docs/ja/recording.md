...
- **新しい画面。** 現在の画面の `crawl.fingerprint(...)` が前のターンと異なる（または最初のターン）
  とき。まだ見ていない画面には画像を渡します。
- **退化したツリー。** `fingerprint` が構造ベースのフォールバックに落ちたとき（セレクタでアドレス指定
  するにはアクセシビリティ識別子が少なすぎる、id を持たないタブバーのような場合で、`tap_point` が唯一の
  入口になります）。ここでは画像を先回りして添付します。このトリガーは意図的に寛容にしてあり、id の
  乏しい画面がエスカレーションの往復に頼らずに済むようにしています。
