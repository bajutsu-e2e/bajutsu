...
デバイスを駆動することはありません。一方、**リモートまたはセルフホスト**の `serve`
（[BE-0015](../../roadmaps/BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting.md) /
[BE-0016](../../roadmaps/BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting.md)）では、デバイスは作者の
目の前になく、デバイス操作の引き取りは成立しません。`respond-human` は `acted` の再開を拒否し、代替手段を
名指しするメッセージを返します——**デバイスのある場所で録り直す**か、**テストビルドの迂回路を配線**して、
実行時に人の操作を要さないようにする、のいずれかです。ブラウザ内にデバイスを鏡像表示する対話的な画面が
あればこの制約は外れますが、その画面はここでは対象外です。
