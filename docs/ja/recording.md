...
ループとモデルを分離するための薄い Protocol です（`agent_protocols.py`）。テストではスクリプト化した fake を使い、
...
5. proposal.steps を順に実行する（1 手以上のバッチ。後述）:
     ...
     - 解決しない手は、そのターンで 1 手も実行していなければ記録を止め、そうでなければバッチを中断する
...
`_render`（`claude_agent.py`）は、エージェントが操作できる要素を落とさないまま、このツリーを軽く保ちます。
...
表示します（`usage.py`、BE-0194）。これにより、トークン削減の効果を測定できます。各カテゴリは記録した
すべての呼び出しを漏れなく分けるので、合計は running total に一致します。この内訳は報告専用で、`usage.py`
の他の部分と同じく pass/fail には一切関与しません。

## Claude オーサリングエージェント

`record` / `crawl` は本番の `agent_protocols.Agent` 実装として `ClaudeAgent` を 1 つ構築します
（`claude_agent.py`、`agent_factory.py` が組み立てます）。モデルへはベンダー中立の `AiBackend`
