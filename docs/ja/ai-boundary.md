...
ため（Anthropic API、Amazon Bedrock、サブスクリプションでログインした Claude Code CLI）、「API キーが
要る」という枠取りは単位として誤りです。誠実な単位は「Claude を使う」であり、プロバイダやバック
エンドが増えても正しいままです。
...
| | `serve` | ローカルの Web UI。何も設定せずに起動し、Claude のタブは優雅に縮退する |
...
| | `run --dismiss-alerts` | アラートガード。ステップを塞いだ OS のプロンプトを Claude が処理する |

分類はコマンド名ではなく経路の粒度です。`triage` は Claude を使わず、`--ai` フラグ一つで Claude の
経路に切り替わります。`run` も Claude を使わず、アラートガード（`--dismiss-alerts`。シナリオごとに
...
- **`serve`** は Claude のタブ（`record` と `crawl`）を見せたうえで、Claude に到達できないときは
  インラインの説明とともに無効化し、UI 内のキー入力欄へ誘導します。キーを設定するか、プロバイダを
  構成するか、Claude Code CLI が使えるようになった時点で、タブはすぐに有効へ戻ります。

## Claude の経路をインストールする

この分離は実行時だけでなく、パッケージングの境界でもあります（[BE-0111](../../roadmaps/implemented/BE-0111-ai-sdk-optional-dependency/BE-0111-ai-sdk-optional-dependency-ja.md)）。
...
- `pip install bajutsu`：決定論的なオーサリングと実行の経路（`run`、`doctor`、`lint`、`codegen`、
  ...
  モデルに到達することはありません。
- `pip install bajutsu[ai]`：Claude の経路（`record`、`crawl`、`triage --ai`、`run --dismiss-alerts`）の
