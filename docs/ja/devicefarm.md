...
> **設定ファイルとシナリオの配置。** クラウドバッチのパッケージは、*クラウドバッチパッケージルート*をルートとして構築されます。このルートは、チェックアウトから起動した場合は `pyproject.toml`・`tests/`・`bajutsu/` パッケージを含む Bajutsu ソースツリーのディレクトリ、それ以外の場合は設定ファイル自身のディレクトリとなります。クラウドバッチのターゲットが参照する設定ファイルと `scenarios` ディレクトリは、そのルートの内側に置く必要があります。ルートの外に置かれた設定ファイルでは、`POST /api/run-set` が 400 エラーを返し、実際のパッケージルートのパスを示します。
