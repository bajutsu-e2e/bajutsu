...
> chromium`、iOS Simulator（idb）backend なら `make deps`（`idb` クライアント、`idb_companion`、xcodegen）に
> 続けて `uv sync --extra worker-idb` です。backend の extra 単体（`web` / `idb`）では入れ足りず、その実行は
> アサーションの時点で遅延的に失敗します。まったく入れずに飛ばすと、実行のディスパッチ時に
...
既定は idb だけの構成で、これは変わりません。web の run をホストするときに web ワーカーを足してください
...
を返します）、Git とアップロードの2つだけを残します。ホスト型のユーザーは共有ワーカーのファイルシステムと
関係を持たないため、オペレータの `--root` を辿っても自分が所有する config はバインドできません。ファイル
ブラウザを外すことで、ログイン済みの全ユーザーにそのツリーのディレクトリ一覧を見せてしまう露出も避けられ
...
`BAJUTSU_SERVER_STORE` という1本の URI で選びます（BE-0204）。`s3://bucket/prefix`（S3 互換。この compose が同梱する MinIO）と `gs://bucket/prefix`
...
ランタイムクロージャ `bajutsu[worker-idb]`（`idb` backend に加え、実行が到達する `visual` / `schema` の
アサーション依存。BE-0173）をインストールし、tailnet 越しにコントロールプレーンへ向けます:
...
（profile の裏）なので、idb だけの構成は変わりません。Linux ノード上でも、コントロールプレーンへ到達できる
...
web ワーカーは **web** ジョブだけを、Mac の idb ワーカーは **idb** ジョブだけをリースします。プールは
...
- **ワーカーの広告集合**は、その `--platform`（backend 軸。Mac の idb ワーカーは `platform:ios`、Playwright の
