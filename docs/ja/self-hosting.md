...
拒否したサインインは `event=oauth.denied` として `WARNING` で記録され、`orgs:` が一致しなかった理由と、
admin Team がそもそも構成されていたかどうかの両方を名指しします。同じ event は、GitHub account を必要と
しない4つの手前の失敗（OAuth が構成されていない、CSRF の state 不一致、例外を発生させた exchange、
identity を返さなかった exchange）も対象にしますが、これらは代わりに `INFO` で記録されます。アラートは
`WARNING` をキーにしてください。そうすれば、callback への匿名の probe がそれを埋もれさせることはありません。
