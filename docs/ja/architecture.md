...
| `platform_lifecycle/` | `Environment` の seam（BE-0009）。1 つの `RunEnvironment`/`CrawlEnvironment` Protocol がプラットフォームごとのアプリ起動・readiness・relaunch・デバイス制御・teardown を担い、`runner/` と `cli/commands/crawl.py` は actuator 名で分岐せず iOS/Android/web を同じ界面から駆動（パッケージ: `protocols` / `factories` / `readiness` / `relaunchers` / `device_control` / `read_session`、加えて `environments/` ── `ios` / `xcuitest` / `android` / `web` / `fake`） | [drivers](drivers.md#環境管理simctl) |
