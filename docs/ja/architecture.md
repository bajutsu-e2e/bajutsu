...
- **Android adb バックエンド**（`drivers/adb.py` ＋ `adb.py`）: 座標ドライバ（`uiautomator dump` → frame 中心タップ）、`AndroidEnvironment` の起動シーケンス、`doctor` の報告、interval 証跡（`video` は `screenrecord`、`deviceLog` は `logcat`。どちらも driver 供給の `driver_interval` seam を通す）と iOS から再利用するモックネットワーク、取得済み XML フィクスチャに対する fast ゲートのユニットテストまで。デバイス制御と codegen は後続で、実機エミュレータの e2e は別の重い経路（BE-0007）
