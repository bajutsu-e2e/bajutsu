...
| `cloud/` | クラウドのデバイスバックエンドをバッチサブミッタとして扱う層。`run`/CI の判定経路の外にあります。`devicefarm`（AWS Device Farm。BE-0235、iOS は BE-0238）はカスタム環境用のテストスペックを描画し、アップロード用パッケージを組み立てます。実行を submit/poll/collect し、判定を Bajutsu 自身の manifest から導きます。ヘッドレスな CLI ラッパー（`scripts/devicefarm_submit.py`）と `serve` のシナリオ単位バッチ fan-out（BE-0336）が共有する再利用可能なコアです | [devicefarm](devicefarm.md) |
...
| `analytics/` | AI の利用量・コストの計上。報告専用で決定的な判定には触れません。`usage` は CLI の `AI usage:` 行が読む、プロセスグローバルな呼び出しごとのトークントラッカーです（BE-0194）。`ledger` は AI 呼び出し 1 件ごとに帰属先と価格を付した JSONL 記録を永続化します（BE-0196）。`stats` はその ledger を provider・model・command・scenario で集計し、`serve` の Usage ダッシュボード向けに日次コスト推移を添えます（BE-0195） | [web-ui](web-ui.md) |
...
- **`serve` 主導の AWS Device Farm fan-out**（BE-0336）: `cloudBatch: devicefarm` を宣言したターゲットは、`serve`（`POST /api/run-set`）からシナリオごとに 1 つの Device Farm 実行をディスパッチします。ヘッドレスなサブミッタがスイート全体を 1 台の予約デバイス上で直列にまとめるのに対し、serve はそのサブミッタのコア（`cloud/devicefarm.py`、BE-0235）を再利用しながら実行を並列化し、プロバイダー単位のデバイス予算（`cloudBatchBudget`）で上限を決めます。データベースバックエンドのデプロイでは `serve` の再起動をまたいでも継続します。スケジュール済みの各実行の ARN をあらかじめチェックポイントしておくため、再起動後のワーカーは再アップロードなしに同じ実行のポーリングを再開します
- **AI 利用量・コストダッシュボード**（BE-0195）: 帰属付きで永続化した利用 ledger（BE-0196）を集計します。AI 呼び出しのたびに、provider・model・command・scenario と、provider がトークン単価を持つ場合のドル換算コストを添えた JSONL 1 行を `runs/usage.jsonl`（`ai.usageLedger` で移動、空文字で無効化）へ追記します。`serve` の **Usage** タブは、その ledger を各軸で集計し、provider・model の比較と日次コスト推移を示します。BE-0102 の run 統計ダッシュボードと同じく、読み取り専用で報告目的に限られます
