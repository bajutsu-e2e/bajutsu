...
- DSL のオーサリング再利用: 再利用可能なパラメータ化コンポーネント（`use` / `${params.*}`）。ファイル横断のコンポーネント（BE-0030）として解決するほか、`/` も `.yaml`/`.yml` も含まないベア名の場合は、同じシナリオファイル内にインラインで宣言した `components:` ブロックに対して解決する——そのファイル内でしか再利用しないブロックなら、別のコンポーネントファイルを起こす必要がない（BE-0422）。データ駆動シナリオ（`data` / `dataFile` と `${row.*}`）、シークレット変数（`${secrets.X}`、値マスク）、シナリオタグ + `--tag` / `--exclude` 選択、`setLocation` / `push` デバイスステップ、起動前の `permissions` フィールド（`simctl privacy` / `pm grant`|`pm revoke`、BE-0276）、`doubleTap` アクション、ファイル単位 + シナリオ単位の `description`
