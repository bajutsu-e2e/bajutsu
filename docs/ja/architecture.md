...
- DSL の `setPickerValue`（BE-0356）: ホイール型のピッカー（`UIPickerView`、またはホイール表示に切り替えた `UIDatePicker`）を、解決済みのホイールに対して XCUITest 自身の `adjust(toPickerWheelValue:)` を呼び出すことで、指定した行へ動かします。`swipe`/`drag`/`scroll` のような座標指定ではなく、`tap` と同じハンドル指定です。ホイールの各行は個別にアドレス指定できる要素ではなく、座標によるドラッグでは目的の行で確実に止められないためです。ホイールが持たない値を指定した場合は、ホイールが止まった位置をそのままにするのではなく、その値を名指ししてステップを失敗させます。複数コンポーネントのピッカー（年のホイールと月のホイールが並ぶものなど）は、セレクタが既に持つ `within` / `traits` / `index` を使って、コンポーネントごとに 1 ステップずつアドレス指定します。`PICKER_WHEEL` の能力を宣言するのは常駐ランナー方式の XCUITest バックエンドと `FakeDriver` だけなので、Android と web はデバイス操作が始まる前の preflight で拒否されます
- DSL の `handleSystemAlert`（BE-0316）: SpringBoard の権限プロンプトのボタンを、ネイティブなアクセシビリティ照会（ランナーの 2 つ目のオンデマンドな SpringBoard ハンドル）で tap する、決定的で iOS 専用のステップです。解決は Python 側の `resolve_unique` に残ります。この能力を宣言するのは XCUITest バックエンドだけなので、Android と web は preflight で失敗します。label が決定的になるのは、XCUITest のライフサイクルが cold spawn のたびに **Simulator 自身の**システム言語を run の `locale` に固定するからです。SpringBoard はアプリの launch 引数が届かない別プロセスなので、グローバル設定ドメインへの書き込みと再起動 1 回で行い、warm な runner の再利用はその locale が一致していることを条件にします。`permissions` で先回りできない 2 つのプロンプト（通知の許可と ATT）については、`sel` の代わりに `prompt` と `choice` も指定でき、固定した locale が描画する label を run が解決します（BE-0320）
...
- **データベースを出どころとする org のライフサイクルとメンバーシップ**（BE-0375）: データベースを繋ぐと、org の `members` / `githubOrgs` / `editorTeam` は config ファイルの `orgs:` ブロックではなく `orgs` テーブルに置かれます。起動時と config の再 bind のたびに、そのブロックから org ごとに一度だけ書き写し、以後はデータベースが持ちます。これに伴い `serve` は admin 専用の `/api/orgs…` エンドポイントを4つと **Orgs** ページを持ち、再デプロイなしでテナントの作成、メンバーシップの置き換え、soft delete ができます。サインインの解決はテーブルだけを見るので、config が読めなくても全ユーザを拒否することはなくなり、データベースが読めないときはストアを名指しする 5xx で答えます。target の識別子は `(org, target)` になり、同じ名前を2つの org がそれぞれ要求できます。target の所有そのものは config に残り（prime directive 3）、データベースを繋がないデプロイは従来どおり `orgs:` を読みます
- **セッション単位の org の選択**: 複数の org に属する login は、いま使っているブラウザのセッションがどの org として振る舞うかをヘッダのセレクトボックスで選び、`POST /api/session/org` でサインインし直さずに切り替えます。2つのウィンドウで2つのテナントを同時に開けます。セッションは、自身のサインインが観測した GitHub の事実に加えて、振る舞い中の org とその org でのロールを記録します。login と org とロールを持つ `Caller` を認証ゲートで1度だけ解決し、operations はユーザ行を読み直す代わりにその `Caller` で範囲を区切ります。`editorTeam` は org ごとなので、ロールは選んだ org に従います。記録した Team は権限のキャッシュであり、その寿命はセッションの寿命です。org の廃止と、生きているセッションが持つ答えを変えるメンバーシップの編集は、該当のセッションをその場で失効させます
