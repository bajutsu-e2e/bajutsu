...
- **config プロジェクトハブ**（`project add`/`ls`/`use`/`rm` と `run --project`、BE-0225）: プロジェクト名を config のソースに束ねる名前付きレジストリで、CLI と `serve` の Web UI が共有します（データベースがあればそこに保存し、なければディスク上の JSON に保存します）。`serve` はヘッダーの**プロジェクト切り替え**と、プロジェクトを一覧・追加・削除・切り替えするトップレベルの **Projects** ページ（BE-0275）を備え、再起動なしにアクティブな config を切り替えます。org が行うバインドは、ファイルブラウザからのものも、Git ピッカーからのものも、バンドルのアップロードも、成果物の三つ組の合成も、すべてその org のプロジェクトとして登録され、その org が記憶する config になります。データベースがある場合、その選択はプロセス内ではなく org の行に保存するため、再起動しても忘れません（[BE-0393](../../roadmaps/BE-0393-per-org-config-memory/BE-0393-per-org-config-memory-ja.md)）。アップロードされたバンドルは、オブジェクトストアを要求する前にレプリカ自身の展開キャッシュから復元するので、ローカルや単一ノードのデプロイでも、まだ手元に持っているバンドルへ切り替え直せます
