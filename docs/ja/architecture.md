...
| `common/devices/` | バックエンド間で共有するデバイス識別のヘルパ（パッケージ）: `os.py`（`device_runtime` ラベルから読む、解析済みのデバイス OS。プラットフォーム/メジャー/マイナー、BE-0358）、`id.py`（各バックエンドの `--udid`/config が共有するデバイス id バリデータ。argv オプションインジェクションを防ぎます）、`errors.py`（`simctl`/`adb` が継承するプラットフォーム中立な `DeviceError` 基底、BE-0260） | [drivers](drivers.md) |
