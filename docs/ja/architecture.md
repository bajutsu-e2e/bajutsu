...
- **iOS クラッシュレポートのシナリオ単位での取得**（BE-0421）: XCUITest のクラッシュ再試行を使い切って失敗した
  ...
  サブディレクトリを直接名指しします
- **アプリクラッシュのシナリオ単位での取得**（BE-0424）: 直前の項目のアプリ側の対応物であり、意図して
  *再試行しません*。バックエンドがクラッシュするとシナリオには救い出せるものが残らないので、
  パイプラインはリースを捨てて再実行します。一方、テスト対象アプリのクラッシュでは、ドライバ、
  バックエンドプロセス、実行中の動画記録がそのまま残るので、**ステップループの内側**で分類します。
  事象を観測したステップを失敗させ、通常の `RunResult` の組み立てがその周りで最後まで進みます。検知は
  事後的で、ステップがすでに失敗した後にだけドライバへ問い合わせるため、緑の run は何も払いません。
  証跡は直前の項目の隣、`app-crash/` サブディレクトリに入ります。ここで再試行すると、同じ形で
  失敗する見込みの高いシナリオがクラッシュリカバリの予算を使い切ります。アプリの本物のリグレッションを
  明示的な失敗として扱う代わりに flakiness として吸収してしまう危険もあります（BE-0049）。対象は iOS
  シミュレータと Android API 30 以上、つまり各プラットフォーム自身のシグナルが事象を実際に確認できる
  範囲に限っています。それ以外では推測せず「確認できない」と答えます
  （[`docs/ja/evidence.md`](evidence.md#アプリのクラッシュ証跡be-0424)）
