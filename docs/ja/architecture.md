...
- **クロスプロジェクトのメトリクス比較ダッシュボード**（BE-0226）: `serve` の **Metrics** タブが、登録済みのプロジェクトを横並びで順位付けする — pass 率、flaky 率、p50/p95 の run 所要時間、そしてプロジェクトごとのトレンドスパークライン。BE-0102 のプロジェクト単位の集計をプロジェクトごとに 1 回ずつ実行して再利用する（`GET /api/metrics/projects`）。BE-0102 と同じく読み取り専用でアドバイザリ
