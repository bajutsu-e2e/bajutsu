...
- `SFSafariViewController` の要素ツリーを、それを描画しているプロセス `com.apple.SafariViewService` から読み取ります
  ...
  なく、要素の live な frame 中心を対象に actuation します。また閉じるボタンの identifier は、iOS のバージョンごと
  に異なる名前を `Close` へ正規化します。これにより、プロトコルの変更なしに、シナリオは `/elements` を通じて
...
- **サインイン時のアクティブ org 選択**（BE-0395）: GitHub のメンバーシップが複数の org に一致するログインは、
  ...
  ありませんでした。サインインはログインが act できるすべての org を、それぞれで持つロールとともに記録し、
  ヘッダーはそれらを単純なバッジの代わりにセレクタとして提示します。アクティブな org は依然として単一のカラム
  であり、`ServeState.org_of` が読む先も変わらないため、org スコープの読み書きは以前と同じ経路を通ります。
  `users.org_selected_at` の印が、ユーザ自身が選んだ org と単に解決されただけの org を区別し、サインインは選択を
  維持したまま、それ以外は解決し直します。単一 org のデプロイではバッジの見え方は変わりません
