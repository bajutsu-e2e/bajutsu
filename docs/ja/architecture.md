...
- idb バックエンドの `back` とデバイス制御（`setLocation` / クリップボード / `push`）を実機上で確認しています。`ios-e2e.yml` の `actuation (idb)` ジョブによる検証です（[BE-0281](../../roadmaps/BE-0281-ios-on-device-actuation-coverage/BE-0281-ios-on-device-actuation-coverage-ja.md)）。
