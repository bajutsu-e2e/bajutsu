...
関連: [concepts](concepts.md) · [開発者ガイド](developer-guide.md)（クラス単位の読解案内） · 各機能ページ（下のリンク）
...
| `common/mailbox.py` | `email` ステップ（BE-0046）向けの純粋でネットワークを使わない照合・抽出ロジック。メールボックスプロバイダのメッセージを正規化し、`to`/`subject`/`subjectMatches` で照合し、ステップ開始後に届いたメッセージだけを選び、正規表現で値を `${vars.*}` に取り出す | [scenarios](scenarios.md) |
| `common/totp.py` | `totp` ステップ（BE-0046）向けの RFC 6238 時刻ベースワンタイムパスワード（TOTP）。共有シークレットと時刻の純粋で決定的な関数なので、ネットワークもデバイスも使わずにコードを導出できる | [scenarios](scenarios.md) |
| `config/` | チーム既定 × アプリ別の解決（`Effective`）（パッケージ: `schema` / `effective` / `resolve` / `accessors`） | [configuration](configuration.md) |
| `common/backends.py` | バックエンド可用性判定、actuator 選択（プラットフォーム対応レジストリ: `ios` / `android` / `web` / `fake`）、Driver 生成 | [drivers](drivers.md#バックエンド選択と-actuator) |
| `common/devices/` | バックエンド間で共有するデバイス識別のヘルパ（パッケージ）: `os.py`（`device_runtime` ラベルから読む、解析済みのデバイス OS。プラットフォーム/メジャー/マイナー、BE-0358）、`id.py`（各バックエンドの `--udid`/config が共有するデバイス id バリデータ。argv オプションインジェクションを防ぎます）、`errors.py`（`simctl`/`adb` が継承するプラットフォーム中立な `DeviceError` 基底、BE-0260。その下に、デバイス操作が失敗ではなく期限の超過で終わったときにどのバックエンドも送出する `DeviceTimeout`（BE-0374）を置きます） | [drivers](drivers.md) |
| `common/backend_cli/` | バックエンドのコマンドラインラッパ（パッケージ）: `simctl.py`（`simctl` ラッパ。erase/boot/launch/openurl/io）、`adb.py`（Android バックエンドのコマンド構築側。clean-state/launch/deeplink/input/screencap。`simctl` の双子）、`adb_resident.py`（常駐 UI Automator サーバチャネルの Python 側、BE-0245） | [drivers](drivers.md#環境管理simctl) |
| `common/platform_lifecycle/` | `Environment` の seam（BE-0009）。1 つの `RunEnvironment`/`CrawlEnvironment` Protocol がプラットフォームごとのアプリ起動・readiness・relaunch・デバイス制御・teardown を担い、`common/runner/` と `crawl/cli.py` は actuator 名で分岐せず iOS/Android/web を同じインターフェースを通じて駆動（パッケージ: `protocols` / `factories` / `readiness` / `relaunchers` / `device_control` / `read_session`、加えて `environments/` ── `ios` / `xcuitest` / `xcuitest_live` / `android` / `web` / `fake`） | — |
...
| `common/runner/` | config + シナリオ → レポート。デバイスプール + launch 手順。`device_provider` の seam が、run のデバイスをどこから調達するかを解決する（組み込みの `local` pass-through に加え、予約済みの実機 iOS デバイスを Appium/WebDriver エンドポイント越しにエンドツーエンドで駆動する `appium` provider、BE-0238。他のクラウドベンダ種別は今後の追加課題）。`recovery` はバックエンドクラッシュのリトライ回数・wall-clock 予算の判定を持ち、オンデバイスのドライバ適合性スイートと共有します（BE-0334）。障害を分類する述語も 2 つ持ちます。`recovers_by_respawn` が再試行を判断し、`is_host_fault` がホスト側に原因のある失敗を診断します。固まったデバイスは、この 2 つに違う答えを返します（BE-0378）。`is_host_fault` が名指しするのはプラットフォーム中立な `DeviceTimeout` なので、この基底を採用したバックエンドはすべて対象に入ります（BE-0374）。後続シナリオを止める run 単位のラッチも `recovery` にあり、復旧を諦めた理由（予算を使い切ったのか、ホストが固まったのか）を保持します。さらに `recovery` は、pool の teardown 箇所・`launch_driver`・オンデバイススイートの lease 破棄が共有する、守られた teardown の方針も持ちます（BE-0342）。`mailbox` は `email` ステップの transport を、`kind` で鍵付けしたレジストリで解決します（出荷済みなのは `http` の JSON アダプタ、BE-0186）。形は `ai/registry.py` を踏襲します（パッケージ: `pipeline` / `pool` / `launch` / `device_provider` / `recovery` / `mailbox`） | [run-loop](run-loop.md#runner実行パイプライン) |
| `common/run_meta/` | 複数の機能が共有する run メタデータのヘルパ（パッケージ）: `files.py`（runs ルート名 + run ディレクトリのパスを介さない読み取り。一覧・manifest）、`id.py`（run id の生成・解析）、`root.py`（run ディレクトリの書き込みプロバイダ、BE-0331。後述の import 契約を参照）、`artifact_perms.py`（run の成果物への owner-only な `0700`/`0600` 権限、BE-0131）、`object_store.py`（証跡アップロードとサーバストレージ向けのバックエンド中立な `ObjectStore`。local/S3/GCS、BE-0110/BE-0204） | [configuration](configuration.md) |
| `run/notify` | `bajutsu run` 完了後に送る run 完了通知（Slack など） | — |
| `common/doctor/` | 規約充足度スコア（id カバレッジ等） | [configuration](configuration.md#doctor規約充足度スコア) |
...
| `common/_yaml.py` | `on`/`off`/`yes`/`no` を文字列のまま読む YAML ローダ | [scenarios](scenarios.md#yaml-の注意点) |
...
ここまでのジョブはどれもデバイスを 1 台しか起動しません。そのため、`common/runner/pool.py` の `device_pool` が並列実行に対して主張する保証を、どのジョブも観測できません。その保証とは、`--workers N` のもとで各ワーカーが固有のデバイスを借り受けることです。さらに各ワーカーは、共有された 1 つの run ディレクトリの下にある固有の `run_dir/<sid>` サブディレクトリにだけ証跡を書きます。モックのポートや索引も、ほかのワーカーのシナリオとは共有しません。[`DESIGN.md`](../../DESIGN.md) §3.3 が述べる「状態を共有しない」不変条件です。
...
**並行デバイスレーン**は、代わりに実際のデバイスを 2 台起動します。`pool (adb)`（`android-e2e.yml`）はエミュレータを 2 台起動します。状態に依存しない showcase のシナリオファイル 4 本を、1 回の `bajutsu run --workers 2` で走らせます。プールは作業を 2 台へ振り分け、両方のワーカーを同時にビジーに保たなければなりません。
...
このジョブは、専用の変更フィルタ（`scripts/e2e_changes.py` の `touches_pool`）で発火します。ほかのすべてのジョブが読む、レーン全体を対象としたシグナルより狭いフィルタです。Android レーンのほかのジョブの 2 倍のデバイスを起動するからです。このジョブは PR ごとのシグナルであり、Android レーンの必須集約チェックからは外してあります。BE-0282 の「まずシグナル、のちに必須」の道筋に乗せる理由は、障害注入レーンと同じです。1 つの runner でエミュレータを 2 台動かす作業は、このレーンでもっともホストの資源に左右されます。

iOS 側の対になるジョブ `pool (xcuitest)` は、Simulator を 2 台起動して macOS レーンで走っていましたが、BE-0298 で取り下げました。このジョブが分離の判定を返せたのは 5 回のうち 1 回だけです。残る 4 回は、プールの検査ではなくホストで崩れました。`SimRenderServer` は自身のディスパッチキューでクラッシュしました。`simctl uninstall` は固まった CoreSimulator に対してタイムアウトし、runner のチャネルは run の途中で到達不能になりました。しかも赤くなるたびに、10 倍課金の runner を 30 分ほど使いました。BE-0361 は、その runner を 3 コア・7 GiB と実測しています。Simulator を **1 台**起動しただけでゲストプロセスが 257 個立ち上がり、物理メモリの未使用分は 189 MB しか残りません。2 台目の Simulator は、すでに飽和した天井に対してゲストの数を倍にします。動画記録、タッチマーカー、シナリオの半分を削っても、崩れる位置が早まっただけで、崩れること自体は変わりませんでした。そのため、実際の並行デバイスに対する分離の検証は Android レーンだけが担い、iOS 側は高速スイートによる記帳の証明だけが残ります。
...
- `SFSafariViewController` の要素ツリーを、それを描画しているプロセス `com.apple.SafariViewService` から読み取ります
  ...
  ショットを合成します。アプリ側が同じサブツリーを重複して映し出す部分は剪定し、二重報告を避けます。`tap`/
  `doubleTap`/`press(forDuration:)` は、ブラウザの chrome が黙って無視してしまう `XCUIElement` 自身の呼び出しでは
  なく、要素の live な frame 中心を対象に actuation します。また閉じるボタンに identifier を与えない iOS の
...
- **複数ターゲットにまたがるシナリオ実行**（BE-0428）: シナリオのトップレベル `targets: list[str]` とステップ単位の
  `target: str` により、複数の宣言済みターゲット（たとえば iOS ターゲットと web ターゲット）へステップを自由に
  織り交ぜて振り分けられます。1 回の `bajutsu run` 呼び出しとして 1 つの合否判定にまとまり、`${vars.*}` はすべての
  ターゲットで共有されます（あるターゲットの `extract` ステップが取り込んだ値を、同じ run 内の別ターゲットに対する
  アサーションから読み取れます）。宣言済みのターゲットはすべて最初のステップの前にまとめて起動し、最後のステップの
  後にまとめて終了します。ステップや `if`/`forEach` の条件、`expect` のエントリが `target` を省略した場合、シナリオが
  宣言するターゲットが 0 個または 1 個であれば今までどおり動作し、2 個以上になると `target` の指定が必須になります。
  シナリオがこの形で自己宣言していれば `--target` は省略可能になり（代わりに `--scenario` が必須になります）、
  `--target` を明示した場合は `scenario.targets` に含まれているかを検査し、黙って上書きされることはありません。
  `RunResult.target_devices` と `StepOutcome.target` が各ステップを実行したデバイスとターゲットを報告し、単一
  ターゲット向けの既存フィールドは空のままなので、既存の JUnit/CTRF リーダーは単一ターゲットの run をこれまでどおり
  解釈できます。`bajutsu crawl` / `record` と `serve` 自身のディスパッチ UI はいずれも従来どおり単一のターゲットしか
  解決しません。`targets`/`target` をそこまで通すのはこの項目の対象外で、今後の課題です
- run パイプラインでの**バックエンドクラッシュ復旧**: シナリオ途中でバックエンドがクラッシュした場合（`base.BackendCrashError`、バックエンドに依存しない）、死んだリースを破棄し、新規に再生成したリースでシナリオ全体をやり直します。上限はリトライ回数（`crash_retries`、既定 1）と、再生成に費やす合計時間の任意の wall-clock 上限（`crash_recovery_budget`、既定は無制限）の 2 つで、クラッシュし続けるシナリオや復旧しないランナーを、黙って合格扱いにしたりジョブをハングさせたりせず、はっきりと失敗させます。再試行は、シナリオが `erase: true` を宣言したときにすでに得られるのと同じ `erase` の precondition を強制します。XCUITest バックエンドでは Simulator 自体の再起動（`simctl shutdown → erase → boot`）、adb バックエンドではアプリレベルのクリーンな状態化であり、直前にクラッシュしたそのデバイスへのその場での respawn ではありません。シナリオが `reinstall: overwrite` を宣言してリースをまたいだアプリのデータ保持を求めている場合、および `erase` の precondition をそもそも受け付けない2つの XCUITest 経路（実機、`xcuitest.deviceType: device`／live WebDriver エンドポイント。そこで強制すれば1シナリオの失敗では済まず run 自体が中断してしまいます）では、この強制をスキップします（単なる `erase: false` では不十分です。CLI はどのシナリオの `erase` も、パイプラインに届く前にすでに具体的な bool 値、多くの場合 `false` に解決してしまっているため、その値だけを見るガードでは、この仕組みがまさに対象としている経路そのもので強制再試行が無効になってしまいます）。強制 erase 用のリース自体がデバイスレベルの不具合（`simctl.DeviceError`/`adb.DeviceError`。`BackendCrashError` とは別系統の型であり、その派生ではありません）で失敗したときは、この不具合をこのループの外まで伝播させて run 全体を中断させるのではなく、同じその場での respawn に切り替えます。ただし、不具合がタイムアウト（`device_errors.DeviceTimeout`）であるときは切り替えず、シナリオを失敗させて run をラッチします（BE-0374）。拒否したデバイスは 1 つの操作についての証拠を出していますが、まったく応答しなかったデバイスが示すのは、あらゆる操作の背後にあるサービスが応答をやめたことだけです。そこへ再試行するのは、タイムアウトが否定したことをそのまま前提に置く行為です。もう1つの run 単位の wall-clock 予算（`run_crash_recovery_budget`、こちらも既定は無制限）は、クラッシュ復旧に使う時間をシナリオ単位でリセットせず run 全体を通して上限を設けるもので、劣化し続けるデバイスは、各シナリオが自分の予算を静かに使い切って外部の CI タイムアウトがジョブを打ち切るのを待つ代わりに、run 自体をはっきりと失敗させます。最終的に成功する復旧にこの run 単位の予算を使い切っただけでは、何もラッチされません。それはデバイスがまだ機能していることを示しているにすぎないからです。あるシナリオ自身のクラッシュ再試行ループが、この予算を主因として実際に失敗した後は初めて、以降のどのシナリオも、自身の最初のリースを試す前の時点で即座に失敗します。この確認はクラッシュ再試行ループの中だけでなく、各シナリオの先頭でも行われます。これにより、すでに復旧できないと分かっているデバイスに対して、残る全シナリオが同じ打ち切りに至るまでにコールド起動の試行を1回ずつ余計に払わずに済みます。デバイスの準備がタイムアウトした場合も、同じ限定を要さずに同じラッチを立てます（BE-0374）。ラッチはどちらの原因で立ったかを保持します。そのため、飛ばされたシナリオは予算ではなく固まったホストを報告します。オンデバイスのドライバ適合性スイートはシナリオ単位の判定を共有しており（`common/runner/recovery`）、Simulator のインフラ障害が起きても、無関係な PR で必須チェックを赤くすることなく同じように復旧します（BE-0334）。Simulator の XCUITest 経路では、再試行は erase より強い段として**デバイスの置き換え**を持ちます（BE-0354）。消えたデバイスを置き換えるときと同じ手順で新しいデバイスを作り、劣化したデバイスはシャットダウンしてプールから外します。erase が消すのはデバイスのデータであって、固まった画面キャプチャの機構ではありません。そのため、強制 erase 付きの再試行がまたクラッシュした場合はこの段へ進みます。映像の録画が書き込みを始めたことを確認できないまま終わった試行も、この段へ直接進みます。その兆候は、erase の効かない劣化を言い当てているからです。置き換えの試行では強制 erase を外します。これから作成するデバイスには消すものがないからです。この段が働くのは、udid を固定しておらず `appPath` を持つ run にかぎるので、`--udid` で名指しした run はそのデバイス上の erase による再試行を保ちます。置き換えが消すものは erase より多いため、erase の段が尊重する `reinstall: overwrite` と `bajutsu run --no-erase` も同じように尊重します
- **iOS クラッシュレポートのシナリオ単位での取得**（BE-0421）: XCUITest のクラッシュ再試行を使い切って失敗した
  シナリオは、そのシナリオ自身の証跡ディレクトリ `runs/<run_id>/<sid>/` の下に `crash-diagnostics/`
  サブディレクトリを持ちます。そこには
  ランナー自身が出力したログの末尾（上限あり）と、クラッシュした `xcodebuild` プロセスに対して macOS が
  書いた `.ips` クラッシュレポート（書かれていれば）が入ります。取得はリースの解放時点で行い、同じ warm な
  environment を後続シナリオが再利用して上書きする前に済ませます。シナリオ自身の失敗文字列も、この
  サブディレクトリを直接名指しします
...
- DSL のオーサリング再利用: 再利用可能なパラメータ化コンポーネント（`use` / `${params.*}`。`/` を含む参照や `.yaml` / `.yml` で終わる参照は、ファイル横断のコンポーネントファイル（BE-0030）として解決します。どちらでもない名前だけの参照は、同じシナリオファイルにインライン宣言した `components:` ブロック（BE-0422）を参照します）、データ駆動シナリオ（`data` / `dataFile` と `${row.*}`）、シークレット変数（`${secrets.X}`、値マスク）、シナリオタグ + `--tag` / `--exclude` 選択、`setLocation` / `push` デバイスステップ、起動前の `permissions` フィールド（`simctl privacy` / `pm grant`|`pm revoke`、BE-0276）、`doubleTap` アクション、ファイル単位 + シナリオ単位の `description`
...
- DSL の `setPickerValue`（BE-0356）: ホイール式のピッカー（`UIPickerView`、またはホイールのみのモードに切り替えた `UIDatePicker`）を、解決済みのホイールに対して XCUITest 自身の `adjust(toPickerWheelValue:)` を呼び出すことで、指定した行へ動かします。`tap` と同じくハンドルベースの操作であり、`swipe`/`drag`/`scroll` のような座標ベースではありません。ホイールの行は座標ドラッグで確実に止められる個別の要素ではないからです。ホイールが持たない値を指定した場合は、ホイールを止まった場所に残したまま、その値を名指ししてステップを失敗させます。複数コンポーネントのピッカー（年のホイールと月のホイールが並ぶような場合）は、セレクタの既存の `within`/`traits`/`index` フィールドでコンポーネントごとに 1 ステップずつアドレス指定します。`PICKER_WHEEL` capability でゲートされており、これを宣言するのは常駐 runner を持つ XCUITest バックエンドと `FakeDriver` だけなので、Android と web はデバイス側の作業に入る前に preflight で拒否されます
- DSL の `selectPhotos`（BE-0433）: 開いている `PHPickerViewController` のグリッドから、序数の `indices` で 1 枚以上の画像を選び、ピッカーの確定コントロールをタップします。確定コントロールは自身の identifier を持たないため、ピッカーのナビゲーションバー内で消去法により構造的に解決します。グリッドの各セルは 1 つの identifier（`PXGGridLayout-Info`）を共有しており、`handleSystemAlert` がラベルのない SpringBoard のボタンを扱うのと同じ方法で、既存の `index` セレクタフィールドによって個々のセルを区別します。actuation は解決したセルの正確なフレーム中心への生の座標タップです。この再利用型のコレクションビューに対しては、同じセルへのハンドルベースのタップが失敗する（`element vanished`/`ElementNotTappable`）ことが確認されているためです。`seedPhotos` precondition（`Preconditions.seed_photos`）は `simctl addmedia` を介して Simulator の写真ライブラリにフィクスチャ画像をシードし、グリッドの内容を再現可能にします。同じ preconditions の `erase: true` を要求し、ロード時に検査します。再利用中のライブラリでは何もシードされないまま黙って通ってしまうためです。`SELECT_PHOTOS` capability でゲートされており、これを宣言するのは XCUITest バックエンドだけなので、Android と web は preflight で拒否されます

#### DSL のシステムアラートと tip の処理

- DSL の `handleSystemAlert`（BE-0316）: SpringBoard の権限プロンプトのボタンを、ネイティブなアクセシビリティ照会（ランナーの 2 つ目のオンデマンドな SpringBoard ハンドル）で tap する、決定的で iOS 専用のステップです。解決は Python 側の `resolve_unique` に残ります。この能力を宣言するのは XCUITest バックエンドだけなので、Android と web は preflight で失敗します。label が決定的になるのは、XCUITest のライフサイクルが cold spawn のたびに **Simulator 自身の**システム言語を run の `locale` に固定するからです。SpringBoard はアプリの launch 引数が届かない別プロセスなので、グローバル設定ドメインへの書き込みと再起動 1 回で行い、warm な runner の再利用はその locale が一致していることを条件にします。`permissions` で先回りできないプロンプト（通知の許可、ATT、そしてプロセスをまたぐペーストの同意。BE-0369）については、`sel` の代わりに `prompt` と `choice` も指定でき、固定した locale が描画する label を run が解決します（BE-0320）。プロンプトを待つ*待機*を持っているのはドライバではなくオーケストレータです（BE-0406）。ドライバは SpringBoard を 1 回照会して tap するだけで、`wait_for_system_alert` がステップ自身の期限までポーリングします。毎ポーリングでまずステップの対象を読み、そのあとに次項のリアクティブなガードを回します。待機をここに置いたからこそ、ステップが待っているあいだにガードが割り込みを片付けられます。ドライバ自身のループの中では他の何も動けず、宣言済みのプロンプトがステップの `timeout` のあいだ画面を覆ったままでした。ガードにはステップのセレクタを渡してあり、それが名指しするアラートには手を出さないので、両者が同じプロンプトに答えることはありません
- そのリアクティブな対極である DSL の `systemAlertHandling`（BE-0315）: ステップまたは `wait` がブロックされたときにだけ発火するアラートガードです。`handleSystemAlert` と同じ SpringBoard 照会を独自の間隔（既定 1 秒。wait のポーリング周期からは切り離しています）でポーリングし、シナリオ自身の `rules` が名指しするプロンプトに答えます — モデル呼び出しはなく、BE-0316 の配管を並行 API を足さずに再利用しています。BE-0402 以降、このガードは全経路が決定的です。決定的な経路がどれも対処できないケース（能力を宣言しないバックエンド、どの規則も同定しないアラート、列挙できないブロック要因のうちどの規則も同定しないもの。規則が同定するものはツリー内のタップが片付けます）では何もせず、見たものが失敗に乗ります。ガード対象の `wait` は必ず注記を運び（そのゲートは毎ポーリングでツリーを見ています）、wait の外のステップは、ネイティブ照会がアラートを列挙できたときだけ運びます。注記は自身のタイムアウトで見たものを名指しします。`… — an unhandled system alert is blocking the screen (buttons: Allow, Don't Allow)` の形、照会が列挙できなかった場合はボタン名を挙げない曖昧な形です。BE-0402 より前は、こうしたケースでスクリーンショットをモデルに読ませる AI 視覚ガードへフォールバックしていました。`visionInstruction` が操作していたのはそのフォールバックだけなので、`run` はどのシナリオも実行する前にこれを拒否します。黙って無視すれば、許可を与えるつもりで書いた設定が既定の dismiss ラベルに落ちて拒否に反転するからです。続く BE-0406 は `rules` をガードの宣言のすべてとし、`labels` を削除しました。ボタンのラベルが名指ししていたのはボタンであってアラートではなく、シナリオが記述していない画面へのタップまで許してしまっていたからです。`rules` は 2 つの層（シナリオ、次にターゲット）を内側から連結し、唯一残ったスカラーである `pollInterval` は値を持つもっとも内側の層を取ります。プロンプトと選択の組をフラグの値 1 つで読みやすく運べないため、コマンドラインが与えられるのは `pollInterval` だけです。ラベル表はプロンプトごとに、3 つの応答面（`handleSystemAlert` ステップ、ネイティブの SpringBoard 照会、ツリー内タップ）のどれに届くかを記録します。iOS がアプリ自身のプロセスに出す `savePassword` は、規則としては宣言でき、答えられるのはツリー内タップだけです。ステップのほうは、空のボタン一覧を期限までポーリングする代わりに、解析時にこれを拒否します。1 回限りの dismiss を行う 2 か所（ステップ末尾の dismiss と `expect` の再試行）は、そのあと画面が動かなくなるまで待ってから読み直します。上限は 1 秒でベストエフォートです。tap されたプロンプトは、まだ消えたプロンプトではありません。この待ちがないと、再試行はアニメーション途中のツリーを判定し、それだけの理由で失敗しえます。前項でオーケストレータへ移した `handleSystemAlert` の待機と同じく、固定の sleep ではなく条件待ちで、上限に達したらそのまま先へ進みます。判定を左右することはありません。BE-0418 以降、この 1 回限りの呼び出し自体が、最大 3 ラウンドまで内部でループします。最初の 1 件を片付けた時点では呼び出しは終わりません。SpringBoard 側のアラートの奥にアプリ自身のシートが控えていても、そのシートを読む前に呼び出しが終わることはなくなりました。ボタンがまだアニメーション途中のスクリムに覆われていてタップが届かなかった場合も、`wait` 中のガードがすでに持っている有界の再試行と同じ仕組みで、次のラウンドが同じタップをやり直します。上で述べた画面静止待ちは、何かを片付けたラウンドや、タップがまだ届かないとわかったラウンドのたびに実行されます。この呼び出しですでに片付けたはずのまだフェード中のアラートへのタップを見送ったラウンドや、ラベルが重なってどの規則も一意に一致しなかったラウンド、問い合わせでは見えていたボタンが実際のタップの時点ではすでに消えていてタップが空振りに終わったラウンド、そして前のラウンドのツリー内タップがまだスクリムに阻まれて届いていない状態でツリーの読み取りが何にも一致しなかったラウンドも同様で、上限に達したラウンドも例外ではありません。呼び出しが戻った直後に画面を読む側は、まだアニメーション中のツリーを読まずに済みます。どの規則も同定しないアラートに行き当たったラウンドも、次のラウンドを続けることで読み取りが変わりうる場合に限り、呼び出しを終えるのではなく settle してから次のラウンドに進みます。続ける価値があるのは、この呼び出しがすでにネイティブのアラートを片付けている場合か、どこかのネイティブの規則の形が（一意にではなく、かつその規則自身の除外ラベルを伴わずに）この読み取りにすでに現れている場合のどちらかです。2つの規則が同じタップ対象のラベルを共有していることがあり（組み込みの notifications と tracking はどちらも許可を選ぶと Allow をタップします）、片方のまだ残るフェードのボタンともう片方の生きたボタンを一緒に読むと、どちらの規則もボタンの一意性チェックを満たせないためです。この衝突は、まだネイティブのアラートを片付けていないこの呼び出し自身の最初のラウンドでも起こり得ます。両方のアラートがすでに画面に並んでいることがあるからです。settle によって得られるのは、両方を同時には読み取らない後の読み取りです。すでに答えたほうのアラートのフェードが抜けることもあれば、もう一方のアラートが画面から消えることもあります。どちらにせよ、生きているほうのアラートが一意に読み取れて、そのラウンドで解決します。このとき報告する注記は、この呼び出しがすでに答えたラベルを1個ずつ差し引いた残りを名指しするため、片付けたアラートを名指しし直すことはなく、それでいて共有ラベルを持つ別のまだ生きているアラート自身の分は名指しされたままになります。ネイティブのアラートを片付けておらず、ネイティブの規則の形も読み取りに現れていなければ、抜けるべきフェードや待つべき衝突は存在しないため、そのラウンドで呼び出しを終えます。ただし、前のラウンドのツリー内タップがまだスクリムに阻まれて届いていない場合は別です。それ自体がこの呼び出しでまだ手を打てる対象であるため、そのラウンドは呼び出しを終えるのではなく settle してから次のラウンドに進みます。後のラウンドは同じ画面を読み直すか、アラートが自然に消えるのを見るだけで、続けるとこのラウンドの診断を失う恐れがあるからです。それ以上手を打つ対象が見つからないラウンドは、そこで呼び出しを終えます。ただし、見つからなかった理由が、この呼び出しですでに片付けた形がまだ残っていることだけだった場合、あるいは前のラウンドのツリー内タップがまだスクリムに阻まれて届いていない場合は別です。前者では settle してから次のラウンドに進み、フェードの奥に控えるシートへ手を伸ばす機会を残します。後者では、何にも一致しない読み取り自体はスクリムが外れた証拠にはならないため、同じく settle してから次のラウンドに進みます。ネイティブ側のアラートは、その静止待ちより長くフェードが続くと、後のラウンドでも同じ内容のまま読み取れてしまいます。この呼び出しですでに片付けた規則と同じ形、またはその形と入れ子になる形（狭いほうでも広いほうでも）にもう一度一致する場合、どちらの面でも、そのラウンドはタップ自体を見送ります。別のアラートとしてではなく、その dismiss がまだ消え切っていないものとして扱うためです。判定にはネイティブ照会が読んだボタン一覧ではなく、一致した規則自体の形を使います。ボタン一覧は SpringBoard がそのとき抱えているアラートすべてをまとめて数え上げたものなので、別のアラートが列に加わったり抜けたりするたびに変わってしまい、片付けたはずのアラート自身は何も変わっていなくても新しいアラートに見えてしまうからです。おかげで、途中で別のアラートが列に加わっていても、再タップが実機に届いてなにもない場所や閉じつつあるアラートの下から現れたものに当たってしまう心配はありません。タップ対象のラベルだけが同じで、一致する規則の形が異なる後続のアラート（組み込みの `notifications` と `tracking` はどちらも許可を選ぶと "Allow" をタップします）は、すでに片付けたほうと一緒に読み取られなくなった時点で、規則の形が違うため別物として通常どおりタップします。両方が同時に読み取れているあいだは、そのラベルの個数を分け合うためどちらの規則も一意に一致せず、そのラウンドはこの面をどの規則も同定しないものとして報告します。すでに答えた形にもう一度一致したからといって、それだけでラウンドが終わるわけでもありません。そのフェードのすぐ隣に、まだ答えていない本物のアラートが並んで見えていることもあるため、見送る前にまだ答えていない形の中を、ネイティブ側とツリー側のどちらでも探し直します。再タップを見送ったラウンドも、照会が読んだボタンをこれまでに答えたすべての規則と突き合わせます。この呼び出しがすでに片付けた規則のすぐ隣に、どの規則も同定しない第二のアラートが並んでいることもあり、見送ったこと自体は残り全体が片付いた証にはならないからです。ツリー側でタップが実際に届いた規則は、その形と、その形と入れ子になる形（狭いほうでも広いほうでも）まで含めて、この呼び出しの残り全体にわたって一致の対象から外れます。savePassword の3つの形のうち2つがまさにこの関係にあり、どちらも同じボタンをタップします。そのため完全一致だけを除外していると、まだ消え切っていない同じシートに狭いほうの形が一致して、二度目のタップが届いてしまいます。判定に使うのは規則の形であって、タップ対象のラベルではありません。1 つのアラートの形を、選択の異なる 2 つの規則が共有していることがあるからです（ターゲットの選択をシナリオの選択が上書きする場合など）。タップ対象のラベルで絞り込むと、すでにタップした規則だけが外れてその片割れが繰り上がり、この呼び出しがすでに答えたはずのアラートに一致して、逆の選択のボタンをタップしてしまいます。形で絞り込めば、同じ形を共有する規則はまとめて外れます。上限まで手が届かなかったボタンは、どの規則も同定しないアラートの注記とは別の、自分自身の注記に名指しされ、単なる要素未検出としては読めなくなります。この呼び出しがタップしたネイティブのアラートも、画面に残っているものがこの呼び出しのすでに答えたもの（この呼び出し自身の前のフェードを含む）だけになった状態で上限を使い切れば、同じ注記を運びます。タップした後、この呼び出しの最後のラウンドの読み取りでもなお、規則が名指ししているアラートが生きたまま残っていることが、タップが実際には届かなかった証拠です。タップ以降の全ラウンドにわたる連続記録ではありません。途中に別の種類のラウンドが挟まれば、その連続は途切れてしまうからです。タップではなく競合で外れたラウンドも、一致した規則の形が最終ラウンドでもまだ列挙されており、かつ同じ呼び出しの前のラウンドでも同じ形が競合していれば、同じ注記を運びます。このときこの呼び出しはタップが実際に届いたことを一度も確認していません。この二度目の競合こそが証拠になります。競合したラウンドの読み取りには、タップが競合する前に規則が一致した、まさにその読み取りである以上、その形が含まれているのは当然だからです。したがって競合が一度きりでは何も証明にならず、それ自身のタップが読み取りに残っているのでない限り、そのラウンドは沈黙したままです。注記はその最終ラウンド自身の読み取りに対して規則を新たに解決し直したもので、前の、すでにタップ済みの形の古い記録には頼りません。そのため、毎ラウンド同じアラートと競合し続ける呼び出しも、黙り込むのではなく名指しします。また、別のアラートをきれいにタップした直後に競合したラウンドも、そのタップ済みの無関係なアラートではなく、実際に競合した規則のほうを名指しします。この呼び出しがタップ済みのツリー内シートも同様で、上限を使い切った時点でまだそのシートが列挙され続けていれば、同じ注記を運びます。判定はその形自身のラベルだけを見て、ツリーの残り全体の同一性は問いません。ただし、まさにその上限を使い切ったラウンド自身がタップしたシートは対象外です。settle を一度挟んだだけでは、まだフェード中のアニメーションと、本当に消え切らないシートを見分けるには足りず、それより前のラウンドのタップであれば必ずある「1 ラウンド分と settle 一度分の間」を、その回のタップは持たないからです。ごく普通に片付いたはずのシートが、そのフェードが settle 一度分より長引いただけで片付いていないと報告され、呼び出し元の再試行を失わせることのないようにしています。シートがタップを受け付けたのに閉じず、バリデーションエラーを再提示することがあります。これは構造上ツリーを変化させるため、ツリー全体が変わっていないことを求めると、そのケースを排除してしまいます。この判定は、この呼び出しがツリーから片付けた形すべてを対象にしており、直近にタップした形だけを見るわけではありません。前の方でタップしたシートが閉じないまま残っていても、その後に別の形のシートをタップして片付けてしまうと、比較対象がそちらだけに移り、前の方のシートは報告されないままになってしまいます。閉じたはずのシートの奥から現れたアプリ画面が、まったく同じボタンのラベルを持つこともあります。その場合は閉じなかったシートと誤認されます。exclude によってこの呼び出しはどちらのケースでも二度とそれをタップしないため、誤った注記が二度目のタップにつながることはありません。ただし、コストは注記の不正確さだけにとどまりません。同じ包含判定は、そのタップが本当に得た `AlertEvent` も取り消してしまいます。本当に閉じたシートが、片付けていないものとして報告されるということです。しかも、それがこの呼び出しにとって唯一の片付けだった場合、呼び出し元の一度きりの再試行は、実際には動いたはずの画面に対して行われずに終わります。この注記がなければ、あとから名指しする機会は二度と訪れません。ツリー側のこの注記は、一致するものが見つからないだけのラウンドでも、どちらの面であれ無関係なアラートを片付けたラウンドでも消えません。ただし、どの規則も同定しない生きたアラートを見つけたラウンドだけは別です。そのラウンドがツリーを読むのは、ネイティブ側の面が正真正銘空だと答えたときに限られるため、ツリー側の診断がまだ有効かどうかをそのラウンド自身は確かめようがなく、いま確認できる、より新しいネイティブ側の所見のほうを報告します。ネイティブ側の、まだ答えていないボタンを名指しした注記は、これほど長くは残りません。消える理由は、後続のラウンドが何をしたかによって2通りあります。無関係なネイティブアラートを片付けたラウンドでは、そのラウンド自身が読んだボタンをもとに注記を作り直すため、それ以前の注記をそのまま引き継ぐことはありません。一方、その回のネイティブ照会自身がその面にアラートがないことを確かめたラウンドでは、アラートがないという決定的な事実に診断が逆らって残ることはできません。どちらの診断も、タップが実際には届かなかったと結論づけた時点で、そのタップが記録した `AlertEvent` を報告から取り消します。取り消さずに残すと、同じプロンプトについて矛盾する2つの事実を運ぶことになります。片付けたと数える記録と、まだ片付いていないと告げる注記です。呼び出し元はその後、一度きりの再試行を、すでに塞がれているとわかっている画面へ無駄に費やしてしまいます。タップは着地した瞬間にしか記録できません。届いたタップと、アプリ側が反応しなかったタップを、その時点では区別できないからです。上限までラウンドを使い切っても同じ形を読み取れることが、はじめてそれを見分けられる瞬間です。この呼び出しが返す値も、ラウンドごとに数えるのではなく、この報告全体の増減から求めます。おかげで、後から取り消しが起きても、この呼び出しが一度も読み直していない、前のほうの本物の片付けはそのまま数えられます。このガードがポーリングするよりも前に、進行中の操作に割り込むアラート自体は XCUITest 自身が処理します。何もしなければ XCUITest はアラート自身の既定ボタンで応答してしまい、シナリオのポリシーをレポートに何も残さないまま黙って上書きします。そこでランナーは割り込みモニタを組み込み、リアクティブなガードと同じく規則が名指すボタン（そのラベルはオーケストレータが解決し、シナリオごとに 1 回 `POST /interruptionPolicy` で送り込みます）を押したうえで、押したボタンを通常の `AlertEvent` として記録します。ガードがシナリオを対象にしているかどうかも、このとき同じ 1 回で送ります。この面が決して出会えない規則は送らずに落とします（プロセス内のプロンプトは、別プロセスの操作に割り込みません）。`handleSystemAlert` ステップ自身が使う prompt/choice の形は、この rules に含まれるとは限りません。ステップという形があるのは、`systemAlertHandling` へ宣言しない選択を認めるためです。オーケストレータは、そのステップが待っている間だけ、シナリオ自身の規則に加えてステップ自身の対象を 1 つ追加で送ります。ステップが答え終わる、失敗する、あるいは待機自体が例外で終わるいずれの場合も、シナリオ本来の方針に戻します。これがないと、先行する操作の割り込みが同じプロンプトと先に出会い、一致する規則がなく、このステップがまさに正しく答えようとしていたアラートのせいで失敗することがあります。ポリシーがボタンを名指ししないプロンプトは、従来どおりXCUITest 自身の既定ハンドラに委ねられます（BE-0399）。この tap 自体は変えられません。変わるのは沈黙のほうです。BE-0406 以降、対象になっているガードは、辞退する前に、規則が同定しなかったボタンの一覧を記録しておきます。その割り込みに遭遇したステップや `expect` は、シナリオに代わって何かが答えたかのように続けるのではなく、ボタン名を添えて失敗します。フォアグラウンドの通知バナーも同じモニタに届きますが、ポリシーを見るより前に別の分岐へ入ります（BE-0416）。バナーは規則が同定できるボタンを持たず、唯一タップできる領域を押すと通知元のアプリが開いてしまうからです。モニタはバナー自身の実測フレームを起点に上方向へスワイプし、消えたことを確認してから割り込みを引き受け、`notificationBanner` 種別の `AlertEvent` として記録します。並ぶ文字列はボタンではなく通知本文だからです。この分岐が `governs` の判定より前にあるのは、アラート側のどちらの結末もバナーには合わないためです。対象になっているポリシーはバナーを未宣言の割り込みとして記録し、本来通るはずのステップを落としていました。対象外のポリシーはバナーを XCUITest が待ち切るまで放置し、割り込まれた操作 1 回あたり約 9 秒かかっていました（割り込みがなければ約 0.6 秒です）。`systemAlertHandling` 自体は既定で有効で、`false` でシナリオ単位に無効化できます。バナーの分岐は、`systemAlertHandling` の指定にかかわらず動きます。この BE-0418 では、上の 1 回限りの呼び出しと合わせて、`wait` 中のガード自身が届く範囲も変わりました。`probed_absent` は `state == "absent"` 単独から `state == "absent" and not buttons` へと絞られました。この状態は、確認と実際のタップの間に一致した規則自身のタップが競合で外れたラウンドにも答えるようになったため、そのラウンドはもうツリー内タップを許しません。空のボタン一覧が示す場合とは違うからです。競合の裏に控えているアプリ自身のシートは、SpringBoard側の読み取りが本当に空になるまでタップされないままです。競合が次のポーリングで解消すれば `poll_interval` 1 回分だけですが、解消しなければ、この後は何もこのタップを許可しなおさないため、wait の残り時間全体に及びます。この空白が始まる前からすでにタップを試みていたシートが、その空白の分を咎められることはありません。ツリー内 dismiss がまだ届かないボタンを見張る待ち時間の上限は、タップを差し止めるポーリング（競合、片付け、どの規則も同定しないアラート、ステップ自身が予約したアラート）が挟まったぶんだけ、後ろへずらされます。リセットではありません。タップが許されていたあいだにすでに費やした時間はそのまま残ります。そのため、そうしたポーリングのあいだにスクリムが外れていたシートは、タップがふたたび許されたポーリングで改めて再試行を受けられる一方、恒久的に塞がれたシートは、差し止めが何度挟まっても give up にたどり着けます。同じポーリングは、この競合を理由に `blocked_note` を無条件に消すこともやめ、代わりにどの規則も届いていないボタンを名指しする自分自身の注記を作れるようになりました。これは、単なる「absent」の答えだけではこれまで出さなかった注記です。「unhandled」の注記も 3 つの点で変わりました。1 つ目は、規則が同定していたアラートで、そのタップがラベルを二重に見つけた場合です。この場合、どの規則も同定しないアラートとしてではなく、ガードが片付けられなかったプロンプトとして名指しするようになりました。2 つ目は、同じ読み取りに並ぶ、どの規則も同定しないボタンです。曖昧だった規則自身の診断がそのボタンを覆い隠して `poll_interval` 1 回分のあいだ報告し損なうのではなく、`raced` 分岐と同じやり方で名指しするようになりました。3 つ目は、ツリー側の give up がまだ立っている間、この注記をそのまま残すようになったことです。ただし、それはそのポーリング自身のツリー読み取りが、シート自身の形がなお一致していると積極的に確認できるとき（ツリーが読める状態で、かつその形が一致しているとき）に限られます。SpringBoard アラートがツリーを畳んでしまったポーリングはどちらとも確認できないため、まさにそのポーリングでは注記のほうが道を譲り、ツリー側がすでに諦めたシートの上に未宣言の SpringBoard アラートがブロックしている `wait` は、より新しく確実なアラート自身のほうを報告します。ラッチそのものが解除されるのは、後続の、ツリーが読める状態のポーリングがそのシートの消失を確認した時点です。この give up は、ネイティブ照会がその面を空だと答えるのを待たずに、そのポーリング自身の読み取りから自ら解除されるようになりました。生きている未宣言のアラートは、その照会自体を空だと答えさせないからです。この変更より前は、シートがすでに閉じたあとも、あるいはアラート自身が立っている間も、give up の注記がそのまま残り続け、実際に画面をふさいでいるアラートが名指しされることはありませんでした。退役はラッチだけでなく、保留中のラベルとタップ回数、記録済みの署名とイベント、タップ不能の猶予も手放します。おかげで、同じシートが新たに現れたときは、すでに使い切ったタップ回数を引き継いで即座に諦められることなく、あらためてタップを受けられます。さらに `_dismiss_from_tree` は、これまでの宣言順ではなく、幅優先の `tree_dedup_rules` で解決するようになりました。この幅優先の解決順は、この変更で 1 回限りの dismiss にも新たに与えられたものです。ツリー内の 2 つの形が入れ子になっているとき、シナリオでの宣言順によらず幅の広いほうを先に一致させます。この 2 つは同じ画面を見る宣言上の双子であり、`wait` が実行中だったかどうかでシナリオの押すボタンが変わってはなりません
...
- **サインイン時のアクティブ org 選択**（BE-0395）: GitHub のメンバーシップが複数の org に一致するログインは、
  従来はランキングが先に置いた org へ固定され、ヘッダーのバッジには他の org が存在することを示す手掛かりが
  ありませんでした。サインインは、そのログインが振る舞えるすべての org を、それぞれで持つロールとともに記録し、
  ヘッダーはそれらを単純なバッジの代わりにセレクタとして提示します。ただし admin Team によって受け入れられた
  admin は例外で、サインイン時に保存した集合ではなく、生存しているすべての org をその都度読み直して提示します。
  サインイン後に Orgs ページで作った org も、再サインインなしにそのまま現れます。アクティブな org は依然として
  単一のカラム
  であり、`ServeState.org_of` が読む先も変わらないため、org スコープの読み書きは以前と同じ経路を通ります。
  `users.org_selected_at` の印が、ユーザ自身が選んだ org と単に解決されただけの org を区別し、サインインは
  選択がなお適格であるかぎりそれを維持し、適格でなくなった場合は解決し直します。選んでいた org から外された
  ユーザは、新しい適格リストの先頭に着地します。単一 org のデプロイではバッジの見え方は変わりません
...
- **有効な config の binding とは独立した、job 単位のアーティファクト差し替え**
  （[BE-0431](../../roadmaps/BE-0431-job-scoped-artifact-override/BE-0431-job-scoped-artifact-override-ja.md)）：
  ホスト型デプロイでは、`POST /api/run` が `binaryArtifact` と `scenariosArtifact` を受け付けます。
  どちらも、保存済みのアーティファクトの sha256 です（BE-0268）。指定した方の脚はそのアーティファクト
  だけを解決先とします — binary はその job 自身の target の `appPath` に配置し、`scenarios` の zip
  はその target の scenarios ディレクトリを置き換えます。指定しなかった脚は従来どおり org の有効な
  config の binding を解決先とするため、そちらは変更されません。この結果、1つのデプロイに対する
  2つの同時 CI ディスパッチが、デプロイのフォールバック binding を奪い合ったり、org の記憶済み設定を
  書き換えたりすることがなくなります。worker はジョブのワークスペースを override の識別子でキー付け
  するため、同じ worker 上の後続ジョブが汚染されることはなく、bundle のキャッシュ木も再取得せずに
  再利用します。単一プロセスの `serve`（`LocalExecutor` は配置を隔離するワークスペースを持たないため）
  と、クラウドバッチの `run-set` ファンアウト（この item が対象とする分割 worker トポロジでは動作
  しません）では拒否されます。差し替えた sha256 はそれぞれ run の manifest の provenance に記録
  されます
...
- ネットワークの実経路を実機上で動かしています。`BajutsuKit` のアプリ内 `URLProtocol` がモックされたリクエストに自前のスタブで応答し、各 exchange をループバック上の collector へ報告する、という一連の流れです。`ios-e2e.yml` の `network (xcuitest)` ジョブ（`make -C demos/showcase e2e-network`。[BE-0282](../../roadmaps/BE-0282-real-backend-network-coverage/BE-0282-real-backend-network-coverage-ja.md)）が PR ごとに検証します。このジョブは `network_mock.yaml`（スタブが 201 で応答する `POST /post`。実サーバなら 200 を返します）と `network_live.yaml`（スタブを介さないカタログの `GET`。観測されたという事実だけをアサートします）を実行し、続いて永続化された `network.json` を `demos/showcase/network/assert_network_evidence.py` でアサートします。モックされた exchange に `mocked` が立ち、その `Authorization` ヘッダと `password` のフィールドがマスクされていること、生の秘密情報がファイルのどこにも残っていないこと、そしてスタブを介さない exchange が `mocked` を false として記録されていること、つまりマッチ範囲の広すぎるモックが、何もスタブしていない通信を自分のものだと主張できないことです。*実際にキャプチャした*資格情報が出荷される証跡でマスクされるかを、iOS 上で観測できるのはここだけです。純粋な redaction のテストは、すべて手で組み立てた exchange をアルゴリズムに渡すからです。このジョブはゲート対象外です。実機の新しいカバレッジはまずシグナルとして着地させます。下記の web 版が `E2E (web)` に加わる前にたどったのと同じ道筋です。
...
| 機能 | 現状 | 場所 |
|---|---|---|
| `mockServer`（外部モックコマンド） | config スキーマのみ。`cmd`/`port` の外部サーバは**未実装**で、シナリオ `mocks`（宣言的なプロトコル内スタブ、実装済み）で代替する | `common/config/schema/mock_server.py` `MockServer` |
