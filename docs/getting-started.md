...
This is the fastest way to confirm everything is healthy — ~150 tests that exercise the
...
[`sample/scenarios/smoke.yaml`](../sample/scenarios/smoke.yaml):
...
This produces `BajutsuSample.app` under `sample/build/…`. (The `.xcodeproj` and `build/` are
