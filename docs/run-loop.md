...
> **Backend-crash recovery (BE-0049).** The proactive refresh above narrows the crash window but
...
> loudly, so flakiness is never absorbed into a silent pass (BE-0049). Because the retry replays the
