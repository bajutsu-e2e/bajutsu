...
- The `serve` local web UI (Tier 1): author (`record` / `crawl`), edit, and run scenarios; **upload + run a `.zip` bundle** of config + scenarios + the built app binary (BE-0073); browse reports and evidence; approve visual baselines; live job streaming — from a browser (not for CI)
