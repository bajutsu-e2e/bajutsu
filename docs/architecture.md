...
- `back` and device control (`setLocation` / clipboard / `push`) on the idb backend, confirmed
  on-device. The `ios-e2e.yml` `actuation (idb)` job proves it
