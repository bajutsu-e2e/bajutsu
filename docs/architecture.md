...
| `common/config_source.py` | Acquire a config (and its scenario tree) from a Git source at an immutable commit SHA into a content-addressed cache (BE-0063) — where a config comes from, distinct from `config/`'s schema and resolution | [configuration](configuration.md) |
