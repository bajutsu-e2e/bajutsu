...
| `record/` | The record loop (observe → propose → execute → emit, `loop.py`) + its CLI command (`cli.py`) | [recording](recording.md#the-record-loop) |
...
| `triage/` | M4 self-heal: rule-based `HeuristicTriageAgent` + structured fixes (`renameId`/`addIndex`/`raiseTimeout`), `--apply`/`--write`/`--rerun` (`heuristic.py`) + its CLI command (`cli.py`) | [cli](cli.md) |
