...
| `runner/` | config + scenarios → report; device pool + launch sequence; `device_provider` seam resolves where the run's devices come from — the built-in `local` pass-through, plus an `appium` provider driving a reserved iOS device end to end behind a live Appium/WebDriver endpoint (BE-0238); a further cloud-vendor kind (e.g. Firebase Device Streaming) stays a future addition; `recovery` holds the backend-crash retry-count/wall-clock-budget decision and infrastructure-fault classification shared with the on-device driver conformance suite (BE-0334), plus the guarded-teardown policy that the pool's teardown sites, `launch_driver`, and the on-device suites' lease discard all share (BE-0342); `mailbox` resolves the `email` step's transport by a registry keyed on `kind` (the shipped `http` JSON adapter; BE-0186), mirroring `ai/registry.py`'s shape (package: `pipeline` / `pool` / `launch` / `device_provider` / `recovery` / `mailbox`) | [run-loop](run-loop.md#runner-the-run-pipeline) |
| `doctor.py` | Convention score (id coverage, etc.) | [configuration](configuration.md#doctor-the-convention-score) |
| `cloud/` | Cloud device backends reached as batch submitters, off the `run`/CI verdict path: `devicefarm` (AWS Device Farm; BE-0235, iOS BE-0238) renders the custom-environment test spec, builds the upload package, and submits/polls/collects a run, deriving the verdict from Bajutsu's own manifest — the reusable core shared by the headless CLI wrapper (`scripts/devicefarm_submit.py`) and `serve`'s per-scenario batch fan-out (BE-0336) | [devicefarm](devicefarm.md) |
...
| `analytics/` | AI usage/cost accounting, reporting only — never touches the deterministic verdict: `usage` is the process-global per-call token tracker the CLI's `AI usage:` line reads (BE-0194); `ledger` persists an attributed, priced JSONL record of every AI call (BE-0196); `stats` aggregates that ledger by provider/model/command/scenario with a daily cost trend for `serve`'s Usage dashboard (BE-0195) | [web-ui](web-ui.md) |
...
- **`serve`-driven AWS Device Farm fan-out** (BE-0336): a target declaring `cloudBatch: devicefarm` dispatches one Device Farm run per scenario from `serve` (`POST /api/run-set`), in parallel and bounded by a per-provider device budget (`cloudBatchBudget`, or a per-request `deviceBudget` that may only lower it), rather than the headless submitter's one run for the whole suite serialized on a single reserved device; the fan-out reuses that submitter's core (`cloud/devicefarm.py`, BE-0235) and is durable across a `serve` restart on the database-backed deployment, which checkpoints each scheduled run's ARN so a worker resumes polling it after a restart with no re-upload
- **AI usage/cost dashboard** (BE-0195) over an attributed, persistent usage ledger (BE-0196): every AI call appends one JSONL line — provider, model, command, scenario, and a dollar cost where the provider prices per token — to `runs/usage.jsonl` (`ai.usageLedger` relocates it, `""` disables it), and `serve`'s **Usage** tab aggregates that ledger by dimension with a provider/model comparison and a daily cost trend; read-only and reporting-only, like the run-stats dashboard (BE-0102) it complements
