...
- Reading `SFSafariViewController`'s element tree from the process that draws it, `com.apple.SafariViewService`
  ...
  identifier is normalized to `Close` across the iOS versions that name it differently — so a scenario
...
- **Choosing the active org on sign-in** (BE-0395): a login whose GitHub memberships match more than
  ...
  distinguishes an org the user picked from one merely resolved for them, so sign-in keeps a pick and
  goes on re-resolving the rest. A single-org deployment renders the badge exactly as before
