...
- **Database-backed org lifecycle and membership** (BE-0375): once a database is wired, an org's `members` / `githubOrgs` / `editorTeam` live in the `orgs` table rather than in the config file's `orgs:` block — seeded from that block once per org at startup and at every config rebind, then owned by the database — so `serve` gains four admin-only `/api/orgs…` endpoints and an **Orgs** page that create, re-member, and soft-delete a tenant without a redeploy; sign-in resolves against the table alone there, so an unreadable config no longer denies every user and an unreadable database answers with a 5xx naming the store; a target's identity becomes `(org, target)`, so two orgs may each claim one name. Target ownership itself stays in configuration (prime directive 3), and a database-less deployment keeps reading `orgs:` unchanged
- **Session-scoped org selection**: a login that belongs to more than one org chooses which one the current browser session acts as, from a header select box, and switches through `POST /api/session/org` without signing out — so two windows can hold two tenants at once. The session records what its own sign-in observed on GitHub, plus the org it acts as and that org's role; a `Caller` (login, org, role) resolved once at the authentication gate carries all three to the operations, which scope by it instead of re-reading the user row. The role therefore follows the chosen org (`editorTeam` is per org), and the recorded Teams are a privilege cache bounded by the session's own lifetime — retiring an org, or a membership edit that changes what a live session holds, revokes the affected sessions outright
