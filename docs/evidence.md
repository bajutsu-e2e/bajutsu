...
> far more steps than intended, and attaching a heavy capture (`video` / `network`) to it quietly
> produces gigabytes. `bajutsu trace --explain <scenario.yaml>` is a read-only dry run that counts
