...
is a **zero-padded, 4-digit, monotonically increasing** ID. Every item lives directly under `roadmaps/` in a
flat layout: its path is fixed the moment its ID is allocated and never moves (BE-0159 retired the per-`Status`
folders BE-0078 introduced — `Status` now decides only the index bucket, below, not the file's location).

When you add a roadmap item:

1. **Allocate the next ID** = the highest existing `BE-NNNN` + 1, over every item under `roadmaps/`. Find the
   current max with:
   ```bash
   ls -d roadmaps/BE-*/ | sort | tail -1
   ```
   Never reuse, skip, or guess a number.
2. **Create the item directory and both language files** directly under `roadmaps/` with `Status: Proposal`
   (a new item is always a proposal first) — `roadmaps/BE-NNNN-<slug>/BE-NNNN-<slug>.md`
