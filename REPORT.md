...
自然言語駆動の iOS Simulator 向け E2E テストツール **Bajutsu** の開発セッション報告。
設計のブラッシュアップ、M1 実装、デバイス層、同梱サンプルアプリまでをまとめる。

- **リポジトリ:** `bajutsu`（ローカル、リモート設定済み）
...
- **`sample/scenarios/`** — 全プリミティブを使う example シナリオ（smoke / auth / settings / list）。
  スキーマ検証を CI で実施。
- **`bajutsu.config.yaml`** — `sample` の app エントリを持つ実 config。
...
静かに壊れていた。`bajutsu/_yaml.py` で bool リゾルバを `true`/`false` のみに制限して修正、
...
## 5. その後の到達点（このセッション以降に出荷済み）

- **M1 — 完了（実機検証済み）。** idb を実出力に対して検証し、起動済み Simulator で
  id ファーストシナリオが決定的に通ること・config だけで対象アプリを切り替えられることを確認。
- **M2 — 完了。** AI ループ `record`（探索 + 自然言語のシナリオ正規化）と capturePolicy のトリガー発火。
- **M3 — 完了。** network 観測（アプリ内 collector + `request` アサーション）+ 決定的 `mocks` +
  appTrace（os_signpost）+ redaction + XCUITest codegen + CI（`ci.yml` / `e2e.yml`）。
- **M4 — 実装済み。** 自己修復トリアージ `triage`（失敗要約 + 構造化 fix `renameId` / `addIndex` /
  `raiseTimeout` の提案、`--apply` / `--write` / `--rerun`、人間レビュー前提・助言のみ）。
- **オーサリング DSL の拡充:** 再利用可能なパラメタ化 `use` コンポーネント・データ駆動
  （`data` / `dataFile`）・`secrets` 変数（値マスク）・タグ + `--tag` / `--exclude`・`setLocation` /
  `push` / `doubleTap` ステップ・ファイル / シナリオ単位の `description`。
- **`serve`:** ローカル Web UI（シナリオ実行 + レポート閲覧。Tier 1・CI ゲート外）。
...
bajutsu run sample/scenarios/smoke.yaml --app sample
bajutsu doctor --app sample
```

## 7. プロジェクト構成

```
bajutsu/                  # ツール本体（Python）
...
├── BajutsuSample/*.swift  project.yml  scenarios/*.yaml  README.md
bajutsu.config.yaml       # チーム既定 × アプリ別
