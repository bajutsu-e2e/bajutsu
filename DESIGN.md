...
```
bajutsu run <scenario.yaml> --app <name> [--backend rocketsim[,idb]] [--udid booted] [--workers N]
...
| マイルストーン | 内容 |
|---|---|
| **M1** | `env.py`（simctl）+ `drivers/base,rocketsim,idb`（共通 IF、idb 側セレクタ解決）+ YAML シナリオ(pydantic) + `assertions.py` + 証跡の軽量 3 点（`screenshot`/`elements`/`actionLog`）と `result:error` 安全網 + `manifest.json` + per-app config（`apps.<name>` 解決, `bajutsu run --app` / `doctor --app`）。**同一シナリオが RocketSim / idb 両方で通る** こと、**config だけで対象アプリを切り替えられる** ことを完了条件とする |
