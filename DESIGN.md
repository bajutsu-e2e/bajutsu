...
バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収します。現状の登録バックエンドは idb（iOS）、adb（Android、idb の双子。まず fast ゲートまでの第一段）、Playwright（web）で、将来の追加（XCUITest 等）に備えてこの抽象を維持します。
