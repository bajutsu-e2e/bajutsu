...
Bajutsu の **最初の dogfood 対象**は、リポジトリ同梱のサンプルアプリ `demos/features/app/`（`com.bajutsu.sample`）。あらゆるプリミティブを試せるよう計装してある。§6 の例（settings 画面を開く env フック、reindex、「正規化設定」）はこのサンプルに実在する UI / フックに対応する。

- **位置づけ**: 同梱サンプルは「最初の現実的なテスト対象」であって Bajutsu のコア依存ではなく、汎用 iOS E2E ツールとして他アプリにも使える
- **含意**: §7 の実装規約はまず同梱サンプルに適用して検証する
- **汎用性**: 同梱サンプルは対象アプリの 1 つめにすぎない。新しいアプリは `apps.<name>` の config エントリ（§8）と §7 適用（§7.1）で増やせ、ツール本体・ドライバ・実行系は不変。Bajutsu は特定アプリに結合しない
...
- **証跡源**: idb はネイティブのネットワーク監視を持たないため `network` は **モックサーバのログから取得**する。未接続時は `manifest` に skip 理由を明示
- **設定**: `bajutsu.config.yaml` の `mockServer`（起動コマンド / ポート / スタブ定義への参照）。Bajutsu 同梱の軽量サーバでも、外部サーバでもよい
...
- **ヘッドレス前提**: idb はヘッドレスで動くため N 台へ水平スケールできる（CI 向き）
- **CLI**: `bajutsu run ... --workers N`（または `--shard i/n`）。既定は 1。`--udid` 明示時は単一デバイス固定
...
バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収する。現状の登録バックエンドは idb のみだが、将来の追加（XCUITest 等）に備えてこの抽象を維持する。
...
> **要点**: セレクタは常に識別子（`AXUniqueId` 由来の id）で書く。idb は `query()` で id → frame 中心に解決してから座標 tap する。actuation は座標だが、選択は id で安定するため、シナリオはバックエンド非依存に保たれる。
...
> ambiguity 検出はここに一元化する。抽象側は **常に `query()` で候補数を検証してから** actuation へ渡すので、「曖昧なら失敗」挙動はバックエンドに依らず一貫する。

### 操作の安定度順（stability ladder）

UI 操作は **最も安定する選択手段から順に試し、成立した手段を採用**する。採用結果は record で凍結し（§6.5）、`run` は凍結済みを決定的に再実行する。安定度で変わるのは **どの要素を選ぶか** であり、actuation は常に frame 中心への座標 tap（idb は semantic tap を持たないため）。下に行くほど壊れやすく、順 3 を使ったら manifest に degradation として明示する（§10）。
...
- **actuation は常に frame 中心へ座標 tap**: idb は `query()` で id を引き当て、その要素の frame 中心を叩く。選択（順 1〜3）は id で安定させ、actuation 自体は常に座標。
- **actuator はリストで最初に利用可能な backend**: `backend` リスト（§8）の先頭から利用可能なものを actuator に確定し、run 中は固定（複数 backend が同一デバイスを操作しない＝§3.3 の決定性を維持）。現状は idb のみだが、将来 backend を追加できるようリスト構造を残す
...
- **バージョン管理**: シナリオはリポジトリ内のただのファイル。履歴は git が持ち、Bajutsu は独自ストアを持たない
...
`bajutsu.config.yaml` を 2 層（`defaults` × `apps`）で持つ。値の解決順は **既定 < アプリ < シナリオ**（右ほど優先 = テストに近いほうが勝つ）。
...
- **可用性を見る**: 証跡源がこの環境で起動できない場合（例: モックサーバ未接続）はスキップして次へ。最終的に mock / simctl、無ければ skip
...
- **過剰マッチ対策**: `bajutsu trace --explain <scenario>` で、どのルールが何回発火するかを事前提示
...
- idb の要素ツリー正規化精度（特に `.searchable` 等の SwiftUI 標準要素）
  - SwiftUI の画面遷移中、idb の `describe-all` が一過性で「ほぼ空」のツリー（観測例: 識別子なし 1 要素）を返すことがある（画面は描画済みでも発生）。単発の `expect`/`assert` や条件 `wait` がこの一瞬のスナップショットを掴むと誤失敗する。対策として `IdbDriver.query()` に上限付きリトライを実装: 一度リッチなツリー（`_max_seen >= 2`）を観測済みで、かつ結果が `< 2` 要素のときのみ「未準備」とみなし短いバックオフで再取得する（最大 ~1s で頭打ち）。`_max_seen` ゲートにより、もともと疎な画面はマスクせず即返す。`_await_ready` の起動時しきい値（`>= 2`）と同一基準。8 秒級で空が続く病的ケースは idb 側のより長いストールで、本層では意図的にマスクしない（`idb_companion` バージョン監視の対象）。
