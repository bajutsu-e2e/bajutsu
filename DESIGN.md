# SimPilot — 自然言語駆動 iOS E2E テストツール 設計指針

> ステータス: 設計確定版 v1.7（実装未着手 / ドキュメントのみ）
> ツール名 `SimPilot` は仮称。unfydqry とは独立したツールとして開発する。
...
SimPilot は unfydqry（隣接リポジトリ。Rust core + UniFFI のクロスプラットフォーム全文検索エンジン）とは **別ドメイン・別リポジトリ・別言語** の独立ツール。両者にコード依存はない。

- **関係**: unfydqry の iOS サンプル `ios/sample/SearchSample` が SimPilot の **最初の dogfood 対象**。§6 の例（`SEARCH_SHOW_SETTINGS` env フック、reindex、「正規化設定」）は SearchSample に実在する UI / フックに対応する（`ContentView.swift` の `SEARCH_SHOW_SETTINGS`、`SearchModel.swift` の reindex 状態）
- **なぜ独立か**: ドメイン（テスト自動化 vs 検索）・言語（Python vs Rust/Swift/Kotlin）・配布（pipx vs SwiftPM/Gradle）が異なる。SearchSample は「最初の現実的なテスト対象」であって SimPilot のコア依存ではなく、汎用 iOS E2E ツールとして他アプリにも使える
- **含意**: §7 の実装規約はまず SearchSample に適用して検証する。現状 SearchSample は `accessibilityIdentifier` 未付与のため、その付与が dogfood の前提条件（M1）
- **汎用性**: SearchSample は対象アプリの 1 つめにすぎない。新しいアプリは `apps.<name>` の config エントリ（§8）と §7 適用（§7.1）で増やせ、ツール本体・ドライバ・実行系は不変。SimPilot は特定アプリに結合しない
...
- **設定**: `simpilot.config.yaml` の `mockServer`（起動コマンド / ポート / スタブ定義への参照）。SimPilot 同梱の軽量サーバでも、外部サーバでもよい
...
- **CLI**: `simpilot run ... --workers N`（または `--shard i/n`）。既定は 1。`--udid` 明示時は単一デバイス固定
...
- 配布: `pipx install simpilot` を想定（idb 本体 = `fb-idb` も Python 製で親和性が高い）
...
| **review / edit** | 人間 | YAML が真実。手で編集してよい。`simpilot trace --explain` でルール発火回数を検証（§9） |
...
- **バージョン管理**: シナリオはリポジトリ内のただのファイル。履歴は git が持ち、SimPilot は独自ストアを持たない
...
4. **`simpilot doctor --app <name>` で検証** — 実行可能ゲート + §7 充足度スコアを出す（測り方は §7.2）
...
```
simpilot run <scenario.yaml> --app <name> [--backend rocketsim[,idb]] [--udid booted] [--workers N]
simpilot record <scenario.yaml> --app <name>   # AI で探索しつつ操作・証跡指示を記録
simpilot doctor --app <name>                   # 環境/権限/接続 + §7 規約の充足を検証
simpilot trace open <runId>                    # 証跡ビューア
simpilot trace --explain <scenario>            # 証跡ルールの発火回数を事前提示（ドライラン）
simpilot codegen <result.json> --emit xcuitest -o UITests/   # M3
```

### 設定の階層（チーム既定 × アプリ別）

`simpilot.config.yaml` を 2 層（`defaults` × `apps`）で持つ。値の解決順は **既定 < アプリ < シナリオ**（右ほど優先 = テストに近いほうが勝つ）。
...
- **過剰マッチ対策**: `simpilot trace --explain <scenario>` で、どのルールが何回発火するかを事前提示
...
| マイルストーン | 内容 |
|---|---|
| **M1** | `env.py`（simctl）+ `drivers/base,rocketsim,idb`（共通 IF、idb 側セレクタ解決）+ YAML シナリオ(pydantic) + `assertions.py` + 証跡の軽量 3 点（`screenshot`/`elements`/`actionLog`）と `result:error` 安全網 + `manifest.json` + per-app config（`apps.<name>` 解決, `simpilot run --app` / `doctor --app`）。**同一シナリオが RocketSim / idb 両方で通る** こと、**config だけで対象アプリを切り替えられる** ことを完了条件とする |
