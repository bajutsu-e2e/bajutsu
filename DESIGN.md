...
バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収します。現状の登録バックエンドは idb（iOS）、XCUITest（iOS）、adb（Android）、Playwright（web）です。XCUITest は idb より安定度の高い第一候補で（[BE-0019](roadmaps/BE-0019-xcuitest-backend/BE-0019-xcuitest-backend-ja.md)）、adb は idb の双子で、fast ゲートとエミュレータ上の e2e CI まで到達済みです（[BE-0208](roadmaps/BE-0208-android-emulator-e2e-ci/BE-0208-android-emulator-e2e-ci-ja.md)）。将来の追加に備えて、この抽象を維持します。
