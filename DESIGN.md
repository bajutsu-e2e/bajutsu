...
- **actuator はリストで最初に利用可能な backend**：`backend` リスト（§8）の先頭から利用可能なものを actuator に確定し、run 中は固定します（複数 backend が同一デバイスを操作しないことで、§3.3 の決定性を維持します）。現状は idb / XCUITest（iOS）、adb（Android）、Playwright（web）で、将来 backend を追加できるようリスト構造を残します
