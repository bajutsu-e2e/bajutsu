...
| `clear` / `delete` / `select` / `copy` | `clear: { into: <Selector> }` ほか（BE-0265） | テキスト編集。`clear` は全消去、`delete` は末尾から `count` 文字、`select` は全選択、`copy` は選択内容をクリップボードへ（要 `select` 先行）。`select` / `copy` は idb / web コンテキスト非対応で codegen 経由の XCUITest に誘導 |
