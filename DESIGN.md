...
Bajutsu の **最初の dogfood 対象**は、リポジトリ同梱のサンプルアプリ `app/sample/`（`com.bajutsu.sample`）。あらゆるプリミティブを試せるよう計装してある。§6 の例（settings 画面を開く env フック、reindex、「正規化設定」）はこのサンプルに実在する UI / フックに対応する。
