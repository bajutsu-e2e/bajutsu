# Bajutsu：自然言語駆動 iOS E2E テストツールの設計指針
...
自然言語で書かれたテストシナリオを受け取り、AI エージェントが iOS Simulator 上でアプリを操作して E2E テストを実行し、結果を検証するツールです。
...
4. **`bajutsu doctor --app <name>` で検証**：実行可能ゲートと §7 充足度スコアを出す（測り方は §7.2）
...
`doctor --app <name>` は **AI 非依存で決定的**です（§3.1 の通り doctor は配線検証であって著者ではありません）。アプリを 1 回起動し、入口画面の `query()` を解析して **(1) 実行可能ゲート** と **(2) §7 充足度スコア** を出します。
...
- **秘匿情報のマスキング**：スクショ / ログ / ネットワークに PII やトークンが写り得ます。`redact:`（マスクするラベル / ヘッダ / フィールド）を保存前に適用します
...
- idb の要素ツリー正規化精度（特に `.searchable` 等の SwiftUI 標準要素）
  - SwiftUI の画面遷移中、idb の `describe-all` が一過性で「ほぼ空」のツリー（観測例: 識別子なし 1 要素）を返すことがあります（画面は描画済みでも発生します）。単発の `expect`/`assert` や条件 `wait` がこの一瞬のスナップショットを掴むと誤失敗します。対策として `IdbDriver.query()` に上限付きリトライを実装しています。これは、一度リッチなツリー（`_max_seen >= 2`）を観測済みで、かつ結果が `< 2` 要素のときのみ「未準備」とみなし、短いバックオフで再取得するものです（最大 ~1s で頭打ちです）。`_max_seen` ゲートにより、もともと疎な画面はマスクせず即返します。`_await_ready` の起動時しきい値（`>= 2`）と同一基準です。8 秒級で空が続く病的ケースは idb 側のより長いストールで、本層では意図的にマスクしません（`idb_companion` バージョン監視の対象です）。
