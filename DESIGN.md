# Bajutsu — 自然言語駆動 iOS E2E テストツール 設計指針

> ステータス: 設計確定版 v1.7 / M1–M4 実装済み（決定的ロジック層は実装・テスト済み、idb バックエンド実行部は実機 Simulator で検証済み）
> ツール名 `Bajutsu` は仮称。
...
自然言語で書かれたテストシナリオを受け取り、AI エージェントが iOS Simulator 上でアプリを操作して E2E テストを実行・検証するツールです。
...
Bajutsu の **最初の dogfood 対象**は、リポジトリ同梱のサンプルアプリ `demos/features/app/`（`com.bajutsu.sample`）です。あらゆるプリミティブを試せるよう計装してあります。§6 の例（settings 画面を開く env フック、reindex、「正規化設定」）は、このサンプルに実在する UI / フックに対応します。

- **位置づけ**: 同梱サンプルは「最初の現実的なテスト対象」であって Bajutsu のコア依存ではなく、汎用 iOS E2E ツールとして他アプリにも使えます
- **含意**: §7 の実装規約はまず同梱サンプルに適用して検証します
- **汎用性**: 同梱サンプルは対象アプリの 1 つめにすぎません。新しいアプリは `apps.<name>` の config エントリ（§8）と §7 適用（§7.1）で増やせ、ツール本体・ドライバ・実行系は不変です。Bajutsu は特定アプリに結合しません
...
> タイミング / 状態 / ネットワーク起因のフレーキーは、環境・待機側で別途対処します。
...
└───────────────┬───────────────────────┘
   ...
   ┌────────────┴───────────────┐
   ▼                            ▼
idb                       (将来) XCUITest backend
(Simulator/ヘッドレス可)    (決定的コード生成)
        │
        ▼
...
決定的なネットワーク（§2「ネットワークをモックへ」）と、idb バックエンドの `network` 証跡源を、**1 つのモックサーバ**で兼ねます。

- **居場所**: アーキ図の通り、アプリと外部ネットワークの間に立つ任意コンポーネントです。アプリは launch env で base URL を差し替えて接続します（§2 の env 注入と同じ流儀で、アプリ側に特別な改造を要求しません）
- **2 つの役割**: (a) **決定性** = 事前定義したスタブ応答を返します、(b) **証跡** = 受けたリクエストをログ化し `network` アーティファクトの取得元になります（§9）
- **証跡源**: idb はネイティブのネットワーク監視を持たないため、`network` は **モックサーバのログから取得**します。未接続時は `manifest` に skip 理由を明示します
- **設定**: `bajutsu.config.yaml` の `mockServer`（起動コマンド / ポート / スタブ定義への参照）です。Bajutsu 同梱の軽量サーバでも、外部サーバでもかまいません

### 3.3 並列実行とアイソレーション

スループットは **Simulator を複数 boot して水平分割**することで稼ぎます。決定性は崩しません（各テストはクリーン環境・機械アサーションで完結し、状態を共有しないためです）。

- **アイソレーション単位 = ワーカー**: 各ワーカーは固有の `udid`（`simctl clone` 等）、固有の `runs/<runId>`、固有のモックサーバ **ポート** を持ちます。これによりポート衝突と成果物の混線を避けます
- **ヘッドレス前提**: idb はヘッドレスで動くため、N 台へ水平スケールできます（CI 向き）
- **CLI**: `bajutsu run ... --workers N`（または `--shard i/n`）です。既定は 1 です。`--udid` 明示時は単一デバイスに固定します
...
│   ├── drivers/
│   │   ├── base.py           # Driver 抽象インターフェース（★ バックエンド差の要）
...
- 外部ツール連携: `subprocess` で `idb` / `xcrun simctl` を呼び、各々の JSON 出力をパース
- LLM: `anthropic` SDK（Claude）。シナリオ・要素ダンプは prompt cache に載せてトークン削減
- 配布: `pipx install bajutsu` を想定（idb 本体 = `fb-idb` も Python 製で親和性が高い）

---

## 5. ドライバ抽象（バックエンド非依存）

バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収します。現状の登録バックエンドは idb のみですが、将来の追加（XCUITest 等）に備えてこの抽象を維持します。
...
| 機能 | idb | 抽象側の扱い |
|---|---|---|
| 要素ツリー取得 | `ui describe-all`（`AXUniqueId` / label / traits / frame） | 共通の `Element` へ正規化 |
| セレクタ解決 | id（`AXUniqueId`）を `query()` で引き当て → **frame 中心へ座標 tap** | `query()` で対象を一意化してから frame 中心を tap |
| 条件待機 | ネイティブ無し | `query()` のポーリングで実装 |
| 実行形態 | ヘッドレスで動作 | `backend` リストの可用性判定で対象を選ぶ（§9） |

> **要点**: セレクタは常に識別子（`AXUniqueId` 由来の id）で書きます。idb は `query()` で id → frame 中心に解決してから座標 tap します。actuation は座標ですが、選択は id で安定するため、シナリオはバックエンド非依存に保たれます。
...
UI 操作は **最も安定する選択手段から順に試し、成立した手段を採用**します。採用結果は record で凍結し（§6.5）、`run` は凍結済みを決定的に再実行します。安定度で変わるのは **どの要素を選ぶか** であり、actuation は常に frame 中心への座標 tap です（idb は semantic tap を持たないためです）。下に行くほど壊れやすく、順 3 を使ったら manifest に degradation として明示します（§10）。
...
- **actuation は常に frame 中心へ座標 tap**: idb は `query()` で id を引き当て、その要素の frame 中心を叩きます。選択（順 1〜3）は id で安定させ、actuation 自体は常に座標です。
- **actuator はリストで最初に利用可能な backend**: `backend` リスト（§8）の先頭から利用可能なものを actuator に確定し、run 中は固定します（複数 backend が同一デバイスを操作しないことで、§3.3 の決定性を維持します）。現状は idb のみですが、将来 backend を追加できるようリスト構造を残します
- **明示指定は固定**: `--backend <one>` で単一指定したらフォールバックしません（`--udid` と同様です、§3.3）
...
- name: 設定を開いて再生成する
  preconditions:
    ...
    launchEnv: { SAMPLE_SCREEN: "settings" }  # 状態注入（launch args / env）
    deeplink: "bajutsusample://settings"     # 起動後に開く（任意・scheme は §8）
...
`capturePolicy`（繰り返し発火する証跡ルール）と `redact` はシナリオ単位またはファイル単位で指定します（§9 A）。識別子・`launchEnv`・deeplink scheme・mock は **アプリ固有** で、§7（規約）と §8（`apps.<name>` config）が供給します。シナリオはアプリの名前空間で識別子を書き、ツール本体は不変です（§7.1）。
...
> `launch` / `deeplink` / `erase` は基本 `preconditions` で宣言しますが、シナリオ途中で再起動・再注入したい場合はステップとしても書けます（`relaunch: { env: {...} }`）。
...
4. **`bajutsu doctor --app <name>` で検証** — 実行可能ゲート + §7 充足度スコアを出す（測り方は §7.2）
...
`doctor --app <name>` は **AI 非依存・決定的**です（§3.1 の通り doctor は配線検証であって著者ではありません）。アプリを 1 回起動し、入口画面の `query()` を解析して **(1) 実行可能ゲート** と **(2) §7 充足度スコア** を出します。

**(1) ゲート（pass/fail・1 つでも ✗ なら実行不能）**

| チェック | 内容 |
|---|---|
| backend | idb_companion の存在・バージョン |
...
> **測定範囲の正直さ**: 既定は **入口画面 + 宣言済み deeplink で直接開ける画面** のみです。全画面の網羅は `doctor --app <name> --from <runId>` で、`record` 実行が残した各画面の `elements` ダンプ（§9）を再利用して算出します。入口だけの測定はその旨を report に明示します。

### 7.3 識別子の命名規約と名前空間（`idNamespaces`）

`accessibilityIdentifier` は **`<namespace>.<element>` のドット区切り階層**です。すべて小文字で、各セグメントは `[a-z0-9-]`（複数語はハイフン）です。先頭セグメント = 名前空間で、`apps.<name>.idNamespaces`（§8）に宣言した集合のいずれかです。
...
2. **非ローカライズ・データ由来** — id に表示文言を使いません（翻訳で壊れます）。動的部分は表示テキストではなく **裏側の安定キー**（recordId 等）から作ります。doctor は id == label を advisory 警告にします（§7.2）
...
- **共有フローは予約名前空間に固定** — ログイン等の **再利用シナリオ / 共有コンポーネント**（§6.1 `setup:`）が複数アプリで動くには、その範囲の id が全アプリで **同一** である必要があります。`auth.*`（認証）/ `nav.*`（共通ナビ）等を **予約名前空間**（§8 `defaults.reservedNamespaces`）として全アプリ共通の意味で使います
...
ツール本体はアプリ非依存です。**アプリ固有の差分はすべて config に寄せ**、同じバイナリ・同じドライバで複数アプリを回します。
...
bajutsu codegen <result.json> --emit xcuitest -o UITests/   # M3
```

### 設定の階層（チーム既定 × アプリ別）

`bajutsu.config.yaml` を 2 層（`defaults` × `apps`）で持ちます。値の解決順は **既定 < アプリ < シナリオ** で、右ほど優先します（テストに近いほうが勝ちます）。

```yaml
defaults:                       # 全アプリ共通の既定
  backend: [idb]                 # actuator=リストで最初に利用可能なもの（§5 / §9）。現状 idb のみ・将来の追加に備えたリスト構造。単一文字列も可
  device:  "iPhone 15"
...
apps:
  sample:                       # ← --app sample で選択
    ...
    launchEnv:      { SAMPLE_UITEST: "1" }             # このアプリ既定のフック（§7）
    idNamespaces:   [home, list, counter, settings]    # 識別子の名前空間（§7 命名規約）
    mockServer:     { cmd: "...", port: 8080, stubs: ./mocks/sample }   # §3.2
    setup:          ./scenarios/sample/_setup.yaml    # 再利用するログイン等（§6.1）
    redact:         { labels: ["カード番号"] }           # アプリ固有の追加（既定にマージ）
```

> アプリを増やすには `apps.<name>` を 1 つ足すだけです。ツールもドライバもシナリオ実行系も変えません。各アプリのシナリオは `scenarios/<name>/` に置き、識別子はそのアプリの名前空間で書きます（§7.1）。
...
`backend` は **安定度順の順序付きリスト**です（§8 / §5 stability ladder）。`tap` / `type` / `swipe` / `wait` / `query`（操作と解決）は **actuator のみ** が行います。**actuator = リストで最初に利用可能な backend** です（run 開始時に確定し run 中は固定です。2 ドライバが同一デバイスを操作する非決定性を避けます、§3.3）。actuator 以外は **能力を補うフォールバック**で、実体は read-only な証跡供給に限ります。`capabilities()`（§5）で各 backend の提供能力を引きます。
...
| `network` | **モックサーバ**（§3.2）から取得 → 無ければ skip（idb はネイティブ監視を持たない） |

- **可用性を見る**: 証跡源がこの環境で起動できない場合（例: モックサーバ未接続）はスキップして次へ進みます。最終的に mock / simctl を見て、無ければ skip します
- **来歴を残す**: 各アーティファクトが **どの provider から来たか** を `manifest` に記録します（例: `network: mockServer（idb はネイティブ監視なし）`）。未取得は capability フラグと共に skip 理由を明示します（§10）

### 注意点（証跡特有）

- **観測者効果**: 録画・ログストリームは僅かに時間挙動へ影響します。決定性検証では `around` 区間の長さを固定し、待機は条件待機のまま保ちます
- **コスト管理**: `video` を全操作に付けると重くなります。ルールはマッチ条件を絞り、既定は `screenshot` + `elements` + `actionLog` の軽量 3 点とし、`video` / `network` はオプトインにします
- **秘匿情報のマスキング**: スクショ / ログ / ネットワークに PII・トークンが写り得ます。`redact:`（マスクするラベル / ヘッダ / フィールド）を保存前に適用します
...
- idb の要素ツリー正規化精度（特に `.searchable` 等の SwiftUI 標準要素）
  - SwiftUI の画面遷移中、idb の `describe-all` が一過性で「ほぼ空」のツリー（観測例: 識別子なし 1 要素）を返すことがあります（画面は描画済みでも発生します）。単発の `expect`/`assert` や条件 `wait` がこの一瞬のスナップショットを掴むと誤失敗します。対策として `IdbDriver.query()` に上限付きリトライを実装しています: 一度リッチなツリー（`_max_seen >= 2`）を観測済みで、かつ結果が `< 2` 要素のときのみ「未準備」とみなし短いバックオフで再取得します（最大 ~1s で頭打ちです）。`_max_seen` ゲートにより、もともと疎な画面はマスクせず即返します。`_await_ready` の起動時しきい値（`>= 2`）と同一基準です。8 秒級で空が続く病的ケースは idb 側のより長いストールで、本層では意図的にマスクしません（`idb_companion` バージョン監視の対象です）。
...
| マイルストーン | 内容 |
|---|---|
| **M1** | `env.py`（simctl）+ `drivers/base,idb`（共通 IF、id → frame 中心のセレクタ解決）+ YAML シナリオ(pydantic) + `assertions.py` + 証跡の軽量 3 点（`screenshot`/`elements`/`actionLog`）と `result:error` 安全網 + `manifest.json` + per-app config（`apps.<name>` 解決, `bajutsu run --app` / `doctor --app`）。**idb でシナリオが通る** こと、**config だけで対象アプリを切り替えられる** ことを完了条件とする |
| **M2** | observe→act→verify の AI ループ（自然言語ステップ・証跡指示の解釈と正規化）+ ルール方式（トリガー）一式 + `video` / `deviceLog` + Reporter(JUnit/HTML) |
| **M3** | `network`（モックサーバ）+ `appTrace`（os_signpost）+ redaction + XCUITest codegen（Tier2）+ CI 統合 |
