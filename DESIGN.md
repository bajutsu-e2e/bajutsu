...
バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収します。現状の登録バックエンドは idb（iOS）、XCUITest（iOS）、adb（Android）、Playwright（web）です。XCUITest は idb より安定度の高い第一候補で（[BE-0019](roadmaps/BE-0019-xcuitest-backend/BE-0019-xcuitest-backend-ja.md)）、adb は idb と対をなす backend で、fast ゲートとエミュレータ上の e2e CI まで到達済みです（[BE-0208](roadmaps/BE-0208-android-emulator-e2e-ci/BE-0208-android-emulator-e2e-ci-ja.md)）。将来の追加に備えて、この抽象を維持します。
...
```yaml
defaults:                       # 全アプリ共通の既定
  backend: [idb]                 # actuator=リストで最初に利用可能なもの（§5 / §9）。idb / XCUITest（iOS）、adb（Android）、Playwright（web）が登録済み・将来の追加に備えたリスト構造。単一文字列も可
  device:  "iPhone 15"
...
- idb の要素ツリー正規化精度（特に `.searchable` 等の SwiftUI 標準要素）
  - SwiftUI の画面遷移中、idb の `describe-all` が一過性で「ほぼ空」のツリー（観測例: 識別子なし 1 要素）を返すことがあります（画面は描画済みでも発生します）。単発の `expect`/`assert` や条件 `wait` がこの一瞬のスナップショットを掴むと誤失敗します。対策として、idb と adb が共有する `CoordinateTreeDriver.query()` 基底クラス（BE-0254）に上限付きリトライを実装しています。これは、一度リッチなツリー（`_max_seen >= 2`）を観測済みで、かつ結果が `< 2` 要素のときのみ「未準備」とみなし、短いバックオフで再取得するものです（基点 0.05 秒を毎回倍加し 0.2 秒で頭打ち、5 回までの合計で最大 ~0.75s）。`_max_seen` ゲートにより、もともと疎な画面はマスクせず即返します。`_await_ready` の起動時しきい値（`>= 2`）と同一基準です。8 秒級で空が続く病的ケースは idb 側のより長いストールで、本層では意図的にマスクしません（`idb_companion` バージョン監視の対象です）。
