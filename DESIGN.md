...
> ステータス: 設計確定版 / M1–M4 実装済み（決定的ロジック層は実装・テスト済みです。idb バックエンドの実行部は実機 Simulator で、Playwright（web）バックエンドはブラウザに対する決定的な `run` で検証済みです）
> ツール名 `Bajutsu` は仮称。
...
自然言語で書かれたテストシナリオを受け取り、backend 非依存のドライバ越しにアプリを操作して E2E テストを実行し、結果を検証するツールです。プラットフォームは 1 つのドライバインターフェースの背後にある backend にすぎず、対象は今のところ iOS Simulator（idb）とブラウザ（Playwright）で、Android は次に予定しています。シナリオ形式・セレクタ解決・決定的ランナー・証跡サブシステム・レポーターはどれもプラットフォームを名指しせず、プラットフォーム固有なのは UI を操作する backend だけです。
...
**やらないこと（スコープ外と明示）**
- 実機 / クラウドデバイスファーム（現時点では iOS は Simulator、web はローカルブラウザを対象とします。実機・デバイスファーム対応は将来課題です）
- ユニット / 結合テスト（XCTest の領域。E2E に集中）
- アプリのビルド自体（既存 `xcodebuild` 成果物を前提に受け取る）

> **補足（なぜ最初に iOS Simulator を選んだか）。** 最初の backend に iOS Simulator（idb）を選んだのは、ヘッドレスで動き、`simctl` / `idb` でスクリプト化でき、`simctl clone` で水平にスケールでき、実機やクラウドデバイスファームを必要としないため、決定性と CI 親和性を最も安く得られたからです。実機・デバイスファームは依然としてスコープ外ですが、これは「Simulator 限定」という恒久的な制約ではなく、最初の足場の選択でした。決定的コアは backend 非依存なので、同じシナリオ形式とランナーのまま web（Playwright）へ広がり、Android へも広がる予定です。
...
- **位置づけ**：同梱サンプルは「最初の現実的なテスト対象」であって Bajutsu のコア依存ではなく、汎用の E2E ツールとして他アプリにも使えます（iOS では idb、web では Playwright を backend にします）
- **含意**：§7 の実装規約はまず同梱サンプルに適用して検証します
- **汎用性**：同梱サンプルは対象アプリの 1 つめにすぎません。新しいアプリは `targets.<name>` の config エントリ（§8）と §7 適用（§7.1）で増やせ、ツール本体、ドライバ、実行系は不変です。Bajutsu は特定アプリに結合しません
...
バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収します。現状の登録バックエンドは idb（iOS）と Playwright（web）で、将来の追加（XCUITest、Android 等）に備えてこの抽象を維持します。
...
- **actuator はリストで最初に利用可能な backend**：`backend` リスト（§8）の先頭から利用可能なものを actuator に確定し、run 中は固定します（複数 backend が同一デバイスを操作しないことで、§3.3 の決定性を維持します）。現状は idb（iOS）と Playwright（web）で、将来 backend を追加できるようリスト構造を残します
...
```yaml
defaults:                       # 全アプリ共通の既定
  backend: [idb]                 # actuator=リストで最初に利用可能なもの（§5 / §9）。idb（iOS）と Playwright（web）が登録済み・将来の追加に備えたリスト構造。単一文字列も可
  device:  "iPhone 15"
...
| マイルストーン | 内容 |
|---|---|
| **M1** | `env.py`（simctl）+ `drivers/base,idb`（共通 IF、id → frame 中心のセレクタ解決）+ YAML シナリオ(pydantic) + `assertions.py` + 証跡の軽量 3 点（`screenshot`/`elements`/`actionLog`）と `result:error` 安全網 + `manifest.json` + per-target config（`targets.<name>` 解決, `bajutsu run --target` / `doctor --target`）。**idb でシナリオが通る** こと、**config だけで対象アプリを切り替えられる** ことを完了条件とする |
