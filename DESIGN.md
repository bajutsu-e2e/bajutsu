...
バックエンドを差し替えられるよう、**共通インターフェースを最初に固定**し、能力差は抽象側で吸収します。現状の登録バックエンドは idb（iOS）、XCUITest（iOS。idb より安定度の高い第一候補、[BE-0019](roadmaps/BE-0019-xcuitest-backend/BE-0019-xcuitest-backend-ja.md)）、adb（Android、idb の双子。まず fast ゲートまでの第一段）、Playwright（web）で、将来の追加に備えてこの抽象を維持します。
