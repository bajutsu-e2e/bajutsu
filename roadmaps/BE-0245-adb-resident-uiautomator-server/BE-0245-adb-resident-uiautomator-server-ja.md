...
- PR-D ([#1042](https://github.com/bajutsu-e2e/bajutsu/pull/1042)) — 作業単位 5。チャネルを
  ...
  なりました。レーンが初めて緑になる過程で、速い読み取りが露呈した常駐経路とダンプ経路の非等価が 2 つ見つかり、
  両経路が同じツリーを返すようにここで両方直しました。1 つは、常駐サーバがダンプの前に `UiDevice.waitForIdle()`
  を呼ぶようにした点です（ダブルタップの非同期なアクセシビリティ値がまだ伝播していないうちに速い読み取りが
  スナップショットしていました。`uiautomator dump` は idle を待つのでこの差は現れませんでした）。もう 1 つは、
  ...
  ツリーを静定済みとして通してしまい、スクロール直後の tap が古い座標に当たっていました）。これで項目は完了します。
