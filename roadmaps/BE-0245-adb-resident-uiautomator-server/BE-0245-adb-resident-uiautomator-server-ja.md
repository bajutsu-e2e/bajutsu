...
- [x] 常駐 UI Automator インストゥルメンテーションのパッケージと起動（アプリに依存しない部品。配布方法を決める）。
- [x] 階層問い合わせのチャネル（ローカルのソケット／HTTP）を定め、その応答を `parse_hierarchy` がそのまま消費できるようにする。
- [x] `AdbDriver._describe()` を常駐チャネルへ切り替え、`uiautomator dump` をフォールバックとして残す。
- [x] 常駐サーバのライフサイクルをデバイスのリースに結びつける（一度起動し、実行の終了・失敗時に停止）。
- [x] 実機で検証し（読み取りが約 0.1〜0.3 秒へ）、Android の e2e レーンで常駐経路とフォールバックの両方を守る。
...
- PR-B（[#1017](https://github.com/bajutsu-e2e/bajutsu/pull/1017)）— 常駐サーバ本体と、その配布方法の決定
  です（作業単位 1〜2 のサーバ側）。`BajutsuAndroidUIAutomatorServer/` を足しました。これは自己完結した Gradle
  プロジェクト（ラッパーを追跡下に置くので、新しいクローンは Android SDK だけでビルドできます）で、
  `androidTest` のインストルメンテーションを収めます。`am instrument -w` が返らないブロッキング `@Test`
  を走らせ、`UiAutomation` セッションを一つ温かいまま保ち、ループバックの素の `ServerSocket`（HTTP
  ライブラリなし）越しに `GET /source` へ `UiDevice.dumpWindowHierarchy()` の XML で答えます。この XML
  の形式は `parse_hierarchy` がすでに解析している `AccessibilityNodeInfoDumper` のものです。配布方法
  は追跡下の `gradlew` を通じた必要時ビルドとし（XCUITest ランナーや showcase の Android ビルドと同じ
  流儀）、ビルド済みの APK をリポジトリに置きません。API 34 のエミュレータで手で確認しました。ソケットを
  束ねるためにホストアプリが必要とする `INTERNET` 権限を足したうえで、サーバは常駐し続け、`GET /source`
  は `parse_hierarchy` がそのまま解析できる階層 XML を返します。この確認は、作業単位 2 に残る等価化の作業
  も明らかにしました。`UiDevice.dumpWindowHierarchy()` は*すべての*ウィンドウをたどるので、プラットフォーム
  の `uiautomator dump` が対象ウィンドウに絞って除く SystemUI のステータスバーのツリー（時計・Wi-Fi・
  電池・通知アイコン、29 ノード）を含みます。アプリの内容は同一ですが、両者が同じ Element を返すように
  常駐側のダンプを対象ウィンドウへ絞る作業は、`adb forward` のクライアントトランスポートと
  `fetch_hierarchy` の配線とともに PR-C へ回します。そこなら端から端まで回帰テストできるからです。チェック
  を付けないのも同じ理由です。その配線が入るまで、読み取りは常駐経路を通りません。レビューでは、受け付けた
  ソケットに足した読み取りタイムアウト（シングルスレッドの受け付けループを堅牢にする際の `soTimeout`）が
  読み取りしか束ねず、応答の途中で相手が止まると書き込み側から同じようにループを塞ぎうる点も指摘されました。
  書き込み側の上限は、この足場ではなく PR-C のトランスポート堅牢化とその回帰テストと一緒に入れます。ビルドの
  CI 配線は後続スライスへ延期します。
- PR-C（[#1032](https://github.com/bajutsu-e2e/bajutsu/pull/1032)）— Python 側のトランスポート
  配線です（作業単位 1 の起動、2、3、4）。`bajutsu/adb.py` に `adb forward`、`am instrument`、install
  のコマンドビルダを足しました。新しい `bajutsu/adb_resident.py` には次を収めます。`fetch_source`（標準
  ライブラリの `http.client` による `GET /source`。デコード失敗を含めチャネルの不具合はすべて
  `AdbResidentError` にして、ドライバが必ずきれいに縮退するようにします）、`narrow_to_active_window`
  （SystemUI の装飾ウィンドウを取り除き、常駐側の `dumpWindowHierarchy` XML が `uiautomator dump` と
  同じ Element に解析されるようにします。作業単位 2 の等価化で、二つのウィンドウを持つ木を手で組んで
  ゲートでテストします）、`ResidentServer` のライフサイクル（両方の APK を導入し、ブロッキングの
  インストルメンテーションを起動し、`adb forward tcp:0` を張り、上限つきの接続リトライで起動を待ち
  （固定スリープではなく条件待ちです）、`stop` でクライアントを回収してデバイス側のパッケージを
  force-stop し、フォワードを外します）。`make_driver` は `fetch_hierarchy` を `AdbDriver` へ渡し、
  `AndroidEnvironment` はリースごとにサーバを起動し、teardown で停止します（失敗や中断でも動きます）。
  読み取りの不具合はラッチします。最初の `AdbResidentError` の後はそのリースの残りでチャネルを無効にする
  ので、実行途中でチャネルが死んでも読み取りごとに再ログや接続タイムアウトの再支払いをしません。チャネルは
  e2e レーンがサーバをビルドして導入するまで `BAJUTSU_ADB_RESIDENT` による**オプトイン**です。設定しなければ
  adb バックエンドはこれまでどおり `uiautomator dump` で読み取ります。応答の途中でサーバが止まった場合は
  クライアントのソケットタイムアウトが上限になり（`AdbResidentError` からダンプへフォールバックします）、
  PR-B が挙げた書き込み側の塞がりは bajutsu の側からは押さえられます。サーバ側の書き込み上限は実機スライスの
  小さな堅牢化として残します。ここまではすべて実機不要でゲートでテストしています。実機での実測（約 0.1〜0.3
  秒）と e2e レーンでの両経路の担保（作業単位 5）は PR-D に入り、そこでチャネルを既定で有効にします。その
  ためこのボックスは未チェックのままです。
- PR-D ([#1042](https://github.com/bajutsu-e2e/bajutsu/pull/1042)) — 作業単位 5。チャネルを
  既定で有効にし、CI で両経路を守ります。`_make_resident` は、サーバの APK がビルド済み（`server_apks_built`、
  `make -C BajutsuAndroidUIAutomatorServer build`）のときは常駐チャネルで読み取り、そうでなければ `uiautomator dump` へ
  フォールバックするようになりました。これにより `BAJUTSU_ADB_RESIDENT` はオプトインのフラグから上書き用の
  スイッチへ変わります（`0` はビルド済みのツリーでもダンプ経路に固定し、`1` は常駐経路を強制して未ビルドなら
  はっきりと degrade します）。Android の e2e レーン（`android-e2e.yml`）はエミュレータを起動する前にサーバを
  ビルドするので、`e2e`／`e2e-golden`／`e2e-visual` は常駐チャネルを通り、新しい `e2e-fallback` ターゲットが
  同じ golden をチャネル無効で走らせ直します。同じ golden が両経路の要素ツリーの一致を示すので、常駐経路と
  `uiautomator dump` フォールバックは毎回の実行で両方とも動きます。ステップごとの実測は表明しません（ハードな
  CI の実測ゲートは BE-0234 と同様に対象外です）。エミュレータ上の常駐チャネルでの golden 一致が、実機での
  正しさの確認です。`drivers.md` と `architecture.md`（両言語）は常駐チャネル優先の読み取りを記述するように
  なりました。レーンが初めて緑になる過程で、速い読み取りが露呈した常駐経路とダンプ経路の非等価が 3 つ見つかり、
  両経路が同じツリーを返すようにここですべて直しました。1 つめは、常駐サーバがダンプの前に `UiDevice.waitForIdle()`
  を呼ぶようにした点です（ダブルタップの非同期なアクセシビリティ値がまだ伝播していないうちに速い読み取りが
  スナップショットしていました。`uiautomator dump` は idle を待つのでこの差は現れませんでした）。2 つめは、
  `AdbDriver._settle` を固定の読み取り回数ではなく実時間の締め切りで区切るようにした点です（BE-0234 の 3 回上限は
  約 2.4 秒の読み取りがループの間隔を作る前提でしたが、約 0.1 秒の常駐読み取りは静定の窓を潰し、まだ動いている
  ツリーを静定済みとして通してしまい、スクロール直後の tap が古い座標に当たっていました）。3 つめは、末尾の
  `expect` を 1 回の読み取りではなく条件待ち（`bajutsu/orchestrator/loop.py`）にした点です。操作がツリーへ映す値は
  操作が返った少しあとに届くことがあり（Compose は `content-desc` を非同期に再コンポーズします）、遅いダンプ読み取りは
