...
- PR-A（_pending_）— 常駐チャネルへの切り替え（作業単位 3）に向けた、Python のみ・実機不要の足場です。
  ...
  ままなので、チェックはまだ 1 つも付きません。`adb forward` のビルダと実際の HTTP トランスポートの
  ファクトリは、それらが dead code でなくなる PR-C へ回します。
