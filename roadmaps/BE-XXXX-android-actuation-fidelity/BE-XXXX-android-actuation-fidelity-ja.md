[English](BE-XXXX-android-actuation-fidelity.md) · **日本語**

# BE-XXXX — Android 実機アクチュエーションの忠実度

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-android-actuation-fidelity-ja.md) |
...
- [ ] system back（`keyevent 4`）と by-scheme deeplink。
- [ ] プラットフォームの double-tap ウィンドウ内での double-tap（単一の低遅延アクチュエーション経路）。
- [ ] scroll-into-view（回数を区切った scroll して再クエリする条件待ち）。
- [ ] ランタイム権限ダイアログの処理（決定論的な付与／消去）。
- [ ] 検証：可能な範囲で fast ゲートのビルダ／ループ、および `notices`／`gestures`／`controls`／権限で止まるフローの実機合格。
