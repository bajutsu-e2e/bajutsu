...
しまいます。`oauth_callback` には、サインインを成功させずに終える経路が4つあり、この項目はその4つ
すべてを記録するようにします。残りの3つ（CSRF の state 不一致、exchange が例外を発生させた場合、
exchange が identity を返さなかった場合）も、同じ `"oauth.denied"` イベントを記録します。上で
述べた理由がそのまま当てはまるためです（ユーザの「サインインできない」という報告と突き合わせる手がかりが
無い、素の 403 や 502 になってしまう）。CSRF の分岐は、別の理由でも自分自身の記録に値します。state の
不一致が繰り返されるのは、期限切れの cookie というより login-CSRF 攻撃の兆候であり、この項目の前は
見えないままでした。これら3つの早い段階では `login` がまだわからないため、その記録には `actor`
フィールドが付きません。`actor` が付くのは、後段のゲート由来の拒否と、すべての成功したサインインだけです。
...
- [x] `oauth_callback` のサインインゲートに、`identity_matches_org` と並ぶ admin Team の迂回を追加する。
      ...
      終える4つの経路すべてを、別の `"oauth.denied"` イベントで記録する。CSRF の state 不一致、
      exchange が例外を発生させた場合、identity を返さなかった場合、そしてどちらの確認にも一致しない
      login（最後のものは、`orgs:` が一致しなかった2つの形、config の読み込み失敗か、実在するが
      一致しない名簿かのどちらだったかも名指しする）。これにより、壊れた、あるいは存在しない
      `orgs:` ブロックに一致する admin Team もない状態が、突き合わせる手がかりの無い素の 403 や 502 の
      ままにならないようにする。identity を永続化する際、既存の login の記録済み org を `default`
...
- [x] テストを追加する。`orgs:` に一致するエントリがない場合と、`orgs:` ブロック自体がない場合の
      ...
      を記録することを確認する。CSRF の state 不一致、例外を発生させた exchange、identity を
      返さなかった exchange のそれぞれも `"oauth.denied"` を記録することを確認する。改名した変数が
