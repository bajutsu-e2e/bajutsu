...
（`/` の個数ではなく、organization 側と slug 側のどちらかが空である場合や、内部に空白がある場合、
slug 側に大文字が含まれる場合も拒否する正規表現で判定します。GitHub の Team の slug は常に小文字化
されるため、GitHub の UI に表示される Team の名前をそのまま書いた slug は、正しい形に見えても
決して一致しません）。スペースやセミコロンで
...
（上の malformed-entry の正規表現がすでに依拠している事実と同じです）。そのため、`admin_teams` の
エントリの organization 側が GitHub に保存されているどの大文字小文字であっても、login の
`identity.teams` の exact-case のメンバーシップと一致し続けなければならず、逆方向についても同様です。
この case-fold がなければ、この項目自身のサインイン迂回は、既存の `editorTeam` ロール判定には
存在しない潜在的な大文字小文字の罠を抱えてしまいます。`editorTeam` は、その大文字小文字を自分で
管理する運用者が一度だけ書き込み、毎回同じ GitHub 側の大文字小文字と比較されますが、`admin_teams`
のエントリは、たまたま大文字混じりで表示される GitHub の org ページから書き起こされたり、org の
rename で保存済みの大文字小文字が変わる前にコピーされたりした場合、許可するはずの login に
静かに一致しなくなってしまいます。これは、上の malformed-entry の warning がすでに防いでいる
「admin がいない、しかも見える原因がない」という同じ失敗であり、ただし大文字小文字の不一致は
構文的には整った形なので、その確認では届きません。case-fold は、空の Team 名を一致に変えることは
ありません。`admin_teams` にはカンマ分割時の `t.strip()` によるフィルタがあるため `""` が
含まれることはないからです。また、下記の入れ子 Team に対する保証にも影響しません。それは
`/` の個数に基づくものであり、大文字小文字には基づかないためです。
...
- [x] `BAJUTSU_OAUTH_ADMIN_TEAM` を `BAJUTSU_OAUTH_ADMIN_TEAMS`（カンマ区切り）に改名し、そのリストを
      ...
      および各エントリが正しい `"<GitHub organization>/<team slug>"` の組でないとき（slug 側に大文字が
      含まれる場合を含む。GitHub 自身の slug は常に小文字なので大文字は決して現れない）、それぞれ起動時に
      大きく警告する。admin を失う間違いをどれも見逃さず、かつトークン認証だけのデプロイに持っていない
      admin ロールについて警告しないようにする。
