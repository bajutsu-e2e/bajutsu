...
自体は、他のどの org にも属さない対象以上の特別な扱いを持たず、admin の `_ADMIN_PATHS` による
...
`targets_for_org` は、その org 自身の `targets:` の指定にかかわらず、対象の所有権判定でこの文字列を
...
- [x] テストを追加する。`orgs:` に一致するエントリがない場合と、`orgs:` ブロック自体がない場合の
      ...
      確認する。迂回した admin が `default` org に配置されることを確認する。
