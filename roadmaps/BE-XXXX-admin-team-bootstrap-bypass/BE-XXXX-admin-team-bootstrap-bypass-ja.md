...
や 403、502 になってしまう）。この4つの早い段階の記録はすべて `WARNING` ではなく `INFO` で発行し
ます。この項目が `WARNING` を与えるのは下記の org ゲートの拒否だけであり、そこに届くには実際に
GitHub との exchange が必要です。残り4つはどれも、実在する GitHub account を証明する必要が
ありません。`oauth is None` はデプロイの静的な性質であり、request ごとの信号ではありません。また
