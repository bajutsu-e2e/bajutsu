...
引数なので、これは3変数すべてが設定されるまで続きます）。このメッセージは、`cid` / `secret` /
...
実際にページングすべき唯一の形、つまり `admin_teams` が空で `orgs:` を直しにサインインできる admin が
1人も残っていない場合だけです。通常の拒否（構成済みの admin Team に単にこの login が一致しなかった場合）も、上記の4つの早い失敗と同じ理由で、依然として記録は残りますが、`INFO` で記録されます。
...
- [x] `oauth_callback` のサインインゲートに、`identity_matches_org` と並ぶ admin Team の迂回を追加する。
      `Identity` は `TYPE_CHECKING` の下でインポートする（`from __future__ import annotations` が
      すでに有効なので、annotation だけの用途にランタイムのインポートは不要）。モジュールレベルの
      インポートにすると、既定の `bajutsu.serve` / CLI パスに `bajutsu.serve.server` を引き込んで
      しまう。これは `bajutsu/serve/server/__init__.py` が「起こらない」と明言している状態である。
      `state.py` はすでに同じモジュールに対してこの前例を確立している。ロール判定のためにすでに
      ...
      すべて `INFO` で記録する。どれも実在する GitHub account を証明する必要がないためである。
      `oauth is None` はデプロイの静的な性質であり、攻撃者は `state_param` と自分自身の `Cookie:`
      header に同じ偽の値を送るだけで CSRF の確認を無料で突破できる。そしてどちらの確認にも一致しない
login（最後のものは、
      ...
      `state.config` 自体が `None` かどうかで見分ける。これにより、壊れた、あるいは存在しない
      `orgs:` ブロックに一致する admin Team もない状態が、突き合わせる手がかりの無い素の 404 や
      403、502 のままにならないようにする。identity を永続化する際、既存の login の記録済み org を
      `default` へ移すのではなくそのまま使うのは、config path が bind されているのに読み込みに
      失敗した場合に限る。config がまったく bind されていない場合（一時的な障害ではなく通常の
...
- [x] テストを追加する。`orgs:` に一致するエントリがない場合と、`orgs:` ブロック自体がない場合の
      ...
      失敗するテストが存在するようになる。独立したテストが、手作業で組み立てた
      `ServeState.startup_warnings` に対して `_emit_startup_warnings` を直接動かすので、この網羅は
      どの起動時チェックがどう書かれているか、あるいはまだ存在するかとは無関係に残る。もう1つの
      独立したテストが、警告すべきものが何もない場合の no-op 経路も固定する。改名した変数が
      ...
      （guard の2つの側を証明する）や、実際に revoke されたメンバーの次回のサインインでは
