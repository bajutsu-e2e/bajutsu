...
- [x] `BAJUTSU_OAUTH_ADMIN_TEAM` を `BAJUTSU_OAUTH_ADMIN_TEAMS`（カンマ区切り）に改名し、そのリストを
      `SessionManager`、`role_for`、サーバーバックエンドの環境変数配線に通す。OAuth を構成しているのに
      結果のリストが空になっているとき（未設定でも、廃止した単数形だけが設定済みでも）、および各
      エントリが正しい `"<GitHub organization>/<team slug>"` の組でないとき、それぞれ起動時に大きく
      警告する。admin を失う間違いをどれも見逃さないようにする。
