...
報告と突き合わせる手がかりが何もない、素の 403 のままではいけません。サインインが成功する
...
`INFO` と単純な「… signed in」という文言）を組み合わせます。これにより `bypass` フィールドは、すべての記録で定数の `True`
...
`False` になる3つの経路のうちどれだったかも名指しします。config の読み込み失敗、GitHub がこの
login に対して org を1つも返さなかった、あるいは `orgs:` のどのエントリにも一致しなかった――
`WARNING` に反応した運用者は、org ゲートがこの login を認めなかったという事実だけでなく、その
理由も知る必要があるためです。config の読み込み失敗なら config 自体を調べるべきであり、残りの2つは
org 名簿を調べるべきです。この区別を、1つの固定された文言の裏に隠してしまわないようにしています。
...
GitHub のどの org にも本当に属していない login――たとえば `members:` に列挙された bot・ops 専用の
...
- [x] `oauth_callback` のサインインゲートに、`identity_matches_org` と並ぶ admin Team の迂回を追加する。
      ...
      唯一のサインイン経路にも、`event` で絞り込める記録を残す。どちらの確認にも一致しない login は、
      別の `"oauth.denied"` イベントとして記録する。壊れた、あるいは存在しない `orgs:` ブロックに
      一致する admin Team もない状態が、突き合わせる手がかりの無い素の 403 のままにならないようにする。
      identity を永続化する際、config 自体の読み込み失敗によって `orgs:` ではなく迂回がその login
...
- [x] テストを追加する。`orgs:` に一致するエントリがない場合と、`orgs:` ブロック自体がない場合の
      ...
      引き続き拒否され、`"oauth.denied"` を記録することを確認する。改名した変数が複数の Team を
      持つリストとしてパースされることを
