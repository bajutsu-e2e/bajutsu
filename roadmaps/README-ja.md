...
両言語のファイルは [`document-writing`](../.claude/skills/document-writing/) スキル、すなわち BE 項目と
ドキュメントに共通する散文規範に従います。このスキルは、どの項目も満たすべき言語非依存の技法を定めます。
上から下へ推敲する、主眼を「はじめに」と「動機」の冒頭で述べる、各文の文末を最も重要な要素のために
空けておく、主語と述語を近づける、能動態を選ぶ、冗語を削る、といった技法です。書く前に呼び出し、
書いたあとではなく執筆そのものを導きます。

日本語版（`*-ja.md`）は、`docs/ja/` と同じく**敬体（ですます調）**で書きます。常体（だ・である調）は
使いません。日本語では、`document-writing` は [`japanese-document-writing`](../.claude/skills/japanese-document-writing/)
規範の上位に立ちます。両方を適用し、英語版の逐語訳ではなく、自然な敬体の日本語として読めるように
します。
...
単一のドライバインタフェースの背後に、プラットフォームごとに 1 つの backend を置きます（[DESIGN §1](../DESIGN.md)）。iOS Simulator（idb / XCUITest アクチュエータ）、Android（adb）、Web（Playwright）、Flutter に対応し、各 backend の操作忠実性を保つ実機検証も含みます。マルチプラットフォーム化は決定的コアの変更ではなく backend の追加として実現します。概要は [multi-platform.md](../docs/ja/multi-platform.md) を参照してください。
...
| [BE-0037](BE-0037-webview-hybrid-support/BE-0037-webview-hybrid-support-ja.md) | WebView / ハイブリッド対応 | 実装済み |
...
| [BE-0221](BE-0221-android-scenario-portability-guarantee/BE-0221-android-scenario-portability-guarantee-ja.md) | Android showcase の共有シナリオが無改修で動くことを保証する | 実装済み |
| [BE-0223](BE-0223-adb-tab-bar-navigation/BE-0223-adb-tab-bar-navigation-ja.md) | adb で tab bar を操作して Android の全タブへ到達する | 実装済み |
| [BE-0227](BE-0227-web-swipe-scroll-fidelity/BE-0227-web-swipe-scroll-fidelity-ja.md) | Web の swipe スクロール忠実度（入力モードに応じたスクロールの振り分け） | 実装済み |
| [BE-0228](BE-0228-web-device-mode-emulation/BE-0228-web-device-mode-emulation-ja.md) | Web のデバイスモード（デスクトップ／モバイルのエミュレーション） | 実装済み |
| [BE-0231](BE-0231-smoke-idb-first-wait-settling/BE-0231-smoke-idb-first-wait-settling-ja.md) | E2E の最初の待機を Simulator の settling flake に対して硬化する | 実装済み |
| [BE-0232](BE-0232-adb-multitouch-gestures/BE-0232-adb-multitouch-gestures-ja.md) | adb ドライバでマルチタッチジェスチャ（pinch / rotate）を実行する | 実装済み |
| [BE-0233](BE-0233-adb-clipboard-fidelity/BE-0233-adb-clipboard-fidelity-ja.md) | adb クリップボードの実機での忠実性 | 実装済み |
...
| [BE-0269](BE-0269-ios-alert-guard-early-wait-intervention/BE-0269-ios-alert-guard-early-wait-intervention-ja.md) | wait ステップ中のシステムアラートガードの介入を早める | 実装済み |
...
各プラットフォームが差し込まれる backend 非依存のコアです。ドライバインタフェースとその conformance スイート、capability モデルと事前チェック、`wait_for` のポーリング契約、backend のライフサイクル、ターゲットやプラットフォーム別の config 解決を扱います。
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0030](BE-0030-parameterized-shared-steps/BE-0030-parameterized-shared-steps-ja.md) | パラメータ化シェアドステップ | 実装済み |
| [BE-0031](BE-0031-data-driven-scenarios/BE-0031-data-driven-scenarios-ja.md) | データ駆動シナリオ | 実装済み |
| [BE-0032](BE-0032-secret-variables/BE-0032-secret-variables-ja.md) | シークレット変数 | 実装済み |
| [BE-0033](BE-0033-scenario-variables-control-flow/BE-0033-scenario-variables-control-flow-ja.md) | シナリオ変数 + 軽い制御フロー | 実装済み |
| [BE-0034](BE-0034-tags-selective-runs/BE-0034-tags-selective-runs-ja.md) | タグ / ラベル + 選択実行 | 実装済み |
| [BE-0035](BE-0035-device-control-primitives/BE-0035-device-control-primitives-ja.md) | デバイス制御ステップ（background・ステータスバー上書き） | 実装済み |
| [BE-0036](BE-0036-utility-steps/BE-0036-utility-steps-ja.md) | HTTP ユーティリティステップ | 実装済み |
| [BE-0046](BE-0046-otp-email-steps/BE-0046-otp-email-steps-ja.md) | OTP・メールの側方チャネルステップ | 実装済み |
| [BE-0052](BE-0052-device-state-timezone-clipboard-shake/BE-0052-device-state-timezone-clipboard-shake-ja.md) | デバイス状態プリミティブ: タイムゾーン・クリップボード・シェイク | 実装済み |
| [BE-0186](BE-0186-mailbox-provider-registry/BE-0186-mailbox-provider-registry-ja.md) | email ステップのメールボックス・プロバイダレジストリ | 実装済み |
| [BE-0265](BE-0265-text-editing-steps/BE-0265-text-editing-steps-ja.md) | テキスト編集ステップ: select・clear・delete・copy | 実装済み |
| [BE-0276](BE-0276-scenario-permission-state/BE-0276-scenario-permission-state-ja.md) | シナリオ単位で宣言する権限状態（simctl privacy / pm grant） | 実装済み |
<!-- /GENERATED:implemented-scenario-authoring -->

### 検証とカバレッジ

実行の合否を、決定的に、多くは UI より下のレイヤで判定する仕組みです。振る舞いやプロトコルのアサーション、ビジュアル回帰アサーションとその比較エンジン、決定性と flaky の監査、E2E カバレッジマップを扱います。これらの土台となる決定的ランナーが最初に着地しました。
...
| [BE-0029](BE-0029-visual-regression-assertions/BE-0029-visual-regression-assertions-ja.md) | ビジュアル回帰アサーション | 実装済み |
| [BE-0048](BE-0048-behavioral-protocol-assertions/BE-0048-behavioral-protocol-assertions-ja.md) | 振る舞い／プロトコルアサーション | 実装済み |
| [BE-0049](BE-0049-determinism-flakiness-audit/BE-0049-determinism-flakiness-audit-ja.md) | 決定性／フレーキネス監査 | 実装済み |
| [BE-0050](BE-0050-e2e-coverage-map/BE-0050-e2e-coverage-map-ja.md) | E2E カバレッジマップ | 実装済み |
...
AI 駆動の `record`（Tier 1）は実装済みです（[recording.md](../docs/ja/recording.md)）。ここでは record → 編集 → 再実行のサイクルを人にとって扱いやすくします。ローカル Web UI ランチャー `bajutsu serve` がその第一歩です。
...
| [BE-0226](BE-0226-cross-project-metrics-dashboard/BE-0226-cross-project-metrics-dashboard-ja.md) | プロジェクト横断のメトリクス比較ダッシュボード | 実装済み |
| [BE-0241](BE-0241-stats-run-drilldown/BE-0241-stats-run-drilldown-ja.md) | Stats ダッシュボードから対象の run へドリルダウンする | 実装済み |
| [BE-0262](BE-0262-serve-author-live-step-picker/BE-0262-serve-author-live-step-picker-ja.md) | Author エディタにライブなステップ選択と target 単位に絞った run を導入する | 実装済み |
| [BE-0263](BE-0263-serve-author-tiling-layout/BE-0263-serve-author-tiling-layout-ja.md) | Author ビューをタイリングレイアウトに組み込む | 実装済み |
| [BE-0267](BE-0267-serve-author-actuator-selection/BE-0267-serve-author-actuator-selection-ja.md) | serve の capture・enrich でコスト順のアクチュエータ選択を再利用する | 実装済み |
| [BE-0273](BE-0273-serve-replay-scenario-viewer/BE-0273-serve-replay-scenario-viewer-ja.md) | Replay タブからシナリオの内容を確認する（生 YAML と構造化ステップ） | 実装済み |
<!-- /GENERATED:implemented-authoring -->

### 自律クロール

アプリの画面を自力で辿って地図化する自律探索を、高速かつ再開可能に保ちます。複数の Simulator やブラウザでの並列クロール、Web クロール、フロンティア全体からの再開を扱います。

<!-- GENERATED:implemented-crawl -->
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0038](BE-0038-autonomous-crawl-exploration/BE-0038-autonomous-crawl-exploration-ja.md) | 自律クロール探索（App Explorer 風） | 実装済み |
...
合格したシナリオを、出力先フレームワークの流儀のネイティブテストに変換します。XCUITest、Playwright、Android（Espresso / UI Automator）に対応します。
...
AI を判定役から外して調査役に限定したまま、回帰の保守コストを下げます。
...
| [BE-0039](BE-0039-self-healing-propose-optin/BE-0039-self-healing-propose-optin-ja.md) | 自己修復は「提案＋opt-in 適用」に限定 | 実装済み |
...
実行結果を、チームが普段使っているサービスへ送り出します。判定後の決定的な転送であり、ランナーが算出した判定を運ぶだけで、LLM の判定は運びません。
...
Tier-1 の AI 経路（`record` / `triage` / `--dismiss-alerts` / `crawl`）は、差し替え可能なプロバイダ経由で Claude を呼びます。ここではそのプロバイダの選択と設定、たとえば直接の Anthropic API に代わる Amazon Bedrock（AWS 認証情報で認証）と、プロバイダ非依存で秘匿化された AI データ経路を扱います。決定的な `run` / CI ゲートはモデルを呼ばず、影響を受けません。

<!-- GENERATED:implemented-ai-provider -->
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0047](BE-0047-ai-data-sovereignty/BE-0047-ai-data-sovereignty-ja.md) | AI データ主権（プロバイダ非依存・秘匿化された AI 経路） | 実装済み |
...
| [BE-0184](BE-0184-persist-serve-ai-provider-settings/BE-0184-persist-serve-ai-provider-settings-ja.md) | serve の AI プロバイダー設定を再起動をまたいで永続化する | 実装済み |
...
| [BE-0229](BE-0229-per-org-provider-settings-resolution/BE-0229-per-org-provider-settings-resolution-ja.md) | serve の AI プロバイダー設定を組織ごとに実行時に解決する | 実装済み |
<!-- /GENERATED:implemented-ai-provider -->

### AI 使用量とコストの可観測性

Bajutsu の AI トークンと金額の消費を可視化します。すべての AI 呼び出しを帰属づけて永続化する台帳と、それを読む serve Web UI のダッシュボードを扱います。
...
CLI 自身のツールを、オーサリングが行われる serve Web UI に取り込みます。シナリオエディタのインライン lint とスキーマ検証が最初に着地しました。
...
| [BE-0272](BE-0272-serve-version-badge/BE-0272-serve-version-badge-ja.md) | serve の Web UI のヘッダーに bajutsu 自身のコミット・バージョンを表示する | 実装済み |
| [BE-0275](BE-0275-serve-projects-management-page/BE-0275-serve-projects-management-page-ja.md) | serve にプロジェクト管理ページを設ける（モーダルではなくトップレベルのビュー） | 実装済み |
<!-- /GENERATED:implemented-serve-cli-features -->

### Web UI のホスティング（クラウド / セルフホスト）

`bajutsu serve` をループバックの外に立ち上げます。既存の標準ライブラリ製サーバを安全に公開するための強化（認証、入力検証）は出荷済みで、ホスティングの全体構成は下の提案に残っています。

<!-- GENERATED:implemented-hosting -->
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0015](BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md) | Web UI の公開ホスティング | 実装済み |
...
| [BE-0239](BE-0239-deletable-runs-serve/BE-0239-deletable-runs-serve-ja.md) | serve の Web UI から run（レポート）を削除できるようにする | 実装済み |
| [BE-0277](BE-0277-docker-build-commit-badge/BE-0277-docker-build-commit-badge-ja.md) | セルフホスト用 Docker イメージにコミットハッシュを埋め込み、バージョンバッジに表示する | 実装済み |
<!-- /GENERATED:implemented-hosting -->

### config の取得元

プロジェクトの config とシナリオの取得元です。Git リポジトリと ref は、CI やセルフホストの `serve` から今すぐ使える取得元で、不変のコミットとして実体化します。
...
| [BE-0224](BE-0224-github-private-repo-config-auth/BE-0224-github-private-repo-config-auth-ja.md) | GitHub を取得元とする config での private リポジトリへのアクセス権限の付与 | 実装済み |
| [BE-0242](BE-0242-config-relative-path-resolution/BE-0242-config-relative-path-resolution-ja.md) | config が宣言するパスを、その config ファイル基準で解決する | 実装済み |
| [BE-0243](BE-0243-upload-bundle-durable-storage/BE-0243-upload-bundle-durable-storage-ja.md) | アップロードした zip config バンドルをオブジェクトストレージへ永続化する（ホスト型 serve 向け） | 実装済み |
| [BE-0268](BE-0268-composable-upload-artifacts/BE-0268-composable-upload-artifacts-ja.md) | config・シナリオ・アプリバイナリを独立した content-addressed な成果物として個別にアップロードし run ごとに合成する | 実装済み |
<!-- /GENERATED:implemented-config-sourcing -->

### セキュリティ強化

決定的コアが触れない周縁を塞ぎます。`serve` の HTTP 面、capture / record / 成果物を通る秘密情報の流れ、ドライバ引数の衛生、CI のサプライチェーンを対象とします。共有マシンでの実行や、信頼できない出所のシナリオを受け取る場面でも、prime directive を弱めずに安全に保ちます。
...
| [BE-0274](BE-0274-serve-scenario-secrets/BE-0274-serve-scenario-secrets-ja.md) | serve の Web UI からシナリオが宣言したシークレットを設定する | 実装済み |
<!-- /GENERATED:implemented-security -->

### Dogfood フィクスチャ（デモアプリ）

コマンドを end-to-end で動かすために用意した被験アプリです。showcase スイートは iOS 側の dogfood ターゲットで、同じアプリを UIKit と SwiftUI で書き、それぞれをアクセシビリティ有効版と無効版で用意しています。これにより `run`、`record`、`doctor`、`crawl` のいずれもが、豊かで代表的な被験対象を 1 つ持てます。画面ごとの契約は [`demos/showcase/SPEC.md`](../demos/showcase/SPEC.md) にあります。
...
Bajutsu 自身の `serve` Web UI は Web アプリなので、Web（Playwright）backend で駆動します。UI に対する決定的な Tier-2 の回帰網であり、iOS の showcase フィクスチャの Web 版に当たります。
...
このリポジトリで並行して動く多数のセッションの摩擦を減らします。マージ衝突を設計の匂いとして扱い、独立した変更が別々のファイルに触れるようファイルの流れを組み替えます。
...
| [BE-0266](BE-0266-contributor-workflow-tutorial/BE-0266-contributor-workflow-tutorial-ja.md) | コントリビューションワークフローチュートリアル: ideation・implement-be・propose-and-build を実際に使うガイド | 実装済み |
| [BE-0278](BE-0278-tech-writing-skill/BE-0278-tech-writing-skill-ja.md) | BE 項目と散文ドキュメントに共通するテクニカルライティングスキル | 実装済み |
<!-- /GENERATED:implemented-developer-experience -->

### コードベース品質・技術的負債

`bajutsu/` 自体の中の、振る舞いを変えないクリーンアップです。重複の解消、肥大化した関数やモジュールの分解、命名の明確化を扱います。上の *開発基盤（コントリビュータ体験）* がこのリポジトリで作業するためのツール（CI、フック、ロードマップ自動化）を対象とするのとは別の領域です。
...
| [BE-0246](BE-0246-claude-client-taxonomy/BE-0246-claude-client-taxonomy-ja.md) | Claude と話すためのモジュール構成を整理する | 実装済み |
| [BE-0247](BE-0247-serve-frontend-es-modules/BE-0247-serve-frontend-es-modules-ja.md) | serve フロントエンドを ES モジュールへ移行する | 実装済み |
| [BE-0248](BE-0248-serve-state-decomposition-continued/BE-0248-serve-state-decomposition-continued-ja.md) | ServeState の分解を継続し、認証系とプロバイダー設定系のマネージャに分離する | 実装済み |
| [BE-0249](BE-0249-dead-claude-client-wrapper-removal/BE-0249-dead-claude-client-wrapper-removal-ja.md) | バックエンドの seam に取り残された、使われていない Claude クライアントのラッパーを削除する | 実装済み |
| [BE-0250](BE-0250-assertions-package-eval-context/BE-0250-assertions-package-eval-context-ja.md) | assertions のパッケージ分割と評価コンテキストの EvalContext への統合 | 実装済み |
| [BE-0251](BE-0251-driver-base-helper-hoist/BE-0251-driver-base-helper-hoist-ja.md) | 重複した driver ヘルパーを drivers.base に集約し、細かな定数を統一する | 実装済み |
| [BE-0252](BE-0252-config-package-split/BE-0252-config-package-split-ja.md) | config をパッケージに分割し、Effective をサブレコードにまとめる | 実装済み |
| [BE-0253](BE-0253-serve-route-registry-unification/BE-0253-serve-route-registry-unification-ja.md) | serve の二重バックエンドのルートテーブルを宣言的レジストリに統一する | 実装済み |
| [BE-0254](BE-0254-coordinate-tree-driver-base/BE-0254-coordinate-tree-driver-base-ja.md) | idb と adb 向けに共有の CoordinateTreeDriver 基底クラスを抽出する | 実装済み |
| [BE-0255](BE-0255-codegen-shared-helper-dedup/BE-0255-codegen-shared-helper-dedup-ja.md) | codegen の識別子と正規表現のヘルパーを codegen_common に集約する | 実装済み |
| [BE-0256](BE-0256-platform-lifecycle-package-split/BE-0256-platform-lifecycle-package-split-ja.md) | platform_lifecycle をパッケージへ分割し、デバイス解決を Environment という seam に通す | 実装済み |
| [BE-0258](BE-0258-serve-step-artifact-seam/BE-0258-serve-step-artifact-seam-ja.md) | serve のステップアーティファクトの読み取りを ArtifactStore seam 経由に統一 | 実装済み |
| [BE-0259](BE-0259-assert-query-snapshot-reuse/BE-0259-assert-query-snapshot-reuse-ja.md) | assert と extract のステップで、確定した query のスナップショットを使い回す | 実装済み |
| [BE-0260](BE-0260-cli-bringup-consolidation/BE-0260-cli-bringup-consolidation-ja.md) | 重複した CLI コマンドの起動処理を統合し、中立な DeviceError を追加する | 実装済み |
| [BE-0261](BE-0261-serve-author-yaml-roundtrip/BE-0261-serve-author-yaml-roundtrip-ja.md) | Author の YAML 編集を serializer で往復させる | 実装済み |
| [BE-0264](BE-0264-serve-uncaught-exception-json/BE-0264-serve-uncaught-exception-json-ja.md) | serve ハンドラの未捕捉例外を JSON エラーで返す | 実装済み |
<!-- /GENERATED:implemented-quality-debt -->
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0257](BE-0257-layer-package-topology/BE-0257-layer-package-topology-ja.md) | 強制されているアーキテクチャレイヤをディレクトリとしてパッケージ化する | 実装中 |
<!-- /GENERATED:in-progress-quality-debt -->
...
| [BE-0279](BE-0279-crossbackend-e2e-required-gate/BE-0279-crossbackend-e2e-required-gate-ja.md) | E2E の必須チェックを全 backend に揃える | 提案 |
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0235](BE-0235-aws-device-farm-submitter/BE-0235-aws-device-farm-submitter-ja.md) | AWS Device Farm batch submitter | 提案 |
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0282](BE-0282-real-backend-network-coverage/BE-0282-real-backend-network-coverage-ja.md) | ネットワークのキャプチャ・モック・アサーションを CI で実バックエンド検証する | 提案 |
| [BE-0285](BE-0285-scenario-feature-real-backend-coverage/BE-0285-scenario-feature-real-backend-coverage-ja.md) | シナリオ作成機能の実バックエンドカバレッジを検証する | 提案 |
<!-- /GENERATED:proposals-verification -->
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0284](BE-0284-docs-single-source-of-truth/BE-0284-docs-single-source-of-truth-ja.md) | 重複するドキュメント規範を1つの基準ファイルへ集約する | 提案 |
<!-- /GENERATED:proposals-developer-experience -->
