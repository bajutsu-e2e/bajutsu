...
| [BE-0054](proposals/BE-0054-web-backend-completion/BE-0054-web-backend-completion-ja.md) | Web backend の完成（リッチな capability と並列実行） | 実装中 |
...
| [BE-0068](proposals/BE-0068-regenerable-reports/BE-0068-regenerable-reports-ja.md) | 再生成できるレポート（保存済み run データから描画する） | 実装中 |
<!-- /GENERATED:accepted-authoring -->
...
| [BE-0052](proposals/BE-0052-device-state-timezone-clipboard-shake/BE-0052-device-state-timezone-clipboard-shake-ja.md) | デバイス状態プリミティブ: タイムゾーン・クリップボード・シェイク | 実装中 | MagicPod |
<!-- /GENERATED:accepted-competitive -->

### 競合調査（Maestro）由来の候補

Bajutsu の「契約としての決定性」という立場を、Maestro の flakiness 許容と対比させ、機械検証可能な具体機能へ落とし込みます。決定性監査（静的な安定度スコア）が最初に着地したスライスです。
...
通過したシナリオを、出力先フレームワークの流儀に沿ったネイティブテストに変換する取り組み。元の XCUITest に加えて web（Playwright）向けが着地しました。残りの未対応は下記の提案に残っています。

<!-- GENERATED:accepted-codegen -->
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0026](proposals/BE-0026-shrink-unsupported-syntax/BE-0026-shrink-unsupported-syntax-ja.md) | 未対応構文の縮小 | 実装中 |
...
（可決済み）が**ファイルの流動**を見直したのに対し、ここでの提案は、コードベースが働き手（人間と
...
| [BE-0078](proposals/BE-0078-roadmap-status-folders/BE-0078-roadmap-status-folders-ja.md) | 状態ごとのロードマップフォルダ（提案 / 保留 / 実装中 / 実装済み） | 提案 |
<!-- /GENERATED:proposals-dev-infra -->

### Dogfood フィクスチャ（デモアプリ）

[BE-0045](implemented/BE-0045-dogfood-showcase-apps/BE-0045-dogfood-showcase-apps-ja.md) が始めた移行を
完了させます。showcase をオンデバイス CI と Web UI ツアーまで含めてカバーし、**唯一**の iOS フィクスチャに
した上で、旧来の `demo` / `sample` / `sample2` を退役させます。BE-0045 が意図的にスコープから外した後続作業です。
