...
単一のドライバインタフェースの背後に、プラットフォームごとに 1 つの backend を置きます（[DESIGN §1](../DESIGN.md)）。iOS Simulator（idb / XCUITest アクチュエータ）、Android（adb）、Web（Playwright）、Flutter に対応し、各 backend の操作忠実性を保つ実機検証も含みます。マルチプラットフォーム化は決定的コアの変更ではなく backend の追加として実現します。概要は [multi-platform.md](../docs/multi-platform.md) を参照してください。
...
AI 駆動の `record`（Tier 1）は実装済みです（[recording.md](../docs/recording.md)）。ここでは record → 編集 → 再実行のサイクルを人にとって扱いやすくします。ローカル Web UI ランチャー `bajutsu serve` がその第一歩です。
...
| [BE-0234](BE-0234-adb-run-performance/BE-0234-adb-run-performance-ja.md) | adb のシナリオ実行を高速化する（uiautomator dump のボトルネック） | 提案 |
<!-- /GENERATED:proposals-platform -->
...
- **クラウド端末ファーム、実機やクラウドでの実行**：スコープ外です。Bajutsu が対象とするのはローカルで CI に載せやすい backend（Simulator、ヘッドレスブラウザ、エミュレータ）であり、実機やクラウド端末ではありません（[DESIGN §1](../DESIGN.md)）。マルチプラットフォームは別途、提案として管理しています（*プラットフォーム対応* の項目）。
