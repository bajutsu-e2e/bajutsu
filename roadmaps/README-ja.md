...
`NNNN` は**ゼロ詰め 4 桁・単調増加**の ID です。すべての項目ディレクトリは **`roadmaps/` の直下**に
置きます。ID を採番した時点でパスが確定し、以後は動きません（BE-0159 で、BE-0078 が導入した
`状態` ごとのフォルダを廃止しました）。`状態` は、項目が並ぶインデックスのバケットを決める唯一の
真実であり続けますが、もうフォルダは決めません。したがって状態が変わってもファイルは移動しません。
...
2. **項目ディレクトリと両言語のファイルを作成**します。`roadmaps/` の直下に置き、提案なら `状態: 提案`、同じ PR で実装も出荷する場合は `状態: 実装済み` にします（新規項目は原則まず提案ですが、コードが一緒に入るなら最初から実装済みにします）。`roadmaps/BE-NNNN-<slug>/BE-NNNN-<slug>.md`（英語）と
...
がこの形を検査します。
**状態**がインデックスのバケットを決めます。`実装済み` / `実装中` / `提案` / `提案（保留）` です。
項目の状態が変わったら（着手した、あるいは出荷した）、`状態` を更新し、`make roadmap-index` で
バケットの表を再生成してください。ディレクトリは移動しません（BE-0159）。同じ `BE-NNNN-<slug>/` の
パスが、状態にかかわらずその項目を最後まで指し続けます。
...
| ID | 項目 | 状態 | 由来 |
|---|---|---|---|
| [BE-0038](BE-0038-autonomous-crawl-exploration/BE-0038-autonomous-crawl-exploration-ja.md) | 自律クロール探索（App Explorer 風） | 実装中 | Autify VAX |
<!-- /GENERATED:in-progress-competitive -->
...
| [BE-0016](BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting-ja.md) | Web UI のセルフホスティング | 実装中 |
| [BE-0110](BE-0110-evidence-store-uri/BE-0110-evidence-store-uri-ja.md) | URI 指定によるオブジェクトストレージへの証跡アップロード | 実装中 |
<!-- /GENERATED:in-progress-hosting -->
...
| [BE-0114](BE-0114-driver-conformance-suite/BE-0114-driver-conformance-suite-ja.md) | backend 非依存の挙動を検査する driver conformance suite | 提案 |
...
| [BE-0128](BE-0128-device-step-capability-preflight/BE-0128-device-step-capability-preflight-ja.md) | デバイス制御ステップをケイパビリティで preflight ゲートする | 提案 |
...
| [BE-0098](BE-0098-unified-authoring-surface/BE-0098-unified-authoring-surface-ja.md) | serve の統合オーサリングサーフェス | 提案 |
...
| [BE-0138](BE-0138-serve-lint/BE-0138-serve-lint-ja.md) | serve エディタでのシナリオ検証（lint / schema） | 提案 |
...
| [BE-0148](BE-0148-serve-doctor/BE-0148-serve-doctor-ja.md) | serve Web UI の doctor 準備状況パネル | 提案 |
<!-- /GENERATED:proposals-serve-cli-features -->
...
| ID | 項目 | 状態 | 由来 |
|---|---|---|---|
| [BE-0107](BE-0107-showcase-tab-navigation-no-launch-shortcut/BE-0107-showcase-tab-navigation-no-launch-shortcut-ja.md) | showcase の各タブへ、起動時の近道ではなくナビゲーションで到達する | 提案 | Dogfooding |
<!-- /GENERATED:proposals-dogfood -->
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0104](BE-0104-vendor-neutral-ai-backend/BE-0104-vendor-neutral-ai-backend-ja.md) | ベンダー中立な AI バックエンドインターフェース | 提案 |
<!-- /GENERATED:proposals-ai-provider -->
...
| [BE-0120](BE-0120-recorded-scenario-secret-tokenization/BE-0120-recorded-scenario-secret-tokenization-ja.md) | 記録された scenario の YAML でシークレットをトークン化する | 提案 |
| [BE-0121](BE-0121-serve-csrf-host-allowlist/BE-0121-serve-csrf-host-allowlist-ja.md) | serve の CSRF・Host allowlist 防御を無条件化する | 提案 |
| [BE-0123](BE-0123-composite-action-input-indirection/BE-0123-composite-action-input-indirection-ja.md) | composite action の入力を env 経由の間接参照にする | 提案 |
| [BE-0124](BE-0124-config-source-owner-repo-validation/BE-0124-config-source-owner-repo-validation-ja.md) | config-source の owner・repo 検証を厳格化する | 提案 |
| [BE-0130](BE-0130-default-network-secret-redaction/BE-0130-default-network-secret-redaction-ja.md) | ネットワークの機密ヘッダーと Cookie を既定で redact する | 提案 |
| [BE-0131](BE-0131-run-artifact-permissions/BE-0131-run-artifact-permissions-ja.md) | 実行証跡ファイルのパーミッションを制限する | 提案 |
| [BE-0136](BE-0136-serve-write-once-secrets/BE-0136-serve-write-once-secrets-ja.md) | serve の秘密情報ストアを書き込み専用にする | 提案 |
...
| [BE-0151](BE-0151-screenshot-secret-capture-warning/BE-0151-screenshot-secret-capture-warning-ja.md) | スクリーンショットと動画が画面上の秘密情報を捉えうることを警告する | 提案 |
| [BE-0153](BE-0153-encode-aware-secret-redaction/BE-0153-encode-aware-secret-redaction-ja.md) | エンコードを考慮した秘密情報の redaction | 提案 |
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0119](BE-0119-scenario-schema-versioning/BE-0119-scenario-schema-versioning-ja.md) | バージョン間の読み込みに備えてシナリオスキーマにバージョンを持たせる | 提案 |
<!-- /GENERATED:proposals-config-sourcing -->
...
| ID | 項目 | 状態 |
|---|---|---|
| [BE-0132](BE-0132-dedupe-crawl-screenshot-helpers/BE-0132-dedupe-crawl-screenshot-helpers-ja.md) | クロールのスクリーンショットヘルパーを重複排除する | 提案 |
| [BE-0135](BE-0135-module-naming-debt/BE-0135-module-naming-debt-ja.md) | トップレベルモジュールの命名の負債を解消する | 提案 |
| [BE-0143](BE-0143-run-command-decomposition/BE-0143-run-command-decomposition-ja.md) | run コマンドの巨大関数を分解する | 提案 |
<!-- /GENERATED:proposals-quality-debt -->
