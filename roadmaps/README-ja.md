...
`NNNN` は**ゼロ詰め 4 桁・単調増加**の ID です。各項目ディレクトリは `状態` ごとに 4 つの
フォルダのいずれかに置きます（BE-0078）。フォルダは進捗の度合いをそのまま映します。

| 状態 | フォルダ |
|---|---|
| `実装済み` | `roadmaps/implemented/` |
| `実装中` | `roadmaps/in-progress/` |
| `提案` | `roadmaps/proposals/` |
| `提案（保留）` | `roadmaps/deferred/` |

ロードマップ項目を追加するとき:

1. **次の ID を採番** = 既存の最大 `BE-NNNN` + 1（**すべて**のフォルダの項目を数えます）。次で確認します:
   ```bash
   ...
   取り合うのを防ぎます。
2. **項目ディレクトリと両言語のファイルを作成**します。提案なら `roadmaps/proposals/` の下に、同じ PR で実装も出荷する場合は `状態: 実装済み` にして `roadmaps/implemented/` の下に置きます（新規項目は原則まず提案ですが、コードが一緒に入るなら最初から実装済みにします）。`BE-NNNN-<slug>/BE-NNNN-<slug>.md`（英語）と
   `BE-NNNN-<slug>-ja.md`（日本語・同一 ID と slug）です。**下の
   ...
   マーカー間の表を再生成してください。項目の `状態`（バケット）+ `Topic` がセクションを決めるので、既存トピックの項目なら
...
`<!-- BE-METADATA -->` … `<!-- /BE-METADATA -->` の中に `提案`・`提案者`・`状態`・
...
がこの形を検査します。
**`状態` が唯一の真実の源**です。項目のフォルダ（上の表）とインデックスのバケットの両方を `状態` が
決めます。`実装済み` → 実装済み、`実装中` → 実装中、`提案` → 提案、`提案（保留）` → 保留、と対応するので、
両者がずれることはありません。項目の状態が変わったら、`状態` を設定し、ディレクトリをその状態が示す
フォルダへ（同じ ID と slug のまま）**移動**して、インデックスを再生成してください。`make roadmap-promote`
を使えば、移動と再生成をまとめて行えます。

---

## 実装済み

出荷済みで `main` に入っています。プロジェクトの**実装の記録**です。各項目の `状態` は `実装済み` で、
`roadmaps/implemented/` の下にあります。
...
マルチプラットフォーム化を、スライスを 1 つずつ着地させて進めています。**Web（Playwright）backend** と、**プラットフォーム対応の backend レジストリ**（`--backend` / `backend:` が bare な actuator に加えてプラットフォームトークン（`ios` / `android` / `web` / `fake`）を受け付ける）です。各プラットフォームの三点セットの残りは、提案の[プラットフォーム拡張](#プラットフォーム拡張android--web--flutter)で扱います。
...
AI 駆動の `record`（Tier 1）は実装済みです（[recording.md](../docs/ja/recording.md)）。これらの項目は、記録 → 編集 → 再実行のサイクルを人が扱いやすくします。ローカル Web UI ランチャ `bajutsu serve` がその最初のステップです。
...
Maestro の flakiness 許容に対し「契約としての決定性」を尖らせた機能のうち、着地したもの。まずは振る舞い／プロトコルアサーション（event・request sequence・response schema）です。
...
コマンドを端から端まで行使するための目的特化のテスト対象です。showcase 群は次世代の dogfood 対象で、同じアプリを UIKit と SwiftUI で書き、各々をアクセシビリティ有/無の変種で出します（2 コードベースで 4 プロダクト）。これにより、`run`（id ベース）、`record`（id 無しフォールバック）、`doctor`（Ready vs Blocked）、そして `crawl`（[BE-0038](in-progress/BE-0038-autonomous-crawl-exploration/BE-0038-autonomous-crawl-exploration-ja.md)）が、1 つの豊かで代表的な対象を持ちます。画面ごとの契約は [`demos/showcase/SPEC.md`](../demos/showcase/SPEC.md) にあります。
...
通過したシナリオを、出力先フレームワークの流儀に沿ったネイティブテストに変換する取り組み。元の XCUITest に加えて web（Playwright）向けが着地し、複合セレクタ・デバイス制御の未対応構文も縮小しました。残りの未対応は下記の提案に残っています。
...
可決され、現在実装中です（PR が進行中）。各項目の `状態` は `実装中` で、`roadmaps/in-progress/` の
下にあります。

### プラットフォーム拡張（着手済みスライス）

着地済みの v1 スライスの上に、Web（Playwright）backend を能力モデルの豊かな端へ引き上げます。ネイティブ network、video / console 証跡、エミュレートした multi-touch、並列実行です。
...
実行レポートを保存済み run データの純粋な描画にし、シナリオを再実行せずに完了済みの run から再生成できるようにします。
...
MagicPod / Autify 由来の候補のうち、現在実装中のもの。自律クロール探索と、次のデバイス状態プリミティブです。
...
Bajutsu の「契約としての決定性」という立場を、Maestro の flakiness 許容と対比させ、機械検証可能な具体機能へ落とし込みます。振る舞いアサーション、決定性監査、カバレッジマップです。

<!-- GENERATED:in-progress-competitive-maestro -->
| ID | 項目 | 状態 | 由来 |
|---|---|---|---|
| [BE-0049](in-progress/BE-0049-determinism-flakiness-audit/BE-0049-determinism-flakiness-audit-ja.md) | 決定性／フレーキネス監査 | 実装中 | Maestro |
...
現状はスコープを **iOS Simulator 限定**としています（[DESIGN §1](../DESIGN.md)）。このセクションは driver / backend 抽象を活用したマルチプラットフォーム化の方向性を扱います。コアのスコープ文の更新を伴う戦略的な判断です。全体像（大枠）は [multi-platform.md](../docs/ja/multi-platform.md) にあり、**プラットフォーム別の具体的な設計**は以下の各項目にあります。[BE-0009](proposals/BE-0009-cross-platform-abstractions/BE-0009-cross-platform-abstractions-ja.md) が共有の抽象、続いて Android、Flutter です。Web（最初に推奨）はすでに着地しています（[BE-0041](implemented/BE-0041-web-playwright-backend/BE-0041-web-playwright-backend-ja.md)）。プラットフォーム対応の backend レジストリも同様です（[BE-0042](implemented/BE-0042-platform-backend-registry/BE-0042-platform-backend-registry-ja.md)）。
...
`bajutsu` が config とシナリオ一式をどこから読むか。現在はローカルパスだが、ここで扱う項目は **ある ref を指す Git リポジトリ**（`github:owner/repo@ref:path`）を指定できるようにし、ホスト型やセルフホストの `serve`、あるいは CI のランナーが、チームのテストリポジトリを直接取得できるようにする（シナリオはもともと git 管理下のファイルである。[DESIGN §6.5](../DESIGN.md)）。
...
このリポジトリを読み書きする多数の並行セッション（人間と AI）のコストを下げます。
[BE-0043](implemented/BE-0043-conflict-resistant-file-flow/BE-0043-conflict-resistant-file-flow-ja.md)
（実装済み）が**ファイルの流動**を見直したのに対し、ここでの提案は、コードベースが働き手（人間と
エージェント）に向けて**自身をどう説明するか**を良くします。
...
検討した上で、意図的に見送った提案です。各項目の `状態` は `提案（保留）` で、`roadmaps/deferred/` の
下にあります。
