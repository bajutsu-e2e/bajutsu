...
1. **次の ID を採番** = 既存の最大 `BE-NNNN` + 1（**両方**のフォルダの項目を数えます）。次で確認します:
   ...
   取り合うのを防ぎます。
2. **項目ディレクトリと両言語のファイルを作成**します（新規項目はまず提案なので `roadmaps/proposals/`
   の下に）。`roadmaps/proposals/BE-NNNN-<slug>/BE-NNNN-<slug>.md`（英語）と
...
| [BE-0068](proposals/BE-0068-regenerable-reports/BE-0068-regenerable-reports-ja.md) | 再生成できるレポート（保存済み run データから描画する） | 提案 |
| [BE-0072](proposals/BE-0072-responsive-web-ui/BE-0072-responsive-web-ui-ja.md) | serve Web UI のレスポンシブ対応（小さい画面・タッチ操作） | 提案 |
<!-- /GENERATED:proposals-authoring -->

### AI プロバイダ設定

出荷済みの Bedrock 対応（[BE-0053](implemented/BE-0053-bedrock-ai-provider/BE-0053-bedrock-ai-provider-ja.md)、可決済み）を超えて、差し替え可能な AI プロバイダを広げる項目です。プロバイダの設定と認証を容易にします。例えば、Bedrock 用の AWS 認証情報を、`serve` を起動したシェルではなく Web UI から取得します。
