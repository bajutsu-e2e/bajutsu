...
- [x] 検証：fast ゲートでコマンドビルダと scroll して再クエリするループを注入した `run` で検証し、
      実機合格（`notices`／`gestures`／`controls`／権限で止まるフロー）は Android エミュレータの e2e
      レーンで確認します。

ログ:

- _pending_ — `back`（バックエンド横断のステップ。Android は `keyevent 4`、iOS／XCUITest は OS の戻る
