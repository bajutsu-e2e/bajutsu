...
| 実装 PR | [#857](https://github.com/bajutsu-e2e/bajutsu/pull/857) |
...
- [x] 検証：fast ゲートでコマンドビルダと scroll して再クエリするループを、注入した `run` で検証済み
      です。4 シナリオ（`notices`／`gestures`／`controls`／権限で止まるフロー）の実機合格は後続作業です。
      エミュレータの e2e レーンの `E2E_SCENARIOS`（`demos/showcase/android/Makefile`）はまだこれらを
      実行しておらず（BE-0007／BE-0208 で先送り）、`permission.yaml` も決定論的な `grantPermissions`／
      `pm grant` 経路にはまだ整えられていません。
