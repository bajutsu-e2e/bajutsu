[English](BE-XXXX-serve-author-live-step-picker.md) · **日本語**

# BE-XXXX — Author エディタにライブなステップ選択と target 単位に絞った run を導入する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-serve-author-live-step-picker-ja.md) |
...
これを回避できた唯一の入口 — 取り込んだばかりのシナリオをそのまま Edit に流す `auOpenSaved` — は Capture が起点ですが、
その Capture 自体が `[ios]` target で壊れています（姉妹提案のアクチュエータ選択で対処）。したがって大半のシナリオで、
...
1. **target/シナリオで絞った run 一覧。** Author の Run ドロップダウンを、現在の選択と target・シナリオが一致する run だけに
   ...
   Author 専用の runs エンドポイント）を優先し、ペイロードを小さく保ち絞り込みを正とします。
2. **run なしのライブなステップ選択。** 選択中の target のドライバを起動し（共通のコスト順アクチュエータ選択を再利用 —
   姉妹提案のアクチュエータ選択を参照 — し、同じ `[ios]` クラッシュを再導入しない）、スクリーンショットと query を取り、画面
   クリックをライブなツリーに対して解決するライブ解決経路を Edit に追加します。Capture の `mark` 解決を写したものです。選ばれた
...
- **グローバル run 一覧のクライアント側フィルタだけ。** 単純ですが、全 run をブラウザに送り続け、絞り込みが正になりません。
