...
どちらも `ANTHROPIC_API_KEY` または AWS の資格情報で認証します。`record` だけは、これとは別に
`--agent claude-code`（`bajutsu/claude_code_agent.py` と `bajutsu/agents.py`）を選べます。
これは Claude Code CLI（`claude -p`）を呼び出すもう一つの経路で、API 利用料の代わりに Claude
Pro / Max のサブスクリプションに課金できます。この二重構造は `bajutsu/ai_availability.py` 自身
の docstring が「既存の仕組みを薄く一般化したものであり、新しい subsystem ではない」と述べて
おり、解消すべき歪みとしてすでに認識されています。しかも実際の恩恵は見た目より狭く、サブスクリ
プション課金が効くのは `record` だけで、しかもそこでもテキストのみです（スクリーンショットを
送らない）。`claude -p` の構造化出力モードは生の Messages API とは別物だからです。
...
- `ANTHROPIC_API_KEY` を発行・ローテーションする代わりに、すでに持っている Pro / Max /
  Console のシートに Claude の利用料を寄せたいチームは、現状では `record` でしか、しかも
  テキストのみでしかそれができません。`crawl`（画面の解釈にスクリーンショットを使います）、
  `triage --ai`（画像を使った原因調査）、`--dismiss-alerts`（画像そのものが判定の入力です）
  にはその選択肢がなく、これは好みの問題ではなく実際の機能差です。
- 「Claude に到達する」経路が構造的に異なる二本立てになっている状態（`bajutsu.ai` レジストリと
  ...
  `alerts` が現状そうであるように「サブスクリプション課金に対応するかどうか」を個別に判断する
  羽目になります。
...
1. **`anthropic_client.py` に三つ目のプロバイダを追加する。** `PROVIDERS` に `"ant"` を
   ...
   `ai.keyEnv` / `ANTHROPIC_API_KEY` を読む代わりに `ant` バイナリを呼んで（例えば `ant auth
...
5. **`claude-code` エージェントを削除する。** `bajutsu/claude_code_agent.py` を削除し、
   `agents.AGENT_KINDS` と `make_agent` の分岐から `"claude-code"` を外します。
   `ai_availability.py` の CLI 専用分岐（`CLAUDE_CODE_MISSING`、`agent_kind` 引数）も、
...
7. **`ant` が未インストールなら自動インストールする。** `ant` は外部バイナリで、`subprocess`
   と `shutil.which` による存在確認は、`ai_availability.py` が今も `claude` CLI を確認して
   いるのと同じやり方です。ただしバイナリが見つからないときは、エラーで止まるだけでなく
   bajutsu が**その場でインストールします**。これは `make serve`
   （[scripts/serve.sh](../../../scripts/serve.sh)）が idb バックエンドの依存をオンデマンドで
   インストールするのと同じ発想で、CI が `actionlint` をインストールするのと同じ位置づけです。
   インストーラは、実行環境の OS・アーキテクチャに合ったリリースバイナリを CLI の GitHub
   リリースから取得し（ドキュメントに載っている `curl` の手順）、**バージョンを固定し SHA で
   検証したうえで**（actionlint インストーラに対する
   [BE-0133](../BE-0133-pin-actionlint-installer/BE-0133-pin-actionlint-installer-ja.md) の
   規範）、`PATH` 上の bajutsu 管理下の場所に置きます。Homebrew や `go install` は、ユーザーが
   好むなら選べる手動の代替手段として残ります。このインストールは `ant-cli-missing` の
   ギャップトークンと同じ仕組みから提供します。`make` のターゲットを設け、ギャップをすでに
   報告している `doctor` / `serve` の準備状況の画面にインストール操作を加えるので、バイナリの
   不在は行き止まりではなく一手で解消できる状態になります。**Python の依存は増えません**。
   `ant` は `pyproject.toml` には入らず、idb クライアントや `idb_companion` と同じく、
   オンデマンドで取得する実行時のツールです。
...
- [ ] `bajutsu/claude_code_agent.py` と `claude-code` エージェント種別を削除する
      （`agents.py`、`ai_availability.py`）
- [ ] `serve` の AI プロバイダ選択 UI を更新する
- [ ] バイナリが未インストールのとき `ant` をオンデマンドでインストールする（バージョン固定 +
      SHA 検証、`make serve` / actionlint と同じ発想）。`make` / `doctor` / `serve` の
      `ant-cli-missing` ギャップと連動させる
...
- [BE-0133 — Pin the actionlint installer by SHA](../BE-0133-pin-actionlint-installer/BE-0133-pin-actionlint-installer-ja.md)
  ―オンデマンドの `ant` インストーラが踏襲する、SHA で固定する規範です
