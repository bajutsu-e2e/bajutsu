...
# BE-XXXX — AI と外部サービスに依存する機能の決定論的な部分を実機で検証する
...
メールボックスと 2 要素認証のメールステップ、自律クロール、そしてデバイスを駆動する MCP サーバ。この 3 つの
機能領域は、スタック全体としては AI 駆動か外部サービス依存であるため、決定論的な CI ゲートでは検証されません。
プライムディレクティブ 1 が、LLM を合否判定の経路に置くことを禁じているからです。

しかしこの 3 つには、LLM を呼ばず実際の外部サービスにも接続せずに実バックエンドで動かせる決定論的な部分が
それぞれあります。メールステップの HTTP 取得から抽出、アサートまでの連鎖、AI を使わないガイドで動かすクロール
エンジン、そして `run` CLI を起動するだけの MCP の `bajutsu_run` ツールです。これらはいまどの CI レーンにも
接続されていません。本項目では、この決定論的な部分を実デバイス上で動かす軽量な確認テスト（スモークテスト）を
追加します。一方、モデルが合否を左右する経路（record、モデルを使うクロール、enrich）は、意図的に決定論的な
CI の外に保ちます。
...
しました）。ループバックのスタブメールサーバがあれば、取得から抽出、アサートまでの連鎖を、実際のプロバイダや
鍵なしに決定論的に走らせられます。したがって OTP とメールステップ
...
すなわち画面変化の検出、クラッシュの検出、`recover` の継ぎ目は、いまは fake の `react` フックで完全に
スクリプトされています。そのため、実アプリの画面が操作に応じて変化するか、`recover` が動けなくなったブラウザを
復旧させるかは未観測です。web ショーケースに対する AI 抜きのクロールが、モデルなしでその契約を動かします。
...
いずれも LLM を判定の経路に置きません。それぞれが、本来は除外される機能の決定論的な部分であり、その境界は
