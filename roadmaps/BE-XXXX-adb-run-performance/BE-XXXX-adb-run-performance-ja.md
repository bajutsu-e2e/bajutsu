...
したがって、現実的な 1 ステップあたりの読み取り回数は次のとおりです。

| ステップ | 読み取り回数 | 約 2.4 秒/回 での実時間 |
|---|---|---|
...
10 ステップのシナリオでは、**ダンプだけでおよそ 48 秒**を費やす計算になります。これが Android で
シナリオを書く体験を重くしている正体であり、その負荷はすべて読み取りに由来します。
...
2. **結果が使われないときは画面を読まない（ランナー、低リスク）。** 各ステップ末尾の
   ...
   **遅延・条件付き**にします。実際に消費されるとき——シナリオが `wants_screen_changed` である、
   ...
   決定性はそのままです。同じ「消費されるときだけ」というゲートを `before` の読み取りにも適用します。
   これはバックエンド非依存で、idb も（読み取りがすでに安いため相対的には小さいものの）速くなります。
...
- [ ] ランナーのステップ末尾の `after`（および `before`）読み取りを遅延・条件付きにし、不要なら省く。
