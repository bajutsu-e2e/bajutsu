...
- `bajutsu/report/ctrf.py` は `YYYYmmdd-HHMMSS` 形状をパースして UTC の開始時刻を導出します。
- `serve/helpers.py` の `valid_run_id` はタイムスタンプ検証ではなく、パス安全性（安全な 1 セグメント）チェックです。
...
- [ ] 4 つの run id 刻印箇所を移行（`audit-` 変種を含む）
