...
- [x] Unit 1 — 往復の境界（ファイル全体か scope 付きブロックか）を決めて記録。
- [x] Unit 2 — Apply（Edit）をパースしたモデル＋serializer 経由に。
- [x] Unit 3 — Accept（Enrich）をパースしたモデル＋serializer 経由に。
- [x] Unit 4 — Save 前のライブな lint/audit 確認を保つ。
- [x] Unit 5 — 文字列照合が誤る整形での往復テスト。

**選んだ境界（Unit 1）：scope 付きブロックの継ぎ足し、PyYAML のみ。** エディタはファイルの生テキストを
（コメントごと）読み込みます。そのためファイル全体を再シリアライズすると、Apply のたびにコメントを落とし、
関係しないシナリオまで整形し直してしまいます。代わりにバックエンドが PyYAML でパースし、変更するステップや
expect のブロックの範囲を `yaml.compose` のソースマークで特定し、そのブロックだけを再シリアライズして生テキストに
継ぎ足します。ブロックの外側にあるコメントは残ります。コメントを保つ `ruamel` での往復は、`bajutsu/_yaml.py` と
`on`/`off` の真偽値解釈が異なる二つ目の YAML エンジンを持ち込むことになるため、採りませんでした。
...
- 実装：新しい `bajutsu/scenario/edit.py`（`apply_selector` / `apply_enrichment`）と、`dump_block`
  ...
  ステップ、ステップ間のコメント、`:`/`#` を含む selector 値、2 シナリオのファイルを対象にします。
