[English](BE-0187-serve-config-view.md) · **日本語**

# BE-0187 — serve の Web UI で読み込み中の config を確認する（生 YAML、構造化ツリー、Git 由来）

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0187](BE-0187-serve-config-view-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0187") |
...
計算し、bind の応答としてすでに返している `source_provenance` の刻印です。ただ、その後それが **UI に**
出ていないだけです。これを UI に出せば、「正しいものが bind されていると信じる」状態が「正しいものが bind
されていると見て確かめられる」状態に変わります。これはこの提案が土台とする Git のパスでとくに効きます。
...
- **config の中身を返す serve オペレーションとエンドポイント。** 読み取りオペレーションが、bind した config の
  生のテキスト、そのパス、パースした構造（プロジェクトの制限付き YAML ローダで読み込むので、`on:` のキーは真偽値に
  変換されず文字列 `"on"` のまま残り、そのうえで JSON で扱える型に整えます）、そして由来（または `None`）を
  返します。両方の serve トランスポート（stdlib のハンドラと FastAPI アプリ）に `GET /api/config/content` として
  公開し、config が bind されていなければ 404 を返します。config の中身の全文は、パスだけの `/api/config` よりも
  開示が広いので、この読み取りは `admin` role に限定します（ローカルでトークンなしのデプロイのときだけ制限が
  かかりません）。
- **プレースホルダはそのまま、ただし秘匿情報の伏せ字ではありません。** テキスト（とパースした構造）は bind された
  ままのファイルです。`${secrets.*}` のプレースホルダは書かれたとおりに現れ、解決されることはないので、Git に
  コミット済みの、あるいはバンドルに同梱されたファイルを超える開示はありません。ただし伏せ字にはしません。ローカルや
  アップロードした config に**平文で**書かれた秘匿情報は、そのまま表示されます。秘匿情報はファイルに書かず
  `${secrets.*}` の参照にしておくのが約束事であり、上記の `admin` ゲートが、いずれにせよ中身を読める相手を絞ります。
...
- **モーダルを使わず、ヘッダに YAML 全文をインライン表示する。** config は長く、ヘッダはタブバーと場所を共有して
  います。クリックで開くモーダルにすれば、既定の UI を散らかさずに済みます。
...
- [x] 中身、パス、パースした構造、由来を返す config-content の読み取りオペレーション。
...
- [#734](https://github.com/bajutsu-e2e/bajutsu/pull/734) — 項目全体を実装しました。`ServeState`
  への由来の保持、`config_content` オペレーションと `GET /api/config/content`（両トランスポート、`admin`
  role に限定）、View ボタンと Structured / Raw のビューアモーダル、そして英日のドキュメントです。
