...
- [ ] `list_crawl_runs()` helper（`screenmap.json` を目印に走査し、件数を要約し、
      `crashes/`/`flows/` の有無を検出する）
- [ ] `/api/crawl/runs` read エンドポイント
- [ ] Crawl タブ内の履歴リスト（既存の `loadGraph()` に接続する）
- [ ] 読み取り専用の見せ方（ライブ操作の無効化、過去の run であることの明示）
- [ ] 選択中の run のクラッシュ/フローファイルへのリンク
