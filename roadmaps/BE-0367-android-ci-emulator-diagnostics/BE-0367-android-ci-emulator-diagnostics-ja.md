[English](BE-0367-android-ci-emulator-diagnostics.md) · **日本語**

# BE-0367 — Android の CI 失敗の原因究明に必要な多層の診断ログを収集する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0367](BE-0367-android-ci-emulator-diagnostics-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0367") |
| 実装 PR | [#1631](https://github.com/bajutsu-e2e/bajutsu/pull/1631) |
...
- **第3層を `android-e2e.yml` の既存の `poll_cpuinfo` サンプラーに統合する、あるいはその入力に
  ...
  **ホスト**自身の負荷を読みます。`poll_cpuinfo` が `pull_request` と `merge_group` のすべての
  ...
  adb トラフィックをまったく発生させず、頻度も10分の1です。そのため第3層は、`poll_cpuinfo` が
...
- [x] Unit 1 — Android の `video` プロバイダの録画バイト増加監視
- [x] Unit 2 — ストール時プローブのフック（階層読み取りのフォールバック、新しいバイト無し警告）
- [x] Unit 3 — `scripts/collect_android_diagnostics.sh` と各ジョブの `script:` への配線
- [x] Unit 4 — composite action `collect-android-diagnostics`（ホストテレメトリのみ）
- [x] Unit 5 — ドキュメント（`docs/ci.md` とその `ja` ミラー）
- [x] Unit 6 — テスト

[BE-0361](../BE-0361-ios-ci-simulator-diagnostics/BE-0361-ios-ci-simulator-diagnostics-ja.md) が先に
着地したため、本項目は2つのトリガーを、隣に別のモジュールを足すのではなく、BE-0361 の
`bajutsu/stall_diagnostics.py` に配線しました。*検討した代替案* が本項目を BE-0361 に依存させることを
退けたときに見込んでいたとおりの結果です。この配線の過程で、どちらの項目も名前を与えていなかった継ぎ目が
見えました。いつ捕捉するか、捕捉が何を費やしてよいか、どこへ書くかは両バックエンドで同一である一方、
何を読むかには共通のコマンドが1つもありません。そこで Unit 2 はこの継ぎ目でモジュールを分けました。
オプトインの門、トリガーごとの上限、サマリを引き受ける `capture` と、各バックエンドが持ち寄る
`ProbeSet` です。2つのレーンのプローブは `simulator_probes` と `device_probes` になりました。3つめの
バックエンドは、プローブ一式を足すだけで済みます。Unit 5 が2つのレーンの `docs/ci.md` の記述を1つの
節にまとめたのも、同じ理由です。

Unit 3 と Unit 4 は、上記 *詳細設計* が除外した `uiautomator (codegen)` も対象にしました。あの除外は
収集の**仕組み**から論じたもので、つまり「このジョブは `runs/` に何も書かないので相乗りできない」という
話です。これをジョブ自身の**必要性**と取り違えていました。アップロードの実態はむしろ逆を示します。
`codegen` がアップロードする経路は、どれも Gradle が生成テストを実行して初めて生まれます。その手前で
落ちる原因は、AVD の boot の固まり、codegen やビルドの失敗、ジョブ自身の `timeout-minutes` の発火です。
いずれも `if-no-files-found: ignore` に当たり、1つもアップロードできていませんでした。
収集の出力先は制約ではなく引数なので、このジョブにも同じ2段とホストのサンプラをそのまま入れ、
アップロードに `runs/` を足しました。第1層だけはフックが2つではなく1つです。テスト時に adb ドライバが
走らないため、ホスト側のレコーダ
（[`demos/showcase/android/screenrecord.py`](../../demos/showcase/android/screenrecord.py)。録画の
バイト増加を確認するようにしました）だけが発火します。もっとも、描画が固まった状態はこのレーンの既知の
フレークなので、そもそもこちらが適切なフックです。
