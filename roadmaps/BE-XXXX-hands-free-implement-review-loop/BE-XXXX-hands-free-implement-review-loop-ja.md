...
PR が収束しない場合の歯止めは二層構造です。主要な歯止めは**24時間の経過時間**で、この上限に達したらループは
現在の状態（CI 状態、未解決コメント数）を報告して終了します。その上に**20周回（レビュー待ちポーリングのみ計上）**
を二次安全網として重ねます。CI 待ちの短いポーリングはこのカウントに含めません。1回の CI ランだけで
キャッシュウィンドウ間隔（60〜270秒）の何倍も掛かり、PR が push→CI を複数回繰り返すこともあるため、CI 待ちを
同じカウンタに含めると24時間に届く前に20周回が消化されてしまいます。これは歯止めが防ごうとしているリスクに
逆行します（緩やかだが収束途中の PR ではなく、暴走するタイトループが対象です）。24時間上限は「人間が一度
確認すべきタイミング」を示す自然なシグナルで、20周回はあくまで暴走ループへのフェイルセーフです。
セッションローカルの `/loop` をスケジュール型クラウドエージェントより選んだのは、設定が簡単で割り込みがしやすい
からです。セッションを止めれば人間はいつでもループを中断・再起動できます。
...
- [ ] Unit 3 — 3つの停止条件（CI 緑・CHANGES_REQUESTED なし・静かな周回2回連続）とエスカレーション（設計変更・コンフリクト）、歯止め（24時間主要・20周回二次）を備えた一定間隔の pr-followup ループ。
