...
PR が収束しない場合の歯止めとして、ループは**レビュー待ちポーリング 20 周回**で停止します。CI 待ちの
短いポーリングはこのカウントに含めません。レビュー待ちに推奨される間隔（ハーネスのキャッシュウィンドウ指針に
よる 20〜30 分程度）では、20 周回はおおよそ **7〜10 時間**に相当します。`ScheduleWakeup` の上限（1 回あたり
3600 秒）を毎回使い続けた場合でも最大 20 時間です。上限に達したらループを続けるのではなく、現在の状態
（CI 状態、未解決コメント数）を報告して終了します。セッションを止めれば人間はいつでもループを中断・再起動
できます。`ScheduleWakeup` の制約上、反復カウントが必ず先に発火するため、24時間の経過時間を別途歯止めとして
設ける必要はありません。セッションローカルの `/loop` をスケジュール型クラウドエージェントより選んだのは、設定が
簡単で、いつでも割り込みが可能なためです。
...
- [ ] Unit 3 — 3つの停止条件（CI 緑・CHANGES_REQUESTED なし・静かな周回2回連続）とエスカレーション（設計変更・コンフリクト）、歯止め（レビュー待ち20周回≈推奨ケイデンスで7〜10時間）を備えた一定間隔の pr-followup ループ。
