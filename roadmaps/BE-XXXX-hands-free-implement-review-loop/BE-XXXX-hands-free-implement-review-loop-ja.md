...
**実装上の注記：** スキルの実行途中にどのようにしてハーネスへ compact を指示するか（たとえばスキルテキストを
`/compact` ディレクティブで終端しハーネスに検知させる形かどうか、あるいは別の組み込みポイントが必要かどうか）は、
Unit 2 を実装するときに確立すべき未解決の問いです。この提案は「手渡しの時点で一度 compact する」という
**意図**を記録するもので、`implement-be` の実装者が具体的な呼び出し方法を決める余地を残します。

### Unit 3 — 一定間隔の pr-followup ループと、その停止条件

compact のあと、`implement-be` はセッションに組み込みの **`/loop` スキル**
...
ループが**停止する**のは、次の**両方**が成り立つときだけです。

1. **CI が緑**。必須チェックがすべて通っています。
2. **レビューの新規コメントがない周回が2回連続**。レビュー面が静かになった状態です（1回の空振りでは足りず、
   2回目で静止を確かめます）。
...
セッションローカルの `/loop` をスケジュール型クラウドエージェントより選んだのは、設定が簡単で割り込みが
しやすいからです。セッションを止めれば人間はいつでもループを中断・再起動できます。24時間の上限は安全の
歯止めであり、実際の PR レビューループは数時間で完結するのが通常の想定です。
...
- [ ] Unit 3 — 2つの停止条件（CI 緑・静かな周回2回連続）とエスカレーション（設計変更・コンフリクト）、歯止めを備えた一定間隔の pr-followup ループ。
