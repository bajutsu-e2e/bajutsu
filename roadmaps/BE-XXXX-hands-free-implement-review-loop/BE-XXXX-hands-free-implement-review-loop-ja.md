...
compact のあと、スキルは**一定間隔のループ**に入ります。各周回で `pr-followup` を呼び、周回のあいだは
待機します（ハーネスの self-pacing の wake-up や `loop` の仕組みを用います）。間隔は標準のキャッシュウィンドウの
指針に従い、CI が動いているあいだ（実行の完了を待つあいだ）は短く、人間のレビューを待つあいだは長くします。
