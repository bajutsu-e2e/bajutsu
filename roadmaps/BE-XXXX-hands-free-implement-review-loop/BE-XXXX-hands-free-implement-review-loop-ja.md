...
現在の状態（CI 状態、未解決コメント数）を報告して終了します。その上に**20周回**を二次安全網として重ねます。
CI 待ちの短いポーリング間隔では1回の CI ラン中に何周も消費され、PR が push→CI を複数回繰り返すこともあるため、
20周回を「収束しなかった」の主要基準にするとレビューの静止条件に達する前に歯止めが掛かりかねません。
24時間上限は「人間が一度確認すべきタイミング」を示す自然なシグナルで、20周回はあくまで暴走ループへの
フェイルセーフです。セッションローカルの `/loop` をスケジュール型クラウドエージェントより選んだのは、設定が
簡単で割り込みがしやすいからです。セッションを止めれば人間はいつでもループを中断・再起動できます。
