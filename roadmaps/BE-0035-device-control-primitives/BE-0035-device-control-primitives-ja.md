...
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0035") |
...
- **前面と背面の遷移。** 多くの挙動は背面化のときにのみ発火します（状態復元、再認証プロンプト、
...
- **`overrideStatusBar`** は、ステップが指定したフィールド（`time`、`batteryLevel`、
  `batteryState`、`cellularBars`、`wifiBars`）だけを上書きし、`simctl status_bar override` に
...
- **アラートガードの vision パス（スクリーンショット + タップ）でシステムを操作する。** このパスは
  ...
  run ゲートに入れるわけにはいきません。却下：run パス上に LLM を置くことになります。
