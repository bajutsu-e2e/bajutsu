...
1. **`CLAUDE.md` と `docs/ai-development.md` への規約追加**（両言語）。`docs/ai-development.md` の
   ...
   （`glossary.md#アンカー` または `docs/ja/glossary.md#アンカー`）へのリンクにします」。あわせて、これが
   CI のゲートではなくレビュー時の規約であることを明記します。隣接する2つの規約がそうである理由と同じで、
   「step」「target」のような普通の英単語の使用が Bajutsu 固有の意味を指しているかどうかの判断には
...
一方で、意図的にスコープ外とするのは、既存のロードマップ項目277件です。ほぼ全項目を英語・日本語の
