[English](BE-0206-serve-state-module-split.md) · **日本語**

# BE-0206 — serve のジョブ状態をジョブ実行から分離する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0206](BE-0206-serve-state-module-split-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0206") |
| 実装 PR | [#825](https://github.com/bajutsu-e2e/bajutsu/pull/825) |
| トピック | コードベース品質・技術的負債 |
<!-- /BE-METADATA -->
...
`serve/helpers.py`（675 行）にも、規模は小さいものの同じ形の問題があります。コマンドビルダー群
...
- [x] `serve/state.py` を新設し状態コンテナを移動（純粋な移動）
- [x] `serve/jobs.py` を実行エンジンに縮小
- [x] 利用側（本体・テスト）を移行し、再エクスポートのシムは残さない
- [x] 分離で不要になった lazy import 回避策を解消
- [x] `serve/commands.py` を `serve/helpers.py` から切り出し、利用側を更新
- [x] モジュール一覧の記述を更新（`docs/architecture.md`（en/ja）は対象の serve サブモジュールを
  名指ししていないため、更新対象は `serve/__init__.py` の docstring のモジュール一覧でした）

ログ:

- [#825](https://github.com/bajutsu-e2e/bajutsu/pull/825) — serve の状態コンテナを `serve/state.py` に、CLI コマンドビルダー群を
  `serve/commands.py` にそれぞれ純粋な移動として切り出しました。`serve/jobs.py` は
  run/cancel/build の実行エンジンを保持し、`state` を一方向に import します。`executor.py` の型
  import は `state` へ付け替えました（`run_job` の遅延 import と `ServeState._env_var_for_secret`
  の `operations.config` 遅延 import は、いずれも解消すると循環するため維持しています）。本体の
  約 28 の利用側と serve のテストを移行し、シムは残していません。`make check` は緑です
  （3214 件成功）。
