...
GitHub user が通ってしまいます。そのため拒否は、同じ先頭の位置のまま行います。`org_for_identity` を
計算し、許可か拒否かを決めるのは、データベースのブロックより前です。ブロックの中にある
`_org_for_login` の呼び出しは、どの org に永続化するかだけを決めます。拒否するかどうかは決めません。
...
- [ ] Organization メンバーシップによるサインインの許可判定を `org_for_identity` と
      `_org_for_login` に折り込む。ただし拒否は `oauth_callback` の先頭（`if state.repository is not
      None:` ブロックより前）で行い、OAuth を設定していてもデータベースを配線していないデプロイで
