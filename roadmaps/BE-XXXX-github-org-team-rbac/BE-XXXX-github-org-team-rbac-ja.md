...
成功する必要がないからです。`_fetch_orgs` は、失敗する場合（非200応答やネットワーク障害、本文の
パース失敗など）でも、すでに空の org 一覧へ fail open します。今日この失敗が送るのは `default` org
だけです。この項目のもとでは、同じ失敗によって、障害が続くあいだ、`githubOrgs` だけに頼る login の
サインインそのものが失敗し続けます。この項目は、この新しい可用性のトレードオフを受け入れます。org
メンバーシップ取得にリトライやキャッシュの仕組みを足すことはしません。`members` に明示登録された
login は、GitHub の `API` を一切呼ばないため、この影響を受けません。`githubOrgs` だけに頼る login は、
...
- [ ] `GET /user/teams` の照会（直接のメンバーシップのみ）と、環境変数 `BAJUTSU_OAUTH_ADMIN_TEAM` を
      追加する。
- [ ] Organization メンバーシップによるサインインの許可判定を `org_for_identity` と
      `_org_for_login` に折り込み、`BAJUTSU_OAUTH_ALLOWED_USERS` と `BAJUTSU_OAUTH_VIEWERS` を
      廃止する。
