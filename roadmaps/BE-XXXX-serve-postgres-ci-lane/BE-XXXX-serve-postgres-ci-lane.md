...
cleanly against SQLite can still fail against Postgres — `test_import_guard.py`'s check that the
`psycopg` import string isn't pulled in eagerly is the closest the suite comes to acknowledging
Postgres exists, and it tests an import statement, not a schema. Because this gap sits entirely
...
- **Run the existing migration test suite against it too.** Parametrize (or duplicate)
  ...
  `test_oauth.py`'s persistence tests) against Postgres, since dialect-specific column/constraint
...
- [ ] Run the migration upgrade/downgrade tests against it.
- [ ] Run the wider DB-touching test suite against it.
