...
| 実装 PR | [#1089](https://github.com/bajutsu-e2e/bajutsu/pull/1089) |
...
- [#1089](https://github.com/bajutsu-e2e/bajutsu/pull/1089) — `bajutsu/serve/handler.py`: `do_POST` / `do_GET` は `_dispatch_post` /
...
- [#1089](https://github.com/bajutsu-e2e/bajutsu/pull/1089)（レビューのフォローアップ）— 生ボディの
  アップロード／アーティファクト経路（`_handle_upload` / `_handle_artifact_upload`）にも境界を広げました。
  これらは do_POST の `try/except` より前で振り分けられるため、`ops.bind_upload_config` /
  `ops.bind_artifact` からの送出も `_respond_uncaught` を通すようにし、これらの経路も本項目が解消しようと
  する本文なしドロップに陥らないようにしました。
