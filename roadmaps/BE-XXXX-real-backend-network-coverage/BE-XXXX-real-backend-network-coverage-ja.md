...
実行時は 2 つのバックエンドに配線されています。iOS はプロセス外でキャプチャし（アプリが各 exchange を
ループバック上の `NetworkCollector` へ POST する）、web はプロセス内でキャプチャします（Playwright ドライバが
`requestfinished` をフックし、`page.route` でモックを応じる）。この実行時を実デバイスや実ブラウザに対して
...
プロセス内の自己ループとして動かされています。どのテストも観測していないのが実際の境界です。アプリ側の
...
これを動かすシナリオは iOS についてはすでに存在し、孤立しています。web 側は、既存のジョブが `--no-network`
...
は false で、adb ドライバは `NETWORK` capability を宣言しません）、まだそこにアクチュエートする対象がないため
