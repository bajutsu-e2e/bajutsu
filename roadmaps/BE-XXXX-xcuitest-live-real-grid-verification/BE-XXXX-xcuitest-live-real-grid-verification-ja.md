[English](BE-XXXX-xcuitest-live-real-grid-verification.md) · **日本語**

# BE-XXXX — Real-grid verification of the XCUITest live device-cloud route

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-xcuitest-live-real-grid-verification-ja.md) |
...
の device provider の seam のために、W3C WebDriver プロトコル越しに Appium に対する device-cloud
経路を駆動します。このテストモジュール自身の docstring は、両方とも「ネットワーク境界で fake化
されているため、ゲートに grid は不要」と述べており、すべてのテストは合成された transport を
...
合成された transport は、`XcuitestLiveDriver` がその自身のテストから渡された WebDriver の
JSON をまさしく組み立て、パースできることを証明します。これはドライバの内部ロジックに対する
...
（[BE-0236](../BE-0236-device-cloud-provider-abstraction/BE-0236-device-cloud-provider-abstraction-ja.md)、
[BE-0237](../BE-0237-firebase-device-streaming-adapter/BE-0237-firebase-device-streaming-adapter-ja.md)）。
それが話すために存在するまさにそのプロトコルをモックしても、本当に重要な性質までは検証できません。
...
- **CI 向けの実際の Appium ターゲット**：実際の Appium サーバを立てる(または既存のものに接続
  する)。Simulator/エミュレータに対するローカルインスタンスで WebDriver のワイヤ契約を検証する
  には十分であり、実際のクラウド grid はローカル経路が実証されたあとの、さらなる任意のステップ
  です。
- **API キーまたは環境で gate したライブセッションテスト**：そのAppium ターゲットに対して、
...
- **WebDriver の JSON 構築がユニットテストされていることを根拠に、合成 transport のテストを
  ...
  語りません。それこそがこの backend が本来主張していることです。
- **具体的な device-cloud provider がこの経路を採用するまで、実検証を先送りする**：この経路は
  device provider の seam を通じて今日すでに到達可能です。特定の provider 統合が着地するまで
...
- [ ] それに対する gated なライブセッションテスト(作成 → 操作 → 破棄)を追加する。
...
- [BE-0236 — device-cloud provider の抽象化](../BE-0236-device-cloud-provider-abstraction/BE-0236-device-cloud-provider-abstraction-ja.md)
- [BE-0237 — Firebase Test Lab / Device Streaming アダプタ](../BE-0237-firebase-device-streaming-adapter/BE-0237-firebase-device-streaming-adapter-ja.md)
