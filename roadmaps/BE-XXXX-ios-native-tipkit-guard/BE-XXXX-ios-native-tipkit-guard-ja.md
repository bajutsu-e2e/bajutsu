...
1. **実現可能性の検証 — 確認済み。** 実際の Simulator を使ったランで、表示中の TipKit tip のアクセ
   ...
   め、これに一致させると `AmbiguousSelector` を招くリスクがあります。そのため本設計では
   `PopoverDismissRegion` だけを検知シグナルにも閉じる対象にも使います。ただしこの検証で決着**しな
   かった**点が 1 つあります。この名前が TipKit 専用かどうかです。名前も画面全体のフレームも
   `UIPopoverPresentationController` を指しているため、アプリ自身が本当に popover として提示している
   ビュー(コンパクト環境での sheet 化を明示的に上書きした SwiftUI の `.popover`)も、同じ scrim を持つ
