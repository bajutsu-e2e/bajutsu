...
**決定と実測。** `BajutsuKit/Package.swift`は`swift-tools-version:6.0`へ移し、パッケージの宣言も
`platforms: [.iOS(.v18), .macOS(.v14)]`とします。そのうえで、すべてのターゲットに
`swiftLanguageMode(.v5)`を固定します。この3つは連動しており、どれも単独で決まったものでは
ありません。単位2はHummingbirdを採用しますが、そのOpenAPIアダプターはiOS 17以上を要求します。
iOS 18はtools 6.0未満では表現できません。`.v18`という指定がそもそも存在しないからです。そして
tools 6.0は、すべてのターゲットをSwift 6の言語モードにデフォルトで置きます。この厳格な並行性
チェックを、ランナーの既存のメインスレッドへの受け渡しは通過できません（`Router.onMain`に対する
`sending 'result' risks causing data races`）。Swift 5モードを固定することで、並行性の移行は
プラットフォーム下限の引き上げに伴う無言の副作用ではなく、意図して選ぶ将来の課題のままになります。
ランナー側のプロジェクトの`deploymentTarget`も、これに合わせてiOS 18へ上げます。

本項目の以前の版は、この前半と反対のこと、すなわちtools 5.9のまま下限も変えないと記録していました。
それは、その版が前提としていたトランスポートにとっては正しい記述でした。`5.9`のマニフェストのまま
でも、`6.1`を宣言するOpenAPI関連パッケージを解決し、プラグインを実行してビルドが通ります。これは
推測ではなく、実際にビルドして確かめた事実です。つまりOpenAPIのツールだけでは、マニフェストの
引き上げも言語モードの決定も強制されません。両方を強制するのはHummingbirdの採用です。この段落が
その選択と一緒に動いたのは、そのためです。
...
**決定：候補H、プラットフォーム下限をiOS 18へ引き上げる。** メンテナはiOS 18という下限を受け
入れました。これにより、候補Hが落ちていた基準は消えます。この下限で両方の候補を測り直しても、
それ以外は何も変わりませんでした。候補Hは62,272 KBのまま、候補Fは15,648 KBのままです。候補Hの
かさばりはSwiftNIOの静的リンクによるものであり、デプロイメントターゲットが左右する性質のもので
はないからです。したがってこの選択は、およそ46 MBのランナーバンドルと、iOS 15から17までの
Simulatorサポートを差し出します。引き換えに得るのは、自前で保守するアダプターの代わりの、
上流で保守されるアダプターです。これはメンテナが下す判断であり、逆方向を示す測定と並べて、下さ
れた決定としてここに記録します。

下限を引き上げたことで、バージョン番号だけからは見えない2つの変更が付随して生じました。記録し
ておきます。`.v18`は`swift-tools-version:6.0`を下回るバージョンでは表現できません。したがって
マニフェストのツールバージョンも、下限の引き上げに合わせて上がります。また、ツールバージョン
6.0はすべてのターゲットをSwift 6の言語モードへデフォルトで移します。その厳格な並行性チェックは、
ランナーが今日持つメインスレッドへの受け渡しを通しません（`sending 'result' risks causing data
races`）。そのためすべてのターゲットで`swiftLanguageMode(.v5)`を固定しています。並行性の移行を
プラットフォームの引き上げに付随する副作用としてではなく、意図して先送りする将来の作業のまま
にしておくためです。
...
- [x] 単位1 — 既存15個のエンドポイントぶんの`openapi.yaml`を書く。Swift OpenAPI Generatorのビ
      ...
      する。tools 6.0と`platforms: [.iOS(.v18), .macOS(.v14)]`とし、すべてのターゲットに
      `swiftLanguageMode(.v5)`を固定する。HummingbirdのiOS 17という下限が、Swift 6の厳格な並行性
      チェックへの移行まで引きずってこないようにするため。プラグインは出荷ライブラリに置くので、
      2つのランナービルドはどちらも`-skipPackagePluginValidation`を払う。その確認の抑止を許容
      できるのは、`exact:`によるバージョン固定と、コミットした`Package.resolved`があるからである。
...
- [x] 単位3 — 勝者となった候補に対して`APIHandler`を実装し、`actuationLock`と同時実行数の上限という
      不変条件を保つ。どちらの不変条件も、1つの直列`operations`キューとして生き残る。このキュー
      は、ロックがそうしていたのと同じようにXCUITestの操作を直列化する。さらに、Hummingbirdの
      イベントループからブロッキングなメインスレッドへの受け渡しを引き取る。そのため、このキュー
      に触れない唯一の操作である`/health`は、長いジェスチャーの最中でも応答し続ける。パリティを検証する
      テストは、生成された各エンドポイントの応答を、同じ入力に対する旧来の`Router`の応答と突き合
      わせる。mutation testingは、ずれたステータス文字列や欠落したオプションのフィールドで、これ
      らのテストが実際に失敗することを確かめる。
