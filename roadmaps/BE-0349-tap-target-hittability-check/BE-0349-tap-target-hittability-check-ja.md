...
配線そのものは、`gestures.py` にある小さなラッパー `_tap_with_recovery(actuate, driver, sel)` です。これは指定された `driver.tap(sel)` / `driver.double_tap(sel)` / `driver.long_press(sel, duration)` を呼びます。`base.ElementNotTappable` を受けると、`_TAP_RECOVERY_DIRECTIONS = ("down", "up")` を `enumerate` した各 `i, direction` について `scroll_until_tappable(driver, sel, direction, None, _TAP_RECOVERY_MAX_SCROLLS * (i + 1))` を呼び、`sel` がタップ可能になった最初の方向で止めたうえで、操作をちょうど一度だけ再試行します。方向を1つに固定すると、よくある2種類の遮蔽の両方には対応できません。`down` は画面上のコンテンツを上方向へ動かすので、下端に張り付いた遮蔽(トースト、スナックバー、固定フッター)は取り除けますが、上端に張り付いた遮蔽(スティッキーヘッダー)の下に入り込んだ対象はさらにその下へ押し込んでしまい、抜け出せません。下端の遮蔽のほうがよくあるケースなので、まず `down` を試し、その回数上限を使い切ってもタップ可能にならなかったときにかぎり `up` を試します。`up` は `down` の各ステップが残した位置から始まるため、自分の分の前進をする前にその変位をまず巻き戻す必要があります。方向ごとの上限を一律にせず `_TAP_RECOVERY_MAX_SCROLLS * (i + 1)` と広げているのはこのためで、`down` は 3 回、`up` は 6 回、最悪でも合計 9 回です。この経路上のどの失敗も、単一の `base.ElementNotTappable` へ収束します。最初の試行自身が投げた例外(`base.raise_if_covered` により、対象を覆っていたものを名指しします)は、捨てられるのではなく最終メッセージへ埋め込まれます。回復のきっかけとなった、最後に試した方向のスクロールの失敗は、`raise ... from exc` で連鎖させ、その隣に添えます。どちらの事実も失われません。`_do_tap`、`_do_double_tap`、`_do_long_press`、そして `_do_type` / `_do_clear` / `_do_delete` / `_do_select` の内部にあるフォーカスタップの呼び出し箇所は、いずれも生の `driver.tap(sel)` 呼び出しをこの 1 つの共有ラッパーへ切り替えます。同じ try/except を 7 か所へ複製するのではありません。方向ごとの上限は小さく保ち、両方向を合わせた 9 回でも `scroll` 自身のデフォルトである 15 を下回ります。これは探索ではなく、一時的なオーバーレイや、スティッキーなヘッダーやフッターといった、よくあるケースのための安全網です。対象を特定の方向、特定のコンテナ越しにスクロールする必要があるとすでに知っている作者は、それでも明示的な `scroll` アクションを書きます。この安全網が保険をかけるのは、作者が想定していなかったケースだけです。
...
- レビューで、`up` フォールバックの回数上限を `down` と同じ値にしていたため、実際には上端に張り付いた遮蔽を取り除けないという指摘を受けました。`up` は `down` が残した分だけ位置がずれた地点から始まるため、自分の分を前進させる前に、まず `down` がすでに却下した範囲を巻き戻す必要があるからです。各方向の回数上限を `_TAP_RECOVERY_MAX_SCROLLS * (i + 1)`(`down` は 3 回、`up` は 6 回)へ広げて修正しました。これにより `up` は `down` による位置のずれをまず巻き戻したうえで、遮蔽を取り除くだけの余裕を持てます。回帰用のフィクスチャ(`_ClearsTopOverlayOnlyViaUpwardRecoveryDriver`)も、両方向で同じ移動量になるよう修正しました。実機のバックエンドはどちらの方向でも同じ `_SWIPE_FRACTION` を使うためです。修正前の非対称な移動量は、実機のバックエンドでは起こり得ない回復を表現しており、回数上限を広げる必要性を覆い隠していました。
- レビューで、web の `_point_hits` フォールバック(解決した対象に `data-testid` がないため、その属性ではなく境界矩形でヒットチェーンを辿る経路)が、矩形の一致だけで最初の祖先要素を対象そのものとみなしてしまうという指摘を受けました。対象そのものかどうかの判定がなく、ボタンをちょうど覆う透明なクリックブロック用オーバーレイは矩形こそ一致するものの名前は一致しないため、フォールバックはそれをそのまま通過させてしまっていました。ヒットした要素自身のアクセシブルな名前(`aria-label`、またはトリミングした `textContent`。`bajutsu/dom.py` の `QUERY_JS` と同じ 200 文字で切り詰めます)も対象の `label` と一致することを要求するよう修正し、対象にアクセシブルな名前が一切ない場合にかぎり矩形だけの一致にフォールバックします(他のバックエンドで `topmost_at_point` がすでに許容している、同一フレームの組に対する残存的な曖昧さと同じです)。実機の Chromium で検証しました(`tests/test_driver_conformance_web.py` の `test_is_tappable_false_for_an_identically_sized_cover_with_no_data_testid`、`test_is_tappable_true_for_a_same_named_element_matched_without_a_data_testid`)。加えて、生成される JavaScript にこのガードが含まれることを固定する配線レベルの単体テストも追加しました。
