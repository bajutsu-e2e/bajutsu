...
- [ ] On-device validation — a pinch/rotate scenario idb cannot run, through the full `run` path on the `e2e.yml` path, before the item flips to Implemented. The `xcuitest (multi-touch)` job runs `scenarios/gestures_multitouch.yaml` (`--backend xcuitest`) against a flat, scroll-free `GestureView` (swapped in by the `SHOWCASE_GESTURES` launch env, mirroring BE-0114's `SHOWCASE_CONFORMANCE`); the box is ticked once that job is green. Scoped single-device: the parallel device pool is covered by BE-0020's own tests and the on-device conformance job, and the idb graceful-degradation fallback by the registry's fast-gate unit tests (select_actuator with an injected availability fn) plus the idb `smoke` job — a macOS runner always has xcodebuild, so xcuitest never actually degrades to idb on-device to observe.
