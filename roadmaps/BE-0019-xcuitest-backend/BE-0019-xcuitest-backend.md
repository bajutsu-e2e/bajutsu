...
| Status | **Implemented** |
| Tracking issue | [Search](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0019") |
| Implementing PR | [#366](https://github.com/bajutsu-e2e/bajutsu/pull/366), [#368](https://github.com/bajutsu-e2e/bajutsu/pull/368), [#374](https://github.com/bajutsu-e2e/bajutsu/pull/374), [#376](https://github.com/bajutsu-e2e/bajutsu/pull/376), [#391](https://github.com/bajutsu-e2e/bajutsu/pull/391), [#394](https://github.com/bajutsu-e2e/bajutsu/pull/394), [#397](https://github.com/bajutsu-e2e/bajutsu/pull/397), [#433](https://github.com/bajutsu-e2e/bajutsu/pull/433), [#672](https://github.com/bajutsu-e2e/bajutsu/pull/672) |
| Topic | Backend expansion (iOS actuators) |
| Related | [BE-0105](../BE-0105-xcuitest-single-snapshot-query/BE-0105-xcuitest-single-snapshot-query.md) |
<!-- /BE-METADATA -->
...
- [x] Runner test target — the concrete XCUITest `ElementProviding` that walks `XCUIApplication` into the normalized `Element` shape (identifier / label / value / traits / frame) and actuates by snapshot handle (tap / gesture / swipe / type / screenshot); a thin UI-testing target (`BajutsuKit/Runner/`) that imports XCTest + `BajutsuRunner`, calls `RunnerServer.startFromEnvironment()`, and stays resident for the run.
- [x] App-agnostic launch — the target bundle id is forwarded to the runner (`BAJUTSU_BUNDLE_ID` alongside the existing `BAJUTSU_LAUNCH_*`): `XcuitestEnvironment` sets it, `RunnerServer.forwardedBundleId` exposes it, the target launches `XCUIApplication(bundleIdentifier:)`. A minimal host app in the project satisfies XCUITest's build-time target-app requirement.
- [x] Runner packaging and example config — `make -C demos/showcase runner-build` builds the generic runner into a `*.xctestrun`; the two `-noax` showcase targets set `xcuitest.testRunner` (with `build` as the on-demand fallback).
- [x] On-device validation — a pinch/rotate scenario idb cannot run, through the full `run` path on the `e2e.yml` path. The `xcuitest (multi-touch)` job runs `scenarios/gestures_multitouch.yaml` (`--backend xcuitest`) against a flat, scroll-free `GestureView` (swapped in by the `SHOWCASE_GESTURES` launch env, mirroring BE-0114's `SHOWCASE_CONFORMANCE`) and is green ([#672](https://github.com/bajutsu-e2e/bajutsu/pull/672)). Scoped single-device: the parallel device pool is covered by BE-0020's own tests and the on-device conformance job, and the idb graceful-degradation fallback by the registry's fast-gate unit tests (select_actuator with an injected availability fn) plus the idb `smoke` job — a macOS runner always has xcodebuild, so xcuitest never actually degrades to idb on-device to observe.
- [x] Query performance — replaced the per-element `/elements` reads (each property its own XCUITest round-trip, ~12s on an 84-element screen, which had forced the driver socket timeout up to 30s) with a single `XCUIApplication` snapshot walk ([BE-0105](../BE-0105-xcuitest-single-snapshot-query/BE-0105-xcuitest-single-snapshot-query.md), [#670](https://github.com/bajutsu-e2e/bajutsu/pull/670)).

Built in slices: #366 / #368 (driver + channel), #374 / #376 (runner server + endpoints), #391 / #394 (run-lifecycle wiring, preconditions), #397 (doctor fallback), #670 (single-snapshot query, BE-0105).

Investigation (2026-07-02): confirmed on iPhone 17 Pro / iOS 26.1 (fb-idb 1.1.7) that `idb ui describe-all` surfaces a UIKit `UITabBarController` and a SwiftUI `TabView` only as one opaque "Tab Bar" group with no per-tab children, whereas XCUITest enumerates each tab as a label-addressable button and `tabBars.buttons["…"].tap()` switches the tab — for both `-noax` showcase apps (no accessibility identifiers). This is the concrete case the runner test target unlocks: tabs become selector-addressable where idb cannot see them, so crawl over xcuitest needs no vision tab locator.

Runner target landed (2026-07-02): implemented the concrete provider + UI-testing target and validated end to end on device — `bajutsu run --backend ios` on `showcase-uikit-noax` resolved `tap: {label: "Search", traits: [button]}` uniquely, switched the tab, and the `exists` assertion passed, with the manifest recording `backend: "xcuitest"`. Also fixed a lifecycle gap surfaced only on device: `xcodebuild` does not forward its process environment into the Simulator test runner, so `XcuitestEnvironment` now patches the run's `BAJUTSU_*` vars into a private copy of the `.xctestrun`'s per-target `TestingEnvironmentVariables` (kept beside the built products so `__TESTROOT__` still resolves).

Query performance resolved (2026-07-05): [BE-0105](../BE-0105-xcuitest-single-snapshot-query/BE-0105-xcuitest-single-snapshot-query.md) ([#670](https://github.com/bajutsu-e2e/bajutsu/pull/670)) replaced the per-property `/elements` reads with a single `app.snapshot()` walk, removing the last blocker in that box. It keeps BE-0019's snapshot-handle addressing intact (Python resolves the unique element; the runner acts on exactly that handle; ambiguous fails immediately; stale fails loudly).

On-device gesture validation wired (2026-07-05): added a flat, scroll-free `GestureView` with `log.pinch` / `log.rotate` two-finger targets, swapped in by the `SHOWCASE_GESTURES` launch env (mirroring BE-0114's `SHOWCASE_CONFORMANCE`, a SwiftUI-only test affordance), the `scenarios/gestures_multitouch.yaml` scenario that pinches/rotates them and asserts each mirrored a11y value flips, and the `xcuitest (multi-touch)` `e2e.yml` job that runs it through the full `run` path on `--backend xcuitest`. idb cannot run this scenario (single-touch → `UnsupportedAction`), so it stays off the idb smoke path. A dedicated screen (rather than the deep-in-a-Form Log-tab targets first tried) because XCUITest's coordinate swipe did not scroll the SwiftUI Form to reach off-screen rows — a separate swipe-to-scroll limitation worth its own follow-up, orthogonal to this actuation proof. The item flips to Implemented once the job is green.
