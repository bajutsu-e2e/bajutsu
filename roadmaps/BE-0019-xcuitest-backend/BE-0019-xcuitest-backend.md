...
On-device gesture validation wired (2026-07-05): added the Log tab's `log.pinch` / `log.rotate` two-finger targets (SwiftUI + the UIKit twin, sharing the identifier contract), the `scenarios/gestures_multitouch.yaml` scenario that pinches/rotates them and asserts each mirrored a11y value flips, and the `xcuitest (multi-touch)` `e2e.yml` job that runs it through the full `run` path on `--backend xcuitest`. idb cannot run this scenario (single-touch → `UnsupportedAction`), so it stays off the idb smoke path. The item flips to Implemented once that job is green.
