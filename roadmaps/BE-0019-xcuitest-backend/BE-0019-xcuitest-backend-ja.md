...
- [ ] 実機検証。idb にはできない pinch ／ rotate のシナリオを、`e2e.yml` 経路でフルの `run` 経路を通して確認します。項目を実装済みへ移す前に行います。`xcuitest (multi-touch)` ジョブが、Log タブに新設した 2 本指ターゲットに対して `scenarios/gestures_multitouch.yaml` を `--backend xcuitest` で走らせます。このジョブが緑になった時点でチェックを付けます。範囲は単一デバイスに絞りました。並列デバイスプールは BE-0020 自身のテストと実機の conformance ジョブが担保し、idb へのなだらかな劣化のフォールバックはレジストリの高速ゲートのユニットテスト（可用性判定を差し込んだ `select_actuator`）と idb の `smoke` ジョブが担保します。macOS ランナーには必ず xcodebuild があるため、実機で xcuitest が idb へ実際に劣化する様子は観測できません。
