...
| 実装 PR | [#366](https://github.com/bajutsu-e2e/bajutsu/pull/366), [#368](https://github.com/bajutsu-e2e/bajutsu/pull/368), [#374](https://github.com/bajutsu-e2e/bajutsu/pull/374), [#376](https://github.com/bajutsu-e2e/bajutsu/pull/376), [#391](https://github.com/bajutsu-e2e/bajutsu/pull/391), [#394](https://github.com/bajutsu-e2e/bajutsu/pull/394), [#397](https://github.com/bajutsu-e2e/bajutsu/pull/397), [#433](https://github.com/bajutsu-e2e/bajutsu/pull/433) |
...
- [x] バックエンドの登録。`PLATFORMS["ios"] = ("xcuitest", "idb")` とし、`xcuitest` を `IMPLEMENTED` に入れ、idb をフォールバックにします（`bajutsu/backends.py`）。
- [x] ランナー側サーバー。`BajutsuKit` の汎用 XCTest ターゲット（`Sources/BajutsuRunner/RunnerServer.swift`）が、ループバックの `/elements` / `/tap` / `/gesture` / `/swipe` / `/type` エンドポイントを公開します（[#374](https://github.com/bajutsu-e2e/bajutsu/pull/374)、[#376](https://github.com/bajutsu-e2e/bajutsu/pull/376)）。
- [x] Python とランナーのチャネル、および `XcuitestDriver`。`bajutsu/drivers/xcuitest.py` が、スナップショットごとに安定したハンドルで要素を操作します（[#366](https://github.com/bajutsu-e2e/bajutsu/pull/366)、[#368](https://github.com/bajutsu-e2e/bajutsu/pull/368)）。
- [x] 実行ライフサイクルの配線と事前条件。リースごとのランナーの起動・終了と、事前条件の転送（[#391](https://github.com/bajutsu-e2e/bajutsu/pull/391)、[#394](https://github.com/bajutsu-e2e/bajutsu/pull/394)）。
- [x] ケイパビリティ、doctor、開示。`doctor --target` が idb の検査を併合し、xcuitest 選択時に idb へフォールバックします（[#397](https://github.com/bajutsu-e2e/bajutsu/pull/397)）。
- [x] ランナーのテストターゲット。`XCUIApplication` を走査して正規化した `Element`（identifier、label、value、traits、frame）へ変換し、スナップショットのハンドルで操作する（tap、gesture、swipe、type、screenshot）、具象の XCUITest 版 `ElementProviding` です。XCTest と `BajutsuRunner` を取り込み、`RunnerServer.startFromEnvironment()` を呼んで run のあいだ常駐する薄い UI テストターゲット（`BajutsuKit/Runner/`）として実装しました。
- [x] アプリ非依存の起動。テスト対象の bundle id をランナーへ転送します（既存の `BAJUTSU_LAUNCH_*` に並べる `BAJUTSU_BUNDLE_ID`）。`XcuitestEnvironment` が値を設定し、`RunnerServer.forwardedBundleId` がそれを公開し、ターゲットは `XCUIApplication(bundleIdentifier:)` で起動します。XCUITest がビルド時に要求するターゲットアプリは、プロジェクト内の最小のホストアプリで満たします。
- [x] ランナーのパッケージングと設定例。`make -C demos/showcase runner-build` が汎用ランナーを `*.xctestrun` にビルドし、2 つの `-noax` showcase target が `xcuitest.testRunner`（オンデマンドのフォールバックとして `build`）でそれを指します。
- [ ] 実機検証。idb にはできない pinch ／ rotate のシナリオを、`e2e.yml` 経路でフルの `run` 経路を通して確認します。項目を実装済みへ移す前に行います。`xcuitest (multi-touch)` ジョブが、`SHOWCASE_GESTURES` 起動 env で差し替わる平坦でスクロール不要な `GestureView`（BE-0114 の `SHOWCASE_CONFORMANCE` に倣った作り）に対して、`scenarios/gestures_multitouch.yaml` を `--backend xcuitest` で走らせます。このジョブが緑になった時点でチェックを付けます。範囲は単一デバイスに絞りました。並列デバイスプールは BE-0020 自身のテストと実機の conformance ジョブが担保し、idb へのなだらかな劣化のフォールバックはレジストリの高速ゲートのユニットテスト（可用性判定を差し込んだ `select_actuator`）と idb の `smoke` ジョブが担保します。macOS ランナーには必ず xcodebuild があるため、実機で xcuitest が idb へ実際に劣化する様子は観測できません。
- [x] クエリ性能。個別プロパティごとの `/elements` 読み取り（各プロパティが個別の XCUITest 往復で、84 要素の画面で約12秒。このためドライバのソケットタイムアウトを 30 秒へ引き上げていました）を、`XCUIApplication` の単一スナップショット走査へ置き換えました（[BE-0105](../BE-0105-xcuitest-single-snapshot-query/BE-0105-xcuitest-single-snapshot-query-ja.md)、[#670](https://github.com/bajutsu-e2e/bajutsu/pull/670)）。

スライスで構築しました。#366 / #368（ドライバーとチャネル）、#374 / #376（ランナーサーバーとエンドポイント）、#391 / #394（実行ライフサイクルの配線、事前条件）、#397（doctor のフォールバック）、#670（単一スナップショットのクエリ、BE-0105）。

調査（2026-07-02）。iPhone 17 Pro、iOS 26.1（fb-idb 1.1.7）で確認したところ、`idb ui describe-all` は UIKit の `UITabBarController` も SwiftUI の `TabView` も、子を持たない一つの不透明な「Tab Bar」グループとしてしか見せませんでした。一方 XCUITest は各タブを label で解決できるボタンとして列挙し、`tabBars.buttons["…"].tap()` でタブが切り替わりました。どちらも `-noax` の showcase アプリ（アクセシビリティ識別子なし）での結果です。これがランナーのテストターゲットで解けるようになる具体例です。idb からは見えないタブがセレクタで解決できるようになるので、xcuitest 経由の crawl は vision のタブロケータを必要としません。

ランナーのテストターゲットの着地（2026-07-02）。具象の provider と UI テストターゲットを実装し、実機で end-to-end に検証しました。`showcase-uikit-noax` に対する `bajutsu run --backend ios` が `tap: {label: "Search", traits: [button]}` を一意に解決してタブを切り替え、`exists` アサーションが通り、マニフェストは `backend: "xcuitest"` を記録しました。あわせて、実機でのみ現れたライフサイクルの穴を修正しました。`xcodebuild` は自身のプロセス環境を Simulator のテストランナーへ渡さないため、`XcuitestEnvironment` は run の `BAJUTSU_*` を `.xctestrun` の各ターゲットの `TestingEnvironmentVariables` に書き込んだ私的なコピーを作って起動します（`__TESTROOT__` が解決できるよう、ビルド成果物の隣に置きます）。

クエリ性能の解決（2026-07-05）。[BE-0105](../BE-0105-xcuitest-single-snapshot-query/BE-0105-xcuitest-single-snapshot-query-ja.md)（[#670](https://github.com/bajutsu-e2e/bajutsu/pull/670)）が、プロパティごとの `/elements` 読み取りを 1 回の `app.snapshot()` 走査へ置き換え、このボックスに残っていた最後の障害を取り除きました。BE-0019 のスナップショットハンドルによるアドレッシング（Python が一意の要素を解決し、ランナーは正にそのハンドルの要素を操作する、曖昧は即失敗、stale は明示失敗）はそのまま保たれています。

実機のジェスチャ検証の配線（2026-07-05）。`log.pinch` ／ `log.rotate` の 2 本指ターゲットを持つ平坦でスクロール不要な `GestureView` を追加し、`SHOWCASE_GESTURES` 起動 env で差し替わるようにしました（BE-0114 の `SHOWCASE_CONFORMANCE` に倣った、SwiftUI だけのテスト用の仕掛けです）。それらを pinch ／ rotate してミラーした各 a11y value が変わることを確認する `scenarios/gestures_multitouch.yaml` を追加し、それをフルの `run` 経路（`--backend xcuitest`）で走らせる `xcuitest (multi-touch)` の `e2e.yml` ジョブを追加しました。idb はこのシナリオを実行できません（単一タッチのため `UnsupportedAction` になります）。そのため idb の smoke 経路からは外してあります。当初は Log タブの Form 奥にターゲットを置きましたが、XCUITest の座標スワイプが SwiftUI の Form をスクロールせず画面外の行へ到達できなかったため、専用画面にしました。これは本項目のアクチュエーション検証とは別の、スワイプによるスクロールの制約であり、別途フォローアップに値します。このジョブが緑になった時点で、項目を実装済みへ移します。
