...
- [x] `android-e2e.yml` の集約ジョブ `E2E (android)`。`needs: [changes, smoke, conformance]`、`if: always()` で、
...
- [x] `web-e2e.yml` の集約ジョブ `E2E (web)`。`needs: [changes, web-e2e, serve-ui-dogfood, web-conformance]`、同じトリガ変換
  （`E2E_LANE=web`）。
