...
| 実装 PR | TBD — このブランチの PR が開いたら記入します（propose-and-build Phase C） |
| トピック | 検証とカバレッジ |
<!-- /BE-METADATA -->
...
2. **シナリオの最終ステップの欠落を閉じます。** ステップ *i* の結果をステップ *(i+1)* のステップ前
   ...
   進む間、最後の**リーフ**ステップの識別情報を追跡します。3 つの並行した `Optional` フィールドでは
   なく、`LastLeafStep(outcome, step_id, driver)` という 1 つの値にまとめます（`loop.py`）。こうする
   ことで、3 つは常に一緒に設定されることが構造上保証され、利用側も 1 回の `is not None` 判定だけで
   3 つとも絞り込めます。`StepLoopState` は単一の `last_leaf: LastLeafStep | None` フィールドを
   ...
   この時点で `state.prev_after` は、すでにそのステップの落ち着いた事後のツリーであることが通常で
   （無条件に維持されています）、追加の問い合わせは発生しません。Unit 1 が頼っているのと同じ再利用を、
   実行の最後にもう一度使うだけです。**実装メモ：** 唯一の例外は、シナリオが `web` ブロックの内側で
   終わる場合です。この場合、`prev_after` はブロックから戻る際にすでに `None` にリセットされている
   ため、そのときは `leaf.driver` を直接再度問い合わせます。これも Unit 1 と同じベストエフォートの
   `try`/`except` で保護されています。この純粋に演出的な、判定確定後のキャプチャが実行される時点で
   web コンテキストがすでに失われていても、すでに確定した（合格したかもしれない）実行をクラッシュ
   させてはならないため、誤ったドライバのツリーを推測するのではなく、キャプチャ自体をスキップします。
...
6. **決定的スイートで発火順序・最終ステップの取得・読み取り回数の不変条件・非回帰を検証します。**
   ...
   ではなく最後の**リーフ**ステップに載ることを検証するケースを追加します。
   ...
   より、この項目がそれを気づかぬうちに壊すことはなくなります。`_step_artifacts` をすでに検証している
   `tests/serve/test_editor_ops.py` には、決め打ちの名前ではなく manifest に記録された名前を
   解決することを検証するケースを追加します。さらに、`extract` / `assert` が引き続き事後の安定した
   ツリーを読んでいることを検証する回帰テストを加えます。
