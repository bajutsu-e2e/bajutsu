...
入れ子の`steps`のあいだだけ指定したアプリへ切り替え、ブロックを抜けたらブロックへ入る前にアクティブだったアプリへ戻します。
...
まったく同じ形です。ブロックへ入るとき、ドライバは名指ししたアプリをactivateし、`.state ==
.runningForeground`になるまで、上限を決めて待ちます。`.activate()`が戻ってくること自体は、前面化の
確認にはなりません。スパイク自身も、最初にツリーを読む前にこの同じポーリングを必要としたからです。
上限内に前面化しなかったバンドルIDは、空の、まだ前面化していないツリーを読むのではなく、名前付きで
（`ElementNotFound`として）そのステップを失敗させます。この保証が成り立つのは、前面化が遅い
（コールドスタート、権限プロンプトなど）**インストール済み**のアプリに対してです。そもそも
インストールされていないバンドルIDには成り立ちません。別のアプリが本当にフォアグラウンドにある
状態でこれを試すと、この有界ポーリングにすら届かず、runnerが応答しなくなることを実機で確認済み
です（この項目自身のログを参照）。`bundleId`がすでにインストール済みのアプリを名指しすることは、
このステップの推奨事項ではなく要件です。ブロックを
...
「そのまま入れ子にする」とは言っても、どのステップも`app:`と安全に組み合わせられるわけでは
ありません。この項目は、そのどちらの隙間もあえて広げず、新しいガードを追加しません。デバイスの
ライフサイクルを操作するステップ（`relaunch`・`foreground`・`background`・`clearKeychain`・
`overrideStatusBar`）があります。これらは`app:`のスタックを経由しません。テスト対象アプリ自身の
simctlに結び付いたバンドルIDに対して働きます。そのため、`app:`ブロックの中でこれらを使うと、スタックの
先頭にどのアプリがあっても、テスト対象アプリのほうに届きます。ブロックの途中でこれらのステップを
使いたいシナリオ作者は、直前に`app:`が入ったアプリではなくテスト対象アプリに届くと想定すべきです。
入れ子の`web:`ブロックは、方向によって挙動が異なります。`web:`の中に`app:`を入れ子にした場合は
きれいに失敗します。`WebContextDriver`は`enter_app`/`leave_app`を実装していません。その狭めた
操作面の外にあるほかの操作すべてと同じように`UnsupportedAction`を送出するからです。`app:`の中に
`web:`を入れ子にした場合はそうなりません。`app:`のスタックにどのアプリがあるかにかかわらず、
テスト対象アプリ自身の`BAJUTSU_WEBVIEW_PORT`ブリッジに結び付いた`WebContextDriver`を開いてしまいます。
これはpreflightが今のところ検出しない誤りです。この項目自身のpreflight要件が使うcapability walkは、
`web:`ステップ・`app:`ステップのどちらについても、入れ子先まで再帰しません。既存の、認識済みの
隙間と同じ扱いです。
...
分岐ではなく、コンパイル時の表面にとどまります。ステップループは、`web:`のように2つ目のドライバの
インスタンスを作るのではなく、`active_driver`をそのままブロックに再利用します。`app:`はネイティブの
操作面（tap、type、ジェスチャ、picker wheel）をそのまま必要とします。同じインスタンスを再利用する
ほうが得るものは大きいからです。ブロックの前後でリセットが要る、run loop側の共有状態が1つだけ
あります。直前のステップのスクリーンショット再利用キャッシュ`prev_after`です。ブロックが読むのは
別のアプリのツリーなので、ブロック直後のネイティブなステップが、ブロック自身の最後のステップが
撮ったスクリーンショットを再利用してはなりません。`_handle_web`が自分自身のコンテキスト切り替えの
前後で、すでにこれをリセットしているのと同じ理由です（BE-0234 Unit 2）。

### Unit 5 — showcaseのフィクスチャとシナリオ

showcaseのシナリオで、実際のシステムアプリに対して`app:`ステップを端から端まで動かします。
流れは次のとおりです。showcaseアプリから`app:`ブロックでSafariをactivateし、その中の要素を
アサートします。ブロックを抜けたら、showcaseの要素が再び読めることをアサートします。スパイクが
手作業で測ったのと同じ往復を、シナリオの記述言語を通して駆動します。
[BE-0396](../BE-0396-ios-sfsafariviewcontroller-tree/BE-0396-ios-sfsafariviewcontroller-tree-ja.md)の
`scenarios/browser.yaml`とは違い、これは同じフィクスチャアプリのパターンではありません。
`browser.yaml`はこのリポジトリ自身が配るページに対してアサートするため、構造上hermeticです。
一方、Safari.appの中でアサートするシナリオは、Appleが提供するUIに依存します。UIはiOSのリリースごとに
変わり、このリポジトリが制御できないオンボーディングやネットワークの状態も持ち込みます。まさに
prime directive 2が回帰ゲートから遠ざけたい種類の依存です。そのため、このシナリオは
`ios-e2e.yml`の自動化されたマトリックスにあえて配線しません。動くのは、専用のopt-inレーンである
`make -C demos/showcase e2e-cross-app`だけです。`e2e-browser`が一括実行の`run-swiftui`/
`run-uikit`ターゲットに混ぜず自分専用のレーンを持つのと同じ理由であり、両方の一括レーンともこの
タグを除外します。
