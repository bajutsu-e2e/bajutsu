...
- [ ] `OrgConfig` / `DEFAULT_ORG` / `org_for_*` / `targets_for_org` を `bajutsu/serve/orgs.py` へ移し（シグネチャを `dict[str, OrgConfig]` に絞り込む）、`load_config` を `parse_config_dict` と `load_config` に分割し、`load_serve_config` を追加する
- [ ] コア側のローダーがトップレベルの `orgs:` を**取り除く**（組織情報を含む config でも `run` が読めるまま保ち、`extra="forbid"` は他の typo を捕まえ続ける）。serve 側の消費点は組織情報を `Config.orgs` ではなく `load_serve_config` から受け取る
...
| 場所 | 内容 |
|---|---|
| `bajutsu/config.py:27` | `_Model` の `extra="forbid"`。他のすべてのフィールドの typo を捕まえ続ける対策（`orgs` はコア側のローダーが検証前に取り除くので弾かれない） |
...
| `bajutsu/serve/__init__.py`、`authz.py`、`jobs.py`、`operations/reads.py`、`server/worker_job.py` | 組織用ヘルパーの現在の呼び出し元。すべてすでに `bajutsu/serve/` の内部 |
