...
- **`_await_ready` / `_await_boot` を、`base.wait_until` のデッドライン規律に統合します。** 現状、
  ...
  ループを手作りしています。このポーリングのロジックを 1 つの共有プリミティブへまとめる（`readiness.py`
  から `base.wait_until` を直接呼ぶか、両者が今重複させているバックオフのループを共通に抽出するか）こと
  で、決定性優先（プライムディレクティブ 2）と足並みをそろえるべき、もう 1 つのデッドライン実装を、
  分割のついでに消せます。どちらの呼び出し元が何を待っているかには手を入れません。
...
- **`Environment.captures_video -> bool`（または既存の `records_video_up_front` 述語の拡張）を追加
  し、`capture_video` を `actuator == "idb"` と書き下す代わりに seam へ問い合わせます。** この項目のなか
  で挙動が変わるのはここだけです。`record.py:259` は今、`XcuitestEnvironment` が `IosEnvironment` と
...
- [ ] `Environment.resolve_device(actuator, udid)` を追加し、`run.py` / `audit.py` / `doctor.py`
      をそこに通す
- [ ] `Environment.captures_video`（または `records_video_up_front` の拡張）を追加し、
