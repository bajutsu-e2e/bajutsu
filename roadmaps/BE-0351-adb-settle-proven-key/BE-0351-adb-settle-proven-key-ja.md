[English](BE-0351-adb-settle-proven-key.md) · **日本語**

# BE-0351 — adb の settle ポーリングの高速経路を、実際に静止を証明したキーだけに限定する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0351](BE-0351-adb-settle-proven-key-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0351") |
...
`hierarchy.raw<suffix>`と、存在する場合は`hierarchy.parsed-input<suffix>`を書き出します
...
- `_bounds()`は、不正な（属性自体は存在するがパースできない）`bounds`を、警告なしに
  `(0.0, 0.0, 0.0, 0.0)`として扱っていました。これは、その属性を本当に持たないノード
...
だけを閉じます。想定していたケースとは`uiautomator dump`経路です。catch-up の
projection-dwell が本物の「2 回の観測が離れて一致した」証明であり、`_settled_key`を設定する
からです。resident channel ではこの最適化を保ちません。マーク postdate によるクローズは
`_settled_key`を設定しません。理由は次の代替案で説明します。そのため`_settle()`は resident
channel 上のあらゆるアクチュエーションのあとに、この見送った代替案とまったく同じコストを
支払います。
高速経路そのものは残っていても、1 回の確認読み取りと`_SETTLE_POLL_S`分のスリープを毎回払う
ことになります。resident channel は本番の経路であり、今回診断したフレークが実際に走っていた
経路でもあるため、本項目はリスクよりこのコストを受け入れます。
...
すれば、本項目が閉じようとしている隙間を 1 手前で再び開けてしまいます。しかもその隙間を
resident channel 上で再び開けることと引き換えに得られるのは、上の代替案が失われると説明した、
まさにその free-settle 最適化です。
