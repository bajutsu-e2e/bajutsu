...
[BE-0049](../BE-0049-determinism-flakiness-audit/BE-0049-determinism-flakiness-audit-ja.md) が退ける
...
`_MAX_ATTEMPTS` と `_BACKOFF_BASE_SECONDS`（3回、0.5秒・1.0秒の指数バックオフ）を再利用します。
そのためループ全体は1秒未満ではなく数秒程度に収まり、本当になくなった要素を長く再試行しません。

再解決は、リトライ経路に限って、バックエンドの「解決したまさにその要素に作用する」保証を緩めます。
本提案は、このトレードオフを隠さずに述べます。再解決で一意に一致することは、セレクタに一致する要素が
存在することは証明しても、その一致が1回目の往復で見つけた要素と「同じ」であることは証明しません。
2回の往復のあいだに画面が変わると（たとえばモーダルが閉じると）、同じセレクタに一意に一致する別の
要素が現れることがあり、リトライはその別の要素に作用してしまいます。このトレードオフは受け入れ可能で、
見かけよりも狭いものです。解決は Python 側にとどまるので、不変条件の目的、すなわち解決の権限を
runner の外に保ち、曖昧な述語が黙って複数の一致から1つを選ぶのを防ぐという目的は、損なわれません。
リトライは Python 側の一意の一致にのみ作用し、複数のうちの1つには決して作用しません。しかも作用する
瞬間に解決し直すのは、まさに idb が行うこと（idb は1手で解決して tap する）なので、リトライは XCUITest
の stale 処理をほかのバックエンドに揃えるものであって、より緩い規則を発明するものではありません。より
厳しい変種、すなわちセレクタだけでなく元の要素の同一性属性にも一致するときにのみ再アクチュエーション
する形は、セレクタだけの一致が緩すぎると分かった場合に `implement-be` の選択肢として残します。
...
解決の権限は Python 側にとどまり、複数の要素に一致しうる runner 側の述語には移りません。stale
リトライは Python 側で、しかも一意の一致にのみ再解決するので（このトレードオフは *詳細設計* が詳しく
述べます）、この権限を Python 側に保ちます。解決を runner に移すと、その権限を手放し、決定論的な核が
防ぐために存在する曖昧さと、stale レースを引き換えにしてしまいます。
