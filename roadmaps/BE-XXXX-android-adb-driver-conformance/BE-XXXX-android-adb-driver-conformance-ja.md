...
- **契約はカバレッジの分だけしか強くありません。** BE-0114 の眼目は、決定性コアの不変条件がすべての実アクチュエータで*同一に*成り立つこと、`FakeDriver` の上だけではないことです。検査されていない backend こそ、乖離が潜む場所です。たとえば、曖昧な adb セレクタが失敗せず最初の一致にタップする（プライムディレクティブ #2 への違反）としても、既存の Android のジョブはすべて通ってしまいます。`smoke` / `golden` / `visual` は構成上どれも曖昧でないセレクタを使うからです。
