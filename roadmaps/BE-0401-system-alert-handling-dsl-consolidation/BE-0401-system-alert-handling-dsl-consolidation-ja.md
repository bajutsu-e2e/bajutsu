[English](BE-0401-system-alert-handling-dsl-consolidation.md) · **日本語**

# BE-0401 — systemAlertHandling のDSLを、答え方の経路ごとに1つのキーへ整理する
...
シナリオに`instruction`を足すと、別のファイルにある設定のブロックが黙って無効になります。
...
`visionInstruction`を与えていないときに限り、フォールバックへ渡すヒントを導出します。ガードは候補を
「Tap the button labeled one of, in order: …」の形に整えて渡します。どの層のものであれ
`visionInstruction`はこのヒントより優先します。ラベルから導いたヒントはフォールバックについての
表明ではなく、明示された`visionInstruction`は表明だからです。

ヒントを導くもとになるのは、**`labels`を与えている層のうちもっとも内側の1つ**です。ネイティブ経路が
たどる連結ではありません。ネイティブ経路はラベルを厳密に比較するので、候補が長くなっても、どこかの
層が名指ししたラベルを探す手間が増えるだけです。一方でフォールバックへの指示は locator がゆるく解釈
する自由文であり、連結したリストを渡すと2つの答えを同時に手渡すことになります。ターゲット設定が
`labels: ["Allow"]`で許可し、シナリオが`labels: ["Don't Allow"]`で拒否している場合を考えます。
「…in order: Don't Allow, Allow」と描画され、locator が後者を取る余地が生まれます。`rules`を
フォールバックから外している理由と同じ危険が、1つ隣のフィールドで起きます。そこで扱いも同じにします。
フォールバックが聞くのは1つの層だけで、それは発言した層のうちもっとも具体的な層です。`rules`がフォールバックへ何も渡さない点は、BE-0382が記録した理由のまま変わり
...
```
ネイティブ経路:   rules  →  labels （どの層も labels を与えていなければ組み込みの拒否ラベル）
                              │ 何も解決しない
                              ▼
...
そこでガードは、構築の時点で通知を出します。通知は、対象のシナリオと、そこで答えることになる
ターゲットの規則を名指しします。この通知は、合成を保ったまま沈黙だけを取り除きます。実装は
`warn_once`（`bajutsu/deprecations.py`）なので、デバイスの操作もモデルの呼び出しも発生しません。

`warn_once`はコードごとにプロセス全体で重複を除きます。ガードはシナリオごとに1回組み立てられるので、
コードにはシナリオ名とその規則のプロンプトを組み合わせます。プロンプトだけを鍵にすると、多くの
シナリオを回す実行では最初の1件だけが警告され、残りはこの節が取り除こうとしている沈黙のまま通り
過ぎます。この鍵は、通知を検査可能にするためにも必要です。2つのシナリオが該当する実行では、2つを
まとめた1件ではなく、シナリオごとに1件ずつ通知が出ます。`warn_once`を持つモジュールは、docstringが
自らの範囲を非推奨の通知に限っています。規則がシナリオへ届くことは非推奨ではないので、同じ変更で
その docstring を、モジュールが実際に担う範囲へ広げます。
...
出すこと。視覚フォールバックが、`visionInstruction`があればそれを受け取り、なければもっとも内側の層の
`labels`から導いたヒントを受け取ること。どちらの場合も`rules`からは何も受け取らないこと。どのアサーションもモデルに依存せず、
モデルへ届く呼び出しも増えません。
