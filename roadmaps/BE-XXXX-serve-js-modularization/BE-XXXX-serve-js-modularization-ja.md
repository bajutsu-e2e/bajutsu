[English](BE-XXXX-serve-js-modularization.md) · **日本語**

# BE-XXXX — serve.js をビルドなしのままセクション別ファイルに分割する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-serve-js-modularization-ja.md) |
...
- [ ] serve.js を既存テンプレート機構で配信するセクション別ファイルに分割
- [ ] `startJob(…)` ヘルパを追加し、run / record / crawl のハンドラを移行
- [ ] `eslint.config.mjs` が新しいファイル群を対象に含む
- [ ] 分割をまたいで dogfood ゲートが緑（隙間が見つかった場合のみシナリオを拡張）
