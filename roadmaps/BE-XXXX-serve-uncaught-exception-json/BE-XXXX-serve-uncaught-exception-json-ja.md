[English](BE-XXXX-serve-uncaught-exception-json.md) · **日本語**

# BE-XXXX — serve ハンドラの未捕捉例外を JSON エラーで返す

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-serve-uncaught-exception-json-ja.md) |
...
3. **サーバ側のトレースバックを保つ。** 例外は引き続き記録し（`oplog` で束ねた request id 付き）、運用者には診断情報を
   ...
   行動に足るだけを返し、必要な箇所では内部パスを漏らさないようにします。
...
- [ ] Unit 1 — POST 振り分けを囲むトップレベル `try/except` → JSON 500。
- [ ] Unit 2 — GET にも同じ境界を（ストリーミング/バイナリ経路を除く）。
- [ ] Unit 3 — サーバ側トレースバックを保持し、クライアント向けメッセージを意図的に決める。
- [ ] Unit 4 — POST・GET の送出経路のハンドラテスト、ストリーミングは不変。
