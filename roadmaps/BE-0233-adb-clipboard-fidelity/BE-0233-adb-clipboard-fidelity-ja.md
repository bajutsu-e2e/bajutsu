...
| 実装 PR | [#949](https://github.com/bajutsu-e2e/bajutsu/pull/949) |
...
- [x] ユニット 1 — 対象の API レベル / ABI にわたって、実機のクリップボードのベースラインを確立する。
- [x] ユニット 2 — 修復か狭めるかを判断し、その判断を記録する（capability の面が変わる場合は
  DESIGN / architecture も）。
- [x] ユニット 3 — 選んだ方向を adb バックエンドの内側で実装する。
- [x] ユニット 4 — ギャップを捕まえられたはずの、実機（または preflight での拒否）のカバレッジを追加する。
- [x] ユニット 5 — 結果に合わせて BE-0208 の `device_android` レーンのシナリオを整合させる。

**ログ**

- ユニット 1（ベースライン、arm64 API 34 エミュレータ）：`cmd clipboard set/get-primary-clip` は
  `No shell command implementation` を返して exit 0 になります（黙って何もしません）。`service call
  clipboard` は `ClipData` のパーセルを手組みする必要があり、トランザクションコードは API レベルを
  またぐと変わり、`ClipboardService.checkAndSetPrimaryClip` を通るため脆く、採れません。shell uid の
  プロセスからはそもそもクリップボードに到達できません（Android 10 以降はフォアグラウンドのアプリと
  既定の IME に限られます）。この結果は ABI ではなくプラットフォーム実装に由来し、PR #934 が x86_64
  イメージで観測したものと一致します。
- ユニット 2（判断）：**修復**を選びました（狭めません）。シェルの手段がどれも動かないので、シナリオ
  実行中は対象アプリがフォアグラウンドにあることを使い、クリップボードをアプリの内側から操作します。
  これはアプリに組み込む一律の opt-in な SDK を活用する方式で、どのアプリも同じライブラリを埋め込む
  ため per-app の設定差ではなく、アプリ非依存を保ちます。iOS で BajutsuKit が通信捕捉に使うのと同じ
  モデルです。`DC_CLIPBOARD` は adb で宣言し続けます（idb のクリップボードが simctl に乗るのと同じく、
  協調するアプリがあればバックエンドが駆動できます）。capability モデルに関するこの整理は `DESIGN.md`
  §8 と `docs/architecture.md` / `docs/drivers.md` に記録しました（BE-0113）。
- ユニット 3（PR #949）：再利用可能なライブラリ `BajutsuAndroid/`（`BajutsuKit/` の Android 版）を
  新設し、クリップボードの `BroadcastReceiver` を置きました。`bajutsu/adb.py` が順序付き `am broadcast`
  で駆動し（両方向を base64 で運び、アプリにレシーバがなければ明確に失敗します）、
  `platform_lifecycle.android_device_control` が package を渡し、`drivers/adb.py` は `DC_CLIPBOARD` を
  維持します。
- ユニット 4：fast ゲートのテストで broadcast のビルダ、結果のパース（base64 と loud-fail）、fake の
  レシーバに対する往復、維持した capability と preflight の admit を確認します。実機では
  `device_android` のクリップボード読み戻しがローカルの arm64 API 34 エミュレータで通ります。
- ユニット 5（PR #949）：`device_android.yaml` にクリップボードの書き込みと読み戻しが戻りました
  （PR #934 が求めていた強い assertion です）。showcase の compose / views アプリは debug ビルドで
  レシーバを組み込みます。
