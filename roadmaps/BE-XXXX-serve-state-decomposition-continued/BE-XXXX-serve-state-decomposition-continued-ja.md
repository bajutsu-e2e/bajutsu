...
2. **プロバイダー設定。** フィールド `provider_settings`、`provider_settings_store`、両者を守る
   ...
   （`state.py:540`〜`578`）は、組織ごとの AI プロバイダー選択（BE-0229）のうちインメモリ側を構成し
...
この2つのクラスターは合わせて、`ServeState` に残るロックのうち3つ（4つ目のジョブレジストリ自身のロ
ックはすでに BE-0198 で移動済みです）と、およそ十数個のフィールドを抱えています。レジストリに次ぐ規
...
`baselines`、`secrets`、`executor`、`repository`、`for_org`/`StoreBundle`）、run の入出力先のまと
