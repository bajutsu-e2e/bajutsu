...
本項目は、同一性が一致する重複アクセシビリティノードのペアを、XCUITest が実際にタップできる要素 1 つ
だけに絞り込み、runner の `/elements` の返信からもう一方を取り除きます。runner の要素問い合わせの
列挙順でたまたま先だった要素に、参照の割り当てを任せたままにはしません。同一性が一致する重複ノードとは、
...
の `refreshSnapshot` も、その同じ最初の出現に対して接尾辞のない参照を割り当てます。どちらにも、
...
1 つのステップの中で BE-0289 が試す 3 回の再解決は、そのすべてが同じ「最初に列挙された複製」を導き直し、
同じ結果を繰り返します。それが XCUITest のネイティブな tap が解決してタップできると判定できる複製で
あれば、その重複の並びで run するかぎり毎回成功します。それがもう一方の複製（識別子・ラベル・traits・
value・frame の報告は完全に同じでも、XCUITest 自身のイベント合成が解決できないもの）であれば、状況が
変わります。
...
成否は、XCUITest 自身のツリーの走査が決める並び順に懸かっており、シナリオにも再解決にも、その
...
ない、2 つの重複それぞれへのネイティブな参照と、その特定のノードが今まさにタップできるかを XCUITest に
...
すべて一致）でグループ化します。traits は集合として比較し、Python 側のキー
`tuple(sorted(set(el["traits"])))` に合わせます。そうすることで、traits の並び順が違うだけの重複ペア
もグループ化できます（単純な `[String] ==` や `SnapshotStore` の配列のハッシュでは、これはできません）。
1 件しかないグループは判定の必要がなく、余分なコストもかかりません。
...
`/elements` が返したものをそのまま読むだけなので、本項目ではそれぞれを別個に導き直しません。ただし、
この候補の減り方は、そのときどきの判定結果に懸かっています。重複する同一性への `count` や `exists` は、
ペアがまだ覆われているあいだは 2 件を報告し（どの要素も `.ok` を返さないため、グループは変わらないまま
です）、いったん落ち着けば 1 件を報告します（ちょうど 1 件が `.ok` を返します）。この 2 つの値は、
本項目自身のオンデバイスのテストが同じ画面について固定する値でもあります。その移り変わりのあいだに
ポーリングする assertion では、ツリーの同一性そのものは変わらないまま結果だけ変わり得ます。しかも
`tap` と違って BE-0289 の再解決が吸収できる `stale` の返事もありません。*進捗* には、下の先送りにした
限界と合わせてこれも記録します。
...
今日と変わらないままにする問い合わせ（下の 2 つの場合のいずれか）で、本項目が後で取り除く要素に
...
が一致する fake の要素を 2 つ用意して `isHittable` の結果を互い違いにすれば、返信はタップできる要素
...
**取り除くのではなく、並べ替えるだけにする。** 本項目の前の版は、タップできる複製をグループの先頭へ
移すだけで、両方をツリーに残していました。BE-0312 が今持っている出現番号のタイブレークの形を変えず、
その上に乗る設計です。並べ替えは、*動機* が説明する失敗そのものを直します。先頭の要素はつねに接尾辞
のない参照を得るからです。しかし、それが効くのは片方だけがタップできる場合だけです。両方タップできる
ときは、並べ替えるだけの設計でもどちらを先頭にするかを問い合わせ順に頼るしかなく、今日と同じだけ
非決定的な場合として残ります。片方だけがタップできる場合、タップできない複製を取り除けば、その複製と
...
内容を受け取ることが一切ありません。また、ネイティブな参照をまったく持たない Python 側から、重複
グループごとに、しかも問い合わせごとに 1 回の追加往復なしに「どちらがタップできるか」を runner へ
尋ねる経路もありません。runner がタップできる要素を告げたあとで Python 側が候補を 1 つ捨てても、
もう一方の複製の参照は runner 側では生きたまま `index` で届くところに残ってしまいます。取り除くこと
自体は、要素が実際に消える場所、つまり返信を組み立てる runner で行う必要があり、runner が送った
ものしか見えない Python 側では足りません。この理由づけは、参照を保つネイティブな runner のプロセスを
持つバックエンドに
...
- [ ] `RouterTests.swift` の実機なしテスト（`GET /elements` を覆う）：`FakeElementProvider` に、
      ...
      返すときは、取り除かれるのは例外を投げた要素だけである。
- [ ] オンデバイスのテスト：実際に片方だけが覆われた重複ボタンを持つ `UIAlertController` で `id` に
      タップすると、覆われていない、生き残ったタップできる要素に着弾すること。その識別子への
