...
現在のロードマップが必要とするのは 2 つの形（dotted / underscore）です。iOS と Android Compose は
SPEC の id をそのまま表出し、Android Views はアンダースコアへ写します。候補リストは、ネイティブ id
構文が異なるプラットフォームが増えるごとに 1 つずつ項目が増え、リストが伸びても「canonical 形を
先頭に」の規則は保たれます。この手作業を今ここで一般化しないのは意図的です。第 3 の id 表記はまだ
具体化しておらず（Flutter バックエンド [BE-0008](../BE-0008-flutter-support/BE-0008-flutter-support.md)
はまだ Proposal です）、失敗は target 単位で loud だからです（候補が欠けたセレクタはその target の
実行で失敗し、どこかで静かに失敗するわけではありません）。取りこぼしは新しい target を動かした瞬間に
表面化します。第 3 のプラットフォームが実際に来たときは、各 target の id 規約と各セレクタ候補を
突き合わせる `audit` 側のカバレッジ検査（候補をすでに列挙している `referenced_ids` の拡張）を、
検証すべき具体的な規約を持つそのプラットフォームと**一緒に**入れるのが自然で、投機的な N 形の
仕組みを今作るのは避けます。
