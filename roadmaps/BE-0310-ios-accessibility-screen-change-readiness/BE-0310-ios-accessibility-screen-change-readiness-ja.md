...
| 実装 PR | [#1309](https://github.com/bajutsu-e2e/bajutsu/pull/1309) |
...
シグナルの出どころは、どのターゲットも同一に組み込むテスト補助パッケージ `BajutsuKit` への
オプトインの追加です。これは UIKit の `UIViewController.viewDidAppear` をフックします。この
メソッドは、出現の遷移が完了した**あと**に呼ばれます。（当初はこの遷移で iOS が投げるアクセシビリティ
通知 `UIAccessibility.screenChangedNotification` を観測する計画でしたが、このシンボルは外向き専用でした。
すなわち `UIAccessibility.post(...)` の enum ケースであって、観測できる `NSNotification.Name` では
なかったため、下の*検討した代替案*で比較した viewDidAppear のフックを採用しました。）
`UINavigationController`・`UITabBarController`・モーダル提示はそれぞれビューコントローラの出現を直接
引き起こし、`SwiftUI` の `NavigationStack`・`.sheet`／`.fullScreenCover`・タブの切り替えは、いずれも
内部で `UIHostingController` を提示します。そのためこのフックは UIKit アプリと SwiftUI アプリを、
コンテナ遷移の粒度で 1 つの仕組みで覆い、テスト対象のアプリには一切変更を求めません。ターゲットが
`BajutsuKit` を組み込まない場合は、readiness ゲートと `settled` 待ちのいずれも従来どおりツリー差分の
ポーリングにフォールバックします。したがってこの変更は、オプトインするアプリにとっての強化ではあっても、
...
ゲートと `settled` 待ちがツリー差分から再構成しようとしているものです。この完了を直接捉えれば、推測を、
それが近似していた事実そのものに置き換えられます。（この事象に名前を与えたのは通知ですが、実装で観測に
使うのは同じ完了を表す `UIViewController.viewDidAppear` です。この通知が観測できる `NSNotification.Name`
ではなかったためで、*詳細設計*の作業単位 1 と*検討した代替案*を参照してください。）

Bajutsu は、どのアプリにも画面のコードを変えさせることなく、この完了を捉えられます。この形の能力を
...
1. **`BajutsuKit` で完了したビューコントローラの出現を観測する（`viewDidAppear` の swizzle）。**
   `UIViewController.viewDidAppear(_:)` を swizzle するオプトインのフックを追加し（`BajutsuURLProtocol`
   がすでに使う `method_exchangeImplementations` の作法を再利用します）、monotonic なタイムスタンプと
   通し番号のカウンタを小さな observable なストアへ記録します。このストアは、ネットワークの exchange に
   ついて `BajutsuExchangeStore` がすでに使っているアプリ内ストアと同じ形です。フックが発火するのは、
   UIKit のコンテナ機構が**自動で**引き起こす出現（ナビゲーションの push／pop、モーダル提示、タブの
   切り替え）だけです。これがアプリの画面変更を要さないことの拠りどころです。（当初はこの出どころとして
   `UIAccessibility.screenChangedNotification` を挙げていましたが、このシンボルは外向き専用で——
   `UIAccessibility.post(...)` の enum ケースであって、観測できる `NSNotification.Name` ではないため——
   *検討した代替案*で比較した viewDidAppear のフックがこの作業単位を実装します。粒度は同じコンテナ遷移の
   ままです。）有効化は、`BajutsuNet` が `BAJUTSU_COLLECTOR` を条件にするのとまったく同じく、注入された
   launch 環境変数を条件にします。これによって、`BajutsuKit` を組み込んでいても Bajutsu の run の外で
   動くアプリは何も観測しません。この作業単位が触れるのは `BajutsuKit` だけで、テスト対象のアプリの画面
   コードには決して触れません。
...
- **アクセシビリティ通知を観測する代わりに `UIViewController.viewDidAppear` を swizzle する。**
  *採用——これが実装された手段です。* 当初の計画は
  `UIAccessibility.screenChangedNotification` を観測することでしたが、このシンボルは外向き専用でした。
  iOS は `.screenChanged` を `UIAccessibility.post(notification:argument:)`（アプリから VoiceOver への
  向き）の enum ケースとして提供するだけで、観測できる `NSNotification.Name` は提供しません。購読すべき
  公開の通知が存在せず、それを試みたコードはコンパイルに失敗しました。この項目は当初、SwiftUI の穴を
  懸念して swizzle を退けていました——「フックは `UIHostingController` の境界でしか発火せず、1 つの
  hosting controller の内側の遷移を取りこぼす」——が、この懸念は、この項目が対象とする**コンテナ**遷移
  については過大でした。`NavigationStack` の push、`.sheet`／`.fullScreenCover` の提示、タブの切り替えは、
  いずれも `UIHostingController` を提示し、その `viewDidAppear` が発火します。したがってフックは UIKit と
  SwiftUI を、コンテナ遷移の粒度で等しく覆います。本当に取りこぼすのは、1 つの hosting controller の
  内側での画面内のビュー差し替えですが、これはコンテナ遷移ではなく画面内の更新であり、次の項目や
  [BE-0299](../BE-0299-settle-value-condition-wait/BE-0299-settle-value-condition-wait-ja.md) と同じ
  スコープ外のケースです。通知の経路が使えない以上、このフックが、アプリの画面変更を要さない残る公開
  API の手段です。
...
- [x] 作業単位 1 — `BajutsuKit` で完了したビューコントローラの出現を観測する（`viewDidAppear` の
  swizzle、オプトイン）。（提案が当初挙げたアクセシビリティ通知ではなく出現フックで実装しました。下のログと
  *検討した代替案*を参照してください。）
...
- 実装中の仕組みの転換：この項目の前提は `UIAccessibility.screenChangedNotification` を*観測*することでしたが、
  このシンボルは外向き専用（`UIAccessibility.post(...)` の enum ケースであって、観測できる
  `NSNotification.Name` ではない）で、購読すべき公開の通知が存在しませんでした。そこで作業単位 1 は
  `UIViewController.viewDidAppear` を swizzle します。これは*検討した代替案*で比較した手段で、その SwiftUI
  への到達は、この項目が対象とするコンテナ遷移（NavigationStack・モーダル・タブがそれぞれ提示する
  `UIHostingController` の出現）を覆うことが分かりました。ワイヤ契約、Python 側のコレクタ、作業単位 2〜4 は
  変わりません。変わったのは iOS の観測の仕組みだけです。作業単位 5 のオンデバイス確認が、この出現フックが
  両ツールキットで想定どおり発火することを裏づけます。
