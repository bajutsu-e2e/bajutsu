...
| `run-set`のクラウドバッチファンアウト（[BE-0336](../BE-0336-serve-device-farm-bounded-fan-out/BE-0336-serve-device-farm-bounded-fan-out-ja.md)） | serve自身のアーティファクトキャッシュのパスへ解決します | 拒否します |
...
ディレクトリの中身へマージするのではなく、**置き換え**ます。`materialize_composition`は2つのアーティ
ファクトの形をすでに扱っており、上書きはそのどちらもそのまま再利用します。zipはルートで展開されます。
単一のYAMLファイルは、runnerの一覧が見つけられる、閉じ込められた`*.yaml`のbasename
（`_safe_scenario_name`）で書き込まれます。置き換えることによって、runは、それを求めたブランチと一致
します。マージにすると、ブランチ側で削除したscenarioがディレクトリに残り続けます。ターゲットのディレクトリ
にあるscenarioすべてを対象とするファンアウトは、それを実行してしまいます。そのrunは、どのコミットにも
対応しなくなります。
...
セグメントと衝突することはありません。単一ファイルのscenariosアーティファクトについては、その表示名が
ダイジェストのソルトになります。同じバイト列が2つの名前で使われるなら、2つのツリーになるべきだから
です。この作り方を再利用することで、キーはbinaryのshaだけでなく、materialsベースのジョブのscenariosも
覆います。

上書きを名指さないジョブは、今日のキーをそのまま保ちます。そのため、既存の挙動は何も変わりません。
上書きを持つジョブだけが、専用のツリーを得ます。これにより、配置は構造的にジョブ単位になり、workerが
削除や再配置の手順を正しく行う必要はなくなります。これは、アーティファクトの同一性はその中身のダイ
ジェストであり、異なる2つのbinaryは異なる2つのツリーになるという、BE-0413自身の原則に従うものです。また、
...
`start_run_set`は、`appPath`をserveのプロセス自身の上で解決済みです。それを
...
- zip形式の`scenarios`上書きは、ターゲットのscenariosディレクトリを置き換えます。そのため、バインド先の
  ツリーにあって上書きにないscenarioは実行されません。
- 単一YAMLの`scenarios`上書きは、runnerの一覧が見つけられる、閉じ込められた`*.yaml`名で配置されます。
...
- [ ] 単位1 — `binaryArtifact`と`scenariosArtifact`のリクエストフィールド。どちらも任意項目であり、
      ...
      引き続きorgのバインディングを通じて解決されます。
- [ ] 単位2 — 本項目は、workerのトポロジーで名指された上書きに署名・配送します。`run-set`のファンアウト
      では、binaryの上書きをserve自身のアーティファクトキャッシュから解決し、scenariosの上書きは拒否
      します。`LocalExecutor`のデプロイでは両方を拒否します。runのマニフェストには、プロベナンスを
      記録します。
...
- [BE-0336 — serve から Device Farm へ投入する、デバイス数を制限したシナリオ単位の分割実行](../BE-0336-serve-device-farm-bounded-fan-out/BE-0336-serve-device-farm-bounded-fan-out-ja.md)
  ——`start_run_set`が駆動するクラウドバッチのファンアウト。そのパッケージングの仕組みが、binaryの上書きを
  serve自身のアーティファクトキャッシュから受け取り、scenariosの上書きを拒否する理由です。
