...
- [x] codegen を実行して生成したソースを返す `POST /api/codegen`（`{target, scenario, emit}`）
      エンドポイントを追加する
- [ ] エディタと Replay ビューに「Generate code」操作を、XCUITest／Playwright の emit セレクタ付きで
      追加する（エディタは完了。Replay ビューは後続 PR）
...
- [#692](https://github.com/bajutsu-e2e/bajutsu/pull/692) — `POST /api/codegen` エンドポイントと、
  ...
  ビューア）。emit のディスパッチは CLI と共有します（`bajutsu/codegen_emit.py`）。Replay ビューの
  同じ操作は、このエンドポイントを再利用する後続 PR です。
