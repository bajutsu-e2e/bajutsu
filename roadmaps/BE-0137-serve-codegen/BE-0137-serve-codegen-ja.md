...
- [ ] codegen を実行して生成したソースを返す `POST /api/codegen`（`{target, path, emit}`）
      エンドポイントを追加する
- [ ] エディタと Replay ビューに「Generate code」操作を、XCUITest／Playwright の emit セレクタ付きで
      追加する
- [ ] 結果を読み取り専用のコードビューアに表示し、クリップボードへのコピーとダウンロードを添える
      （ファイル名はシナリオと出力先から導く）
- [ ] 選択中の backend で有効な emit ターゲットだけを提示し、codegen の既存の診断を出す
