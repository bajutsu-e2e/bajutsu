...
- **`bajutsu/runner/pipeline.py:run_one`（およそ 140〜229 行）は、シナリオ単位の実行関数で、`run_all`（74〜229 行）
  に入れ子になっています。** この関数は外側スコープの `eff`、`mailbox`、`baselines_dir`、`schemas_dir`、
  `actuator`、`golden_context`、`progress`、`total`、`on_blocked_for`、`lease` を捕捉します。クロージャである
...
- **`run_one` を明示的なシグネチャのトップレベル関数に昇格する。** クロージャを、捕捉していた 10 個の値を引数に
...
- **どちらの関数もそのままにして、コメントに頼る。** [BE-0143](../BE-0143-run-command-decomposition/BE-0143-run-command-decomposition-ja.md)
  ...
  `[0]` の箱入りクロージャ状態と 10 変数の捕捉こそ、コメントでは安全に編集できるようにできない部分です。
...
- [ ] `run_one` を 10 個の値を取る明示的なシグネチャのトップレベル関数に昇格する。
