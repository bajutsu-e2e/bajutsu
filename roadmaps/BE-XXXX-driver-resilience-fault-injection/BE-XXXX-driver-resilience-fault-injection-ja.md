[English](BE-XXXX-driver-resilience-fault-injection.md) · **日本語**

# BE-XXXX — Real-device fault-injection coverage for driver resilience paths

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-driver-resilience-fault-injection-ja.md) |
...
常駐ランナーが強制終了またはフリーズしたときのために存在しますが、そのテストは Python の
lambda から合成例外を送出しているだけです。実際に実機で動く conformance suite は、どちらの経路も
踏むことがありません。その画面はあらかじめ seed され準備完了を待ってから使われるため
...
- **両方ともまず非 gating の signal として着地させる**：障害注入レーンは、既存の conformance
...
- **制御フローのロジックがユニットテストされていることを根拠に、合成フィクスチャを信頼する**：
  ...
  これらの機構が保証するために存在する性質です。
- **実運用でこれらの機構が失敗するまで実際のカバレッジ追加を待つ**：リトライ/復旧の経路が現場で
...
- [ ] idb/adb 向けに、実際の transient-empty 障害注入をまず非 gating で追加する。
- [ ] XCUITest 向けに、実際の crash-recovery 障害注入をまず非 gating で追加する。
- [ ] それぞれ安定後に必須化する。

## 参考

- [BE-0254 — idb と adb のための共有 CoordinateTreeDriver 基底の抽出](../BE-0254-coordinate-tree-driver-base/BE-0254-coordinate-tree-driver-base-ja.md)
- [BE-0207 — XCUITest ランナーチャネルを一過性のタイムアウトに対して頑健にする](../BE-0207-xcuitest-channel-transient-retry/BE-0207-xcuitest-channel-transient-retry-ja.md)
- [BE-0287 — マルチタッチ操作下での XCUITest ランナーチャネルの耐障害性](../BE-0287-xcuitest-runner-multitouch-resilience/BE-0287-xcuitest-runner-multitouch-resilience-ja.md)
