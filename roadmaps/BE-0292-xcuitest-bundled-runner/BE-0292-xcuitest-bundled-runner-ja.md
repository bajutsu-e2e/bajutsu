...
| 実装 PR | [#1221](https://github.com/bajutsu-e2e/bajutsu/pull/1221) |
| トピック | プラットフォーム対応 |
...
- [ ] **doctor / 開示**：解決したランナーの出所を報告し、Xcode/SDK の不一致を上書きの逃げ道とともに
  知らせます。「解決したランナーの出所を報告する」半分は実装済みです（`doctor --target` が
  `xcuitest runner: bundled (wheel-shipped Simulator runner)` / `testRunner: <path>` を表示します）。
  Xcode/SDK の不一致検知は別途フォローアップとして未実装のまま残します。
- [ ] **検証**：高速ゲートの解決テスト(同梱デフォルト、上書きの優先、実機エラー、キャッシュ再利用、
  同時展開の競合)は実装して通っていますが、`testRunner` を持たない実機 e2e run はまだありません。
  showcase の config は今もすべてのターゲットで `testRunner` を指定しており、`ios-e2e.yml` も
  `make -C demos/showcase runner-build` を走らせ続けているため、同梱ランナーの経路は Simulator 上で
  未検証のままです。別途のフォローアップとして残します。
