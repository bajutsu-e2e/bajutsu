...
ヘルパ [`scripts/be_claims.sh`](../../../scripts/be_claims.sh) が、この API と `git ls-remote` の上に
`list` / `claim` / `release` を被せます。
...
（`max(used) + 1`、予約済み・claim 済みの番号は飛ばす）を保ちます。
...
[`roadmap-claims-gc.yml`](../../../.github/workflows/roadmap-claims-gc.yml) の 2 つのトリガがそれを徹底します。
...
2. そうでなければ、その番号を持つ **open な PR のうち最小の番号**。`ROADMAP_LOWER_PR_IDS` で渡されます
   （[`scripts/open_pr_be_map.sh`](../../../scripts/open_pr_be_map.sh) からワークフローが算出）。
...
- **失敗するチェックで衝突を検出し、修復は手作業のみとする。** 赤く目立つチェックは、すり抜けるたびに手作業
  ...
  `make roadmap-id-repair` は残します。
- **まず提案として書き起こす（`BE-XXXX` プレースホルダ）。** ここでは不要です。作業は同じ変更の中で実装
  されているので、*開発基盤* の下に BE-0043 の姉妹として、最初から実装済みとして提出します。リポジトリが
...
- [CLAUDE.md](../../../CLAUDE.md) · [docs/ai-development.md](../../../docs/ja/ai-development.md) — 本項目が
  固めるロードマップ ID のルールと、採番・修復の流れ。
- [`scripts/allocate_roadmap_ids.py`](../../../scripts/allocate_roadmap_ids.py)（採番 + 修復）、
  [`scripts/be_claims.sh`](../../../scripts/be_claims.sh)（claim の台帳）、
  [`scripts/open_pr_be_map.sh`](../../../scripts/open_pr_be_map.sh)（open な PR のタイブレーク入力）、
