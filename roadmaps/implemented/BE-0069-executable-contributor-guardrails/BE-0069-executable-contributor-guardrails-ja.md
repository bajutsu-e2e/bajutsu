...
4 つのメカニズムをすべて出荷しました。**A** `make new-roadmap-item`（`ideation` skill が呼び出す）、**B** `make lint-roadmap`（`make check` に組み込み）、**C** `make worktree` / `make preflight`（ドキュメントの手順をコマンドへ向け替え）、**D** `make lint-pr` と `pr-title.yml` の CI タイトルゲートです。`CLAUDE.md`・`docs/ai-development.md`（および `docs/ja/`）・`ideation` / `implement-be` skill のプローズは、手順を語るのではなくコマンドを指すようにしました。phase 5 の任意の末尾も着地しました。追跡対象の **`.githooks/commit-msg`** フック（pre-push ゲートと同じく `core.hooksPath` で配線）が、`lint_pr.py --commit-msg` でスコープなしのコミット subject をコミット時にブロックします。merge / revert / fixup / squash はスキップし、`uv` が無ければ no-op で、機械的な判定のみ（判断を要するものは扱いません）。
...
- **専用の新トピックを設ける。** 代わりに *開発基盤（コントリビュータ体験）* の下（BE-0043 のトピック）に
  ...
  （[BE-0065](../../implemented/BE-0065-docstring-standard-api-reference/BE-0065-docstring-standard-api-reference-ja.md)）に
  ならいます。
