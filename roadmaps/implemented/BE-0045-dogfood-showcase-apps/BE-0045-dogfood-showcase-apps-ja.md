...
`sample2`）に分散しています。showcase は操作面のすべて——5 タブ、ナビゲーションスタックの push、4 つの
モーダル様式（detent 付き sheet、フルスクリーンカバー、アクションシート、一過性トースト）、テキスト入力、
非同期ロード、通信（実通信＋BajutsuKit によるモック可能）、そして意図的に OS レベルのアラートを上げる
タブ——を 1 つの一貫したアプリに収めます。本当に枝分かれの多いアプリ（5 タブ × push × 4 モーダル様式）で
...
- **1 つの識別子契約、4 プロダクト。** 2 つの `-a11y` アプリは識別子/launch フック/deeplink を完全同一
  （byte-for-byte）に露出するので、`scenarios/*.yaml` がどちらのツールキットにも無改変で通る。`accessibilityID(...)`
