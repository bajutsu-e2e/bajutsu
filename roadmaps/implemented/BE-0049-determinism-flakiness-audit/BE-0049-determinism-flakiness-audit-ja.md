...
本項目はこの差別化を実体のあるものにします。「私たちは決定的だ」を、チームが実行できるレポートに変えます。シナリオを K 回実行し、結果やタイミングがばらついたステップやアサーションを表面化させ、各セレクタを安定性のはしご（一意に解決する `id` が `label` ／ `traits` に勝り、それらが `index` ／生座標に勝る）に照らして静的に採点し、タイムアウトのない待機や緩すぎる条件の待機を指摘します。これは `doctor` の規約スコア（[BE-0024](../BE-0024-doctor-onboarding/BE-0024-doctor-onboarding-ja.md)）を、「id は適切に命名されているか」から「このスイートは再現可能か」へと自然に広げるものです。
