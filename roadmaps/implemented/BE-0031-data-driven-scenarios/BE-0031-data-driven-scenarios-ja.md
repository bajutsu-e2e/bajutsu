...
**外部のテストマトリクスハーネス（ツールの外からパラメータ化する）。** 不採用。入力の表をシナリオファイルの外のラッパースクリプトに押し出し、「シナリオ YAML が共有のハブ」という前提と、入力をステップと並べてレビューできる利点を壊します。`data`/`dataFile` をシナリオ内に置けば、ケースはそれが動かすフローとともにバージョン管理・レビューできます。
