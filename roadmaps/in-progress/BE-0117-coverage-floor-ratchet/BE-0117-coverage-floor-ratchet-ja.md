...
- CLI テストの追加により、`lint.py` は 17.6% から 100% へ、`worker.py` は 23.3% から 97% へ、
  `mcp.py` は 40% から 100% へ、`trace.py` は 66.7% から 100% へ、`schema.py` は 71.4% から
  100% へ、`audit.py` は 71.9% から 81% へ（残る未カバーは実機が必要な `--repeat` 実行経路のみ）
  上がりました。`crawl.py` は実機を必要としないオプション検証の分岐をカバーしています（コマンドの
  残りは実機の actuator が必要です）。ブランチカバレッジの合計は 89.34% まで上がったため、
  `Makefile` のフロアを `--cov-fail-under=87` から `89` へ引き上げました（`85` までずれていた
  `docs/ci.md` の記述と、PR テンプレートの例も揃えました）。
