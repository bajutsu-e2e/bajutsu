...
- [ ] On-device validation — across the parallel device pool: a pinch/rotate scenario idb cannot run, and the idb graceful-degradation fallback, on the `e2e.yml` path — before the item flips to Implemented. Single-device `run` is validated (see log).
- [ ] Query performance — the runner's `/elements` walk reads each property over its own XCUITest round-trip (~12s on an 84-element screen), which forced the driver's socket timeout up to 30s. Replace the per-element reads with a single `XCUIApplication` snapshot walk before this ships as `Implemented`.
