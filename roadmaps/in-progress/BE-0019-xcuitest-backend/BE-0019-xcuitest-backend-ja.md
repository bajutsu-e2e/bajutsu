...
- [ ] 実機検証。並列デバイスプール全体で、idb にはできない pinch ／ rotate のシナリオと、idb へのなだらかな劣化のフォールバックを、`e2e.yml` 経路で確認します。項目を実装済みへ移す前に行います。単一デバイスの `run` は検証済みです（ログを参照）。
- [ ] クエリ性能。ランナーの `/elements` の走査は各プロパティを個別の XCUITest 往復で読むため（84 要素の画面で約12秒）、ドライバのソケットタイムアウトを 30 秒へ引き上げる必要がありました。実装済みへ移す前に、個別読み取りを `XCUIApplication` の単一スナップショット走査へ置き換えます。

スライスで構築しました。#366 / #368（ドライバーとチャネル）、#374 / #376（ランナーサーバーとエンドポイント）、#391 / #394（実行ライフサイクルの配線、事前条件）、#397（doctor のフォールバック）。
