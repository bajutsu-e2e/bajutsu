...
| 関連 | [BE-0106](../../implemented/BE-0106-post-completion-worker-model/BE-0106-post-completion-worker-model-ja.md) |
<!-- /BE-METADATA -->
...
- ワーカー死活監視とジョブ再投入。lease を更新するハートビート（`POST /api/worker/heartbeat`）、死んだワーカーの lease を再投入し試行回数の上限を超えたジョブを失敗にする `reclaim_expired_leases`（`lease_job` のたびに掃引）、そしてワーカー側のハートビートループを実装しました。あわせて、本項目の Tier B スタック、図、残作業の各項目を BE-0106 の Redis を使わない HTTP ワーカーモデルに合わせて再構成しました（[#TODO — PR 作成時に記入]）。
