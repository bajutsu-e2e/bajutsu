...
| 状態 | **実装中** |
| 実装 PR | [#103](https://github.com/bajutsu-e2e/bajutsu/pull/103), [#154](https://github.com/bajutsu-e2e/bajutsu/pull/154), [#365](https://github.com/bajutsu-e2e/bajutsu/pull/365), [#367](https://github.com/bajutsu-e2e/bajutsu/pull/367) |
...
- **ジョブ分配**：コントロールプレーンはサーバ側の `QueueExecutor` を介して Redis（RQ：Redis Queue）へ
  ...
  **⚠ [BE-XXXX](../../proposals/BE-XXXX-post-completion-worker-model/BE-XXXX-post-completion-worker-model-ja.md)
  で見直し中** — 完了後連携 worker モデルにより、Redis ブローカーとライブログバスを HTTP ベースのジョブ
  分配と完了後結果収集に置き換え、このスタックから `redis` コンテナを排除します。
...
- [x] Tier B の単一ノード。コントロールプレーンの compose スタック、GitHub OAuth + RBAC、組織間の分離、エフェメラルな Simulator を使う Redis RQ のジョブ分配、全体・ユーザーごとのクォータ（`deploy/self-host/`）。
...
- [ ] ワーカーの死活監視とジョブの再投入（RQ の liveness / ack-late）。
- [ ] コントロールプレーンのスケールアウト。ロードバランサー配下の N 個のアプリレプリカ。
- [ ] 高可用性。Redis Sentinel、Postgres のプライマリとレプリカ、冗長なロードバランサー。
