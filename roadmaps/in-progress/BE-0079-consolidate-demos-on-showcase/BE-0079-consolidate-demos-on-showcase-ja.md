...
- [ ] 残るパリティ（E.1）。不足する操作対象（ジェスチャー、デバイス・システム状態のミラー、コントロール）、証跡の一巡、最初の一見のスライス。
- [ ] 切り替え（E.2）。デモメニュー（`demos/Makefile`、`demos/demo.config.yaml`）と二言語のドキュメントを showcase へ向け直します。
- [ ] 退役（E.3）。レガシーの `demos/app/`・`demos/features/app/`・`demos/record/app/` と孤立した設定・スクリプトを削除し、最後にドキュメントを整理します。
