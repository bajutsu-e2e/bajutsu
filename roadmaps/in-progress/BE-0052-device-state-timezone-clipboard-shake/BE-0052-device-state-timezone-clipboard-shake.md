...
- [x] `setTimezone` — deferred until a reliable `simctl` actuation exists (shipping a command that does not take effect would violate determinism-first). Implementation triage confirmed no such actuation exists today: neither `simctl` nor `idb` has a `timezone` subcommand, and the Simulator's timezone is inherited from the host Mac rather than settable per device. A launch-time `SIMCTL_CHILD_TZ` only moves the app process's libc `localtime`, not the `TimeZone.current` most iOS date UI reads, so it would appear to succeed without changing the UI under test. Changing the host Mac's own timezone would work but is global, `sudo`-gated, and mutates the developer/CI clock across every booted Simulator — out of contract for a per-device primitive.
- [x] `shake` — deferred: there is no `simctl` / `idb` shake command (it is a Hardware-menu gesture). It can be triggered deterministically via GUI automation (AppleScript driving the Simulator's Device ▸ Shake menu item, or a third-party tool such as RocketSim), but that path needs the Simulator GUI running plus an Accessibility grant for the controlling process, is scoped to the focused Simulator, and cannot run in headless CI — so it does not fit as a `simctl`-level primitive today. RocketSim's own CLI does not expose shake (or timezone) either; only its `tap`/`swipe`/`type`/`button`/inspect commands are scriptable.
