...
- PR1（バッチ 1）：一時的な二層対応の走査（`roadmap_ids.iter_item_dirs` がフラットな直下と旧来の
  フォルダの両方を走査）を、索引・採番・フォーマット・同期・ダッシュボード・lint の各ツールに導入し、
  新規項目をフラットに生成するようにし、実装済みの項目をフラットな直下へ `git mv`（索引を再生成し、
  クロスリンクを修復）しました。`promote` は残るフォルダ内の項目を引き続き管理します。BE-0159 を
  `実装中` に移しました。
