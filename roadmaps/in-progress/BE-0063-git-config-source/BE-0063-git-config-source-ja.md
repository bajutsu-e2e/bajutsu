...
残り: 解決した SHA を `manifest.json` に **run の来歴**として記録すること（設計が約束する決定性のアンカー）、`--config-offline` / `--require-pinned-config` スイッチ、Git ソースでの `build` の作業ディレクトリ、そして serve の「Git から」ピッカーです。
