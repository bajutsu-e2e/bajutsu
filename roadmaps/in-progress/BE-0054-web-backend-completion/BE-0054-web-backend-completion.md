...
The first slice of the Web (Playwright) backend ([BE-0041](../../implemented/BE-0041-web-playwright-backend/BE-0041-web-playwright-backend.md))
...
| console / page-error log | — | `page.on("console")` / `on("pageerror")` → a `deviceLog`-equivalent kind |
| `multiTouch` (pinch / rotate) | `UnsupportedAction` | synthesized touch points → advertise `MULTI_TOUCH` |
| parallel lanes | single-lane (`workers = 1`) | N `BrowserContext` lanes in the pool's web branch |
...
- **Fold these back into BE-0041.** Rejected: BE-0041 has shipped its slice and is now
  *Implemented*; tracking the remaining rich-end work as its own item keeps the landed
