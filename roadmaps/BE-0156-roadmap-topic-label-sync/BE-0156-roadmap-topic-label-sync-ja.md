...
| 実装 PR | [#612](https://github.com/bajutsu-e2e/bajutsu/pull/612) |
...
2. **「この PR がロードマップ項目をどう変更したか」を「付け外しすべきラベル」に変換するスクリプ
   ...
   リネームの場合は変更前のパス）を受け取り、`roadmaps/proposals/`、`roadmaps/in-progress/`、
   `roadmaps/deferred/` 配下の項目ファイル（`-ja` ではない方）だけに絞り込んだうえで、まず PR に
   **あるべき** `topic:<key>` の集合を求めます。
...
3. **ワークフロー `.github/workflows/roadmap-topic-labels.yml` を追加します。**
   トリガーは `pull_request`（既定で `opened` / `reopened` / `synchronize`）で、パスを
   `roadmaps/proposals/**`、`roadmaps/in-progress/**`、`roadmaps/deferred/**` に絞ります。
   `roadmaps/implemented/**` は意図的に含めません（理由は「動機」を参照してください）。トークンが
   読み取り専用になるフォークからの PR は、`roadmap-promote` と同様にスキップします。手順は次の
   とおりです。
...
- [x] `scripts/sync_roadmap_topic_labels.py`：変更ファイル一覧から `topic:<key>` の add/remove
      ...
      再利用する。
- [x] `.github/workflows/roadmap-topic-labels.yml`：未実装以外のロードマップディレクトリ配下で
