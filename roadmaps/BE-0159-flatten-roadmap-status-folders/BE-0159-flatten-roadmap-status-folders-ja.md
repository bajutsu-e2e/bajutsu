...
- [x] 物理的な配置のフラット化：既存のすべての項目を 1 回の `git mv` でまとめて移動し、新しい
      （フラットな）パスで索引を再生成する。
- [x] `scripts/build_roadmap_index.py`：リンク生成からカテゴリ別ディレクトリを取り除く。生成される
      表の状態による分類そのものは変えない。
- [x] `scripts/promote_roadmap_items.py` / `roadmap-promote.yml`：まるごと廃止しました（スクリプト、
      ワークフロー、ゲートテスト、`make roadmap-promote` ターゲット）。移動先のフォルダがなくなり、残る
      「状態変更時の索引再生成」は `make roadmap-index` と索引のゲートテストがすでに担っているので、
      昇格の専用ステップは不要です。
...
- [x] `scripts/check_roadmap_format.py` / `scripts/roadmap_ids.py`：`_items()` の 4 フォルダ走査を
      単一ディレクトリの走査にまとめる。共有のフラット走査は `roadmap_ids.iter_item_dirs` に置き、
      6 か所に重複していた `CATEGORIES` ループを置き換えました。ID の形を見る述語自体は変えていません。
...
- [x] `scripts/sync_roadmap_tracking_issues.py`：`issue_body()` の `href` から `item.category` を
      取り除きました（現行バグの本丸の修正）。既存の開いているトラッキング Issue を直す一度限りの修復は
      マージ後の作業です。フラットなパスは、この変更が `main` に入って初めて解決するからです。
- [x] 文章：`CLAUDE.md`、`roadmaps/README.md` / `README-ja.md`、`docs/ai-development.md`（および
      `docs/ja/` のミラー）、`docs/roadmap-workflow.md`（および ja）、`ideation` / `implement-be`
      スキルのフォルダ全単射の規則を「単一ディレクトリ。状態が決めるのは索引の分類だけ」に書き換えました。
- [x] `.github/workflows/roadmap-proposal-approvals.yml`：`roadmaps/proposals/` のパス接頭辞による
      ゲート（フラット化によって無言で無効になるはずだったもの）を、状態による判定に置き換えました。PR
      head で変更された各項目ファイルの `Status` を読み、いずれかが `Proposal` のときに 2 承認を要求します。
      当初の作業分解にはなく、フラット化が引き起こす governance の回帰として実装中に浮かび上がったものです。
- [x] [BE-0154](../BE-0154-roadmap-promote-base-sha/BE-0154-roadmap-promote-base-sha-ja.md)
      に、`promote_roadmap_items.py` が廃止された（対象スクリプトがもう存在しない）ことを踏まえ、見直しか
      終了が必要である旨の印を付ける。
...
- [#625](https://github.com/bajutsu-e2e/bajutsu/pull/625)：151 項目すべてを `git mv` でフラットな `roadmaps/BE-NNNN-<slug>/` 構成へ移し、4
  ...
  ロードマップスキルを更新し、`make lint-roadmap --fix` で 1300 本以上のクロスリンクを修復しました。`make
  check` は緑です。フォローアップ：マージ後に既存のトラッキング Issue を `gh issue edit` で一度だけ修復し、
  BE-0154 を見直すか終了します。
