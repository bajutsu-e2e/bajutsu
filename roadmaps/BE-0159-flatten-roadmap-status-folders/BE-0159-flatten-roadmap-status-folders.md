...
- [x] Flatten the physical layout: one `git mv` sweep of every existing item, index regenerated at
      the new (flat) paths.
...
- [x] `scripts/promote_roadmap_items.py` / `roadmap-promote.yml` — retired outright (script,
      workflow, gate test, and `make roadmap-promote` target): with no folder to move to, the
      residual "reindex on Status change" is already what `make roadmap-index` and the index gate
      test cover, so no separate promote step is needed.
...
- [x] `scripts/check_roadmap_format.py` / `scripts/roadmap_ids.py` — collapse `_items()`'s
      four-folder walk to one directory. The shared flat walk lives in `roadmap_ids.iter_item_dirs`,
      which replaced the six duplicated `CATEGORIES` loops; the id-shape predicates are unchanged.
...
- [x] `scripts/sync_roadmap_tracking_issues.py` — drop `item.category` from `issue_body()`'s
      `href` (the concrete-bug fix). The one-time repair pass over already-open tracking issues is a
      post-merge action: the flat paths only resolve once this lands on `main`.
- [x] Prose — `CLAUDE.md`, `roadmaps/README.md` / `README-ja.md`, `docs/ai-development.md` (+ the
      `docs/ja/` mirror), `docs/roadmap-workflow.md` (+ ja), and the `ideation` / `implement-be`
      skills: replaced the folder-bijection rule with "one directory; `Status` decides only the
      index bucket."
- [x] `.github/workflows/roadmap-proposal-approvals.yml` — replace the `roadmaps/proposals/`
      path-prefix gate (which the flatten would have silently disabled) with Status-based detection:
      read each changed item file's `Status` at the PR head and require two approvals when any is
      `Proposal`. Not in the original breakdown; surfaced during implementation as a governance
      regression the flatten would otherwise cause.
- [x] Flag [BE-0154](../BE-0154-roadmap-promote-base-sha/BE-0154-roadmap-promote-base-sha.md)
      for revision or closing now that `promote_roadmap_items.py` is retired (its target script no
      longer exists).
...
- [#625](https://github.com/bajutsu-e2e/bajutsu/pull/625): `git mv`'d all 151 items to a flat `roadmaps/BE-NNNN-<slug>/` layout; collapsed
  ...
  detection; updated the bilingual prose and the two roadmap skills; and repaired 1300+ cross-links
  with `make lint-roadmap --fix`. `make check` green. Follow-ups: run the one-time `gh issue edit`
  repair over existing tracking issues after merge, and revise or close BE-0154.
