...
すでに作られている「call」フェーズのレポートは読めますが、**自分より後にteardownされる、別のフィクス
...
からです。フィクスチャは、自分自身の2つの`start_*`呼び出しがどちらも成功したあとになって初めて、自分の
ディレクトリをこのフックが掃除してよい対象として登録します。フィクスチャ自身のセットアップが失敗した
場合は、このディレクトリが未登録のまま残り、既定で掃除されずに済みます。成功したテストの実行では何も
アップロードされないため、既存の「Upload run artifacts」ステップの`if-no-files-found: ignore`はクリー
ンなスイートに対しては引き続き何もせず、実際の失敗が最初に起きたときだけ内容が入り始めます。
...
録画は、それを引き継いだ試行によって同じファイルパス上でもう上書きされています。各試行自身の「setup」
