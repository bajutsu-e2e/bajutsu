[English](BE-0291-xcuitest-runner-reuse-across-scenarios.md) · **日本語**

# BE-0291 — シナリオをまたいで XCUITest ランナーを再利用し、コールド起動を償却する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0291](BE-0291-xcuitest-runner-reuse-across-scenarios-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0291") |
| 実装 PR | [#1226](https://github.com/bajutsu-e2e/bajutsu/pull/1226) |
| トピック | プラットフォーム対応 |
...
### Unit 2 — 同一 actuator のシナリオ間での、アプリだけの引き継ぎ
...
- **起動を償却せずに iOS のデフォルトを XCUITest へ切り替える。** ランナーをシナリオごとに起動し直したまま XCUITest をデフォルトへ切り替えると、すべての iOS run でシナリオごとのコールド起動をまるごと払うことになります。これはまさに、コスト順の actuator 解決器（[BE-0240](../BE-0240-ios-capability-aware-actuator-selection/BE-0240-ios-capability-aware-actuator-selection-ja.md)）が idb を今なお安価なデフォルトにとどめている、そのコストです。先に起動を償却しておくことが、既知の後退を飲み込むのではなく、そのデフォルトを誠実に再検討できるようにします。
...
- **actuator によらず 1 つのウォームランナーを保持する。** 解決された actuator を無視する、デバイスごとに 1 つのランナーを考えます。この設計は idb シナリオをウォームな XCUITest ランナーに対して走らせてしまいます。その結果、run のマニフェストが記録する actuator を黙って変え、シナリオごとに 1 actuator という規則に反します（[BE-0240](../BE-0240-ios-capability-aware-actuator-selection/BE-0240-ios-capability-aware-actuator-selection-ja.md)）。キャッシュのキーを actuator にすれば、選択の主導権は解決器に残ります。
...
- [x] Unit 1 — `(udid, actuator)` をキーにした、デバイスごとのウォームランナーのキャッシュ。
- [x] Unit 2 — 既存の relaunch 経路による、同一 actuator のシナリオ間でのアプリだけの引き継ぎ。
- [x] Unit 3 — ランナーの所有権をプールへ移し、run set の終了、 actuator の切り替え、障害でティアダウンする。
- [x] Unit 4 — 死んだウォームランナーをキャッシュミスとして扱うクラッシュ回復。
- [x] Unit 5 — 償却を決定論的に証明しました（複数シナリオのスイートでランナーの spawn がデバイスごと
  1 回になることを `tests/runner/test_pool.py` / `tests/test_xcuitest_environment.py` で検証）。名前を
  付けた Simulator + Xcode 環境でのスイート単位の起動時間のウォールクロック計測は、fast gate が持たない
  実機での確認事項として残します。

**ログ**

- 再利用は、`Environment` シームへの 2 メソッドの追加（`has_reusable_resident` / `end_lease`。いずれも
  「ウォームな常駐なし」を既定値とする）、`runner/pool.py` の `(udid, actuator)` をキーとしたデバイスごと
  のウォームランナーキャッシュ、そしてランナーが健全なときはアプリだけを再起動する再開対応の
  `XcuitestEnvironment.start` として実装しました。これによりプールは actuator 非依存のままで、XCUITest
  以外のバックエンドは変わりません。対象は Simulator 経路に限定します（実デバイスの XCUITest と live
  WebDriver 環境はリースごとにコールドのままです）。詳細は実装 PR を参照してください。
...
- [BE-0240 — 能力を踏まえた iOS actuator 選択](../BE-0240-ios-capability-aware-actuator-selection/BE-0240-ios-capability-aware-actuator-selection-ja.md) — キャッシュのキーが尊重する、シナリオごとの actuator 解決。
