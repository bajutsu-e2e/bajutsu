...
対する純粋なオーケストレータのロジックであり、十分にユニットテストされています。実際に変化する要素ツリーの下での
...
実装済みの最適化のうち 2 つは、どの fake も動かせない前提の上に成り立っています。読み取り回数の削減
...
構造的に表現できません。生きてドリフトする UI の上のシナリオが、それらを観測できる唯一の場所です。

純粋なロジックをデバイスで再テストしても、何も足しません。要点は、実ハードウェアにしか存在しない 2 つの
性質、すなわち反復中に変化するツリーと、スナップショットのあいだの UI のドリフトを動かすことです。
...
- **これらの機能をユニットテストのレベルにとどめる。** 純粋なロジックは検証されていますが、`forEach` の
  変化するツリーでの挙動と、スナップショットのドリフトや再コンポーズのタイミングという前提は fake では表現できません。
