...
| 実装 PR | [#531](https://github.com/bajutsu-e2e/bajutsu/pull/531), [#636](https://github.com/bajutsu-e2e/bajutsu/pull/636) |
...
- [ ] Worker 側の HTTP PUT アップローダー（presigned URL モード）
- [x] 直接 SDK アップロードのフォールバック（スタンドアロン `bajutsu run` モード）
- [ ] `runner/pipeline.py` への run 完了後アップロードステップ追加（モード選択）— スタンドアロンは
      CLI 層（`run.py`、判定の後、`--zip` と同じ扱い）に配線済み。Worker の presigned アップロードは
      `bajutsu worker` の HTTP ループで run の後、`console.log` と並べて配線する（pipeline ではない）
...
- [x] `/api/runs` エンドポイントの `evidence_prefix` パラメータ
- [x] オプショナル依存の宣言（`s3` / `gcs` / `cloud` extras）
- [ ] テスト — スタンドアロンのスライス分（URI パース、S3/GCS ストア（`content_type` と presigned
      GET/PUT 生成を含む）、`upload_tree`）と serve エンドポイント分（`generate_upload_urls` の
      キー生成・検証、両方の HTTP シェル、`evidence_prefix` の受け渡し、ストアの配線）は完了。
      Worker の HTTP PUT アップローダーのテストは Worker スライスで入れる
- [ ] ドキュメント — `run --evidence-store` を記載済み（英語・日本語の `docs/cli.md`）。serve の
      トポロジのドキュメントは Worker スライスと一緒に入れる
...
- **スライス 2a — presigned な serve エンドポイント（サーバー側）**（実装中）: `POST /api/runs/<run_id>/upload-urls`
