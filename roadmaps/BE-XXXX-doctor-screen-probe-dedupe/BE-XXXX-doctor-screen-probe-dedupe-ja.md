[English](BE-XXXX-doctor-screen-probe-dedupe.md) · **日本語**

# BE-XXXX — doctor の画面プローブを CLI と serve で共有する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-doctor-screen-probe-dedupe-ja.md) |
...
- [ ] 環境注入と型付きエラーを備えた共有プローブを `bajutsu/doctor.py` に追加
- [ ] CLI 側アダプタ（typer.Exit への写像）を移行
- [ ] serve 側アダプタ（state.simctl、カンマ区切り udid）を移行
- [ ] 検査項目のずれを解消（serve パネルと CLI）、判断を記録
- [ ] 共有プローブのユニットテスト
