[English](BE-XXXX-record-enrich-shared-replay.md) · **日本語**

# BE-XXXX — record と enrich の重複したリプレイヘルパを統合する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-record-enrich-shared-replay-ja.md) |
...
独立に保守される複製は徐々にずれていきます。
BE-0132 が `record._screenshot_bytes` への統合で解消しました（`enrich.py` はすでにこれを
import しています）。本項目は、同じファイルペアに残る 2 ペアについて同じ統合を完了させる
ものです。
...
- [ ] wait 失敗フック付きの共有ステップリプレイ実行器を導入し、両呼び出し側を移行
- [ ] `_clear_blocking` を 1 実装（record の報告版）に統一し、enrich を移行
- [ ] 異なる 2 つの wait 失敗挙動を固定するユニットテスト
