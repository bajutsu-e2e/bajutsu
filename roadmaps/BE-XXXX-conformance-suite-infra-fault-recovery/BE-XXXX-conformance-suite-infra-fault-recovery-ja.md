[English](BE-XXXX-conformance-suite-infra-fault-recovery.md) · **日本語**

# BE-XXXX — 実機 conformance スイートに run パイプラインと同じインフラ障害からの復旧を持たせる

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-conformance-suite-infra-fault-recovery-ja.md) |
...
実機のドライバ conformance スイートは、この復旧を1つも受け継いでいません。そして `run` 以外で
**マージを止める**唯一の iOS ジョブでもあります。スイートは module スコープの `pytest` fixture から
`launch_driver` を直接呼んでデバイスを得るので、復旧を持つ run パイプラインは経路上にありません。iOS
ワークフロー自身がコメントでそう述べています。「`bajutsu run` ではなく pytest の実機ハーネスである」と。
