[English](BE-XXXX-serve-author-yaml-roundtrip.md) · **日本語**

# BE-XXXX — Author の YAML 編集を serializer で往復させる

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-serve-author-yaml-roundtrip-ja.md) |
...
アサーションと settle の wait を挿入する（Enrich の Accept）— YAML を JavaScript で手作業に、テキストとして編集します。
`bajutsu/templates/serve.author.js` の `auApply` と `enrichApply` は、textarea を改行で分割し、`trimStart()` や
...
正しさの利得に加え、親和性の利得（serializer が 1 つになり、「ステップの見た目」という概念が 2 つに割れない）でもあります。
...
2. **Apply（Edit）をモデル経由に。** `auApply` の行継ぎ足しを次に置き換えます。パースしたモデルでステップを特定し、selector を
   設定し、シリアライズする。`:`/`#` を含む値の引用符付けを serializer が所有すれば、selector→YAML ヘルパー（`auSelectorYaml`）と
   その手作業の引用符付けは不要になります。
