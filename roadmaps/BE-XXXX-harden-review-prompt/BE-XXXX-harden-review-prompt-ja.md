[English](BE-XXXX-harden-review-prompt.md) · **日本語**

# BE-XXXX — 調査に基づく方針で自動 PR レビュープロンプトを強化する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-harden-review-prompt-ja.md) |
...
- [ ] 「ゲートが見えないものをレビューする」という先頭原則を追加する（項目 1）
- [ ] 設計・アーキテクチャの観点を追加する（項目 2）
- [ ] コメント/docstring のずれ（コメント腐敗）の観点を追加する（項目 3）
- [ ] `Protocol`/抽象メソッドの docstring だけの本体を検出する観点を追加する（項目 4）
- [ ] 未エスケープな構造化データ補間の観点を、ゲートの及ばない JS テンプレートに絞って追加する（項目 5）
- [ ] テストの脆さ・フレーキーさの観点を追加する（項目 6）
- [ ] インラインの指摘に Conventional Comments のラベルを採用する（項目 7）
- [ ] 文言・用語の一貫性の観点を追加する（項目 8）
- [ ] 新しい観点それぞれを実際の PR で検証する（項目 9）
