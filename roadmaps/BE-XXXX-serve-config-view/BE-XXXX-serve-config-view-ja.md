...
計算する `source_provenance` の刻印です。ただ、それが表に出ていないだけです。これを表に出せば、「正しい
...
- **config の中身を返す serve オペレーションとエンドポイント。** 読み取りオペレーションが、bind した config の
  ...
  FastAPI アプリ）に `GET /api/config/content` として公開します。読み取りなので role による制限はなく、config が
...
- [ ] bind した状態への Git 由来の保持（Git bind で設定、ローカル / バンドルの bind でクリア、起動時から伝搬）。
- [ ] 中身、パス、パースした構造、由来を返す config-content の読み取りオペレーション。
- [ ] 両トランスポート（stdlib のハンドラと FastAPI アプリ）への `GET /api/config/content`。
- [ ] UI：Structured / Raw トグル付きの View ボタンとビューアのモーダル。
- [ ] ドキュメント（英日）がビューアと由来の行を説明する。
