...
は `make -C demos/showcase runner-build` もランナーのパス指定も不要になり、バックエンドが設定なしで
動きます。明示的な
...
Hatchling は、`bajutsu` パッケージツリーの内側に置かれた非 Python ファイルを、`force-include` の指定
なしにすでに既定で含めています（`pyproject.toml` に `templates/` 向けの `force-include` 設定が存在
しないことがその証拠です）。products ディレクトリを wheel のビルドが走る前に `bajutsu/_xcuitest_runner/`
の下へ置けば、同じ既定の仕組みで取り込まれます。`force-include` が要るのは、リリースパイプラインが
パッケージツリーの外でビルドして流し込む場合だけです。同梱するのは Simulator ランナーだけです。
...
対象に動きます。デバイスプールは1つの run set の中で複数デバイスを並行にリースするため
（[BE-0291](../BE-0291-xcuitest-runner-reuse-across-scenarios/BE-0291-xcuitest-runner-reuse-across-scenarios-ja.md)
のデバイスごとのランナーキャッシュも同じ並行性に依拠しています）、まだ何も入っていないバージョン鍵付きの
キャッシュディレクトリへ複数のリースが同時に届くことがあります。展開は、キャッシュの隣に作った一時
ディレクトリへコピーしたうえで、それをキャッシュの場所へ atomic rename する形を取ります。これにより、
並行して読みに来た側は、キャッシュディレクトリがまだ存在しない状態か、完全に展開し終わった状態の
どちらかしか見ません。
...
- [ ] **ランナー解決**：XCUITest 環境で `testRunner`/`build` の下に同梱デフォルトの段を加えます。明示的な
  config は引き続き優先し、`deviceType: device` は引き続き明示ランナーを要求します。
- [ ] **キャッシュへの展開**：同梱 products をバージョンごとの書き込み可能なキャッシュへコピーし、
  `testRunner` をコピー側へ解決します。run ごとのパッチ済みコピー手順はそのままにします。
- [ ] **パッケージング**：ビルド済みの Simulator products を、wheel のビルドが走る前に package data
