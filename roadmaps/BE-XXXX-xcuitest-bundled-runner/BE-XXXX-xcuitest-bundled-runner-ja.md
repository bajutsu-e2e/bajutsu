[English](BE-XXXX-xcuitest-bundled-runner.md) · **日本語**

# BE-XXXX — XCUITest ランナーを同梱して testRunner を省略可能にする

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-xcuitest-bundled-runner-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **提案** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-XXXX") |
| トピック | プラットフォーム対応 |
| 関連 | [BE-0019](../BE-0019-xcuitest-backend/BE-0019-xcuitest-backend-ja.md)、[BE-0288](../BE-0288-ios-device-signing-batch-build/BE-0288-ios-device-signing-batch-build-ja.md) |
<!-- /BE-METADATA -->
...
は `make runner-build` もランナーのパス指定も不要になり、バックエンドが設定なしで動きます。明示的な
`testRunner` や `build` を書けば従来どおり同梱ランナーより優先され、Bajutsu が同梱できない署名付き
ランナーを要する実機の run は、これまでどおり明示指定のままです。
...
このずれは、XCUITest を初めて使うときの手間として現れます。利用者は `make runner-build`（または同等の
...
はランナーを2段で解決します。`xcuitest.testRunner` のファイルが存在すればそれを使い、なければ
`xcuitest.build` を走らせてそのパスが生成されることを期待し、どちらもランナーを得られなければ明確に
失敗します。本項目は両者の下に第三の段を加えます。Simulator の run が、使える `testRunner` と `build`
のどちらも欠く状態で解決に達したとき、環境は失敗する代わりに同梱ランナーへ解決します。優先順位は明示
指定がデフォルトに勝つ形を保ちます。既存の `testRunner` パスが最優先、次に `build` コマンド、そして
同梱ランナーが、よくある場合には設定を一切要らなくするフォールバックです。解決は LLM と固定 sleep の
いずれも持たない決定論的なファイルパスの判断のままなので、prime directive 1 と 2 には触れません。
アプリに
依存しない成果物をターゲットごとの設定から外すことは、directive 3 を曲げるのではなく強めます。
...
Hatchling は `bajutsu` パッケージ配下の非 Python ファイルをすでに含めています（`templates/` の資産が
その証拠です）。products ディレクトリも同じ仕組みで force-include します。同梱するのは Simulator ランナー
だけです。実機ランナーは操作者の Apple Developer team による署名を要し
...
products ディレクトリをバージョンごとの書き込み可能なキャッシュ（たとえば
`~/.cache/bajutsu/xcuitest-runner/<version>/`）へ展開し、`testRunner` をキャッシュ側のコピーへ解決
します。コピーは Bajutsu のバージョンで鍵付けするので、アップグレードで更新され、暖まったキャッシュは
再コピーせず再利用します。run ごとのパッチ済みコピーと unlink は、site-packages ではなくキャッシュを
対象に動きます。
...
- **高速ゲート（端末なし）。** 解決を、注入した偽の同梱ランナーディレクトリでユニットテストします。
  ...
  実機ランナーエラーで失敗すること、キャッシュへの展開が一度だけコピーしてバージョン鍵付きの暖まった
  キャッシュを再利用することを確かめます。端末も LLM もゲートに載せません。
...
- **ビルド済み products をリポジトリにコミットする。** ビルド済み products をバージョン管理に置けば、
  ...
  wheel には同梱できます。
- **ランナーを別の同伴パッケージとして配布する。** extra としてインストールする macOS 専用の
...
- [ ] **ランナー解決**：XCUITest 環境で `testRunner`/`build` の下に同梱デフォルトの段を加える。明示的な
  config は引き続き優先し、`deviceType: device` は引き続き明示ランナーを要求する。
- [ ] **キャッシュへの展開**：同梱 products をバージョンごとの書き込み可能なキャッシュへコピーし、
  `testRunner` をコピー側へ解決する。run ごとのパッチ済みコピー手順はそのままにする。
- [ ] **パッケージング**：ビルド済みの Simulator products を package data ディレクトリの下に
  force-include し、それを生成するリリースパイプラインのビルド手順を加える。基底の wheel は Linux に
  インストールできる状態を保つ。
- [ ] **doctor / 開示**：解決したランナーの出所を報告し、Xcode/SDK の不一致を上書きの逃げ道とともに
  知らせる。
- [ ] **検証**：高速ゲートの解決テスト(同梱デフォルト、上書きの優先、実機エラー、キャッシュ再利用)と、
  config に `testRunner` を持たない実機 e2e run。
