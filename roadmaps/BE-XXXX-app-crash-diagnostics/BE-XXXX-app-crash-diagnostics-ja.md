...
理由はどちらも同じ1つの規則に行き着きます。端末側のレンダリングをホスト側でエポックへ
解決してはなりません。その変換は端末の時区ではなくホストの時区を通るため、ホストとは
別の時区で動く UTC のエミュレータを駆動すれば、あらゆる新しいタイムスタンプが目印から
...
2つ目のレンダリング（`'+%Y-%m-%d %H:%M:%S'`）と比較し、エポックの `launched_at()` とは
比較しません。`logcat -t` もこの同じエポック値を消費できませんが、こちらはもっと単純な
...
読み取りを取り、そのパイプ区切りの3つのフィールドをホスト側で分割します（プレーンな
文字列分割であり、時区の解決ではないため、上の規則はそのまま成り立ちます）。3回別々に
...
- [ ] Unit 4 — Android：`backends.make_driver` から `AdbDriver.__init__` へ、`fetch_clock` と
      ...
      ウォールクロックのレンダリングであり、エポックではありません——を、Unit 5 自身の
      組み合わせ済みの起動の瞬間の読み取りが持つ2つ目のフィールドと比較します。
      ...
      からです。
- [ ] Unit 5 — Android：各起動の箇所での `AndroidEnvironment.app_launched_at`。
      `adb shell date '+%s|%Y-%m-%d %H:%M:%S|%m-%d %H:%M:%S.000'` という**1回**の組み合わせ
      読み取りから得て、そのパイプ区切りの3つのフィールドをホスト側で分割します
      （プレーンな文字列分割であり、時区の解決ではありません）——エポック
      （`app_launched_at` 自身）、exit-info ポーリング向けのレンダリング（Unit 4）、
      `logcat -t` 向けのレンダリングです。3回別々の `date` 呼び出しからではなく、まとめて
