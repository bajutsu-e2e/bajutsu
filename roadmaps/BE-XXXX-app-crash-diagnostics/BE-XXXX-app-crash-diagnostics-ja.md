...
`'%Y-%m-%d %H:%M:%S'` の device-clock レンダリングの両方——をその場で読む注入された
...
隣に記録された `'%Y-%m-%d %H:%M:%S'` の device-clock レンダリング——以降であるまで
...
2つ目のレンダリング（`'+%Y-%m-%d %H:%M:%S'`）と比較します。これは `AdbDriver` が
受け取る `launched_at()` タプルのレンダリング側であり、エポック側ではありません。
...
- [ ] Unit 4 — Android：`backends.make_driver` から `AdbDriver.__init__` へ、`fetch_clock` と
      ...
      組み合わせ読み取りが作る exit-info 用のレンダリングも含めてその場で読みます。下の
      exit-info の比較にはレンダリングが要り、`float` ではそれを運べないためです。
      ...
      ウォールクロックのレンダリングであり、エポックではありません——を、`launched_at()`
      タプルのレンダリング側と比較します。エポック側とは比較しません。`timestamp=` を
      ホスト側でエポックへ解決すれば、ホスト自身の時区を通ってしまい、この設計がそもそも
