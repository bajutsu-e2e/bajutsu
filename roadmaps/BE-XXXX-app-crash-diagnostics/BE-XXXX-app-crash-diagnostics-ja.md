...
——`ApplicationExitInfo` 自身の、端末の時区によるウォールクロックのレンダリングであり、
オフセットを持ちません——が、この起動の瞬間を表す3つ目の device-clock レンダリング
（`adb shell date '+%Y-%m-%d %H:%M:%S'`）以降であるまで読み直します。エポックの
`launched_at()` とは比較しません。生の `timestamp=` をホスト側でエポックへ解決すれば、
ホスト自身の時区を通ってしまい、ホストと別の時区で動く端末では常識的な構成のもとで
すべてのタイムスタンプを目印から何時間もずらしてしまいます——この設計がそもそも
避けているはずの時計のすり合わせを黙って呼び戻すことになります
（今回の起動より前の古い項目を除外するため
...
として消費します。残る2つの消費者はどちらもこの同じエポック値を使えません。理由はどちらも
同じ1つの規則に行き着きます。端末側のレンダリングをホスト側でエポックへ解決しては
なりません。その変換は端末の時区ではなくホストの時区を通るため、ホストとは別の時区で
動く UTC のエミュレータを駆動すれば、あらゆる新しいタイムスタンプが目印から何時間も
離れた位置へずれてしまい、この設計がそもそも避けているはずの時計のすり合わせを
黙って呼び戻してしまいます。`dumpsys activity exit-info` の `timestamp=` フィールドは
`ApplicationExitInfo` 自身の、端末の時区によるウォールクロックのレンダリングであり、
オフセットを持ちません。そこで上の exit-info ポーリングは、これを起動の瞬間の3つ目の
レンダリング（`adb shell date '+%Y-%m-%d %H:%M:%S'`。各起動箇所で他の2つの隣に記録
します）と比較し、エポックの `launched_at()` とは比較しません。`logcat -t` もこの同じ
エポック値を消費できませんが、こちらはもっと単純な理由からです。`adb logcat -t` は
オーバーロードされており、整数の引数は
「もっとも新しい N 行」という*行数*として読まれ、時刻の境界としては読まれません。
`'MM-DD hh:mm:ss.mmm'` という引用符付きの文字列だけが時刻として読まれ、これは exit-info の
レンダリングとも別物です（`logcat` 側には年がなく、exit-info 側にはあります。どちらも
互いの代わりにはなりません）。そこで各起動
箇所は、同じ瞬間のもう1つのレンダリング、`adb shell date '+%m-%d %H:%M:%S.000'` も記録し、
...
クラッシュも、このシナリオ自身のものとしてすり抜けてしまいます。収集する層は2つです。
両方を取得するという、本項目自身が
決めた対象範囲に合わせたものですが、同じ呼び出しからでも、同じ時点でもありません。
...
1. **`logcat` のクラッシュバッファ**は、常に試みます。昇格した権限は要りません。
   ...
   前述の `logcat` 用にレンダリングした目印を（エポックの `app_launched_at` ではなく）
...
- [ ] Unit 4 — Android：`backends.make_driver` から `AdbDriver.__init__` へ、`fetch_clock` と
      ...
      exit-info <package>`（最新の項目のみ。その `timestamp=` フィールド——端末の時区による
      ウォールクロックのレンダリングであり、エポックではありません——を、起動の瞬間の
      3つ目の device-clock レンダリング（`adb shell date '+%Y-%m-%d %H:%M:%S'`）と比較します。
      エポックの `launched_at()` とは比較しません。`timestamp=` をホスト側でエポックへ解決
      すれば、ホスト自身の時区を通ってしまい、この設計がそもそも避けているはずの時計の
      すり合わせを呼び戻してしまいます）で裏付けます。iOS の
...
- [ ] Unit 5 — Android：各起動の箇所での `AndroidEnvironment.app_launched_at`（端末自身の
      時計、`adb shell date +%s`、エポック値）。同じ瞬間のもう1つのレンダリングを exit-info
      ポーリング向けに（`adb shell date '+%Y-%m-%d %H:%M:%S'`、Unit 4）、さらにもう1つを
      `logcat -t` だけ向けに記録します（`adb shell date '+%m-%d %H:%M:%S.000'`）。
      `logcat -t` は整数の引数を行数として読み、時刻の境界としては読まないため、
      エポック値をそのまま通すことはできません。`dumpsys activity exit-info` 自身の
      `timestamp=` もまた別のレンダリングです（端末の時区によるもので、`logcat` の形式には
