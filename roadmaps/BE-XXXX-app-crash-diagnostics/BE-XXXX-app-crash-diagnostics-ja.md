...
守るはずが効きすぎてしまいます。`_finish_outcome` は代わりに、確定した*ふつうの*
outcome が `outcome.ok is True` を返した時点でこのフラグを解除します。`if`・`forEach`
自身のラッチする outcome は決して数えません——その成功自体が何も証明しないためです。
...
フィクスチャ——`preconditions.launchEnv` を通して起こす、メインスレッドでの未捕捉の
例外——は、`e.launch` のあとに記録した目印がまさに取り逃してしまうクラッシュです。
ホストの時計ではなく、
...
- [ ] Unit 7 — `run_scenario` / `_step_runner.py`：新しい `_finish_outcome` ヘルパーを、
      ...
      以降のあらゆる確認まで黙らせてしまうからです。`_finish_outcome` は代わりに、確定した
      *ふつうの* outcome が `outcome.ok is True` を返した時点でこのフラグを解除します——
      `if`・`forEach` 自身のラッチする outcome は除きます。空の `else` 分岐やゼロ件の
      一致でも `ok=True` は確定してしまい、その先のアプリが問い合わせに答えなくても
      送出しないため、死んだアプリ自身の SpringBoard だけのツリーが、まさにそうあっては
      ならない理由でこのフラグを解除してしまいます。本物のアクチュエーション・`wait`・
