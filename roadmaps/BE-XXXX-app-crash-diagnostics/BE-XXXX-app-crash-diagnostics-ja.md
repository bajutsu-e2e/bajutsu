...
守るはずが効きすぎてしまいます。`_finish_outcome` は代わりに、`outcome.ok is True` の
確定した outcome を一つでも見た時点でこのフラグを解除します——実際に成功したステップこそ
が、シナリオの最初のステップに欠けていたまさにその肯定的な観測だからです。これにより、
最初に失敗するステップへの保護を保ったまま、たまたま弱い readiness の判定しか持たない
ターゲットで以降のあらゆる確認を黙らせてしまうことを避けられます。その最初の成功が
...
- [ ] Unit 7 — `run_scenario` / `_step_runner.py`：新しい `_finish_outcome` ヘルパーを、
      ...
      以降のあらゆる確認まで黙らせてしまうからです。`_finish_outcome` は代わりに、
      `outcome.ok is True` の確定した outcome を一つでも見た時点でこのフラグを解除します。
      実際に成功したステップが供給する、その肯定的な観測をもってです。この Unit では `ReadinessResult` 自身の
