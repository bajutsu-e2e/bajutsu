...
- [ ] `DeviceProvider` / `DeviceLease` の protocol
- [ ] local provider（既定、挙動不変）
- [ ] provider の registry（`kind` キー、fail-closed）
- [ ] config の表面（`targets.<name>` 上の `deviceProvider`、遅延解決）
- [ ] `RunEnvironment` のクラウド用フック（起動待ち省略 / 任意インストール / capability 縮退）
- [ ] テスト（fake の provider）
- [ ] ドキュメントとスコープ更新（DESIGN §1、README「Not adopting」）
