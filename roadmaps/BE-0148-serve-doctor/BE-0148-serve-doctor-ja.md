...
- [ ] serve のジョブとしてチェックを実行し、runnability の結果と規約スコアを返す
      `POST /api/doctor`（`{target, udid?, backend?}`）エンドポイントを追加する
- [ ] それ単独でも、Record と Replay の実行前チェックとしても開ける準備状況パネルを追加する
- [ ] パネルをプラットフォーム対応にする（web ターゲットでは Simulator 専用のコントロールを隠す）
