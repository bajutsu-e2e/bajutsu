...
`default` のエントリ自体を宣言しないのが通常だからです。`targets_for_org` を経由することで
変わる挙動は1つだけありますが、その変化は整合性を高める方向のものです。`default` という名前の
org を、独自の `targets:` 付きで
...
`targets` だけを宣言したエントリは、空の名簿で引き込むのではなく、引き込みの手前で除きます。運用者
が config を削ぎ落とす順序によってカットオーバーの結果が変わらないようにするためです。ドキュメント
は `targets:` だけを残すよう案内していますが、その時期には触れていません。したがって、先に削ぎ落と
した行為が、その org を「誰も受け入れない」状態で固定してしまってはいけません。そうしたエントリを
引き込めば、まさにそれが起きます。誰も書いていない名簿でその行のマーカーを使い切り、しかも上記の
警告は `targets:` だけのエントリを報告しないので、そのことを何も告げません。`members:` を書き戻して
も届かず、Orgs ページで名簿を手入力し直すほかに戻る道がなくなります。除いた場合は行自体が作られま
せんが、これはユニット1の論理がそのまま許すところです。target の所有情報を解決するのは config で
あって、テーブルではありません。したがって、後からメンバーシップを宣言した config は、順序に
かかわらずその org を引き込めます。

アップロードをバインドする3つの経路（`bind_upload_config`、3点セットを合成するバインド、
`activate_uploaded_project`。いずれも
[`bajutsu/serve/operations/upload.py`](../../bajutsu/serve/operations/upload.py)）は、各所が引き込み
の呼び出しを覚えておくのではなく、`bind_upload_and_seed` という1つのヘルパー経由で引き込みへ到達し
ます。どこか1つが忘れると、両方向とも無言だからです。バインド時には何も警告されず、運用者が最初に
気付くのはサインインでメンバー全員が弾かれるテナントを見たときです。
...
- [x] 6 — バインドされた設定ファイルの `orgs:` ブロックをデータベースへ引き込む処理を追加する。
      ...
      起動時・再バインド時に警告する。`targets` だけを持つエントリは、警告の対象外とし、引き込み
      自体も行わない。最初の引き込みより前に config を削ぎ落としても、その org が「誰も受け入れな
      い」状態で固定されるのではなく、未引き込みのまま残るようにする。
