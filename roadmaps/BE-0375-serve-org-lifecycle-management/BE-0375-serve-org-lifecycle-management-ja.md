...
`orgs` テーブルに有効な行が存在しないこと。GitHub がこの login に対して Organization を1つも
返さなかったこと。そして、実在する名簿にマッチしなかったことです。1つめは、何ひとつ作成されず、
引き込みもされていない状態であり、ユニット7の迂回が存在するのはこの形のためです。WARNING は、
運用者が手を打てる1つ、すなわちテーブルが空であることのために取っておきます。
