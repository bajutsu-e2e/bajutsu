...
- [x] 実際のデバイスを 2 台同時に起動し、`--workers 2` を実行する（`android-e2e.yml` のエミュレータ）。
  `ios-e2e.yml` の Simulator 側は着地させたあと取り下げました。ホスト提供の macOS runner は
  Simulator を 2 台起動した状態を保てません（実測はログの最後の項目に記録しています）。
- [x] `udid` と `run_dir/<scenario_id>` サブディレクトリのワーカーごとの分離を検証する。
- [x] まずゲート対象外として着地させる。
- [ ] 安定を確認してから Android レーンを必須化する。
...
- [#1732](https://github.com/bajutsu-e2e/bajutsu/pull/1732) — iOS 側の `pool (xcuitest)` を取り下げ、このジョブしか使っていなかった配線もあわせて外しました。
  **5 回の実測**は、4 回の崩壊と 1 回の判定です。最初の 2 回は、当初のシナリオセットを動画記録と
  ...
  3 回目はキャプチャを軽くした構成で走り、11 分の沈黙のあと、どのシナリオにも到達しないまま
  `simctl uninstall` で固まりました。4 回目はシナリオに到達し、`SimRenderServer` が自身の
  ディスパッチキューでクラッシュしました。`/screenshot` は 2 つのワーカーで 1 秒とたがわず
  タイムアウトしています。5 回目は、分離の判定が許す最小のセット（リーフステップ 10 個、動画なし、
  タッチマーカーなし）で通りました。4 回の赤のあとの 1 回の緑であり、安定の記録ではありません。
  **削減が尽きた理由**：削るたびに失敗はなくならず、場所が移りました。3 回目の実行では、レーンが
  手放せるどのキャプチャよりも上流で CoreSimulator が固まっています。実行ループがリーフステップ
  ごとに記録する `before.png` / `after.png` は、capture の一覧では落とせません（BE-0341）。
  そのためシナリオセットが最後のレバーでしたが、いまはデバイスをまたぐ重なりの検査が要求する
  下限にあります。BE-0361 はこの runner を 3 コア・7 GiB と実測しており、Simulator を **1 台**
  起動した時点で物理メモリの未使用分は 189 MB しか残りません。残るレバーはより大きな runner だけで、
  それは有料であり、ここでは範囲外です。**その間のコスト**：赤くなるたびに 10 倍課金の runner を
  30 分ほど使いました。しかもこのジョブは `touches_pool` で発火し、その対象には runner パッケージ
  そのものが含まれます。つまりコストは、プールを変更する PR、すなわちこの赤をもっとも無視しにくい
  PR にかかっていました。**ジョブとともに外したもの**：`demos/showcase/showcase.pool.config.yaml`、
