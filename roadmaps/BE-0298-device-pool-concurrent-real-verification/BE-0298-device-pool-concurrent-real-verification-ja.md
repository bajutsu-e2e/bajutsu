...
- [#1666](https://github.com/bajutsu-e2e/bajutsu/pull/1666) — 両レーンをゲート対象外の
  ...
  より分かれるだけなので、渡された台数より*少ない*デバイスで走った run だけを失敗させます。**iOS**：`pool (xcuitest)` は Simulator を
  ...
  スナップショットのマシン構成は、保存時の構成と一致しなければならないからです。iOS 側が入力で
  ...
  そして run には、同じシリアルが 2 つ、あるいは何も確かめていないデバイスが渡ってしまいます。**変更
