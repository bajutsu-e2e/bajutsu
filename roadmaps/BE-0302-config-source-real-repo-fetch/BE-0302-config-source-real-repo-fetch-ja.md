[English](BE-0302-config-source-real-repo-fetch.md) · **日本語**

# BE-0302 — config source の実リポジトリ fetch 検証

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0302](BE-0302-config-source-real-repo-fetch-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **提案** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0302") |
| トピック | config の取得元 |
...
- **実際の使い捨てテスト用リポジトリ**：小さく安定した、低権限の公開リポジトリ（あるいは、
  ...
  fetch そのものに認証情報を必要とせず、そこで personal access token（PAT）を使うのはあくまで認証済みの高いレート制限
...
- **恒久的にゲート対象外とする**：
  ...
  の、新しいバックエンドのカバレッジをまずシグナルとして着地させる前例に従います。ただし
  BE-0282 自身の `network (playwright)` ジョブ（シークレットを必要としないため、フォークからの
  ものを含めどんな `pull_request` でも安全にトリガーできます）とは違い、このジョブは実際の
  認証情報を必要とするため、トリガーは `main` への `push`（または `schedule`）のみとし、
  `pull_request` では実行しません。`roadmap-id.yml` が `AUTOMATION_BOT_PRIVATE_KEY` /
  `AUTOMATION_BOT_APP_ID` をフォークからの実行にさらさないためにすでに使っているトリガー制限と
  同じ考え方です。マージ後にしか走らないチェックは `main` のコミットに対して報告され、PR の
  head SHA に対してではないため、BE-0282 のジョブのようには必須ステータスチェックになり
  得ません。これは必須化へ向けた途中経過ではなく、この設計が恒久的にとる形です。認証情報を
  PR トリガーの実行にさらす以外に、それを変える方法はないからです。
