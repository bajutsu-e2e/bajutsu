...
| 実装 PR | [#1069](https://github.com/bajutsu-e2e/bajutsu/pull/1069) |
| トピック | コードベース品質・技術的負債 |
<!-- /BE-METADATA -->

## はじめに

コードベースの各所に、ほぼ同一かまったく同一の小さなロジックが四つ重複しています。実 driver
に存在する単発判定の `wait_for` の本体、三つの driver に重複するフレーム中心座標の
...
`wait_for` は実 driver の間で同一です。`coordinate_tree.py`（BE-0254 で idb と adb が継承する
共有基底）、`bajutsu/drivers/xcuitest.py`、`bajutsu/drivers/playwright.py` のいずれも、本体は
`return len(base.find_all(self.query(), sel)) >= 1` という一文だけです。このメソッドは、
共有の `base.wait_until` が持つ締め切りポーリング（BE-0118）の内側にある単発判定であり、
その正しさは決定性に直結します。driver 適合性テスト（BE-0114）はすでにすべての backend の
挙動が一致することを検証していますが、一部のコピーだけを直して別のコピーを直し忘れても、
テストが気づくまでは静かに乖離が生まれます。これは、backend ごとの挙動の乖離を排除しようとする
...
これらはいずれも、今日時点では挙動のバグではありません。共有された `wait_for` の本体も、三つの
...
- **`wait_for` を `drivers.base` へ集約する。** 現在の単発判定
  ...
  `base.default_wait_for(driver: Driver, sel: Selector) -> bool` を追加し、各 backend の
  `wait_for` からこれを呼び出すようにします（`return base.default_wait_for(self, sel)`）。
  BE-0254 で共有基底 `CoordinateTreeDriver` が導入されたため、idb と adb はそれぞれが独自の
  `wait_for` を持つのではなく、そこで一つの `wait_for` を継承します。したがって呼び出し箇所は
  `coordinate_tree.py`（両方の座標 backend を担う）、`xcuitest.py`、`playwright.py` です。各
  backend は `Driver` プロトコルが要求する `wait_for` メソッド自体は引き続き持つため、将来、
  単発判定＋ポーリングという共有の契約を介さずネイティブに待機できる backend が現れても、その
  driver は自分の `wait_for` を上書きするだけで済みます。移動するのはデフォルトの本体だけで、
  プロトコルの形は変わりません。
...
- [x] `base.default_wait_for` を追加し、各 backend の `wait_for` からこれを呼び出すように
      する。呼び出し箇所は `coordinate_tree.py`（BE-0254 で idb と adb が継承する共有基底）、
      `xcuitest.py`、`playwright.py`。
- [x] `base.frame_center` と `base.gesture_anchor` を追加し、呼び出し元をこれら経由に
      書き換える。`idb.py`、`adb.py`、`playwright.py` に加え、提案の棚卸しが見落としていた
      frame-center 計算の五つ目のコピーを持つ `orchestrator/actions/handlers/gestures.py` も
      対象に含めました。
- [x] `_no_net` と `_no_network` を一つの定義に統合し、`runner/pipeline.py` と
      `orchestrator/loop.py` の双方からインポートするようにする。
- [x] `cli/_shared.py` が `config_source.DEFAULT_CONFIG` を再定義せず再エクスポートするように
      する。

ログ:

- [#1069](https://github.com/bajutsu-e2e/bajutsu/pull/1069) — 四つの単位すべてを、振る舞いを
  変えない一つの PR にまとめました。`base.default_wait_for` / `base.frame_center` /
  `base.gesture_anchor` を追加し、各 backend と `gestures.py` をこれら経由に書き換えています
  （BE-0254 の `CoordinateTreeDriver` へリベースしたため、idb と adb はこの共有基底を通じて
  `wait_for` を委譲し、それぞれが独自に持つことはありません）。`_no_network` と
  `DEFAULT_CONFIG` は単一の所有者へ統合しています。
...
- [`bajutsu/drivers/base.py`](../../bajutsu/drivers/base.py) — 本項目が集約したヘルパーを
  ...
  `wait_until`）。
- [`bajutsu/drivers/coordinate_tree.py`](../../bajutsu/drivers/coordinate_tree.py)（BE-0254 で
  idb と adb が継承する共有基底）、
  [`bajutsu/drivers/xcuitest.py`](../../bajutsu/drivers/xcuitest.py)、
  [`bajutsu/drivers/playwright.py`](../../bajutsu/drivers/playwright.py) — 本項目が集約する、
  同一な `wait_for` 本体。
