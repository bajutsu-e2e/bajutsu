[English](BE-0319-xcuitest-cold-spawn-resilience.md) · **日本語**

# BE-0319 — XCUITest のコールド runner 起動を診断可能で自己修復的にする
...
| 実装 PR | [#1327](https://github.com/bajutsu-e2e/bajutsu/pull/1327)、[#1403](https://github.com/bajutsu-e2e/bajutsu/pull/1403) |
...
作業は独立した 6 つの単位に分かれます。単位 1 と 2 は PR #1299 の捕捉の接合部
（`_open_runner_output` / `_runner_log_hint`）の上に直接立ち、単位 3〜5 は新規です。単位 6 は、
単位 1〜5 が PR #1327 で出荷されたあとに CI が表に出した、残りの問題を解きます。
...
6. **コールド起動の待機を、上限の満了ではなく、テスト実行の終了で打ち切る。** 単位 1〜5 の出荷後
   も、`actuation (xcuitest)` と `bundled-runner (xcuitest)` の 2 つのレーンは、シナリオが 1 つも
   走らないうちに失敗し続けました。引き金は、単位 3 と単位 4 の届かない種類のもの、すなわち
   Simulator のアプリ起動タイムアウトです。`xcodebuild` は
   `Failed to launch <bundle id>: Timed out attempting to launch app` を報告し、XCTest の
   スイートは終了しますが、`xcodebuild` のプロセスはそのあとも居座ります。テスト実行ではなく
   プロセスを見張る単位 3 の死活確認は、生きたハンドルを見続けるため、待機は上限まで走ります。
   PR [#1398](https://github.com/bajutsu-e2e/bajutsu/pull/1398) で観測された 2 件の失敗では、
   スイートが失敗を報告し終えたあと、300 秒の上限のうち 225 秒と 127 秒が空費されていました。
   この空費が、単位 4 の再試行を届かなくしていた仕組みです。上限は試行のあいだで共有される予算
   なので、上限まで走った試行は再試行に予算を残しません。その結果、再試行が吸収するために存在
   するはずの失敗こそが、再試行の決して届かない唯一の失敗になり、発生のたびに
   `attempt 1/2: health never ready` が 1 行だけ上がっていました。単位 1 が既定で有効にした捕捉
   から、実行の終了を示す印を読み取り、そこで待機を終えます。アプリ起動タイムアウトは数分では
   なく数秒で失敗するようになり、使わずに済んだ予算が再試行を賄います。
...
- **health の待機とは別に、runner の内部でアプリ起動を再試行する。** Swift の runner が自身の
  `XCUIApplication.launch()` のタイムアウトを捕まえ、`xcodebuild` を立て直さずに起動をやり直す
  方法もあります。単位 4 の runner ごと再試行するやり方より安価です。それでも単位 6 の Python 側
  での検知を選ぶ理由は 2 つあります。すでに詰まった XCTest の自動化セッションを持つ runner の
  内部でやり直すと、たった今タイムアウトしたセッションをそのまま使い回すことになります。また、
  修正がロジックコアではなく Swift のパッケージに入るため、取り込むにはすべてのレーンで runner
  の再ビルドが必要になります。単位 6 のあともアプリ起動タイムアウトが残る場合にかぎり、あらためて
  検討します。
...
- [x] 単位 6 — テスト実行の終了でコールド起動の待機を打ち切り、Simulator のアプリ起動タイム
      アウトが即座に失敗して再試行の予算を残すようにする。
