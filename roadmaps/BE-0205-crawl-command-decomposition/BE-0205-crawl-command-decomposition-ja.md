...
| 実装 PR | [#866](https://github.com/bajutsu-e2e/bajutsu/pull/866) |
| トピック | コードベース品質・技術的負債 |
<!-- /BE-METADATA -->
...
- [x] `_CrawlPlan` レコード
- [x] `_resolve_warm_start` の切り出し
- [x] レーン計画 / コールバック / guard 配線のヘルパ
- [x] `crawl` 本体をオプションと薄い列に縮小（typer シグネチャは不変）
- [x] 切り出したヘルパのユニットテスト

**ログ**

- [#866](https://github.com/bajutsu-e2e/bajutsu/pull/866)：`crawl` 本体を frozen な `_CrawlPlan`
  レコードと `_resolve_warm_start` / `_plan_lanes` / `_make_callbacks` / `_wire_health` の各ヘルパ、
  そして `_execute` の列に分解しました。BE-0143 が `run` に与えた形に揃えています。切り出した
  ヘルパ向けに `tests/test_crawl_lanes.py` を追加しました。挙動は変えておらず、typer シグネチャは
  インラインのままです（BE-0134 のミラー制約）。
