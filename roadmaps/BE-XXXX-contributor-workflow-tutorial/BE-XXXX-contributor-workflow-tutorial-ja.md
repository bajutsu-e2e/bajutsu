[English](BE-XXXX-contributor-workflow-tutorial.md) · **日本語**

# BE-XXXX — コントリビューションワークフローチュートリアル: ideation・implement-be・propose-and-build を実際に使うガイド

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-contributor-workflow-tutorial-ja.md) |
...
1. **新しいチュートリアルページ**を `docs/contributor-workflow-tutorial.md`（`docs/ja/` の
   ...
   - `/ideation` を起票し、既存のロードマップを踏まえて `BE-XXXX` の提案を書き上げるまで。
...
- [ ] 1. `docs/contributor-workflow-tutorial.md` を書く（ideation の起票 → 提案の merge →
      implement-be の実行 → 実装 PR の merge までの一本のチュートリアル）
- [ ] 2. 実際の merge 済み BE 項目を参照した、良い提案・粗い提案の対比実例を追加する
- [ ] 3. `ideation`・`implement-be` と釣り合う分量まで `propose-and-build` の解説を拡張する
- [ ] 4. `CONTRIBUTING.md`、`docs/roadmap-workflow.md`、`docs/ai-development.md`、
