...
この項目は、この欠落を2つの側面から埋めます。主となる貢献は、`locale` がすでにアプリに与えている決定論を SpringBoard 自身にまで広げることです。Simulator が起動する前に、システム言語を既知の値へ固定します。これにより `label`/`labelMatches` はどのマシン上でも `locale` の値どおりに解決され、アプリと SpringBoard のあいだで言語の認識がずれなくなります。補完となる貢献は、この項目がまず対象とする通知許可と App Tracking Transparency (ATT)について、ロケール別のラベル対応表を用意することです。この2つは、[BE-0276](../BE-0276-scenario-permission-state/BE-0276-scenario-permission-state-ja.md)の権限状態の事前設定が届かないプロンプトでもあります。これにより、「許可する」「拒否する」という意図だけを書きたい作成者が、固定したロケールで実際に表示される文言を手で書き写す必要がなくなります。
...
1. **Simulator のシステム言語を、シナリオ単位で決定論的に解決して固定します。** `XcuitestEnvironment._prepare_simulator`(`bajutsu/platform_lifecycle/environments/xcuitest.py`)は、コールドスタートでもウォームレジューム(実行中ランナーの再利用)でも、シナリオのたびに権限状態を再設定しており、再利用されたランナーもコールドリースと同じ既知の状態からシナリオを始められるようにしています。この既存のデバイス準備シーケンスを拡張し、アプリが起動する前に、システム全体の言語とリージョンを書き込みます。手段は、`xcodebuild` ベースのスクリーンショットツールがすでに使っている手法と同じです。`xcrun simctl spawn <udid> defaults write -globalDomain AppleLanguages -array <language>` と、対応する `AppleLocale` の書き込みです。SpringBoard も、シミュレートされたデバイス上の他のどのプロセスとも同じグローバル環境設定データベースからこの値を読み取ります。書き込む値は、`_launch_params` がアプリ自身の起動引数についてすでに解決している優先順位に、そのまま従います。具体的には、シナリオ単位の `Preconditions.locale` オーバーライドを、ターゲットの `locale` 設定フィールドにフォールバックさせる `pre.locale or eff.locale` です。こうすることで、自分の locale をオーバーライドするシナリオは SpringBoard もそのオーバーライドに合わせて固定され、起動時に一度だけ固定した値を後続のシナリオのオーバーライドが静かに裏切る、という事態がなくなります。この書き込みはシナリオ1回につき1度だけ行われ、ポーリングの tick ごとには行われないため、[BE-0315](../BE-0315-ios-native-system-alert-handling/BE-0315-ios-native-system-alert-handling-ja.md) の詳細設計が慎重に守ろうとしているランナーの tick ごとの負荷には影響しません。実際の `simctl spawn` 呼び出しは、Simulator のグローバル環境設定への書き込みが off-Simulator ゲートでは観測できない性質のものである以上、実機での確認(ユニット4)を経て初めて完了したと言えます。
...
- [ ] ユニット1 — Simulator のシステム言語とリージョンを、シナリオ単位で決定論的に解決して固定する。`_launch_params` がすでに従っている `pre.locale or eff.locale` という優先順位に従う。
...
- [ ] ユニット4 — 実機検証: 異なる `locale` の値を固定した2台の Simulator に対して、同じ `handleSystemAlert` シナリオを実行し、両方で決定論的に閉じることを確認する。
...
- [`bajutsu/platform_lifecycle/environments/xcuitest.py`](../../bajutsu/platform_lifecycle/environments/xcuitest.py) — `XcuitestEnvironment._prepare_simulator` と `_launch_params`。ユニット1が拡張するデバイス準備シーケンスと、再利用する優先順位です。
