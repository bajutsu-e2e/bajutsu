...
（`bajutsu/serve/state.py:262`）は今日、`--runs` を基準にしたきょうだいディレクトリをデフォルトに
...
**`release_upload` への影響。** `ServeState.release_upload()`（`bajutsu/serve/state.py:523`）は、
...
**起動時の一掃処理への影響。** `serve()`（`bajutsu/serve/__init__.py:498`）は現在、起動のたびに
...
- [ ] 2 — `state.uploads_dir` のデフォルトを、`--runs` 基準のディレクトリから、BE-0063 の Git
      ...
      `if not root.exists(): _extract_into(...)` をなぞったものである。`release_upload` は切り替
      えのたびにそのディレクトリを削除するのをやめ、`serve()` の起動時の一掃処理
      （`bajutsu/serve/__init__.py:498`）も起動のたびにそれを消すのをやめる。これにより、キャッ
