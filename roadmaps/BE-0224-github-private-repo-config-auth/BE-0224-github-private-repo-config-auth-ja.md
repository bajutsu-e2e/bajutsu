[English](BE-0224-github-private-repo-config-auth.md) · **日本語**

# BE-0224 — GitHub を取得元とする config での private リポジトリへのアクセス権限の付与
...
private リポジトリへのアクセスを、互いに排他で全体を覆う (MECE) 5 つに整理します。いずれも github.com 限定
です。#4 は単独で出せます。
...
- **認証の失敗を読み取れるようにします。** トランスポートの `HTTPError` を包み、ステータス*と*サブタイプで
  振り分けます。具体的なケースを先に調べ、常にアクセス不足のせいにするのではなく本当の原因を名指しします。
  - **`X-RateLimit-Remaining: 0`（または `Retry-After` ヘッダ）を伴う 403** → レート制限／濫用検知の
    メッセージ（待つか、認証して上限を上げる）。アクセスのメッセージ*ではありません*。リポジトリ権限を
    増やしてもレート制限は解消しません。
  - **`X-GitHub-SSO` レスポンスヘッダを伴う 403**（org の SAML シングルサインオンの認可が必要）→「この
    認証情報を org の SSO に対して認可してください」。これもリポジトリ権限の付与とは別です。
  - **401** →「渡されたトークンが拒否されました」。
  - **404**、またはその他の **403** →「リポジトリが見つからない、*または*アクセス権限が付与されていません。
    `<owner>/<repo>` に Contents: read を持つ認証情報を渡してください」。最も不足していそうな付与を名指し
    することが要点ですが、上記の具体的なケースの後の受け皿としてのみ用います。

  「Contents: read」を提案する前に 403 のサブタイプ（ヘッダ）を調べることが、運用者を誤った道へ送らない
  ための要点です。これは認証情報プロバイダの変更を必要とせず、最初に出せます。
