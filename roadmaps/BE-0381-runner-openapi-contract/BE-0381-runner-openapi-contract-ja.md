...
**BE-0323の直列化は、プロバイダー経由ではなくキューそのもので固定します。** 単位4が`Router`を本番の
経路から外すと、この不変条件を持つのは`APIHandler.operations`だけになります。そこで
`APIHandlerConcurrencyTests`が、それを支えている性質を直接確かめます。このキューが同時に受け入れる
操作は1つだけであり、キューを`attributes: .concurrent`にするとこのテストは失敗します。
...
です。したがって単位5では、Hummingbird上で直列化を担うものに対して、このキューのアサーションを
立て直す必要があります。`Router`との突き合わせと一緒に削除してはいけません。
...
- [x] 単位4 — すべてのエンドポイントを生成されたハンドラーで提供する。`LegacyBackedTransport`が
      ...
      越しに各エンドポイントを旧来の`Router`と突き合わせる。`APIHandlerConcurrencyTests`が、
      `APIHandler.operations`の直列性という観測できる形でBE-0323を固定する。記録した通信上の差分は3つあり、どれも
