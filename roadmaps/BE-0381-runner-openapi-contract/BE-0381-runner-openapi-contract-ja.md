...
**BE-0323の直列化は、プロバイダーが数える重なりではなく、キューの占有で固定します。** 単位4が
`Router`を本番の経路から外すと、この不変条件を持つのは`APIHandler.operations`だけになります。そして
捕まえるべき退行は、誰かがキューを並行に宣言することではなく、ある操作がこのキューを通らなくなる
ことです。そこで`APIHandlerConcurrencyTests`は、操作を1つメインスレッドで留め、キューへプローブを
投入して、操作が戻るまでプローブが動かないことを求めます。これが成り立つのは、操作がキューを
占有しているあいだだけです。2つの退行のどちらにも反応することを測って確かめました。プロバイダーを
その場で呼ぶように変えた読み取りでは、その系統だけが失敗して他は通ります。キューを
`attributes: .concurrent`にすると、3つとも失敗します。

`RouterConcurrencyTests`が使うプロバイダー側のプローブは、この代わりになりません。このスタックで
測ったところ、`attributes: .concurrent`にしたキューが実際に2つの操作を並行して走らせていても、
プローブは重なりを報告しませんでした。メインキューのブロックを実行している最中に、libdispatchが
別のメインキューのブロックを実行することはありません。そのため2つ目の操作の
`DispatchQueue.main.sync`は、何が直列化していようと、1つ目のメインブロックが戻るまで待ちます。
BE-0323が報告された異常終了が、XCUITest自身の実行ループのもとでしか現れなかったのは、これが理由
です。`Router.actuationLock`をまるごと外しても`RouterConcurrencyTests`の対応するアサーションが通って
しまうのも、同じ理由です。したがって単位5では、Hummingbird上で直列化を担うものに対して、これらの
アサーションを向け直す必要があります。`Router`との突き合わせと一緒に削除してはいけません。

**BE-0362の宣言は、このトランスポートが足す唯一の受け渡しにも届かせます。** 各応答を運ぶタスクは、
優先度を継承せず`.userInitiated`を宣言します。このタスクの中の処理、つまり生成されたコードによる
リクエストの復号と応答の符号化（スクリーンショットのPNGを含みます）は、`operations`とその`qos:`を
通らないからです。この受け渡しは、`.utility`の呼び出し元から駆動するテストで固定します。
`HTTPServer`自身の`.userInitiated`のキューが呼び出し元であるあいだは継承しても見分けがつかず、
単位5のイベントループが呼び出し元になった瞬間に見分けがつくようになるからです。
...
からです。1つは、生成されたデコーダーが拒んだ本文に対する400の`message`が、`Router`の経路ごとの
文言ではなく、ステータスコードの一般的な理由句になったことです。ハンドラー自身が返す400は、
`Router`と同じ文言を保ちます。`APIHandlerParityTests`の`testBadRequestParity`が、これをフィールド
単位で固定しています。もう1つは、生成されたデコーダーが、`Router`なら受け入れていた本文を拒むこと
です。整数でない`taps`や`count`、正しい`handle`と一緒に送られた壊れた`point`、本文を必要としない3つ
の操作への規格外の本文が該当します。`Router`は値を変換したり本文を無視したりして動作させて
...
によるアクセプトループと1バイトずつの
パーサー、`Router.swift`の手書きのswitch文とJSONの組み立てコードを削除します。
`APIHandlerConcurrencyTests`のキューのアサーションは、そこで直列化を担うものへ向け直します。
`Router`との突き合わせが消えたあとにBE-0323を守るのはこのアサーションだけであり、リスナーの
入れ替えは、まさにこのアサーションが見落とす変更です。この時点では、
...
- [x] 単位4 — すべてのエンドポイントを生成されたハンドラーで提供する。`LegacyBackedTransport`が
      ...
      越しに各エンドポイントを旧来の`Router`と突き合わせる。`APIHandlerConcurrencyTests`が、占有と
      いう形でBE-0323を固定する。留めた操作は、`APIHandler.operations`へ投入したプローブを動かせない。
      応答を運ぶタスクが宣言する`.userInitiated`（BE-0362）も、より低いQoSの呼び出し元から固定する。記録した通信上の差分は3つあり、どれも
      ドライバーからは観測できない。JSONの応答が`charset=utf-8`を伴うこと、生成されたデコーダーが
      拒んだ場合の400の`message`が一般的な理由句になり、ハンドラー自身が返す400は`Router`と同じ
      経路ごとの文言を保つこと、`Router`が受け入れていた壊れた本文のいくつかをそのデコーダーが拒む
      ことである。
