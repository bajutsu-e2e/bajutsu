...
Hummingbirdへの依存であり、それは単位1ではなく単位4で加わります（詳細は後述の「測定結果」を
参照してください）。下限をめぐる問いも、この依存とともに単位4へ持ち越します。
...
**この依存そのものは、単位2ではなく単位4で加わります。** `swift-openapi-hummingbird`はiOS 17
を要求します。そして、サーバーが`APIHandler`へ配線されるまで、Hummingbirdをインポートするコード
はどこにもありません。単位3が最新の作業である今この依存を宣言すれば、誰も呼び出さないコードの
ためだけにSwiftNIOをwheel同梱のランナーへリンクし、同じ「誰も呼び出さない」ことのためだけに
パッケージの下限を引き上げることになります。単位4がこの依存を追加し、下限をiOS 17へ引き上げ
...
- [x] 単位1 — 既存16個のエンドポイントぶんの`openapi.yaml`を書く。Swift OpenAPI Generatorのビ
      ...
      も強制しないためである。強制するのはHummingbirdへの依存だけであり、それは単位4で加わる。
...
- [ ] 単位4 — 上記4つのまとまりに沿って、比較の仕組みを挟みながら16個のエンドポイントを1つずつ
      移行する。
