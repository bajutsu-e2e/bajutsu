...
| 実装 PR | [#700](https://github.com/bajutsu-e2e/bajutsu/pull/700) |
...
を `serve` Web UI に出します。シナリオの静的な安定性スコア（安定性のはしごで採点したセレクタ、具体的な条件を
持たない（ゆるい）`until` の待機、生座標のジェスチャ）を、シナリオをオーサリングし閲覧するその場で見せます。
読み取り専用で AI を使わず、ゲートには決してなりません。
...
`traits` に勝り、それらが `index` / 生座標に勝ります）、具体的な条件を持たない（ゆるい）`until` の `wait`
ステップを指摘し、安定した `id` で置き換えられる座標ジェスチャを指摘する、読み取り専用の静的スコアです
（`bajutsu/audit.py`）。Bajutsu の
...
- **「Audit」パネル／バッジ**を、エディタと Replay ビューでシナリオに添え、`POST /api/audit` を叩きます。
  ...
  決定性スコアを、セレクタごとの採点と指摘（具体的な条件を持たない（ゆるい）`until` の待機、生座標の
  ジェスチャ、はしごから外れたセレクタ）とともに返します。
...
- [#700](https://github.com/bajutsu-e2e/bajutsu/pull/700) — 静的監査 `bajutsu/audit.py` をラップする `POST /api/audit`（ライブのエディタ内容には
