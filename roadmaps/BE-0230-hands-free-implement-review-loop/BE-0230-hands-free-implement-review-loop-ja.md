...
2. **「変更を要求」レビューが残っていない**。`reviewDecision` が `CHANGES_REQUESTED` でないことを確認します。
   ...
   トップレベル body だけの異議は `pr-followup` には見えません。ループ層が `CHANGES_REQUESTED` かつ
   アクティブなインラインコメントスレッドがゼロであることを検出した場合は、マージコンフリクトと同様に
   **即時エスカレート**します。レビュー待ち上限が静かに消化されるのを待つのではなく、直ちに人間に知らせます。
   また、GitHub の `CHANGES_REQUESTED` 状態は**同じレビュアー**が再レビューするかメンテナが stale レビューを
   却下しない限り解除されません。インラインスレッドを resolve しても `reviewDecision` は変わりません。そのため
   インラインスレッドがすべて解決されても `CHANGES_REQUESTED` が残っている場合、ループはレビュアーへ
   再レビューを依頼する PR コメント（`gh pr comment`）を投稿します。ただし、この催促は stale レビュー
   一件につき一度だけとします（直近の PR コメントがすでにこの催促であれば再投稿しません）。こうすることで、
   レビュアーが全スレッドの解決を知らずにいてループがレビュー待ち上限を消化してしまうのを防ぎつつ、数時間
   離席しているだけのレビュアーに 20〜30 分おきの周回のたびに同じコメントで通知が飛ぶことも防ぎます。
...
- [x] Unit 2 — ループ前の compact step を、そのトークン経済の理由とともに追加する。
- [x] Unit 3 — 3つの停止条件（CI 緑・CHANGES_REQUESTED なし・静かな周回2回連続）とエスカレーション（設計変更・コンフリクト）、2つの歯止め（レビュー待ち20周回・CI待ち30周回、別カウント）を備えた一定間隔の pr-followup ループ。
...
- [#932](https://github.com/bajutsu-e2e/bajutsu/pull/932) — 5つの Unit をまとめて反映しました。implement-be の step 10 は step 10〜12（ドラフト
  PR の自動作成、compact、停止条件とエスカレーション条件と歯止めを備えた一定間隔の
  `/loop /pr-followup`）になり、pr-followup には位置づけの注記を、CLAUDE.md の PR 規則には BE 作成と
  実装の二経路への分割を加え、両スキルが合成された流れを相互参照するようにしました。Unit 2 の compact
  は step 11 の冒頭の指示で `/compact` を発行する形にしました（*Detailed design* の未決事項を、最も
  単純な組み込み地点として決着させたものです）。
