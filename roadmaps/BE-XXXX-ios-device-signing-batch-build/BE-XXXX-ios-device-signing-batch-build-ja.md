[English](BE-XXXX-ios-device-signing-batch-build.md) · **日本語**

# BE-XXXX — バッチ経路向け iOS デバイス署名ビルド

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-ios-device-signing-batch-build-ja.md) |
...
1. **Device Farm 用 iOS 設定。** `demos/showcase/devicefarm/showcase.devicefarm.ios.config.yaml`
   は、ローカルの `showcase-swiftui` ターゲットを写しつつ `xcuitest.deviceType: device` を持ち、
   `appPath` / `build` を持ちません。Device Farm がアップロードした `.ipa` をインストールする
   ため、実行はインストール済みビルドを駆動します。`testRunner` はアップロードパッケージ内の
   デバイスランナーを指します。これは既に出荷済みの Android 用 Device Farm 設定およびライブ
   経路の設定を写したものです。
...
- **デバイス成果物を今すぐ CI でビルドする。** 先送りします。署名済みデバイスビルドは、
  ...
  Developer アカウントを必要とするため、CI ジョブはアカウントが用意されるまで follow-on と
  します。
...
- [ ] ユニット 1 — Device Farm 用 iOS 設定（`showcase.devicefarm.ios.config.yaml`）
- [ ] ユニット 2 — 署名済みデバイスアプリビルド（`swiftui-archive-device` / `swiftui-ipa-device`、`ExportOptions.plist`）
- [ ] ユニット 3 — 署名済みデバイスランナービルド（`runner-build-device`）
- [ ] ユニット 4 — 認証情報の衛生（`require_team` ガード、プレースホルダーテンプレート、gitignore 済み出力）
- [ ] ユニット 5 — Device Farm 投入のエンドツーエンド実証（Apple Developer と AWS のアカウントが必要。`make check` の外の手動手順）

ログ:

- 2026-07-20: ユニット 1〜4 をブランチ `claude/devicefarm-ios-config`（コミット `7f9b3bb4`）で
  起草し、署名の手前まで検証しました。両プロジェクトはデバイス SDK（`PLATFORM_NAME =
  iphoneos`）に解決し、`DEVELOPMENT_TEAM` 未設定時に `require_team` ガードが発火し、
  `ExportOptions.plist` は lint を通り、`make check` は緑です。実装 PR: _pending_。
...
- [実機と device cloud 上の iOS](../../docs/ja/ios-device-cloud.md) · [AWS Device Farm](../../docs/ja/devicefarm.md)。
