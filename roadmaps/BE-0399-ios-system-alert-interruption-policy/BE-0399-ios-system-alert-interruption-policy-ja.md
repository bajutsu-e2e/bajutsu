[English](BE-0399-ios-system-alert-interruption-policy.md) · **日本語**

# BE-0399 — 割り込んできたシステムアラートに答えるのはシナリオの方針だけにする

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0399](BE-0399-ios-system-alert-interruption-policy-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0399") |
...
### ユニット 6 — ステップ終了時のガードでもアプリ側のプロンプトを片付ける

ガードは待機中のゲートだけではありません。ステップや `expect` が失敗したとき、
`AlertGuardConfig.__call__` が塞いでいるものに 1 度だけ挑みます。ここにはネイティブ経路と vision の
フォールバックしかなく、アプリ側のプロンプトは `gone` の待ちと同じように見えていませんでした。実測では、
iOS 26.3、26.4、26.5 で保存アラートがシナリオ最後の待ちより**あと**に届きます。そのとき出会える立場に
あるガードは、もうここだけです。

そこで同じ narrow な面に対する 1 回きりのツリー内消去を足します。条件は同じで、新しい `"absent"` の
答えが得られたときだけです。待機中の版が持つ表示ごとの記帳（再タップの遅延、タップ上限、辞退の上限）は
持ちません。失敗したステップごとに 1 度走るだけで、抑えるべき流れがないからです。

### ユニット 7 — showcase で本物のプロンプトを両方の経路から出す
...
いずれも不要です。iOS は Safari と同じく、web フォームのオリジンに対して資格情報の保存を申し出るから
です。

アプリ自身のサインイン画面がもう 1 つの経路で、こちらはその 3 つすべてを必要とします。実測では
entitlement の宣言だけでは足りませんでした。`apple-app-site-association` を配る HTTPS のサーバと、
Simulator のキーチェインに入れた認証局が揃うまで、プロンプトはまったく出ません。

そこで `e2e-savepassword-native` が 4 つの仕事をします。実行ごとに使い捨ての認証局を生成し、
`localtest.me` から association を配り、認証局を Simulator へ入れ、終わったらすべて片付けます。鍵を
リポジトリに置くことはありません。

この画面については、さらに 2 つの性質を推測ではなく実測で決めました。1 つは、フィールドを素のビュー
コントローラに置いた素の `UITextField` にした点です。SwiftUI の `Form` の中に同じ content type を
置いても Password AutoFill は働きませんでした。もう 1 つは、送信で別のビューコントローラを **push**
することです。AutoFill の合図は、資格情報を持つビューが去ることだからです。
...
それぞれで働くこと。打ち切りが時計に基づくこと。`gone` の待ちがアプリ側のプロンプトを片付けること。あわせて、ステップ終了時
のガード自身の 1 回きりのツリー内消去と、その 3 つの辞退も覆います。runner 側は Swift でこのゲートの外に
着地するので、ユニット 7 のシナリオが検証を担います。
...
**ネイティブ経路のために、固定のテスト用認証局をコミットする。** そのレーンの再現性は上がり、可動部も
減ります。とはいえ採りません。目的が何であれ、秘密鍵がリポジトリに入れば履歴の中の秘密になるからです。
`make lint-secrets` も当然それを咎めます。実行ごとに gitignore 済みの `tmp/` へ生成して終了時に消す方式
なら、`openssl` の数秒で済み、あとには何も残りません。

**ネイティブ経路に専用の showcase アプリを与える。** 釣り合わないので採りません。`SHOWCASE_SIGNIN` は
`SHOWCASE_GESTURES` と同じように画面ごと差し替えるので、他のシナリオのタブバーは動かず、2 つ目の成果物を
ビルドする必要もありません。
...
- [x] ユニット 6 — ステップ終了時 / `expect` のガードでもアプリ側のプロンプトを片付ける
- [x] ユニット 7 — showcase で iOS 本物の保存アラートを両方の経路から出す
- [x] ユニット 8 — オーケストレータ側を決定的なテストで覆う
