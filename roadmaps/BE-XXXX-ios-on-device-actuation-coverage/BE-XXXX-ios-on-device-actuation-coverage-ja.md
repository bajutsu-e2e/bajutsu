...
# BE-XXXX — 読み取り専用の iOS CI を実機アクチュエーションのカバレッジに変える
...
iOS E2E レーンの idb シナリオ（smoke、golden、visual）は読み取り専用です。`wait` と `expect` を使い、
...
Android は圧倒的に最もよく動かされているバックエンドであり、`swipe`、`scroll`、`back`、`longPress`、
...
バックグラウンド）は idb と XCUITest が `DEVICE_CONTROL_ALL` で広告し、simctl のビルダーも単体テストされて
