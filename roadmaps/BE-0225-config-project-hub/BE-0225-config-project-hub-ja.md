...
- [ ] 3. API：org スコープの五つの `/api/projects…` エンドポイント（既存の単一 config 向けへの追加）。（#921 のレビューで挙がったユニット 2 からの持ち越しがあります。解決済みの `project_id` を `job_spec` に載せてリモートワーカーの `_persist_run` が刻印できるようにすること、および自動有効化を org 対応にすることです。それまでは `default` 以外の org には active プロジェクトが付きません。）
