[English](BE-0341-pre-action-evidence-capture.md) · **日本語**

# BE-0341 — ステップの動作前に、ステップごとのレポート証跡を取得するようにする

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0341](BE-0341-pre-action-evidence-capture-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0341") |
...
1. **ステップ前の取得呼び出しを追加し、ベースラインを事後の呼び出しから外します。**
   ...
   ベースラインは残りません。
   **遅延クエリという設計が受け入れている副作用：** 「今日と同じコストになる」という主張は、
   `elements` を消費しないシンクでは成り立ちます。また、消費するシンクでも `screenChanged`
   ポリシーがない通常の場合には成り立ちます。ただし、1 つの狭い組み合わせでは成り立ちません。
   消費するシンク（`FileSink`）、新鮮な `before` を必要とする `screenChanged` /割り込みポリシー、
   **ネイティブ**なステップ、そして `prev_after` が未設定（シナリオの最初のステップ、または
   ツリーを残さないステップの直後）という組み合わせです。ここでは、`write_elements` 自身の
   遅延クエリ（この呼び出しが `elements=None` を渡すために発火します）と、直後にある `before`
   の計算（`prev_after` がまだ未設定のために発火します）が、同じ動作前の状態にそれぞれ独立に
   問い合わせます。今日は 1 回の事後読み取りが両方の消費者に応えていたのに対し、問い合わせが
   1 回多くなります。直後で説明する `web` ブロックの最初のステップのケースとは異なり、これを
   閉じるには、`screenChanged` ポリシーが有効なあいだ、シナリオの最初のステップだけでなく
   ネイティブなステップすべてで先読みの問い合わせをする必要があります。しかもその先読みの問い
   合わせは、`web` のケースのように `isinstance(sink, NullSink)` でゲートできません。
   `tests/orchestrator/test_read_count.py` の `_KindsSink` スタブ（`NullSink` ではないものの、
   実際に消費するシンクとは異なり、自分でドライバへ問い合わせることは一度もないシンク）に対して
   過剰に発火してしまい、正しさの修正がテストの回帰に変わってしまうからです。レビューが立て込む
   このタイミングで作り直すのではなく、受け入れた狭い副作用として残します。本当に閉じるには、
   この 1 つの組み合わせに限って `before` の計算をベースライン取得より前に動かし、1 回の新鮮な
   問い合わせで両方に応えるようにする必要があります。`web` のケースの先読みと同じ形を、逆方向に
   適用するイメージです。これは将来の課題であり、この項目自体の欠陥（証跡が動作の前ではなく後に
   取得されていたこと）をふさぐこととは別の話です。
...
2. **シナリオの最終ステップの欠落を閉じます。** ステップ *i* の結果をステップ *(i+1)* のステップ前
   ...
   Unit 1 のステップ前ベースライン取得も、この locale 解決より前に動くようにしています。これは
   意図した配置です。このベースラインは、ステップが動作の対象とする画面だけに依存し、ステップ
   自身の解決済みフィールドには一切依存しないからです。そのため、この失敗も、ほかのすべての
   リーフステップと同じ証跡の約束を受け取ります（実際に書き出されるかどうかはシンク次第で、
   `NullSink` ならほかのステップと同じく何も書き出しません）。最終取得だけではありません。
   ...
   最後のリーフがまだ `after.png` を記録していないことをゲートにします。`capturePolicy` ルール
   （`screenshot.after`、または既定で `after` になる素の `screenshot`）が、この同じ最後のリーフに
   対して事後に発火していれば、すでに 1 枚記録済みです。よくあるのは、最後のステップが失敗した
   ときの安全網 `{on: {result: error}, capture: [screenshot.after]}` です。ここでもう一度取得する
   と、同じファイル名のせいでルール自身のショットを少し後のものへ静かに上書きしてしまい、上の
   Unit 5 が挙げた「マニフェストを直接読む」読者にとって、リーフの証跡に `screenshot` /
   `after.png` の重複した項目が残ってしまいます。
...
5. **ハードコードされた唯一の利用箇所を修正します。** `bajutsu/serve/operations/reads.py` の
   ...
   `or None` という補正も復元し、さらにこのファイル自身の `_valid_step_id`（クライアントから戻って
   きた `stepId` に対して `resolve_scenario_pick` がすでに適用しているのと同じゲート）で検証も行い
   ます。`str` でない、あるいは `..` や絶対パスの形をした `sid` は、`sid` が欠けている場合と同じよう
   に `[]` へ抜けます。この関数が返すすべての `stepId` の元になる壊れた、あるいはパストラバーサルの
   形をしたステップ ID を組み立てる代わりにです。このステップごとの参照は、各 outcome 自身が記録
   した証跡パスから取り出した実行時のステップ
   ...
   でスラッシュを含み、`_valid_step_id` で安全と判定できる）`name` を持たない証跡を読み飛ばします。
   単なるスラッシュの有無だけでなく `_valid_step_id` も見るのは、パストラバーサルの形をした `name`
   （`../../../run2/...` など）自体がキーになってしまわないようにするためです。そうなると
   `name.rsplit("/", 1)[0]` から作られるキーは、どの本物の `step_id` にも一致しないため、同じ
   ステップに記録されている他の正当な証跡までまとめて隠してしまいます。書き込みが途中で止まった
   manifest のどこかに壊れた要素が1つあっても、そのステップの証跡が欠けるだけで済み、500 エラーに
   なることも、隠れた正当な証跡が生まれることもありません。
...
改訂しています。あわせて読んでください。ステップ後の取得は `elements` を常に記録し、`screenshot.after` は
`_handle_action` が動作の直後に自分で撮るようになりました。そのため実行が残すツリーは、`capture` が
要求したかどうかによらず、どのステップでも動作後のものになります。Unit 4 の優先順位は、シナリオが何を要求したかについての規則
ではなくなり、「`elements` は動作前のツリーのまま」が成り立つのは、動作する前に失敗したステップ
だけです。`rawTree` も同じ理由で全ステップがステップ後の取得に移りました。動作前のダンプは、隣に
置かれるツリーと対応しなくなるためです。動作前のダンプは、`rawTree.before` 修飾子ができるまで
取得できません。

Unit 2 の実行終了時の取得も、あわせて削除しました。動作したステップは、その動作の直後に自分で
`after.png` を撮ります。この安全網が届くのは、そもそも動作せずに戻るステップだけでした。
`UncoveredSystemAlertLocale` で失敗するステップです。そのステップに実行終了後のスクリーンショットを
与えると、動作前のツリーと実行終了後の画素とを組み合わせることになります。現在は、ステップ前の
baseline が残した `before.png` と動作前のツリーの対応をそのまま保ちます。

この項目が定めたこと自体は、どちらの改訂でも変わっていません。どのステップも、動作する前に
`screenshot.before` と `elements` を取得します。
