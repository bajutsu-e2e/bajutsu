...
- [x] `SHOWCASE_TAB` を退役（両ツールキット）。deeplink は機能デモとして残しました。
- [x] タブをまたぐシナリオを `--backend ios` へ移し、idb/XCUITest の分担を確定。smoke と Stable タブの
  golden は idb に残し（BE-0005/BE-0006 のシグナルを保全）、Log タブのコントロール golden は
  `golden_xcuitest.yaml` へ分離し、CI のコメントも合わせました。
- [x] `SPEC.md` §3〜§5（英日）と showcase / scenarios / codegen のドキュメントを更新。
- [x] ナビゲーションで到達する golden をオンデバイスで録り直す（XCUITest の Log コントロールの `controls.json`）。

Android のタブナビゲーション（共有シナリオの Android 版）は BE-0007 の後続作業として残します。Android
アプリはまだ `SHOWCASE_TAB` を読んでおり、adb でネイティブのタブバーを操作するのは、本アイテム（iOS
限定）の外にある別の driver の課題です。adb もタブバーをタップできないため、adb の e2e レーン
（`demos/showcase/android/Makefile`）は当面 Stable タブのシナリオ（`smoke`、`firstlook`）だけを実行します。
`search` / `data_driven` / `relaunch` / `system` は、adb backend がタブを操作できるようになった時点で戻します。

- 2026-07-10: `claude/be-0107-tab-navigation` で実装（PR は保留）。SwiftUI と UIKit のアプリから
  `SHOWCASE_TAB` を退役させ、タブをまたぐシナリオの先頭にタブタップを足し、golden を idb（Stable）と
  XCUITest（Log）に分割し、`ComponentsUITests.swift` を再生成し、ドキュメントを更新しました。
- 2026-07-10: iPhone 17 Pro（Simulator）でオンデバイス検証。`controls.yaml` と `notices.yaml` が
  `--backend ios` で通過し（タブタップでタブに到達。run manifest で `backend: xcuitest`、step 0 が tap と確認）、
  `golden_xcuitest.yaml` が再録した XCUITest の `controls.json` に対して通過し、`golden.yaml` と
  `smoke.yaml` が idb で引き続き通過しました（BE-0005/BE-0006 のシグナルは維持）。
