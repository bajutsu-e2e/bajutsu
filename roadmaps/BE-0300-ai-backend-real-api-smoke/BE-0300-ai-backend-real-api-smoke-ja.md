...
| 実装 PR | [#1348](https://github.com/bajutsu-e2e/bajutsu/pull/1348) |
| トピック | AI プロバイダ設定 |
<!-- /BE-METADATA -->
...
opt-in で API キーによりゲートされた、ゲート対象外のスモークレーンを1つ追加し、実際のレスポンスをベンダー中立な
...
- **API キーでゲートした最小限のライブ呼び出しテスト。契約のみを検証する**：`AnthropicBackend` に
  ...
  条件にするのは安全なゲートにはなりません。`record`（`CLAUDE.md` が前提とする環境）または
  `triage --ai`（`bajutsu/cli/commands/triage.py`）のためにすでにキーを export している
  contributor のセッションでは、通常の `make check` で実際に課金される呼び出しが走ってしまう
  ためです。このテストは、アダプタが正規化した
...
- **アダプタごとに1つ、opt-in かつゲート対象外の CI レーン**：直接 API / Bedrock / `ant` それぞれに
  ...
  ジョブを用意します。トリガーは `workflow_dispatch` のみとし、`pull_request` では実行しません。
  フォークからの PR で実行されると認証情報が漏れてしまうためで、
  `.github/workflows/devicefarm.yml` にすでに明記されているハードな境界と同じです。
...
- **実際の呼び出しを一度録画した VCR 形式のカセットを使う**：繰り返し実行する分には安価で決定的
  ...
  アダプタが実 API を二度と観測しなくなります。定期的なライブスモークレーンだけが、現実を観測し
  続ける設計です。
...
- [x] 直接 Anthropic API アダプタ向けに opt-in かつゲート対象外の CI レーンを組み込む。Bedrock と `ant` の CI レーンは、ライブな AWS ロールや OAuth CLI のセッションを CI シークレットとして用意できないため、ログに記したとおり見送っています。
...
- opt-in のゲートには、設計が例として挙げた `BAJUTSU_LIVE_AI_SMOKE` 環境変数ではなく **`live` の
  ...
  ままです。`live` マーカーを導入するのは本 PR が初めてです（`main` の `addopts` は `-m 'not web and
  not ondevice'`）。兄弟項目の BE-0295 も実モデルスモークを *提案* していますが、こちらは除外マーカー
  ではなく `pytest.mark.skipif` によるゲートです。実行は `-m live` で opt-in します。
