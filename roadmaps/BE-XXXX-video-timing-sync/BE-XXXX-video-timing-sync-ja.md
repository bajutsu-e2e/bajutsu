...
どちらのポーリングも、期限つきの条件待ちであり、固定`sleep`ではありません。プライム・ディレクティブ2（決定性優先）に合致します。ポーリングがタイムアウトした場合は`true_start`を`None`のままにし、補正は現状と同じ無効化（ノーオペレーション）に留まります。当て推量の数値を入れることはありません。web アクチュエータにはポーリングが不要です。`PlaywrightDriver`は、`new_context()`が返った直後に`time.monotonic()`を記録します。ブラウザコンテキストの生成にかかる時間は、サブプロセスの起動に比べて無視できるほど小さいためです。
