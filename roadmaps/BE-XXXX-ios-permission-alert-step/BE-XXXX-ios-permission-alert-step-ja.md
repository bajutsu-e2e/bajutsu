...
本項目は `systemAlert` を追加します。これは、シナリオの中でプロンプトが現れるとまさにその瞬間に、
...
`systemAlert` はその拡張にあたります。シナリオの著者はその時点でどのプロンプトが現れ、どのボタンが必要
...
- **シナリオスキーマ。** 新しいステップアクション `systemAlert: { sel: <Selector>, timeout: <秒> }`
...
- **iOS（XCUITest）ランナー側の対応。** `BajutsuKit/Runner/Sources/RunnerUITest.swift` は、テスト
  ...
  `systemAlert` ステップは、SpringBoard 自身のバンドル識別子である `com.apple.springboard` 用の
...
- **ケーパビリティトークンと preflight。** このステップを表すケーパビリティを、既存の operation
  ...
  の権限プロンプト自体がありません。`systemAlert` をこのどちらかのバックエンドに対して指定したシナリオ
...
- **codegen。** iOS の XCUITest codegen は、ネイティブなイディオムをそのまま生成します。
  ...
  すべてのバックエンドでフィールドを適用しますが、`systemAlert` は mid-flow のステップであり、その
...
- **ドキュメントとフィクスチャ。** [`docs/scenarios.md`](../../docs/scenarios.md) とその日本語版、
  ...
  *ガード*であり、`systemAlert` は著者がプロンプトを見込む地点に置く決定的でネイティブな*ステップ*です。
  どちらも同じ種類の OS レベルのアラートを操作するので、その共通の対象を、正反対の仕組みとあわせて
  述べることが、よく似た名前による誤読を防ぎます。フィクスチャは既存の `permission.yaml` に足すのではなく、
  ...
  除外せずそのまま実行します。同じファイルに `systemAlert` を使うシナリオを足すと、iOS（XCUITest）
  バックエンドしかこのケーパビリティを宣言していないため、Android の必須ジョブが preflight で
  落ちてしまいます。ファイルを分ければこれを避けられます。新しいシナリオは通知の認可を mid-flow で
  リクエストし `systemAlert` 経由で "Allow" をタップするもので、`demos/showcase/scenarios/tabs.yaml`
...
- **`dismissAlerts` を任意のタイミングで mid-step に発火させられるようにする。** 却下しました。
  ...
  保たれ、`systemAlert` 自身も決定的で、`steps` の中に現れた地点で無条件に働くという、より単純な契約を
  持てます。
- **一般の `Selector` に `system: true` のようなスコープを足し、`tap` / `wait` / `assert` が直接
  ...
  `systemAlert` アクションであれば、変更を1つの新しいステップ実装だけに閉じ込められ、他のあらゆる
...
- **権限プロンプトに限らず、SpringBoard レベルのアラート全般（パスワード保存の確認、ペーストの許可、
  ...
  は、アラートの出どころを区別しないので、後から `systemAlert` を任意の SpringBoard アラートへ広げる
...
- [ ] シナリオスキーマ — `systemAlert` ステップ、ラベルベースの `Selector` サブセット、必須の `timeout`。
...
- [ ] ドキュメント（scenarios.md + 日本語版、DSL 文法）と、新しい iOS 専用の showcase フィクスチャ
      ファイル（`permission_system_alert.yaml`、タグ `xcuitest`）。Android の `smoke (adb)` ジョブが
