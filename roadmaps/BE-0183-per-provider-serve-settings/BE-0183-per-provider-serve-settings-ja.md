...
- [x] serve が保持するプロバイダーごとの設定構造を定義する。
- [x] `GET /api/provider` からプロバイダーごとの設定一式を返す。
- [x] `POST /api/provider` の書き込みを選択中プロバイダーの枠に限定する。
- [x] Settings 画面を、取得済みマップからプロバイダーごとに表示欄を切り替えるよう更新する。

ログ:

- [#762](https://github.com/bajutsu-e2e/bajutsu/pull/762) — `ProviderSettings` データクラスと `ServeState.provider_settings` を追加し、
  `GET /api/provider` がプロバイダーごとの設定を `providers` として返すようにしました（現在有効な
  プロバイダーの枠は環境変数から埋めます）。`POST /api/provider` の書き込みは選択中プロバイダーの
  枠だけに限定しつつ、その枠をジョブが読む環境変数へ引き続き反映します。あわせて Settings の
  JavaScript を、取得したマップを保持してプロバイダー切り替え時に model・effort・region の各欄を
  差し替えるよう作り直しました。
