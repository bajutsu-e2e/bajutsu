...
| 実装 PR | [#1075](https://github.com/bajutsu-e2e/bajutsu/pull/1075) |
| トピック | コードベース品質・技術的負債 |
<!-- /BE-METADATA -->
...
`screenshot_file.is_file()`）には、seam に軽量な存在確認メソッドが必要です。`get()` はそれには
使えません。ローカル backend の `LocalArtifactStore.get`（`bajutsu/serve/artifacts.py:125-130`）は
`Artifact(body=target.read_bytes())` を組み立てる、すなわちファイル本文を丸ごと読むため、
`store.get(...) is not None` を存在確認に使うと、URL を出すか決めるためだけに各 `after.png` の
本文をメモリに読み込むことになり、置き換え対象の `is_file()` より厳密に高コストです。
（2 つの `get` 実装はここで挙動が分かれます。`ObjectStorageArtifactStore.get`
（`server/artifacts.py:56-61`）は安価な `exists` と presigned リダイレクトなので、`get()` を存在
確認に使うと一方の backend では無料、もう一方では全読み込みになります。まさに seam が抱え込むべきでない
非対称です。）そこで修正では、`ArtifactStore` protocol に `exists(rel) -> bool`（HEAD 相当）を追加し、
ローカルは `Path.is_file`、リモートはオブジェクトの `exists` で実装して、確認を
`state.for_org(org).artifacts.exists(...)` 経由に通します。`elementsUrl`/`screenshotUrl` は、これまでどおり
...
- [x] 1 — `_step_artifacts`（`reads.py`）を `state.for_org(org).artifacts` 経由にし、直接の
      `runs_dir` マニフェスト読み取りとステップごとの存在確認を `open_bytes`/`get` に置き換える。
- [x] 2 — `resolve_scenario_pick`（`reads.py`）の `elements.json` 読み取りを
      `state.for_org(_org).artifacts` 経由にする。
- [x] 3 — `coverage_view`（`operations/coverage.py`）に、`read_exchanges`/`read_observed_ids`
...
- [x] 4 — `operations/capture.py:62` に、ライブのキャプチャセッションのスクラッチスクリーン
