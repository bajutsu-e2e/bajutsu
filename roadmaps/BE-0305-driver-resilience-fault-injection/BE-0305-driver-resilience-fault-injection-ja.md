...
- **凍結したランナーの復帰は期待できません。凍結時間を3回縮めた判断は、変数を取り違えていました。**
  ランナー自身のログが機構を明かしています。「Restarting after unexpected exit, crash, or test
  timeout」——XCTest は到達できないランナーを回収します。したがって `SIGCONT` は、XCTest が既に
  破棄を決めたプロセスを再開させることが多く、続く再送は接続断、次いで接続拒否に行き当たります。
  約 51.5 秒、約 12.5 秒、約 4.5 秒がいずれも同じ形でランナーを失ったのは、このためです。
  ウォッチドッグが反応するのは、ランナーに到達できないという事実であって、その継続時間ではあり
  ません。どの長さを選んでも生き延びられなかったわけです。記録された復帰ログを見ると、ランナーが
  失われる前に検証対象の機構はすべて動いています。2回の試行のタイムアウト、リトライ予算を超えた
  ミッドラン・クラッシュとしての分類、`health` による確認、そして再送です。そこで凍結のテストは、
  このログから読み取れる2つの事実だけを表明する形にしました。ハングがクラッシュとして分類された
  ことと、リカバリが再送したことです。再送そのものの結果は XCTest に委ねます。あわせて
  `retrying: timed out` を明示的に検証します。これは停止したプロセスだけが生む症状なので、凍結が
  いつのまにか kill に劣化していた場合に、凍結のケースとして通過することはありません。アプリが
  戻ってくることまで表明するのは、ウォッチドッグとの競争に勝ったことを表明するのに等しくなります。
- kill のテストは、ポートの状態をあるがままに受け取る形にしました。このテストの前提は終了した
  ランナーであり、先行する凍結のテストは、XCTest が停止プロセスを回収したときにその状態を残します。
  つまり前提が満たされた状態で到達するのであって、環境が壊れているわけではありません。`nothing is
  listening` で失敗していたのは、まさにこの理由によります。ポートを保持するプロセスがあれば依然と
  して kill します。`xcodebuild` ホストの回収も引き続き要求するので、駆動する `runner_alive` の
  分岐はどちらの経路でも同じです。`lsof` が使えない場合と、異なる複数のリスナーがいる場合は、
  引き続き失敗します。値として扱うようにしたのは、ポートが未束縛の場合だけです。
