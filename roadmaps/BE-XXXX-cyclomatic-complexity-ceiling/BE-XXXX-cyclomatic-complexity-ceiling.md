**English** · [日本語](BE-XXXX-cyclomatic-complexity-ceiling-ja.md)

# BE-XXXX — Enforce a cyclomatic complexity ceiling

<!-- BE-METADATA -->
| Field | Value |
|---|---|
| Proposal | [BE-XXXX](BE-XXXX-cyclomatic-complexity-ceiling.md) |
...
Add `C901`, `PLR0911`, `PLR0912`, and `PLR0915` to `[tool.ruff.lint]`'s `select` list, with mccabe's
...
- [ ] Add `C901` with `max-complexity = 25`; triage the 10 functions this flags (refactor or a
  targeted `# noqa: C901`), resolving the `_make_handler` / `make_app` folding question first.
- [ ] Add `PLR0911`, `PLR0912`, and `PLR0915` at their default thresholds; triage the 87 combined
  findings.
