[English](BE-XXXX-serve-replay-scenario-viewer.md) · **日本語**

# BE-XXXX — Replay タブからシナリオの内容を確認する（生 YAML と構造化ステップ）

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-serve-replay-scenario-viewer-ja.md) |
...
- [ ] Replay の Form に「View scenario」の操作を追加する（`serve.html.j2`／`serve.panels.js` の
      マークアップと配線）。取得は既存の `GET /api/scenario` を使う。
- [ ] 生 YAML と構造化ステップを切り替えるビューアのオーバーレイまたはペインを実装する（BE-0187 や
      テーマエディタのモーダルの慣習に従う）。
- [ ] `read_scenario` の `steps` の構造部分の gate を外し、`run_id` なしでランナー由来のステップを返せる
      ようにする（実行由来の `elementsUrl`／`screenshotUrl` は `run_id` 依存のまま）。そこから構造化表示を
      描画する。
- [ ] `data-testid` を付け、`demos/serve-ui/scenarios/replay-tools.yaml` の隣に dogfood E2E シナリオを
      追加する。
