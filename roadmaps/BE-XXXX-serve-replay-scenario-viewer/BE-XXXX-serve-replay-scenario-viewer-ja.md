...
- **既存の読み取りエンドポイントを再利用する。** ビューアは選択中のシナリオ本体を既存の
  `GET /api/scenario?target=<t>&path=<p>`（`{"yaml": …}` を返す）から取得します。生 YAML 表示のために
  新しいサーバルートを追加する必要はありません。構造化ステップ表示はその YAML を**クライアント側で
  解析して**導出するので、サーバの契約は変わりません。可能な範囲で機能全体をフロントエンドに閉じます。
...
- **生 YAML と構造化を切り替えるモーダルまたは専用ペイン**（BE-0187 のパターンに合わせます）。
  ...
  - **構造化ステップ**：シナリオごとに、人が読める一覧を出します。各シナリオの `name`／`description` に
    続けて、順序どおりのステップをコンパクトに描画します（アクションとセレクタまたはターゲット、主要な
    引数、たとえば `tap { id: nav.replay }`、`assert exists { id: … }`）。これは「何をするか」をざっと
    把握するための表示で、正確なソースが必要なら切り替え 1 つで生 YAML に戻れます。
...
- **サーバ側に「シナリオを描画する」エンドポイントを新設する。** 不要です。既存の `GET /api/scenario` が
  すでに YAML を返しており、構造化表示をクライアント側で導出すればシナリオ構造の 2 つ目の表現をサーバに
  持たずに済みます（それはランナー自身の解析からずれる恐れがあります）。設計としてフロントエンドに閉じます。
...
- [ ] 生 YAML と構造化ステップを切り替えるビューアのオーバーレイまたはペインを実装する（BE-0187 や
      テーマエディタのモーダルの慣習に従う）。
- [ ] 取得した YAML から構造化ステップ表示をクライアント側で導出する。
...
- 既存エンドポイント：`GET /api/scenario?target=&path=`（`bajutsu/serve/handler.py`、
  `bajutsu/serve/operations/reads.py` の `ops.read_scenario`）。
