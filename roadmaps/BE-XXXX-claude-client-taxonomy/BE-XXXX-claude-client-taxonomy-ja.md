[English](BE-XXXX-claude-client-taxonomy.md) · **日本語**

# BE-XXXX — Claude と話すためのモジュール構成を整理する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-claude-client-taxonomy-ja.md) |
...
- [ ] `anthropic_client.py` を、プロバイダ非依存の設定解決を表す名前へリネームし、純粋に
      Anthropic の SDK に固有なコードはベンダー名を冠したモジュールに残す
- [ ] 2 組の `credential_gap` / プロバイダ解決（`anthropic_client.py` と `ai/registry.py`、
      および `ai_availability.availability` ラッパー）を `bajutsu/ai/` 内の 1 つの解決処理へ
      統合する
- [ ] 7 つの `Claude*` クラスの `_ensure_backend` / 使用量記録の定型処理のために、共有の
      `ClaudeBackedAgent` 基底クラスを導入する
- [ ] `agent.py` / `agents.py`（またはファクトリを `agent.py` へ統合）をリネームし、
      単数形・複数形の見分けにくさという罠を取り除く
