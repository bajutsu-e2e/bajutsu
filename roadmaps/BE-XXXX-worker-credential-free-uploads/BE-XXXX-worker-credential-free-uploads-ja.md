...
この 3 つすべてから Worker の認証情報を取り除けば、認証情報レスな Worker という目標が完成します。Worker の extra は `boto3` を丸ごと落とせ（HTTP クライアントだけで済みます）、オブジェクトストレージの認証情報はただ一か所、コントロールプレーンにだけ置けます。そうすればコントロールプレーンは、より厳しい権限のバケットや別アカウントのバケットを、シークレットをフリートへ再配布せずに使えます。
...
1〜3 が入れば、Worker のパス（`worker.py`、`worker_job.py`）から `_object_store()` / `object_store_from_env()` を取り除きます。Worker はクラウドクライアントを組み立てず、`BAJUTSU_S3_*` や AWS の認証情報も読みません。`worker` extra は `boto3` を落とし、Worker のネットワーク依存は既存の HTTP クライアントだけになります。**コントロールプレーン**は今までどおり `server` extra（boto3 / GCS）を保ちます。
...
- [ ] presigned のキービルダーと検証を、複数の保存先（証跡 / アーティファクト / シナリオ）に対応するようサーバー側で一般化する
- [ ] presigned PUT によるアーティファクトアップロード（Worker が org アーティファクトプレフィックスの下で URL を要求し PUT）
- [ ] presigned GET によるベースラインダウンロード（コントロールプレーンが一覧し署名、Worker が run 前に GET）
- [ ] presigned PUT による生成シナリオの保存（`record` ジョブ）
- [ ] Worker から `object_store_from_env()` / `_object_store()` を取り除き、`worker` extra から `boto3` を落とす
- [ ] テスト — アーティファクト / ベースライン / シナリオの presigned パスを実 HTTP サーバーに対して（Worker の認証情報なしで）検証、org 境界の再検証
- [ ] ドキュメント — `docs/self-hosting.md`（Worker は `BAJUTSU_S3_*` や AWS 認証情報を必要としない）とその日本語版を更新
