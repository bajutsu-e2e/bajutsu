...
3. **位置パスによる backing と属性の再照合。** 各ノードは、ルートからのインデックス位置パスを
   ...
   危険が残ります。照合こそが決定性の安全を保つ要です。（[BE-0287](../BE-0287-xcuitest-runner-multitouch-resilience/BE-0287-xcuitest-runner-multitouch-resilience-ja.md)
   では、この照合を後に `identifier` / `label` / `traits` へ狭め、`frame` を除外しました。スナップショットは
   UI がまだ落ち着く前に撮られるため、変化していない要素の frame もスナップショットとタップの間で正当に
   ずれ、frame で照合すると有効な要素を stale と読んでしまうためです。）
...
- [x] `/elements` のベースライン計測と記録（時間の所在、計測環境の明記）。
- [x] `app.snapshot()` による単一スナップショット `queryElements()`（正規化済み `Element[]` 出力は同一）。
- [x] 位置パスによる backing と属性の再照合（食い違いで `stale`）。
- [x] 暫定のドライバソケットタイムアウトを有界な値へ差し戻す。
- [x] 検証：fast gate のアドレッシング正しさテストと、目標に対する実機での遅延計測。

ログ：

- 2026-07-05: 実装スライス。単一スナップショット走査と決定性を保つアドレッシングを、純粋な
  `BajutsuRunner` ライブラリへ寄せ、`swift test` ゲートで検証できるようにしました。`PositionPath.swift`
  に、正規化済みの `SnapshotNode`、`flattenSnapshot(root:)`（pre-order、ルート除外、要素ごとに
  1 つの `PositionPathBacking`）、`attributesMatch(...)` を追加し、`PositionPathTests.swift` で
  位置パスの付与・フィールドの複製・一致／不一致を検証します。XCTest に触れる
  `XcuitestElementProvider` は `app.snapshot()` を 1 回取り、`tap` / `gesture` の時点で記録した
  インデックス位置パスから実要素を再導出し、属性が食い違えば `.stale` を返します。Python ドライバの
  暫定 60 秒ソケットタイムアウトは、有界な `_SOCKET_TIMEOUT_SECONDS` へ差し戻しました
  （`tests/test_xcuitest.py`）。`/elements` のベースラインと実機での受け入れ数値（目標 1〜2 秒）は、
  提案の検証の切り分けに従い残します。
- 2026-07-05: 実機での計測と検証。iPhone 17 Pro Max、iOS 26.5、Xcode 26.6（17F113）で、showcase
  SwiftUI 画面（約 112〜114 要素）に対して計測しました。`GET /elements` の中央値は、旧来の属性
  個別読みの **約 18.6 秒**から、単一 `app.snapshot()` の **約 0.034 秒**へ短縮し、目標の 1〜2 秒を
  十分に下回りました。旧走査は差し戻した 15 秒のソケットタイムアウトを実際に超えており、暫定処置が
  効いていたことも裏づけられました。アドレッシングの正しさもエンドツーエンドで確認しています。実機の
  ドライバ conformance スイート（`tests/test_driver_conformance_ondevice.py`、xcuitest + idb）は
  18 ケースすべて成功し、位置パスの再導出が解決した要素を正確にタップし、stale と曖昧の扱いも
  保たれています。これで 5 つの作業分解が完了しました。
