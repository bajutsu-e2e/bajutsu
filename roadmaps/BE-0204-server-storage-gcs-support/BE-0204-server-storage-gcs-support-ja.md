...
`BAJUTSU_SERVER_STORE`（`s3://bucket/prefix` または `gs://bucket/prefix`）が `BAJUTSU_S3_BUCKET` /
`BAJUTSU_S3_PREFIX` を置き換えます。`BAJUTSU_S3_ENDPOINT` / `BAJUTSU_S3_REGION` は変更せず、引き続き
