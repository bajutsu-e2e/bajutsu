...
各後続項目は、出荷済みの土台、具体設計、検証方法を示します。機械的に検証できる契約を持つのは二つだけです。
トピック別に挙げます。

1. **組織間で公平な重み付きジョブ分配**。org 単位の上限による 429 拒否を*保留*へ変えます。org 別の
   待機キューとラウンドロビンのディスパッチャで、逼迫時にも希少なプールを公平に保ちます。ゲートで
   検証できる契約を持つ唯一の残りの部分です（Mac を使わず `ServeState` に対して単体テストします）。
...
- 残るプール化の作業を五つの焦点を絞ったロードマップ項目（重み付き公平分配、能力別ルーティングキュー、制御プレーンのスケールアウト、高可用性、可観測性）へ分割し、このアンブレラを実装済みへ移して、出荷済みのセルフホストのベースラインを記録しました。
