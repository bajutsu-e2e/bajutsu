...
見送ると決めた提案を示します。変更が及ぶのは、メタデータの語彙、ダッシュボード生成スクリプト、
5本のロードマップ関連スクリプト、それらのゲートテストです。
...
記入済みです。BE-0159 は、BE-0154 の提案全体が前提としていた状態別フォルダを廃止したため、もはや
昇格の起点となるベース SHA は残っていません。BE-0154 はブロッカーを待っているのではなく、提案
...
開かないかぎり見分けがつきませんでした。BE-0078 はこの区分を割り、`In progress` に独立した値を
与えました。今回も同じ論法が当てはまります。`Deferred` 区分は、「まだ見直す価値がある」と
...
がない」という事情）や、項目自身が挙げる将来の具体的なニーズ（BE-0027 が挙げる、「宣言的な
スタブとして表現するには状態を持ちすぎる、あるいはプロトコルに重すぎるバックエンド」という条件）
です。作業はしていませんが、その項目が提起する問いは開いたままです。
...
- [`roadmaps/README.md`](../README.md) と [`README-ja.md`](../README-ja.md)：
  「ロードマップ項目の追加」の注記にある1行の状態一覧に `Rejected` を加えます。
...
- BE-0027 は、本当に判断が割れるケースです。本文の「はじめに」は、宣言的な `mocks` によって
  すでに無効化されたと述べる一方で、復活の具体的な条件（宣言的なスタブの言語を超える、状態を
  持ちすぎる、あるいはプロトコルに重すぎるバックエンド）も名指ししており、`Superseded by`
...
[`roadmaps/README.md`](../README.md) の「不採用」節は別の仕組みであり、そのままとします。
これは、番号付きのBE項目になる前に不採用と決まったアイデアを記録するものであり、未成熟な
アイデアをまず「未整理のアイデア」に置き、対象範囲が固まってから初めて番号付き項目へ昇格する
というロードマップ自身の昇格ルールに従います。`Rejected` は `roadmaps/BE-NNNN-<slug>/` として
すでに存在する項目にのみ当てはまります。本項目は、「不採用」の箇条書きを却下するためだけに
番号付き項目へ昇格させることを提案しません。それは、その箇条書きがすでに一度記録している
内容のために、恒久的なBE IDを1つ費やすことになるからです。

### プライムディレクティブとの整合

対象となるのは、メタデータの語彙、5本のスクリプト、それらのゲートテストだけです。どの経路にも
...
でした。`Deferred` を枠組み付きのままにするなら平らな `In progress` / `提案` / `実装済み`
...
場合があるからです。`状態` を他のすべての区分と同じく手で設定したままにすることは、
`実装済み` を除けば他のどのロードマップフィールドからも導かれないという既存の慣行と
一致します。
