...
同じ3つの試行を、iOS 26.5とiOS 18.6の両方のSimulatorで繰り返し、結果は同一でした。これは
...
上で機能しないステップの出荷を認めません。この項目を再開するには、Apple側の修正か文書化された
回避策、選択を確実に登録できるまだ試していないアクチュエーション手法、あるいは`selectPhotos`
を実機・Intel Simulator限定にするという意図的な決定のいずれかが要ります。今回の調査では、
そのいずれも見つかりませんでした。
...
- 2026-09-16 — showcaseアプリに対してUnit 3を調査した(Xcode 26.6、iOS 26.5とiOS 18.6の
  両Simulator、Apple silicon Mac)。試したアクチュエーション手法はどれもセルの選択を成立
  させられなかった。他の3つのUnitだけを出荷せず、この項目を保留とした。PRなし: 中核となる
  仕組みが動かない項目からは何もマージしない。
