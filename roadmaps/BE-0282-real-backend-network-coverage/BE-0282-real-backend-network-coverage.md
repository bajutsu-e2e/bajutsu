...
- [#1701](https://github.com/bajutsu-e2e/bajutsu/pull/1701) — closed the web redaction check's over-redaction blind spot: a body replaced wholesale by the placeholder satisfied both of its body rules, so the check now requires the Sync request's non-secret `account` key to survive, the rule the iOS twin already carries. Brought the twin's executed-checker tests across with it — `assert_redaction.py`'s `main` ran in no gate test at all, and the browser lane only ever hands it correctly masked evidence, so its rules passed whether or not they still said anything.
