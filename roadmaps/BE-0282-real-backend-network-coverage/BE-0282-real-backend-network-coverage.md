...
| Implementing PR | [#1183](https://github.com/bajutsu-e2e/bajutsu/pull/1183) (web slice), [#1267](https://github.com/bajutsu-e2e/bajutsu/pull/1267) (promote web job to gate), [#1701](https://github.com/bajutsu-e2e/bajutsu/pull/1701) (web over-redaction rule), [#1693](https://github.com/bajutsu-e2e/bajutsu/pull/1693) (iOS slice) |
...
- [x] iOS collector real path: connect `network_mock.yaml` / `network_live.yaml` as a non-gating job.
- [x] Record the Android network gap explicitly (out of scope pending a native monitor).
- [x] Land as signal first; promote to required only after it proves stable. (Web `network (playwright)` job promoted into the `E2E (web)` gate after proving stable in CI; the iOS `network (xcuitest)` job landed as signal and awaits the same stability record.)
...
- [#1693](https://github.com/bajutsu-e2e/bajutsu/pull/1693) — iOS slice, the last unit: a `redact: { fields: [password] }` policy on `showcase-swiftui`, `demos/showcase/network/assert_network_evidence.py`, the `make -C demos/showcase e2e-network` target running `network_mock.yaml` + `network_live.yaml` into a throwaway runs dir, and the non-gating `network (xcuitest)` CI job. The redaction check also asserts the unstubbed exchange carries `mocked` false, so an over-broad mock matcher cannot claim traffic nothing stubbed — a check the web twin, holding only mocked traffic, cannot make. `scripts/e2e_changes.py` keys the job on its Makefile-declared scenarios (BE-0338), and `tests/test_showcase_network_demo.py` pins the scenarios, the policy, and the app's secret literals on the fast gate. Verified on a booted iPhone 17 Pro Simulator: both scenarios PASS, the captured `POST /post` is recorded `mocked: true` with status 201 and both secrets masked to `[REDACTED]`, and the observed `GET /horses` is recorded `mocked: false`.
