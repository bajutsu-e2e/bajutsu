...
| 実装 PR | [#1144](https://github.com/bajutsu-e2e/bajutsu/pull/1144) |
| トピック | セキュリティ強化 |
...
- [x] バインド中の config が宣言する `secrets:` の名前を解決する（ターゲット横断の和集合）
- [x] `GET /api/secrets`（読み取り操作）。ロールゲートなし、平文の値を含まない
- [x] `POST /api/secrets`（書き込み操作）。admin ゲート、未宣言の名前は拒否
- [x] `ServeState._env_var_for_secret` をシナリオのシークレット名向けに拡張する
- [x] UI: `serve.core.mjs` に「シナリオのシークレット」セクションを追加し、config 変更時に再取得する
- [x] 両バックエンドが名前ごとに保存する。ローカルの `EnvSecretStore` と（セルフホストの）
      `DbSecretStore` — seam の切り替えは継承されるため、新しい保存コードではなくテストと検証の項目
- [x] テスト: 宣言済み名前の反映、未宣言名前の拒否、マスクのみの往復、spawn されたローカル run への
      環境引き継ぎ、ホスト型 `DbSecretStore` の org ごとの往復（`ciphertext` が平文を持たない）
- [x] ドキュメント更新（`docs/web-ui.md`、`docs/self-hosting.md`、両言語）

**ログ**

- チェックリストの項目を一つの変更でまとめて実装しました。`operations/config.py` の
  `GET`/`POST /api/secrets` と `declared_secret_names`、`state.py` の宣言済み名前を恒等に写す
  `_env_var_for_secret`、共有ルートと `_ADMIN_PATHS` のゲート、Settings の「シナリオのシークレット」
  UI、そして両言語のドキュメントです。保存は両バックエンドで確認し、セルフホストの消費側は follow-up
  のままです（詳細設計7）。PR: [#1144](https://github.com/bajutsu-e2e/bajutsu/pull/1144)。
