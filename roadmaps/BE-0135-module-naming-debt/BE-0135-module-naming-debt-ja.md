[English](BE-0135-module-naming-debt.md) · **日本語**

# BE-0135 — トップレベルモジュールの命名の負債を解消する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-0135](BE-0135-module-naming-debt-ja.md) |
| 提案者 | [@0x0c](https://github.com/0x0c) |
| 状態 | **実装済み** |
| トラッキング Issue | [検索](https://github.com/bajutsu-e2e/bajutsu/issues?q=is%3Aissue+label%3Aroadmap-tracking+in%3Atitle+"BE-0135") |
| 実装 PR | [#640](https://github.com/bajutsu-e2e/bajutsu/pull/640) |
| トピック | コードベース品質・技術的負債 |
| 関連 | [BE-0063](../BE-0063-git-config-source/BE-0063-git-config-source-ja.md)、[BE-0044](../BE-0044-scenario-provenance/BE-0044-scenario-provenance-ja.md) |
<!-- /BE-METADATA -->
...
2. **`bajutsu/environment.py` → `bajutsu/lifecycle.py`**（モジュール一覧を並べたときに
   ...
   します。この名前の決定は、将来のフォローアップ項目として想定している「backend のライフサイクルを型システムに
   載せる」（TBD）と調整する必要があります。もしその項目がこのモジュール内に `Lifecycle` という
...
- [x] `bajutsu/env.py` → `bajutsu/simctl.py` にリネームする。
- [x] `bajutsu/environment.py` → `bajutsu/platform_lifecycle.py` にリネームする。姉妹項目の [BE-0141](../BE-0141-backend-lifecycle-protocol/BE-0141-backend-lifecycle-protocol-ja.md)（まだ提案の段階です）に `Lifecycle` という Protocol 名を残すため、素の `lifecycle.py` ではなく `platform_lifecycle.py` を選び、本項目を先に反映しました。
- [x] `bajutsu/dotenv.py` と `bajutsu/config_source.py` はリネーム不要であることを確認する（記録のみ、対応なし）。
- [x] `bajutsu/capture.py` → `bajutsu/record_capture.py` にリネームする。
- [x] `bajutsu/provenance.py` → `bajutsu/from_grouping.py` にリネームする。

ログ：

- 4 つのリネームは、挙動を変えない純粋な `git mv` とインポートパスの更新としてまとめて反映しました。対応するテストファイルも名前を揃え（`test_env.py` を `test_simctl.py` へ、`test_capture.py` を `test_record_capture.py` へ、`test_provenance.py` を `test_from_grouping.py` へ、`test_environment_seam.py` を `test_platform_lifecycle_seam.py` へ）、注入可能なランナーの引数名が `simctl` である `serve` の 3 ファイルでは、`simctl` モジュールを `_simctl` として別名で取り込みました。あわせて `docs/`（両言語）と `DESIGN.md`、および [BE-0038](../BE-0038-autonomous-crawl-exploration/BE-0038-autonomous-crawl-exploration-ja.md) にある唯一の実リンクの参照も更新しました。

## 参考

- `bajutsu/simctl.py:1-5`（旧 `bajutsu/env.py`） — モジュール docstring：`simctl` コマンドビルダーのラッパー。
- `bajutsu/platform_lifecycle.py:1-11`（旧 `bajutsu/environment.py`） — モジュール docstring：プラットフォームごとの `Environment`
  Protocol。
- `bajutsu/dotenv.py:1-7` — モジュール docstring：`.env` ローダー。
- `bajutsu/config_source.py:1-11` — モジュール docstring：Git からの config 取得（BE-0063）。
- `bajutsu/record_capture.py:1-5`（旧 `bajutsu/capture.py`） — モジュール docstring：record 時のアクションキャプチャ
  （BE-0012）。
- `bajutsu/from_grouping.py:1-7`（旧 `bajutsu/provenance.py`） — モジュール docstring：`from:` の表示グルーピング
  （BE-0044）。
...
- 関連するロードマップ項目：[BE-0063](../BE-0063-git-config-source/BE-0063-git-config-source-ja.md)
  （Git config source — `config_source.py` を名指しする項目）、
  [BE-0044](../BE-0044-scenario-provenance/BE-0044-scenario-provenance-ja.md)
