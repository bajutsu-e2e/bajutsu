...
- 起動済みの iPhone 17 Simulator（iOS 26.5）上のショーケース SwiftUI アプリから、4つのフィクスチャ
  ...
  伸ばしかけるボタンは、削除するボタンだからです。捕捉は `tests/test_alert_fixtures_ondevice.py` が行います。
  各ボタンの frame は目で採寸するのではなくデバイスから読み取ります。3つの OS プロンプトは
  `handleSystemAlert` が解決に使うのと同じアクセシビリティクエリ（BE-0316）から、アプリ自身の
  ダイアログはアプリの要素ツリーから取得します。期待するボタンがデバイスから実際に報告されなければ
  フィクスチャの
