...
| 実装 PR | [#653](https://github.com/bajutsu-e2e/bajutsu/pull/653) |
| トピック | 外部サービスとの連携 |
<!-- /BE-METADATA -->
...
- [x] アーティファクトの `kind → MIME` 対応表。
- [x] `ctrf_json()` ビルダー（summary / tests / steps / attachments / environment / `extra`、直列 + マトリックス）。
- [x] レポート組み立て（`run`）と `bajutsu report` の再生成への配線。
- [x] テスト：直列実行とマトリックス実行でスキーマ適合の `ctrf.json`、フィールドの往復。
- [x] 日英ドキュメント：`ctrf.json` を実行アーティファクトとして記載し「CI で消費する」注記を添える。
- [ ] *(オプション)* 実行／シナリオへの絶対エポック開始時刻の記録で `start`／`stop` を正確化。`manifest.json` のスキーマ更新。

**ログ**

- 2026-07-04 — 出力機能を出荷しました。`bajutsu/report/ctrf.py` に `kind → MIME` の対応表と
  `ctrf_json()` の射影を実装し、`write_html_and_junit` に配線して `run` と `bajutsu report` の両方が
  `ctrf.json` を出すようにしました（再生成の経路のため provenance を `RenderModel` に通しています）。
  あわせて `ctrf.json` を秘匿値のスクラブ対象に加えました。テストは直列実行とマトリックス実行を、
  vendor した公式 CTRF スキーマに対して検証し、対応づけたフィールドを往復させています。日英ドキュメントも
  追記しました。オプションの絶対エポックの精緻化は後続に回します。
