[English](BE-XXXX-serve-author-tiling-layout.md) · **日本語**

# BE-XXXX — Author ビューをタイリングレイアウトに組み込む

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-serve-author-tiling-layout-ja.md) |
...
- [ ] Unit 1 — 既定木と optional ペインつきで `view-author` を `SPECS` に登録。
- [ ] Unit 2 — YAML エディタを第一級のリサイズ可能ペインに。
- [ ] Unit 3 — マークアップを整合させ、冗長な `#view-author` 固定グリッドを外す。
- [ ] Unit 4 — `bajutsu-tiles` 下での永続化・有効性の再利用。
- [ ] Unit 5 — ブラウザ検証とタイラー spec のテスト網羅。
