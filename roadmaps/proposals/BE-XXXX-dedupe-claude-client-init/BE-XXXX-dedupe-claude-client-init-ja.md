...
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-dedupe-claude-client-init-ja.md) |
...
- [ ] `bajutsu/anthropic_client.py` に、クライアント初期化とキャッシュを行う共有実装を追加する
- [ ] `_ensure_client` の 6 つのコピー（`claude_agent.py`、`claude_triage.py`、
      `claude_enrich_agent.py`、`alerts.py`、`crawl_guide.py`、`crawl_tabs.py`）を、共有実装
      への呼び出しに置き換える
- [ ] 共有実装に対するユニットテスト（注入によるショートサーキット、一度生成したら再利用する
