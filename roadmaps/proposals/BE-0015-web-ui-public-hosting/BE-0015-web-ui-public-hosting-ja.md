...
org が複数になって初めて意味を持つ部分は後回しです。**org スコープの強制**（org をまたぐアクセスは 403）、
`ScenarioStore`／オブジェクトストレージ／`BaselineStore`（すでに `<org>/` prefix でパラメータ化済み）と run 一覧
（すでに org で絞り込み済み）の **tenant prefix へ解決済み `org_id` を渡すこと**、そして org モデルそのもの
