...
app-agnostic を保ち、追加の GitHub scope も要りません。

```yaml
orgs:
  acme:
    members: [alice, bob]   # GitHub login
    apps: [demo, checkout]
  globex:
    members: [carol]
    apps: [other]
...
- **org 解決（8b）**: OAuth ログイン時にユーザを config の org に割り当て（`org_for_user`、user 行に保存）、
  以後のリクエストはその行から actor の org を解決します（`ServeState.org_of`）。run 一覧・run レコード・監査
  エントリは解決した org の下で読み書きします。
