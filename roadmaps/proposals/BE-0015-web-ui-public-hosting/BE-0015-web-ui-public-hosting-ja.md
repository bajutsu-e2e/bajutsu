...
（`Repository` は `record_run`／`get_run`／`list_runs` を備えますが、run の一覧は当面オブジェクトストレージから
取得します。DB からの org スコープ一覧はマルチテナント化を待ちます。）
...
org が複数になって初めて意味を持つ部分は後回しです。**DB からの org スコープ run 一覧**（7c-4）、**org スコープの
強制**（org をまたぐアクセスは 403）、`ScenarioStore`／オブジェクトストレージ／`BaselineStore`（すでに `<org>/`
prefix でパラメータ化済み）の **tenant prefix へ解決済み `org_id` を渡すこと**、そして org モデルそのもの
（GitHub org への対応、複数 org）です。それまでは、上記のシングルテナント backend が出荷済みのベースラインです。
