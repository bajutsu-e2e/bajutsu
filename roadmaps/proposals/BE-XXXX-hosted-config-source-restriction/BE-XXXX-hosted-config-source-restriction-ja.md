[English](BE-XXXX-hosted-config-source-restriction.md) · **日本語**

# BE-XXXX — ホスティング時は config の取得元をアップロードと Git だけに絞る

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-hosted-config-source-restriction-ja.md) |
...
[BE-0015](../../in-progress/BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md)）には意味を
...
- **バックエンドではなく非ループバック公開を判定に使う。** `server` バックエンドに限らず、`serve` が
  ...
  （[BE-0016](../../in-progress/BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting-ja.md) の Tier A）は
...
- [ ] `ServeState` の `hosted: bool` を server バックエンドが設定する
- [ ] `/api/config`（`config_info`）で `configSources` を提示する
- [ ] `fs` を提示しないとき、フロントエンドがファイルブラウザの取得元を隠す
- [ ] ホスティング時に `browse_fs` / `bind_config` の path 分岐を拒否する（サーバ側の強制）
- [ ] テスト。ホスティング時は `fs` を外し拒否する、ローカルは三つとも保つ
- [ ] ドキュメント更新（取得元がデプロイ形態に依存する旨）。両言語
