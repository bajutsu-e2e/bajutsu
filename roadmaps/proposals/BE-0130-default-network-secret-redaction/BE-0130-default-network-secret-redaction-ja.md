...
- [ ] 組み込みの機密ヘッダー既定集合を無条件で隠す
- [ ] 既定の一部を無効化する、明示的かつ見える形の抜け道（暗黙のオプトアウトなし）
- [ ] ヘッダー名照合の正規化（大文字小文字を区別しない、`cookie` と `set-cookie` の連動）
- [ ] テスト：`redact:` なしでの既定マスキング、`cookie` が `Set-Cookie` も隠すこと、
      抜け道によるオプトアウト
- [ ] ドキュメント更新（日英両方）
