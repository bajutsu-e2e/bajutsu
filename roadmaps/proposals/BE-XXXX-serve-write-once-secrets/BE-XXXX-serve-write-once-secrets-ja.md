...
- **設定**（`POST /api/apikey`）には `admin` ロールが必要です。`/api/apikey` は
  `authz.py:122` の `_ADMIN_PATHS` に含まれています。
- **平文での読み出し**（`GET /api/apikey?reveal=1`）にはロールがまったく要求されません。
  `authz.py:147` の `required_role` はすべての `GET` に対して `None` を返すため、認証済みの
  セッションであれば、BE-0015 7c-2 で定義された読み取り専用ロールである `viewer` でも、
  より高い権限を持つ管理者が設定した認証情報をそのまま取得できます。認証済みの `serve`
  セッションに到達できる人物なら誰でも、組織の Claude・Bedrock の認証情報を引き出せてしまいます。
...
そのまま流用します。値は AEAD 暗号（`cryptography` パッケージの `Fernet`。既存の `db` extra の
下に新規依存として追加します)で暗号化し、鍵にはオペレーターが用意するマスターキー
...
- [ ] `SecretStore` の Protocol（`set` / `describe`。HTTP ハンドラから到達できる `get` は持たない）
- [ ] ローカル実装 — 今の `os.environ` による挙動を差し替え口の内側へ移す
- [ ] hosted 実装 — `secrets` テーブル（`org_id`、`name`、`ciphertext`）、`BAJUTSU_SECRETS_KEY`
      で鍵付けした `Fernet` 暗号化、`db` extra の下への追加
- [ ] `GET /api/apikey` から `reveal` を削除し、どのロールでも平文の `value` を返さないようにする
- [ ] `api_key_info` / `set_api_key` を `SecretStore.describe("aiApiKey")` /
      `.set("aiApiKey", value)` へ一般化する
- [ ] テスト — ロール・reveal の削除、設定してから読み出す一連の操作、hosted 側の `ciphertext` に
      平文が含まれないことの確認
- [ ] ドキュメント更新（`docs/getting-started.md`、`docs/self-hosting.md`）を両言語で
...
関連項目として、[BE-0015](../../in-progress/BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting-ja.md)
