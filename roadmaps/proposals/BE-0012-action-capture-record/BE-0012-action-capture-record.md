...
- **The stability ladder** is already encoded in `crawl.py:Action.as_selector()` and `crawl_repro.py:_selector()` (`id` → `label` (+`index`) → coordinate). `crawl_repro` already refuses to emit a step for an unaddressable coordinate ("faithful or nothing"); capture follows the same stance and stops the ladder at `label` — an id-less and label-less tap is refused, never degraded to a coordinate (Decision 1).
