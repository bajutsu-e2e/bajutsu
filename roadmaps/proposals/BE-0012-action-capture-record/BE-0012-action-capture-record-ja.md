...
- **安定度ラダー**はすでに `crawl.py:Action.as_selector()` と `crawl_repro.py:_selector()` に符号化されています（`id` → `label`（+ `index`）→ 座標）。`crawl_repro` は指定できない座標についてはステップを出さない（「忠実に再現できないなら出さない」）方針で、キャプチャも同じ立場を取り、ラダーを `label` で止めます。id も label も無いタップは座標へ落とさず拒否します（決定 1）。
...
`id` があれば一意性を検証して出力します（画面内で id が重複していれば曖昧として提示します。これは `doctor` の `Blocked` 状態です）。なければ `label` で一意なら出力し、そうでなければ `index` を足して解消を試み（フレーキーとして印を付けます）、それでも駄目なら提示します。`id` も `label` も無ければ**キャプチャを拒否し**、その要素には `accessibilityIdentifier` が必要だと著者に伝えます（決定 1、「忠実に再現できないなら出さない」）。座標 tap のステップ型は意図的に**設けません**。`Step` は `tap: Selector` だけを持ち、`id` / `label` で指定できないタップが座標として書き出されることはありません（これは指定できないタップを出さずに飛ばす `crawl_repro` の方針と一致します）。`el → Selector` のラダーは現状 3 箇所（`crawl`、`crawl_repro`、本キャプチャ）で実装されるため、本提案では別の小さなリファクタとして 1 つの共有ヘルパへ集約することを推奨します。
