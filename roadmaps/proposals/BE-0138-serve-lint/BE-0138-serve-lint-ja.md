...
（[BE-0013](../../implemented/BE-0013-scenario-gui-editor/BE-0013-scenario-gui-editor-ja.md)）に先んじて、
かつそれを補完します。
...
* **構造化 GUI エディタ
  （[BE-0013](../../implemented/BE-0013-scenario-gui-editor/BE-0013-scenario-gui-editor-ja.md)）が検証を
...
- [ ] `bajutsu/lint.py` から行に紐づいた診断を返す `POST /api/lint`（`{yaml}`）エンドポイントを
      追加する
- [ ] シナリオの JSON Schema（`lint.scenario_json_schema`）をクライアントに渡し、それで検証する
- [ ] 診断をその場に表示する（溝のマーカーと問題一覧）。編集しながらデバウンスし、要求に応じても出す
- [ ] シナリオ文法のスキーマ駆動の補完／ホバーを追加する
