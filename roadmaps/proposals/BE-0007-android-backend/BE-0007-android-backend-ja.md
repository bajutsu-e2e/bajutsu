...
Android は **idb の構造的双子**です。subprocess 駆動、座標 actuation、遷移中の一過性に空なツリー —— その
...
この共通の形ゆえに、idb の *retry 付き解決・曖昧は即失敗* パターンをそのまま再利用します —— ツリーを
...
Android は **第 2 段階**で、Web（Playwright）バックエンド（[BE-0041](../../proposals/BE-0041-web-playwright-backend/BE-0041-web-playwright-backend-ja.md)）の
...
- **Web より先に Android を作る。** 展開順の理由で却下: Android が構造的に近い双子であっても、Web は macOS も
  エミュレータも不要で、現行 Linux ゲートに最低コストで収まります。[BE-0041](../../proposals/BE-0041-web-playwright-backend/BE-0041-web-playwright-backend-ja.md) を参照。
