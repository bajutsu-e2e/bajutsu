[English](BE-XXXX-config-source-owner-repo-validation.md) · **日本語**

# BE-XXXX — config-source の owner・repo 検証を厳格化する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-config-source-owner-repo-validation-ja.md) |
...
- [ ] `_GITHUB_RE` と `_GIT_URL_RE` の owner/repo の文字クラスを GitHub の実際の文字集合へ
      厳格化し、`.`、`..`、`%` を拒否する。
- [ ] この拒否が、パース不能な source と同様にパース失敗として現れることを確認する。
- [ ] 拒否されるべき形と既存の正当な形の両方をカバーする単体テストを追加する。
