...
### 2. open な roadmap PR を、main の変化に合わせて再検査する
...
この対策は対策1に依存します。未採番のPRのマージ結果を再検査しても、プレースホルダー対応のフォーマット検査が存在して初めてプレースホルダーの形のドリフトを捉えられるからです。したがって対策2は対策1より先に出荷せず、対策1と同時か、その後に出荷します。ブランチに push しないという制約は譲れません。BE-0089は、承認後にPRブランチへコミットが push されるとbranch protectionの「stale approvalの取り消し」が働き、auto-mergeが止まることをすでに明らかにしています。採番処理自体を承認時点の経路から外したのはそのためです。読み取り専用のマージシミュレーションであれば、ブランチに一切触れずにドリフトを報告できるため、レビューの承認を取り消すことはありません。

対策2が担うのはフィードバックの速さであって、安全性そのものではありません。対策1と対策3を合わせれば、不適合な項目が`main`に到達しないことはすでに保証されています。対策3はブランチがどれだけ古くても、マージ時点で無条件に検証するからです。対策2は、その最終チェックが捕まえるまでの間、古びたPRが検知されずに放置される日数を縮めるだけです。
...
- [ ] テンプレートに関わるコミットが`main`に着地したときに起動し、open な roadmap PR を現在の`main`に照らして読み取り専用で再検査するワークフローを追加する（上記に依存）
...
- [BE-0089](../../implemented/BE-0089-merge-time-be-id-allocation/BE-0089-merge-time-be-id-allocation-ja.md)：合流時点でのID採番。そのFeasibility節が、この項目で狭めるプレースホルダー除外の出所であり、その「bypass アイデンティティの保護」節が、対策3を依存関係なしに保つべき理由です
