...
元のゴールはシナリオ単位で保持します。`record`（Tier 1・AI）が自然言語を構造化シナリオへ正規化する際に
...
保存することを求めています — 「由来を `# from:` コメントか sidecar として残す」。
[BE-0002](../../implemented/BE-0002-m2-ai-loop-and-evidence/BE-0002-m2-ai-loop-and-evidence-ja.md) は M2 の
...
4. **決定性は無傷。** `from:` は純粋なメタデータです。`run` は決して読まないので、ゲートに LLM 呼び出しを
   **一切**増やさず、合否に影響しません — オーサリング側に完全にとどまります
...
- **グループ化は新構文ではなく自然な創発。** 1 つの発話が複数ステップを生む場合、それらのステップは
  **同じ** `from:` 文字列を持ちます。レポート（と `trace`）は同一 `from:` の連続を 1 つのラベル付き
  グループにまとめます。span/range 構文は導入しません — round-trip を単純に保ち、各ステップ単位で
  独立に手編集できるようにします。
...
- `claude_agent.py`: 各アクションツール（`tap` / `type_text` / `wait_for`）に任意の `intent` 引数を
  追加します — そのアクションの*理由*を表す短い語句で、`Step.from_` に保存します。`finish` ツールの
...
- [BE-0002 — AI authoring loop & evidence (M2)](../../implemented/BE-0002-m2-ai-loop-and-evidence/BE-0002-m2-ai-loop-and-evidence-ja.md) — 来歴が先送りされた箇所
