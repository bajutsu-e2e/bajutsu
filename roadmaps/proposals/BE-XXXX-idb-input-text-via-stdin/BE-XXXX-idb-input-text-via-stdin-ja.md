...
- `idb ui text` への入力を argv ではなく stdin 経由で渡し、プロセスのコマンドラインに値が一切現れないようにします。`idb` の `ui text` サブコマンドは位置引数のテキストを渡さない場合に stdin を読む（stdin が空のときのみ argv にフォールバックする）ため、これは idb 側の新しい機能を必要としない、呼び出し方だけの変更です。
