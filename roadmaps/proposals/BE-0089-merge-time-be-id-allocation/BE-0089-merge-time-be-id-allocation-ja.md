...
マージのすべてを通じて `BE-0089` プレースホルダのままで、ブランチは `BE-0089` を残した *そのまま*、
...
マージ後に `main` で採番すれば、この問題ごと回避できる。レビュー済みブランチは承認されたとおり、`BE-0089`
...
**そのまま** 再利用する。これはすでに、ワーキングツリー内の各 `BE-0089-<slug>/` プレースホルダを見つけ、
...
1. `ideation` スキルが項目を `BE-0089-<slug>` として書き起こす（従来どおり）。PR は `[BE-0089]` タイトルで
   開かれる。
2. レビュアーが `BE-0089` の内容をレビューして承認する。**ブランチ上では採番は起きない。**
3. auto-merge（または merge queue）がブランチを `BE-0089` を残した **そのまま** マージする。承認後に
...
PR タイトルは書き換えない。作成からマージまで `[BE-0089]` 接頭辞のままにする。実番号はブランチ上
では決して分からず（採番はマージ後にだけ行われる）、タイトルの書き換えはマージ後に編集する手間に見合う
機能上の利点がない。割り当てた ID を記録し、マージ済み PR と結びつける自然で永続的な場所は bot コメント
である。
...
```bash
out="$(python3 scripts/allocate_roadmap_ids.py)"      # BE-0089 ディレクトリをその場でリネーム
...
- **`main` 上の一時的な `BE-0089` でもゲートは緑のまま。** ロードマップ系の 3 つのツールはいずれも
  ...
  [`promote_roadmap_items.py`](../../../scripts/promote_roadmap_items.py) である。したがって `BE-0089`
...
- **出力を縛り、検証する。** 正規の push は常に同じ狭い機械的差分になる。`BE-0089` → `BE-NNNN` の rename と
...
多層防御としてそのまま残す（害はない）ことであり、claim／repair／gc の複雑さを退役させることは、
...
- **`main` への直接 push ではなく、bot の採番 PR で採番する。** 直接 push を避ける手に見えるが、bypass 要件を
  ...
  ので、結局 bot がそれを bypass（または充足）する必要がある。採番ごとに 2 本目の PR が増え、`BE-0089` が
...
- [`scripts/allocate_roadmap_ids.py`](../../../scripts/allocate_roadmap_ids.py)（そのまま再利用）、
  ...
  [`scripts/promote_roadmap_items.py`](../../../scripts/promote_roadmap_items.py)：いずれも `BE-0089`
