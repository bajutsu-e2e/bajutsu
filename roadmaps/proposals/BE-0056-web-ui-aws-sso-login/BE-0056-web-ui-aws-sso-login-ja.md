...
> 作業の進行に合わせて最新に保ちます。チェックリストは *詳細設計* の作業分割に対応します。

- [x] バックエンドの継ぎ目 — `SsoEngine` プロトコル + `NativeSsoEngine`(boto3 `sso-oidc` デバイスフロー、botocore トークンキャッシュ)。`ServeState` に注入。
- [x] serve の操作 + 両バックエンドのルート — `/api/sso`, `/api/sso/login[/<handle>]`, `/api/sso/logout`（BE-0051 に従い admin 制限）。
- [x] Settings パネルの UI（Bedrock 時のみ）: profile + サインイン、検証 URL とコード、完了までポーリング、セッション状態 + サインアウト。
- [x] 注入した fake エンジンによるテスト（AWS なしでゲート緑）、ドキュメント（`cli.md` + ja）。
- [ ] 実際の IAM Identity Center に対する `NativeSsoEngine` の検証（AWS 呼び出しは Linux ゲートでは実行不可）。
- [ ] CLI 委譲エンジン（`aws sso login --no-browser`）を同じ `SsoEngine` インターフェースの裏に。
