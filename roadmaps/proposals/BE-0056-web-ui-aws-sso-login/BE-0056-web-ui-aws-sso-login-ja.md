...
（[BE-0016](../BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting-ja.md) Tier A）では唯一の入口でも
...
- **リモートの `serve` では `aws sso login` がそもそも使えない。** このコマンドは **serve ホスト側** で
  ...
  [BE-0016](../BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting-ja.md)）と、ブラウザが開く場所が違い
...
- **ローカルとリモートの両 `serve`。** 検証 URL とコードをブラウザに表示するので、Tailscale 越しのホスト
  （[BE-0016](../BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting-ja.md) Tier A）でもローカルと同様に
  動きます。
- **単一オペレーター。** serve プロセスは、到達できる全員に対して 1 つの SSO セッションを保持します。共有の
  ...
  [BE-0016](../BE-0016-web-ui-self-hosting/BE-0016-web-ui-self-hosting-ja.md)（OAuth/IdP、組織ごとに
  スコープした秘密情報）の領分です。
