...
* **提案された修正を自動適用する。** 不採用です。自己修復のガード規則（AI は提案し、人がオプトインで適用す
  る。[BE-0039](../../implemented/BE-0039-self-healing-propose-optin/BE-0039-self-healing-propose-optin-ja.md)）
  と衝突し、テストを黙って緩めてしまう危険があります
  （[BE-0023](../../implemented/BE-0023-self-healing-guards/BE-0023-self-healing-guards-ja.md)）。修正は
  常に利用者が受け入れる差分です。
...
- [ ] 既存のジョブ／SSE／キャンセルの仕組みを再利用してトリアージを serve のジョブとして実行する
      `POST /api/triage`（`{runId, scenario, ai?}`）エンドポイントを追加する
- [ ] 失敗した run の Replay レポートと履歴一覧に「Triage」操作を追加し、提案された修正には差分プレビューを
      添える
- [ ] AI（`--ai`）経路をオプトインで組み込み、既定は決定的なヒューリスティックエージェントのままにする
...
* [BE-0021 — AI トリアージ](../../implemented/BE-0021-ai-triage/BE-0021-ai-triage-ja.md)、
  [BE-0022 — 構造化された修正](../../implemented/BE-0022-update-structured-fixes/BE-0022-update-structured-fixes-ja.md)、
  [BE-0039 — 提案＋オプトイン適用](../../implemented/BE-0039-self-healing-propose-optin/BE-0039-self-healing-propose-optin-ja.md)、
  [BE-0023 — テストを緩めないためのガード](../../implemented/BE-0023-self-healing-guards/BE-0023-self-healing-guards-ja.md)
