[English](BE-XXXX-serve-zip-bundle-upload.md) · **日本語**
...
- **UI。** **Upload & run** パネルです。`.zip` をドロップし、展開後にパースした束の config の `apps:` から `--app` を選び、実行を押します。ジョブはログをストリームし、ほかの run とまったく同じくレポートを生成します。そして [BE-0060](../BE-0060-run-report-zip-export/BE-0060-run-report-zip-export-ja.md) のダウンロードが往復を閉じます（スイートをアップロード → 実行 → 結果をダウンロード）。
