...
# BE-XXXX — CLI コマンド層の残りをテストしてから、カバレッジフロアをラチェットする
...
- [ ] `lint.py` のコマンド各分岐（見つからない・読み取れない・lint エラーあり・advisory の有無
      それぞれのクリーンなケース）をユニットテストする
- [ ] `worker.py` の `_post_json` / `_object_store` / `_write_console_log`、および `worker()`
      ポーリングループの1周分をユニットテストする
- [ ] `mcp.py` のトランスポート検証、`fastmcp` の import ガード、`stdio` / `sse` の成功経路を
      ユニットテストする
- [ ] `trace.py` の「run が見つからない」分岐と `_explain` のエラー分岐をユニットテストする
- [ ] `crawl.py` のオプション検証・ディスパッチの各分岐（未知の agent、利用できないバックエンド、
      AI クレデンシャル欠如）をユニットテストする
- [ ] `schema.py` のコマンド本体に対する `CliRunner` テストを追加する
- [ ] `audit.py` の使用方法エラーの各分岐と `_history_audit` のグルーピング/判定ロジックを
      ユニットテストする
- [ ] `Makefile` の `--cov-fail-under` を新しい実測フロアまで引き上げ、`make check` がクリーンに
      通ることを確認する
