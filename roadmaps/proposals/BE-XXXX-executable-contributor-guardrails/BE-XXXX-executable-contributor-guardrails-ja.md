[English](BE-XXXX-executable-contributor-guardrails.md) · **日本語**

# BE-XXXX — コントリビュータ向けガードレールの実行可能化（手順をコマンドに）

* 提案: [BE-XXXX](BE-XXXX-executable-contributor-guardrails-ja.md)
...
- **散文だけのもの** — 読み手が手でたどるために書かれた、複数手順のレシピ。
  - **新しい roadmap 項目の雛形づくり** — ディレクトリと、*両方*の言語のファイルを、決まった
    Swift-Evolution 形式で作り、メタデータブロックを揃え、`BE-XXXX` プレースホルダを置き、author の
...
- `roadmaps/proposals/BE-XXXX-<slug>/` を作り、`BE-XXXX-<slug>.md` と `BE-XXXX-<slug>-ja.md` の
  両方を雛形から埋めます。日英のヘッダリンク、メタデータブロック（`Proposal` / `Author` / `Status` /
  `Track` / `Topic`）、そして Swift-Evolution の五つの節（`Introduction` / `Motivation` /
  `Detailed design` / `Alternatives considered` / `References`）を `TBD` で置きます。
- **常に `BE-XXXX` プレースホルダを書き**、番号は書きません。ID は恒久で単調増加であり、採番は CI の
  アトミックな仕事です（[BE-0061](../../implemented/BE-0061-be-id-allocation-hardening/BE-0061-be-id-allocation-hardening-ja.md)）。
...
- `TOPIC` を [`scripts/build_roadmap_index.py`](../../../scripts/build_roadmap_index.py) の既知の
  ...
  作成時に捕まえる価値のある、鋭い角です。）既定値は `Status=Proposal`、`Track=Proposals`、author は
  `git config` から解決します（`HANDLE=` で上書き可）。
- **索引の行は手で足しません。** 生成ツールは `BE-XXXX` の項目を飛ばすので、プレースホルダの間は
...
- slug がディレクトリ名と一致し、ファイル内の `BE-XXXX`/`BE-NNNN` のトークンがディレクトリと一致する；
- メタデータブロックが揃っている（`Proposal`・`Author`・`Status`・`Track`・`Topic` のすべてがある）；
...
- `BE-XXXX` の項目が、*別の* `BE-XXXX` 項目を相互参照していない（採番は項目ごとの書き換えなので、
...
- roadmap に触れる変更は、PR タイトルに `[BE-NNNN]`（または `[BE-XXXX]`）の接頭を持つ；
...
文章の質）では、あえて止めません。それらは散文と、人間・レビュアーの判断に残します。
...
- **`make` ターゲットではなく、出荷する `bajutsu dev …` CLI にする。** いまは却下します。`make <verb>` は
  ...
  できます。
- **専用の新トピックを設ける。** 代わりに *開発基盤（コントリビュータ体験）* の下（BE-0043 のトピック）に
...
- [BE-0043 — コンフリクトに強いファイル流動](../../implemented/BE-0043-conflict-resistant-file-flow/BE-0043-conflict-resistant-file-flow-ja.md)：
  手で編集する台帳を生成物に変えた、*開発基盤* の先例。
- [BE-0067 — コード品質ゲートの強化](../../implemented/BE-0067-code-quality-gate-hardening/BE-0067-code-quality-gate-hardening-ja.md)：
