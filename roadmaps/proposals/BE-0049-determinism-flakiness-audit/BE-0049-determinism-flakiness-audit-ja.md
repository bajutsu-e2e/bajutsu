...
フレーキネスを許容するのではなく、シナリオの決定性を*証明する*読み取り専用の診断です。同一条件で繰り返し実行して非決定的な結果を検出し、セレクタと待機の安定性を静的に採点します。これは助言にとどまり、ゲートの完全に外側にあります。ばらつきは所見として報告するもので、黙って通過させることも、判定に使うこともありません。
