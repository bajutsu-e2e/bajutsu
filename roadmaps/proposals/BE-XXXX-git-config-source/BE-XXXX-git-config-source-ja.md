...
現状、各コマンドの `--config <path>` はローカルファイルシステム上のパス（既定は `bajutsu.config.yaml`）で、config 内の `scenarios` / `baselines` / `setup` / `appPath` / `build` は作業ディレクトリ基準の相対パスである。本提案では、`--config`（および serve の config ピッカー）に、代わりに **ある ref を指す Git リポジトリ**を `github:<owner>/<repo>@<ref>:<path>` の形で指定できるようにする。Bajutsu はその ref におけるリポジトリ部分木を実体化し、そこから config を読み込み、config の相対パスを、チェックアウトした木に対して解決する。変わるのは config とシナリオ一式の取得方法だけで、スキーマ、ランナー、ドライバ、決定的ゲートはそのままである。
