...
Bajutsu には公開向けのウェブサイトがありません。このプロジェクトが何であり、なぜ決定性を最優先するのか、どう使い始めるのか。その説明はリポジトリの `README` と `docs/` にしかなく、読み手は GitHub を辿って読むしかありません。サイトとして公開されているのは自動生成の API リファレンス（[BE-0065](../../implemented/BE-0065-docstring-standard-api-reference/BE-0065-docstring-standard-api-reference-ja.md)）だけで、それさえもまだ稼働していません。リポジトリで GitHub Pages が有効になっておらず、デプロイのジョブが手動実行に限定されているからです。
