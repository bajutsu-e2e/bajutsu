...
|---|---|---|---|
| **Presigned URL**（serve） | Server（control plane） | `httpx`（既存依存） | `serve` 経由の実行 |
| **直接 SDK**（スタンドアロン） | Runner 自身 | `boto3` / `gcs`（optional） | `bajutsu run --evidence-store` |
...
3. Server がファイルごとに presigned PUT URL を生成
...
- [ ] `S3ObjectStore` 実装（既存コードの再利用、`presigned_put_url` 追加）
- [ ] `GCSObjectStore` 実装（V4 signed PUT URL を含む）
- [ ] Presigned URL アップロードエンドポイント（`POST /api/runs/<run_id>/upload-urls`）
- [ ] Worker 側の HTTP PUT アップローダー（presigned URL モード、httpx ベース）
- [ ] 直接 SDK アップロードのフォールバック（スタンドアロン `bajutsu run` モード）
- [ ] `runner/pipeline.py` への run 完了後アップロードステップ追加（モード選択）
...
- [ ] テスト（URI パース、presigned URL 生成、HTTP PUT アップロード、serve との結合）
- [ ] ドキュメント（英語、日本語）
