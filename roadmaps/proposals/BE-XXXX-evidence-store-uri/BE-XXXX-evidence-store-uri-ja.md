...
| 関連 | [BE-0015](../../in-progress/BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting.md), [BE-0106](../BE-0106-post-completion-worker-model/BE-0106-post-completion-worker-model.md) |
<!-- /BE-METADATA -->
...
現在の `serve` アーキテクチャ（BE-0015、BE-0106）には、`serve/server/object_store.py` に S3 向けの `ObjectStore` プロトコルがありますが、サーバー内部のアーティファクト読み出しにのみ使用されています。この提案では、そのインターフェースを run 完了後の証跡アップロードに拡張し、GCS 対応を追加し、`serve` の単一 URI フラグで設定を統一します。
...
### 3. run 完了後のアップロードステップ

アップロードは、run パイプラインが完了し判定が確定した**後に**実行されます。ローカルの `runs/<run_id>/` ディレクトリツリーを走査し、設定されたプレフィックスの下に相対パス構造を保ったまま各ファイルをアップロードします。
...
この `serve` インスタンスで完了したすべてのジョブが、この証跡ストアにアップロードされます。run ID が常にパスに含まれるため、run 同士が衝突することはありません。

CI はジョブ投入時にプレフィックスをパラメータとして渡すことで、パスを制御します（`/api/runs` エンドポイントがオプショナルの `evidence_prefix` を受け付け、サーバーのベース URI に追記します）。
...
`serve` インスタンスがバケットと認証情報を管理し、CI はパスのみを制御します。責務の分離が明確になります。

### 5. スタンドアロン `bajutsu run` での利用

ローカル実行やスタンドアロン CI（`serve` を介さない場合）でも、同じフラグが使えます。

```bash
bajutsu run --evidence-store gs://bucket/feature/pr-42/ scenarios/
```

仕組みは同一で、サーバーレベルではなく CLI レベルで設定するだけです。

### 6. オプショナル依存

`boto3` と `google-cloud-storage` はいずれも必須依存にはしません。オプショナルの extras として宣言します。
...
シークレットを Bajutsu の設定に含めず、CI プラットフォームのネイティブな認証機構（GitHub Actions OIDC など）をそのまま利用できます。
...
- [ ] URI スキームのパースと `StoreURI` データクラス
- [ ] `ObjectStore` プロトコルのトップレベルモジュールへの昇格
- [ ] `S3ObjectStore` 実装（既存コードの再利用）
- [ ] `GCSObjectStore` 実装
- [ ] `runner/pipeline.py` への run 完了後アップロードステップ追加
- [ ] `serve` の `--evidence-store` フラグと `BAJUTSU_EVIDENCE_STORE` 環境変数
- [ ] `bajutsu run` の `--evidence-store` CLI フラグ
- [ ] `/api/runs` エンドポイントの `evidence_prefix` パラメータ
- [ ] オプショナル依存の宣言（`s3` / `gcs` / `cloud` extras）
- [ ] テスト（URI パース、モック済みストアによるアップロードロジック、serve との結合）
- [ ] ドキュメント（英語、日本語）
...
- [BE-0015](../../in-progress/BE-0015-web-ui-public-hosting/BE-0015-web-ui-public-hosting.md) — Web UI の公開ホスティング（リモートストレージの動機となるサーバー構成）
- [BE-0106](../BE-0106-post-completion-worker-model/BE-0106-post-completion-worker-model.md) — 完了後ワーカーモデル（アップロードが組み込まれる非同期ジョブパイプライン）
- [BE-0060](../../implemented/BE-0060-run-report-zip-export/BE-0060-run-report-zip-export.md) — run レポートの ZIP エクスポート（関連：ポータブルなアーティファクトパッケージング）
