...
- [ ] `bajutsu/adb.py` に 2 接触点の protocol B の pinch / rotate 列を追加する。
- [ ] adb ドライバに `pinch` / `rotate` を実装する（root 化したデバイス限定、フォールバックなし）。
- [ ] adb ドライバの静的 capability セットに `multiTouch` を表明し、root の前提を文書化する。
- [ ] Compose の `SHOWCASE_GESTURES` ジェスチャ画面（`log.pinch` / `log.rotate` のミラー）を追加する。
- [ ] `gestures_multitouch` を BE-0208 の e2e レーンに組み込み、`docs/ci.md`（と日本語版）を更新する。
- [ ] コマンドビルダとドライバの root ゲートを単体テストし、必要なら conformance スイートを拡張する。
