...
- **使い捨ての GitHub OAuth App を用意し、一度だけ捕捉する**：テスト専用の最小限の OAuth App を
  登録します（クライアント ID/secret は一度きりの手動捕捉にのみ使い、CI には一切保存しません）。
  メンテナが手動で、その App に対して実際の認可コード交換を一度完了し、生の HTTP レスポンス
  (トークン交換と、代表的な org 一覧ページ)を保存します。
- **CI のライブログインではなく、捕捉したレスポンスを実際のコードに対して再生する**：最初の
  ...
  OAuth クライアントシークレットよりもはるかに機微な secret を扱うことになります。代わりに、
  ...
  ことで、Authlib の呼び出しシグネチャの破壊や GitHub のレスポンス形状の変更を、CI 上のライブな
  ネットワーク呼び出しや認証情報を一切必要とせずに検出できます。
...
- [ ] 使い捨ての GitHub OAuth App を登録し、実際のトークン交換と org 一覧のレスポンスを
  一度手動で捕捉する。
