[English](BE-XXXX-xcuitest-picker-wheel-adjust.md) · **日本語**

# BE-XXXX — XCUITest 向けに pickerWheel の値を決定的に設定するステップを追加する

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-xcuitest-picker-wheel-adjust-ja.md) |
...
**テスト。** `tests/test_select_option.py` のパース・ディスパッチ・fake への記録という形に、上記の `picker_wheel_options` を組み合わせて、次の3つのユニットテストを追加します。
...
- `Capability.PICKER_WHEEL` を持たないバックエンド（`adb` または `playwright`）に対して `setPickerValue` を使うシナリオを実行すると、actuation を試みる前に preflight の時点で失敗すること
...
- [ ] テスト: 値が見つかる場合、値が存在しない場合（`ElementNotFound`）、ケーパビリティ不足による
      preflight 失敗、複数コンポーネント（年・月）の場合
