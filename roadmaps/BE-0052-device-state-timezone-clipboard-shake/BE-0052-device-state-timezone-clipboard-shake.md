**English** · [日本語](BE-0052-device-state-timezone-clipboard-shake-ja.md)
...
| Related | [BE-0157](../deferred/BE-0157-shake-device-primitive/BE-0157-shake-device-primitive.md), [BE-0158](../deferred/BE-0158-timezone-device-primitive/BE-0158-timezone-device-primitive.md) |
| Origin | MagicPod |
<!-- /BE-METADATA -->

## Introduction

The remaining device-state primitives carved out of [BE-0035](../BE-0035-device-control-primitives/BE-0035-device-control-primitives.md)
...
- [x] `setTimezone` — implementation triage found no reliable `simctl` actuation exists today (shipping a command that does not take effect would violate determinism-first): neither `simctl` nor `idb` has a `timezone` subcommand, and the Simulator's timezone is inherited from the host Mac rather than settable per device. A launch-time `SIMCTL_CHILD_TZ` only moves the app process's libc `localtime`, not the `TimeZone.current` most iOS date UI reads, so it would appear to succeed without changing the UI under test. Changing the host Mac's own timezone would work but is global, `sudo`-gated, and mutates the developer/CI clock across every booted Simulator — out of contract for a per-device primitive. Carved out into its own deferred item, [BE-0158](../deferred/BE-0158-timezone-device-primitive/BE-0158-timezone-device-primitive.md), to wait on a verified mechanism.
- [x] `shake` — implementation triage found there is no `simctl` / `idb` shake command (it is a Hardware-menu gesture). It can be triggered deterministically via GUI automation (AppleScript driving the Simulator's Device ▸ Shake menu item, or a third-party tool such as RocketSim), but that path needs the Simulator GUI running plus an Accessibility grant for the controlling process, is scoped to the focused Simulator, and cannot run in headless CI — so it does not fit as a `simctl`-level primitive today. RocketSim's own CLI does not expose shake (or timezone) either; only its `tap`/`swipe`/`type`/`button`/inspect commands are scriptable. Carved out into its own deferred item, [BE-0157](../deferred/BE-0157-shake-device-primitive/BE-0157-shake-device-primitive.md), to wait on a verified mechanism.

Shipped across [#206](https://github.com/bajutsu-e2e/bajutsu/pull/206) / [#257](https://github.com/bajutsu-e2e/bajutsu/pull/257) / [#378](https://github.com/bajutsu-e2e/bajutsu/pull/378). `setTimezone` and `shake` had no reliable actuator and were split out into their own deferred items, [BE-0158](../deferred/BE-0158-timezone-device-primitive/BE-0158-timezone-device-primitive.md) and [BE-0157](../deferred/BE-0157-shake-device-primitive/BE-0157-shake-device-primitive.md) respectively, so this item is complete for the primitives it could ship.

## References

Split out of [BE-0035 — Device-control steps](../BE-0035-device-control-primitives/BE-0035-device-control-primitives.md).
`setTimezone` and `shake` were in turn split out into
[BE-0158 — Timezone device primitive](../deferred/BE-0158-timezone-device-primitive/BE-0158-timezone-device-primitive.md) and
[BE-0157 — Shake device primitive](../deferred/BE-0157-shake-device-primitive/BE-0157-shake-device-primitive.md).
