...
自体に対する実モデル検証を追加します。別項目として提案する transport 水準のアダプタ検証
(AI backend の実 API 契約 smoke レーン)とは異なり、それを補完する位置づけです。

## 動機

姉妹項目の AI backend 提案は「アダプタは実サービスとそもそも往復できるのか」という transport の
...
このギャップは、一般的な AI 呼び出しよりも重い意味を持ちます。`record` の出力はシナリオファイル
そのものであり、いったん書き込まれれば決定的な `run` ゲートが完全に信頼する唯一の成果物だから
...
- [ ] `record` の propose ループ向けに、API キーで gate したライブ smoke テストを追加する。
- [ ] `crawl` のナビゲーションループ向けに、API キーで gate したライブ smoke テストを追加する。
...
- `bajutsu/agents/claude.py`、`bajutsu/crawl/guide.py`、`bajutsu/crawl/tabs.py`、
  ...
  `tests/test_crawl.py`、AI backend アダプタの実 API 契約 smoke レーンを扱う姉妹提案
