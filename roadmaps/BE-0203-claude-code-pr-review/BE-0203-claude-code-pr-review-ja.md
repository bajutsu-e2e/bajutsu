...
- [x] 助言用のレビューワークフロー `.github/workflows/claude-review.yml` を追加する（自動起動、インラインの
...
- [ ] ライブのテスト PR で手動検証する（自動レビュー、再レビュー、提案、非必須チェック）（項目 9）。マージ後の
      手動手順です。レビューの挙動はライブの PR とプロバイダー呼び出しを要するため、決定論的なゲートには
      置けません（BE-0122 と同じ形）
...
- 助言用のレビューワークフロー、リポジトリ仕様のレビュープロンプト、二言語のドキュメントを出荷しました
  （項目 1〜8）。GitHub OIDC（`aws-actions/configure-aws-credentials`）経由で Amazon Bedrock に認証し、
  `AWS_BEDROCK_ROLE_ARN` の Environment シークレットが用意されるまでは green no-op に固定します。項目 7 の
  移行実施（リポジトリ／組織設定での Copilot 無効化）と項目 9 のライブ検証は、PR の差分では実行できない
  マージ後の手動手順です。実装 PR: [#807](https://github.com/bajutsu-e2e/bajutsu/pull/807)。
...
- レビュー（#807）を受けて堅牢化しました。オンデマンドのコメント経路は、正確な `@claude review` フレーズと
  信頼された actor（OWNER/MEMBER/COLLABORATOR）の両方を要求するようにし、フォーク PR のシークレット露出の
...
- Claude Code 組み込みの `code-review` スキル（`--comment` でインラインの PR コメントを投稿） —
  このワークフローが呼び出すレビュアー。組み込みスキルであり、[`.claude/skills`](../../.claude/skills)
  （リポジトリ側で定義したスキル群）の配下にはありません。リポジトリの
  [`implement-be`](../../.claude/skills/implement-be/SKILL.md) が著者側ですでに使っています（観点や
  pr-review-toolkit を伴うレビュー手順）。この項目が補い再利用するパスです。
