...
- push ごとの自動トリガーが生んでいた再レビューの「ラリー」を解消しました。以前は push のたびに PR 全体の
  差分を、過去のコメントを覚えないまま再レビューしていたため、同じ指摘を毎回投稿していました。現在は再 push
  （`synchronize`）のとき差分を**増分**（`gh api …/compare/{before}...{after}`。force-push やリベースの後は
  全体の `gh pr diff` にフォールバック）に絞り、**すでに PR にある指摘を繰り返しません**（先に `gh api
  …/pulls/N/comments` で既存のインラインコメントを取得します）。ワークフローの `prompt`（イベントの action と
  before／after の head SHA を渡すようにしたもの）と、対応する `Bash(gh api:*)` の allowlist、レビュー契約への
  *スコープ* 節の追加で実現しています。依然としてアドバイザリーであり、必須チェックにもしていません。ゲートは
  変えず、レビュアーの挙動だけを変えた変更です。
