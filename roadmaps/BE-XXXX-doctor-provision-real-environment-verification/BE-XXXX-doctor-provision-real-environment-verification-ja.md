...
score only)"`）にすぎず、終了コードを握りつぶし、レンダリングされた出力から判定をアサーション
...
- **既存の E2E レーン内で `bajutsu doctor` を実際に実行する**：
  ...
  終了コードが 0 で、レンダリングされた出力が Ready または Partial の判定であることを検証する
  ステップを追加します。これにより `simctl.py` の実際の JSON パースと `preflight.py` の実際の
  ツールチェックを一度に検証します。
- **意図的に壊した環境のケースを1つ追加する**：ツールを意図的に欠落させる、または誤設定した（例：`idb` を
  含まない `PATH`）ジョブ（またはジョブのステップ）で、`doctor`/`preflight` が非ゼロの終了コードで
  Blocked の判定をまさしく報告することを検証します。これは、注入された fake によるテストでは
  今日証明できない失敗側です。
...
- [ ] iOS・Android・web の E2E レーン内で `bajutsu doctor` を実際に実行し、Ready/Partial
  の判定を検証する。
