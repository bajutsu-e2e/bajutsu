[English](BE-XXXX-config-aware-environment-installer.md) · **日本語**

# BE-XXXX — config を踏まえた環境インストーラー

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-config-aware-environment-installer-ja.md) |
...
- **`make setup`** は `uv sync --group dev` を実行するだけで、Python のツールチェーンと AI フリーの
  基本パッケージしかインストールしません。どのバックエンド向けの何もインストールしません。
...
逆算しなければなりません。そして今後追加されるバックエンド（Android — [BE-0007](../BE-0007-android-backend/BE-0007-android-backend.md)、
Flutter — [BE-0008](../BE-0008-flutter-support/BE-0008-flutter-support.md)）も、この同じパターン
――独自の Makefile ターゲットやシェルスクリプトを一から書くこと――を繰り返しかねません。このロジックが
乗る唯一の拡張可能な置き場所がない限り、そうなります。
...
3. **1 つの実装を共有する 2 つのエントリポイント。** contributor 向けの Makefile ターゲット（`make setup`
   と `make deps` を 1 つにまとめる、あるいは両者と並ぶ `make install` を新設し、`git clone` の直後――
   今日 `make setup` を実行するのと同じタイミング――で走らせる想定）と、その裏側のロジックを切り出した
   スクリプト（`Makefile` 本体にインラインで重複させない）です。こうすることで実装は 1 つのままとなり、
...
（アップロードされたバンドル config からのコマンド実行を統制・サンドボックス化する）が意図的に閉じた境界を
...
- [BE-0090](../BE-0090-uploaded-config-command-execution/BE-0090-uploaded-config-command-execution-ja.md) — アップロードされたバンドル config からのコマンド実行を統制・サンドボックス化する
- [BE-0111](../BE-0111-ai-sdk-optional-dependency/BE-0111-ai-sdk-optional-dependency-ja.md) — AI SDK をオプションの extra にする
