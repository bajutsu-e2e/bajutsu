[English](BE-XXXX-slim-web-worker-image.md) · **日本語**
...
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-slim-web-worker-image-ja.md) |
...
`deploy/self-host/worker-web.Dockerfile`（名前は仮）をマルチステージビルドとして追加します。

- **ビルドステージ**: `bajutsu[worker-web]` を `uv` で仮想環境にインストールします（editable なチェックアウトではなく、ビルドした wheel から）。
- **最終ステージ**: スリムな Python ベース + 仮想環境 + Playwright が必要とする Chromium ブラウザだけを載せ、`server`/`db`/`oauth`/`cloud`/`ai` からは何も入れません。Chromium バイナリは `playwright install --with-deps chromium` で入れます。あるいは最終ステージを upstream の Playwright Python イメージ（ブラウザと OS ライブラリが導入済み）の上に構築する手もあります。両者の比較は Alternatives で扱います。
...
最終ステージを `mcr.microsoft.com/playwright/python` の上に構築すると、ブラウザと OS ライブラリがバージョン整合済みで導入された状態になりますが、ベースが大きく制御しにくくなり、外部ベースへの依存が増えます。Chromium を自分でスリムな Python ベースに入れると、ベースは最小で完全に自分たちの管理下に置けますが、`--with-deps` のシステムライブラリ導入を保守する手間が生じます。イメージはどちらかを選び、理由を記述すべきです。本提案はサイズ比較を前提に、制御しやすさを重視して自前導入のスリムベースに傾いています。
...
- [ ] `worker-web` / `worker-idb` の実行時閉包 extra を `pyproject.toml` に定義する
- [ ] `deploy/self-host/` にマルチステージの web worker Dockerfile を追加する
- [ ] optional な `worker-web` compose サービス + `docs/self-hosting.md`（と日本語版）の異種フリート記述
- [ ] worker エントリの cold start / import 閉包ガードテスト
- [ ] BE-0166 に `backend` の capability 軸を追記する（相互参照）
