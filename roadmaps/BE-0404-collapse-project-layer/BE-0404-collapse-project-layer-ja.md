[English](BE-0404-collapse-project-layer.md) · **日本語**

# BE-0404 — project 階層を org と target に畳む
...
ます。バインド中の label に一致する run が1件もないときは、デフォルトは絞り込みなしに退避します。
label より古い履歴しか持たないデプロイが、空のページで開くことを防ぐためです。この退避が、単位5の
backfill が移行後の履歴を見えない絞り込みの裏に隠すことを防ぎます。
...
label も空のままです。実際には、`default` 以外のホスト型 org の run の大半がこれにあたります。これらを
見える状態に保つのが、単位4の退避です。すべての run が label を持たない org では、バインド中の label に
一致する run が1件もありません。したがって各画面は、何も表示しないのではなく、絞り込みなしで履歴全体を
開きます。
