...
| 実装 PR | [#1619](https://github.com/bajutsu-e2e/bajutsu/pull/1619) |
...
実装の過程で、この設計の誤りが5つ実測で判明しました。どれも、放置すれば「静かに何も集めない
...
- **捕捉上限を共有にすると、肝心のトリガーが飢えます。** 上の動機に記したとおり、
  `recordVideo produced no new bytes` は緑の実行を含む全シナリオで発火し、1つのジョブは
  1つの `bajutsu run` プロセスで十数個のシナリオドキュメントを走らせます。したがってプロセス
  ごとに3回という単一の上限は3番目のシナリオの映像警告で使い切られ、ランナーのクラッシュ、
  すなわちこの捕捉が説明したい失敗であり、デバイスの id を伴って到達する唯一のトリガーは、
  残りがない状態で届きます。上限はトリガーごとに持たせます。そして捕捉を断ったことは debug では
  なく warning で記録します。このレーンはログハンドラを設定しないため、debug では記録が
  まったく残らないからです。

- **捕捉は、自分のものでない時計に課金されます。** BE-0353 の `CrashRecoveryBudget` は、*最初の*
  クラッシュ時点で置かれ、以降のクラッシュごとに再確認される実時間の締切です。したがって捕捉が使う
  1秒は、復帰が使えなくなる1秒です。この変更の最初の CI 実行がそれを示しました。`visual` ジョブが
  300秒の復帰締切を75秒超過し、その窓の内側に最大30秒の捕捉が2回入っていました。これは復帰可能な
  劣化を失敗した実行に変える診断であり、観測者がしてはならないことそのものです。1回あたりの予算は
  そのレーンの300秒に対して10秒とし、スクリーンショットの上限は5秒とします（ランナー自身の読み取り
  タイムアウトが15秒なので、`simctl` の経路が5秒で応答しなければ、それだけで問いには答えています）。
  そして裸の数値ではなく、レーンが設定している予算との関係をテストで固定します。
