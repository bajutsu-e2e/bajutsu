[English](BE-XXXX-step-latency-driver-internal-tuning.md) · **日本語**

# BE-XXXX — 証拠読み取りの重複排除とドライバ内部の調整による、ステップ実行の高速化（iOS と Android）

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-step-latency-driver-internal-tuning-ja.md) |
...
- [ ] `roadmap-id` ワークフローが `main` 上で 4 項目の ID を採番したら、この項目と
  ...
  新規項目どうしを `BE-XXXX` で相互参照できないため、この作業はどの項目もマージ時点
  では持てない。
