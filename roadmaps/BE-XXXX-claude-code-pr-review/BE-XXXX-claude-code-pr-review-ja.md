[English](BE-XXXX-claude-code-pr-review.md) · **日本語**

# BE-XXXX — プルリクエストの自動コードレビューを Claude Code で行う

<!-- BE-METADATA -->
| 項目 | 値 |
|---|---|
| 提案 | [BE-XXXX](BE-XXXX-claude-code-pr-review-ja.md) |
...
**Claude Code** に置き換えます。GitHub Actions のワークフローから、リポジトリ自身の
[`code-review`](../../.claude/skills) スキルをすべてのプルリクエストに対して走らせ、オープン時と各プッシュ時に
...
- **部品はすでに揃っており、この項目はそれを PR に配線するだけです。**
  [`code-review`](../../.claude/skills) スキルはすでに指摘を生成し、`--comment` を付ければ**それらを
...
1. **レビューのワークフロー。** 新規の `.github/workflows/claude-review.yml` を追加します。
   `pull_request: [opened, synchronize, reopened]` で起動し（オープン時の自動レビュー、各プッシュ時の再レビュー。
   Copilot と同じ）、Anthropic 公式の `claude-code-action`（本リポジトリの他のすべてのアクションと同様、
   コミットの完全な SHA に固定する）を実行して、リポジトリ自身の **`/code-review --comment`** を呼び出し、指摘を
   **インラインの PR コメント**として着地させます。権限はこの表面が必要とする最小限にとどめます。
   `pull-requests: write`（レビューコメントの投稿）と `contents: read`（差分の読み取り）だけで、それ以外は
   与えません。`ci.yml` にならって `concurrency: claude-review-${{ github.ref }}` と `cancel-in-progress: true` を
   持たせ、短時間に連続するプッシュでは古いレビューを積み重ねずに打ち切って置き換えます。
...
3. **Copilot との機能同等性。** Copilot のレビュー機能のそれぞれが、ここでは具体的な部品に対応します。
   ...
   | Copilot の機能 | この項目 |
   |---|---|
   | PR オープン時の自動レビュー | `pull_request: [opened]` |
   | 各プッシュ時の再レビュー | `pull_request: [synchronize]` |
...
- [ ] 助言用のレビューワークフロー `.github/workflows/claude-review.yml` を追加する（自動起動、インラインの
      `/code-review --comment`、最小権限、concurrency）（項目 1）
- [ ] 助言のガードレールを担保する。必須チェックにしない、結果を指摘から切り離す（項目 2）
- [ ] Copilot 同等に到達する。インラインコメント、提案ブロック、要約（項目 3）
- [ ] 任意の `@claude review` オンデマンド経路を追加する（項目 4）
- [ ] リポジトリ仕様のレビュープロンプト（prime directive、観点、プロジェクトの慣行）をコミットする（項目 5）
- [ ] 資格情報を Environment でスコープし、フォークの制約をドキュメント化する（項目 6）
- [ ] 並行運用してから切り替える移行を実施し、手動の Copilot 無効化手順をドキュメント化する（項目 7）
- [ ] `docs/ai-development.md`（EN + JA）にレビュアーをドキュメント化する（項目 8）
- [ ] ライブのテスト PR で手動検証する（自動レビュー、再レビュー、提案、非必須チェック）（項目 9）
...
- [`.claude/skills`](../../.claude/skills) — このワークフローが呼び出す組み込みの `code-review` スキル
  （`--comment` でインラインの PR コメントを投稿）。
- [`.claude/skills/implement-be/SKILL.md`](../../.claude/skills/implement-be/SKILL.md) — この項目が補い再利用する、
  著者側のレビュー手順（観点、pr-review-toolkit）。
