...
`bajutsu/` を mccabe 自身のデフォルトの上限（10）で測定すると、59個の関数がこれを超えます。
この分布は平坦でなく急です。上限を20まで上げると、件数は13個に落ちます。そのなかでも2つの関数
だけが残りから大きく突出しています。複雑度100の `_make_handler`
（[bajutsu/serve/handler.py:90](../../bajutsu/serve/handler.py)）と、複雑度55の `make_app`
...
（44、`bajutsu/runner/pool.py:71`）、`_wait`（35、`bajutsu/orchestrator/waits.py:376`）、
...
`_handle_action`（31、`bajutsu/orchestrator/loop.py:1000`）、`lease`
（24、`bajutsu/runner/pool.py:228`）、`_step_selectors`（21、`bajutsu/analysis/audit.py:94`）です。

Pylint 由来の関数サイズルールが測っているのは、関連はしていますが別の性質です。分岐点の数では
なく、文・分岐・`return` がどれだけ積み重なっているかを測ります。デフォルトのしきい値でも、より多くの
指摘が見つかります。`PLR0911`（return が多すぎる、デフォルトの上限は6）は44件、`PLR0912`（分岐が多す
ぎる、デフォルトの上限は12）は24件、`PLR0915`（文が多すぎる、デフォルトの上限は50）は15件です。
`bajutsu/` の外では、上限25の `C901` はクリーンですが、`PLR0911` と `PLR0912` が `tests/`・
`demos/`・`scripts/` にわたってさらに5件（`PLR0911` 3件、`PLR0912` 2件）を持ち込むため、
`ruff check .` のもとでの合計は83件ではなく88件になります。同じ refactor 層にある他の2つの
...
3. **手順1の一覧が片付いた段階で、`max-complexity` を段階的に締めます。** 20まで下げると3個、
   15まで下げるとさらに6個、12まで下げるとさらに13個が新たに対象へ入ります。前の上限の一覧が
...
*動機* は1つの問いを実装の段階へ残していました。`_make_handler` の複雑度100と `make_app` の55は、
...
### 件数が絡まったロジックではなく場合の数を表している箇所

件数がその関数自身の分岐を反映している場合、多くなる理由は2つあります。絡まったロジックと、
その関数が扱わなければならない場合の数です。振り分けを通して、件数が場合の数から来ている関数が
2種類、繰り返し現れます。どちらも分割しません。1つ目は、閉じたスキーマに対する網羅的な
ディスパッチです。3つのバックエンドの `_emit_step` と、対になる `_emit_assertion` は、シナリオの
step 種別または assertion 種別ごとに1つの分岐を持ちます。件数を決めているのはシナリオスキーマの
大きさであり、分割すると、新しい step 種別を書き加える場所が1つに定まらなくなります。
`required_role` のルーティング方針も同じで、ゲートされる経路の分類ごとに1つの return を持ちます。
2つ目は、return がすべて検証ガードであり、それぞれ異なる HTTP ステータスを返すリクエスト
ハンドラです。これは `RET505` が求める早期 return の形そのものです。どちらの種類の関数にも、
2つのうちどちらにあたるかを名指しした `# noqa` を添えます。
...
- **`max-complexity` を ruff のデフォルト値（10）へただちに設定する** — 却下します。それでは59個の
...
- [ ] `max-complexity` を15まで締め、新たに対象となる6個の関数を振り分ける。
- [ ] `max-complexity` を12まで締め、新たに対象となる13個の関数を振り分ける。
