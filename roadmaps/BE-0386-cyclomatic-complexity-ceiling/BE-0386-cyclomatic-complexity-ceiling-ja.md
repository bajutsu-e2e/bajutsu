...
件数が多くなる理由は2つあります。絡まったロジックと、その関数が扱わなければならない場合の数です。
振り分けを通して、件数が場合の数から来ている関数が2種類、繰り返し現れます。どちらも分割しません。
1つ目は、閉じたスキーマに対する網羅的なディスパッチです。3つのバックエンドの `_emit_step` と、
対になる `_emit_assertion` は、シナリオの step 種別または assertion 種別ごとに1つの分岐を持ちます。
件数を決めているのはシナリオスキーマの大きさであり、分割すると、新しい step 種別を書き加える場所が
1つに定まらなくなります。`required_role` のルーティング方針も同じで、ゲートされる経路の分類ごとに
1つの return を持ちます。2つ目は、return がすべて検証ガードであり、それぞれ異なる HTTP ステータスを
返すリクエストハンドラです。これは `RET505` が求める早期 return の形そのものです。どちらの種類の
関数にも、2つのうちどちらにあたるかを名指しした `# noqa` を添えます。
