...
This is the counterpart to [`ideation`](../ideation/workflow.md): that skill *authors* a BE
proposal, this one *ships* it. When the author is confident enough to write the proposal and
its implementation at once, [`propose-and-build`](../propose-and-build/workflow.md) runs both in
...
keep only their synthesis in the main thread. Use the host's planning facility to draft
...
- **Docs are bilingual.** If you change a *documented* behavior, update `docs/` **and** its
  ...
  [`japanese-document-writing`](../japanese-document-writing/workflow.md) skill — natural Japanese, not
...
subagent context** for each iteration when the host supports subagents. Keep only the short
structured summary in the main thread.

### 12. Run bounded PR follow-up

Use the host's bounded wait or monitoring facility: a **short** interval while CI is actively
running and a **longer** interval while waiting on human review. Each iteration does three things:
...
- [`pr-followup`](../pr-followup/workflow.md) — the skill steps 11–12 loop over: after this skill
...
- [`ideation`](../ideation/workflow.md) — the upstream skill that authors the proposal this one builds.
