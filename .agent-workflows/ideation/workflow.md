...
[`implement-be`](../implement-be/workflow.md) skill (the deterministic counterpart that ships an
...
[`propose-and-build`](../propose-and-build/workflow.md), which authors the proposal and implements
...
invoke the [`document-writing`](../document-writing/workflow.md) skill — it is the authoritative norm for BE
...
[`japanese-document-writing`](../japanese-document-writing/workflow.md) skill (敬体; the Japanese layer beneath
...
**The implement pass.** Unlike the CI workflow — which only posts comments, since prime directive 1
