...
- [`implement-be`](../implement-be/workflow.md) — ships an *already-numbered* item from its id.
...
- **Fall back to the serial path** ([`ideation`](../ideation/workflow.md) →
  [`implement-be`](../implement-be/workflow.md)) when the design is genuinely uncertain, wide-
...
and Phase B delegate to, keyed on the same `BE-XXXX` placeholder throughout. The checkpoints
themselves are inherited this way; each host's adapter still has to say how to dispatch them
cheaply, since an adapter loads only its own workflow, never a sibling skill's adapter.
...
- Scaffold `roadmaps/BE-XXXX-<slug>/` with `make new-roadmap-item` at `Status: Proposal`,
  fill the `TBD` sections under the [`document-writing`](../document-writing/workflow.md) skill (the
  authoritative prose norm for both languages, invoked *before* drafting), and localize the
  Japanese side under the [`japanese-document-writing`](../japanese-document-writing/workflow.md) skill
...
- [`ideation`](../ideation/workflow.md) — the upstream skill this composes for Phase A (authoring
  only, never implements).
- [`implement-be`](../implement-be/workflow.md) — the skill this composes for Phase B (steps 3–9); the
