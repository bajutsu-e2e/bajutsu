...
This skill is the plain-issue sibling of [`implement-be`](../implement-be/workflow.md), which ships a
...
2. Point at the skill that fits instead —
   [`ideation`](../ideation/workflow.md) when the design still needs shaping, or
   [`propose-and-build`](../propose-and-build/workflow.md) when the design is settled enough to
...
Run [`implement-be`](../implement-be/workflow.md) steps 3–5 with the issue body in place of a BE
...
Run [`implement-be`](../implement-be/workflow.md) steps 6 and 7: build to the plan matching the
...
yourself**, exactly as
[`implement-be`](../implement-be/workflow.md) step 10 does and for the same reason: this skill's
...
Run [`implement-be`](../implement-be/workflow.md) steps 11 and 12 against the new PR, unmodified:
each iteration's [`pr-followup`](../pr-followup/workflow.md) work runs in a fresh context so the
implementation transcript stays out of it, and the loop uses the same merge-conflict check, the same
...
- [`implement-be`](../implement-be/workflow.md) — the BE-item counterpart this skill mirrors, and
  the source of steps 4–8's procedure.
- [`ideation`](../ideation/workflow.md) and
  [`propose-and-build`](../propose-and-build/workflow.md) — step 3's two escalation targets.
- [`pr-followup`](../pr-followup/workflow.md) — the skill step 8's loop runs each iteration.
- [`task-select`](../task-select/workflow.md) — the read-only survey that ranks plain issues as
